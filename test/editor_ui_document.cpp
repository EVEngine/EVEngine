...
TEST_CASE("editor.ui.style_inspector_preview_pick_and_anchor_gizmo_are_revision_safe") {
    ...
    REQUIRE(root.value); CHECK(document.applyDomainOperation(*root.value).accepted());
    ...
    REQUIRE(child.value); CHECK(document.applyDomainOperation(*child.value).accepted());
    ...
    REQUIRE(styled.value); CHECK(document.applyDomainOperation(*styled.value).accepted());
    ...
    CHECK(gizmo.accepted());
    auto rename = document.makeRename(ObjectId("play"), "Play now"); REQUIRE(rename.value);
    CHECK(document.applyDomainOperation(*rename.value).accepted());
...
TEST_CASE("editor.ui.document_hierarchy_operations_are_reversible_and_cycle_safe") {
    ...
    for (const CreateUiWidgetRequest& request : {
        ...
        REQUIRE(commitUi(target, transactions, *operation.value, "ui.create." + request.id.value()).accepted());
    }
    ...
    REQUIRE(commitUi(target, transactions, *rename.value, "ui.rename").accepted());
    ...
    REQUIRE(commitUi(target, transactions, *reparent.value, "ui.reparent").accepted());
    ...
    REQUIRE(commitUi(target, transactions, *remove.value, "ui.delete").accepted());
...
TEST_CASE("editor.ui.multi_selection_layout_inspector_is_atomic_and_reports_mixed_values") {
    ...
    for (const char* id : {"left", "right"}) {
        ...
        auto operation = target.makeCreate(request);
        REQUIRE(operation.accepted());
        REQUIRE(commitUi(target, transactions, *operation.value, std::string("ui.create.") + id).accepted());
    }
    ...
    REQUIRE(size.accepted());
    REQUIRE(commitUi(target, transactions, *size.value, "ui.multi-size").accepted());
    ...
    REQUIRE(move.accepted());
    REQUIRE(commitUi(target, transactions, *move.value, "ui.move-one").accepted());
...
TEST_CASE("editor.ui.snapshot_load_is_atomic_and_rejects_invalid_hierarchies") {
    ...
    REQUIRE(root.value);
    REQUIRE(source.applyDomainOperation(*root.value).accepted());
    auto child = source.makeCreate({ObjectId("child"), ObjectId("root"), "text", "Child", {8, 8, 100, 20}});
    REQUIRE(child.value);
    REQUIRE(source.applyDomainOperation(*child.value).accepted());
    ...
    CHECK_EQ(static_cast<int>(restored.loadSnapshot(EditorValue(std::move(invalidRoot))).status),
             static_cast<int>(EditorStatus::Rejected));
    CHECK(restored.widget(ObjectId("child")).accepted());
}
