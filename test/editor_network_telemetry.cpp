...
TEST_CASE("editor.network.telemetry_computes_rates_and_bounds_history") {
    ...
    REQUIRE(model.ingest(a).isAccepted());REQUIRE(model.ingest(b).isAccepted());
    ...
    NetworkTelemetrySample c=b;c.revision=3;c.timeSeconds=4;c.sentBytes=600;REQUIRE(model.ingest(c).isAccepted());CHECK_EQ(model.samples().size(),std::size_t{2});
...
TEST_CASE("editor.network.telemetry_handles_reset_and_reports_health") {
    ...
    REQUIRE(model.ingest(a).isAccepted());REQUIRE(model.ingest(reset).isAccepted());
