...
TEST_CASE("editor.network.telemetry_computes_rates_and_bounds_history") {
    ...
    REQUIRE(model.ingest(a).ok());REQUIRE(model.ingest(b).ok());
    ...
    NetworkTelemetrySample c=b;c.revision=3;c.timeSeconds=4;c.sentBytes=600;REQUIRE(model.ingest(c).ok());CHECK_EQ(model.samples().size(),std::size_t{2});
    CHECK_EQ(static_cast<int>(model.ingest(c).code()),static_cast<int>(EditorStatus::Conflict));
}
TEST_CASE("editor.network.telemetry_handles_reset_and_reports_health") {
    ...
    REQUIRE(model.ingest(a).ok());REQUIRE(model.ingest(reset).ok());
