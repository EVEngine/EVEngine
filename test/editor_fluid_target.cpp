...
TEST_CASE("editor.fluids.settings_are_reversible_persistent_and_budgeted") {
    ...
    REQUIRE(radius.value); CHECK(fluid.applyDomainOperation(*radius.value).accepted());
    auto support = fluid.makeSet(selection, PropertyPath("supportRadius"), 0.32, PropertySetMode::Absolute);
    REQUIRE(support.value); CHECK(fluid.applyDomainOperation(*support.value).accepted());
    ...
    CHECK(fluid.applyDomainOperation(undo).accepted());
    CHECK_EQ(fluid.settings().supportRadius, 0.20);
    FluidSimulationTarget restored("waterfall"); CHECK(restored.loadSnapshot(fluid.snapshotValue()).accepted());
