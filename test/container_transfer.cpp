...
public:
    ...
    [[nodiscard]] eve::Result<ContainerSnapshot> snapshot() const override {
        return eve::Result<ContainerSnapshot>::success(
            ContainerSnapshot{descriptor_.id, revision_, entries_});
    }
    [[nodiscard]] eve::Result<void> validateInsert(
        const ContainerObject&, std::optional<SlotIndex>,
        std::optional<MembershipId>) const override {
        return eve::Result<void>::success();
    }
    [[nodiscard]] eve::Result<std::unique_ptr<IContainer::PreparedState>> prepare(
        const ContainerSnapshot&, const ContainerSnapshot&) override {
        ++prepareCalls_;
        return eve::Result<std::unique_ptr<IContainer::PreparedState>>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::Failed,
                                   "injected destination prepare failure"));
    }
...
TEST_CASE("container.zoneUsesStrongCoordinateSpacesAndValidatesShapes") {
    ...
    auto screen = Zone<ScreenSpace>::create(
        Rectangle<ScreenSpace>{{10.f, 20.f}, 100.f, 50.f}, Capacity::fixed(2));
    REQUIRE(screen.ok());
    CHECK(screen.value().contains({20.f, 30.f}));
    CHECK(!screen.value().contains({0.f, 0.f}));

    auto invalid2d = Zone<World2DSpace>::create(
        Rectangle<World2DSpace>{{0.f, 0.f}, -1.f, 2.f});
    CHECK(!invalid2d.ok());
    CHECK_EQ(invalid2d.code(), eve::StatusCode::Rejected);

    auto invalid3d = Zone<World3DSpace>::create(
        Box3D{{0.f, 0.f, 0.f}, 1.f, 2.f, -3.f});
    CHECK(!invalid3d.ok());

    auto sphere = Zone<World3DSpace>::create(
        Sphere3D{{0.f, 0.f, 0.f}, 2.f});
    REQUIRE(sphere.ok());
...
TEST_CASE("container.cardDeckHandDiscardTransferEmitsOneStrongEvent") {
    eve::card::Card card;
    REQUIRE_EQ(card.registerCardsFromJson(
                   R"([{"id":"creature","kind":"creature"},{"id":"spell","kind":"spell"}])"),
               2);
    ...
    eve::card::CardContainerAdapter source(cid("card:deck"), eve::card::CardContainerKind::Deck,
                                           deck);
    eve::card::CardContainerAdapter destination(
        cid("card:hand"), eve::card::CardContainerKind::Hand, nullptr, hand,
        nullptr, Capacity::fixed(2));
    std::vector<eve::container::TransferEvent> events;
    eve::container::TransferRequest request{&source, &destination,
                                            mid(creature->identity()->id.c_str()), SlotIndex(0),
    ...
    eve::card::CardContainerAdapter discardAdapter(
        cid("card:discard"), eve::card::CardContainerKind::Discard, nullptr, nullptr, &discard);
    auto toDiscard = eve::container::TransferService::transfer(
        eve::container::TransferRequest{&destination, &discardAdapter,
                                        mid(creature->identity()->id.c_str()), SlotIndex(0),
                                        std::nullopt, destination.revision(), discardAdapter.revision()},
...
TEST_CASE("container.cardTransferRejectsFilterAndDuplicateWithoutMutation") {
    ...
    REQUIRE(creature != nullptr);
    deck->push(creature);
    eve::card::CardContainerAdapter source(cid("card:deck-filter"), eve::card::CardContainerKind::Deck,
                                           deck);
    eve::card::CardContainerAdapter destination(
        cid("card:spell-hand"), eve::card::CardContainerKind::Hand, nullptr, hand, nullptr,
        Capacity::fixed(2), {"spell"});
    auto rejected = eve::container::TransferService::transfer(
        eve::container::TransferRequest{&source, &destination,
                                        mid(creature->identity()->id.c_str()), SlotIndex(0),
                                        SlotIndex(0), source.revision(), destination.revision()});
    CHECK(!rejected.ok());
    ...
    eve::card::CardContainerAdapter unrestrictedDestination(
        cid("card:duplicate-hand"), eve::card::CardContainerKind::Hand, nullptr, hand, nullptr,
        Capacity::fixed(2));
    auto duplicate = eve::container::TransferService::transfer(
        eve::container::TransferRequest{&source, &unrestrictedDestination,
                                        mid(creature->identity()->id.c_str()), SlotIndex(0),
                                        SlotIndex(0), source.revision(), unrestrictedDestination.revision()});
    CHECK(!duplicate.ok());
    CHECK_EQ(deck->count(), 1);
    CHECK_EQ(hand->count(), 1);
}
...
TEST_CASE("container.transferRestoresSourceAfterApplyFailure") {
    ...
    REQUIRE(creature != nullptr);
    deck->push(creature);
    eve::card::CardContainerAdapter source(cid("card:rollback"), eve::card::CardContainerKind::Deck,
                                           deck);
...
TEST_CASE("container.inventoryBagAndEquipmentTransferRejectsStaleAndFits") {
    ...
    auto equipped = eve::container::TransferService::transfer(
        eve::container::TransferRequest{&bagAdapter, &equipmentAdapter, object, SlotIndex(0),
                                        SlotIndex(0), oldRevision, equipmentAdapter.revision()});
    REQUIRE(equipped.ok());
    CHECK(bag.isSlotEmpty(0));
    CHECK_EQ(equipment.getSlotItemId("mainhand"), std::string("sword"));

    auto stale = eve::container::TransferService::transfer(
        eve::container::TransferRequest{&bagAdapter, &equipmentAdapter, object, SlotIndex(0),
                                        SlotIndex(0), oldRevision, equipmentAdapter.revision()});
    CHECK(!stale.ok());
    ...
    auto unequipped = eve::container::TransferService::transfer(
        eve::container::TransferRequest{&equipmentAdapter, &bagAdapter, object, SlotIndex(0),
                                        SlotIndex(1), equipmentAdapter.revision(), bagAdapter.revision()});
...
TEST_CASE("container.sameContainerMoveUsesFinalSlotAndRejectsOccupiedSlot") {
    ...
    auto moved = eve::container::TransferService::transfer(
        eve::container::TransferRequest{&adapter, &adapter, first, SlotIndex(0), SlotIndex(2),
                                        revision, revision});
    REQUIRE(moved.ok());
    ...
    auto occupied = eve::container::TransferService::transfer(
        eve::container::TransferRequest{&adapter, &adapter, second, SlotIndex(1), SlotIndex(2),
                                        afterMoveRevision, afterMoveRevision});
    CHECK(!occupied.ok());
    CHECK_EQ(bag.getSlotInstanceId(2), std::stoi(first.value().substr(10)));
    ItemRegistry::clear();
}
