...
TEST_CASE("editor.v2.selection_and_focus_are_presentation_neutral_channels") {
    ...
    int                    notifications = 0;
    REQUIRE(selection.subscribe("park.hud", [&](const SelectionSnapshot&) { ++notifications; }).isAccepted());
    ...
    REQUIRE(selection.clear("world").isAccepted());
