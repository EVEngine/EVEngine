...
public:
    EditorResult<eve::editor::AuthorityPlan> preflight(
        const TransactionSpec&, std::span<const DomainOperation>) override {
        ++preflightCount;
        return EditorResult<eve::editor::AuthorityPlan>::error(
            EditorStatus::Conflict, eve::editor::RuleId("test.preflight.rejected"),
    ...
    EditorResult<TransactionReceipt> commit(const eve::editor::AuthorityPlan&) override {
        ...
        return EditorResult<TransactionReceipt>::error(
            EditorStatus::Failed, eve::editor::RuleId("test.commit.unexpected"),
    ...
    EditorResult<TransactionReceipt> compensate(const TransactionReceipt&) override {
        ++compensateCount;
        return EditorResult<TransactionReceipt>::error(
            EditorStatus::Failed, eve::editor::RuleId("test.compensate.unexpected"),
...
std::unique_ptr<eve::editor::IntFieldEditCommand> makeTileCommand(
    eve::editor::TileBufferTarget& target, int value) {
...
TEST_CASE("editor.transactionConsumer.secondParticipantFailureCompensatesEditor") {
    ...
    auto result = consumer.commit(
        std::span<ITransactionParticipant*>(additional.data(), additional.size()));
    CHECK(!result.ok());
    CHECK_EQ(buffer.getGid(0, 0), 0);
    ...
    auto duplicateCommit = consumer.commit(
        std::span<ITransactionParticipant*>(additional.data(), additional.size()));
