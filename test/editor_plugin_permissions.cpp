...
TEST_CASE("editor.plugins.permissions_are_narrow_reversible_and_default_to_ask") {
    ...
    CHECK(permissions.applyDomainOperation(*set.value).isAccepted());
    ...
    CHECK(permissions.applyDomainOperation(undo).isAccepted());
...
TEST_CASE("editor.plugins.permission_snapshot_is_atomic_and_rejects_duplicate_semantics") {
    ...
    REQUIRE(set.value); CHECK(permissions.applyDomainOperation(*set.value).isAccepted());
