...
inline EventId eventId(std::uint64_t serial) {
    ...
    for (int index = 0; index < 8; ++index)
        bytes[8 + index] = static_cast<std::uint8_t>(bytes[8 + index] ^
                                                     (serial >> ((7 - index) * 8)));
...
inline GameEvent envelope(std::uint64_t serial, std::string type, std::string source,
                          std::string subject, std::string causation,
                          std::string correlation, std::uint64_t tick, std::uint32_t flags,
    ...
    if (!causation.empty()) {
        ...
        result.causation = parsed ? CausationRef::fromEventId(*parsed)
                                  : CausationRef::fromLegacy(std::move(causation));
    }
    if (!correlation.empty()) {
        const auto parsed = EventId::parse(correlation);
        result.correlation = parsed ? CorrelationId::fromEventId(*parsed)
                                    : CorrelationId::fromLegacy(std::move(correlation));
    }
    if (const auto schema = eve::LogicalId::fromParts("game_event", result.type))
