...
TEST_CASE("editor.timeline_filters_correlations_and_rejects_stale_pages") {
    ...
    CHECK(timeline.append({0, 10, "network", "connected", "peer-1", "session-a",
                           DiagnosticSeverity::Info, EditorValue::Object{}}).accepted());
    CHECK(timeline.append({0, 12, "network", "timeout", "peer-2", "session-b",
                           DiagnosticSeverity::Warning, EditorValue::Object{{"retry", int64_t{2}}}}).accepted());
    ...
    CHECK(timeline.append({0, 13, "filesystem", "modified", "asset", {},
                           DiagnosticSeverity::Info, {}}).accepted());
...
TEST_CASE("editor.timeline_capacity_drops_oldest_and_payload_is_bounded") {
    EditorEventTimeline timeline;
    CHECK(timeline.setCapacity(2).accepted());
    CHECK(timeline.append({0, 1, "a", "one", {}, {}, DiagnosticSeverity::Info, {}}).accepted());
    CHECK(timeline.append({0, 2, "a", "two", {}, {}, DiagnosticSeverity::Info, {}}).accepted());
    CHECK(timeline.append({0, 3, "a", "three", {}, {}, DiagnosticSeverity::Info, {}}).accepted());
