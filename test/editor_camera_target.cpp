...
TEST_CASE("editor.camera.timeline_has_stable_keys_reference_safety_and_scrub") {
    ...
    CHECK_EQ(static_cast<int>(target.makeDeleteRig(ObjectId("hero")).code()), static_cast<int>(EditorStatus::Conflict));
    CameraPreview preview; auto pose=preview.evaluate(target,2); REQUIRE(pose.ok());
    CHECK_EQ(pose.value().rig.value(),std::string("hero")); CHECK_EQ(pose.value().fov,70.f);
    auto overlay=preview.gizmo(target,2); REQUIRE(overlay.ok()); CHECK_EQ(overlay.value().primitives.size(),static_cast<std::size_t>(3));
}
...
TEST_CASE("editor.camera.invalid_snapshot_and_runtime_generation_are_atomic") {
    ...
    CameraDocumentRuntime runtime; REQUIRE(runtime.publish(target,nullptr).ok());
    auto* generation=runtime.controller(); const auto revision=runtime.revision();
    CameraDocumentTarget empty("empty"); REQUIRE(runtime.publish(empty,nullptr).ok());
