...
TEST_CASE("card.playConditionAdapterReadsCardAndExternalCapabilities") {
    ...
    queries.policy = [&policyCalls](std::string_view name,
        ...
        ++policyCalls;
        return name == "card.safe" ? std::optional<ConditionResult>(ConditionResult::success())
                                    : std::nullopt;
    };
    ...
    const auto blocked = CardPlayConditionAdapter::evaluate(
        card, local, Condition::stateEquals("card.phase", "hand"), queries);
...
TEST_CASE("card.playConditionIsStoredOnDefinitionAndEvaluatedByCardFacade") {
    ...
    Card mod;
    REQUIRE_EQ(mod.registerCardsFromJson(
                    R"JSON([{"id":"ward","kind":"spell","cost":2,"tags":["ward"]}])JSON"),
                1);
