...
public:
    ...
    void        registerEditor(IEditorExtensionRegistry& registry) override {
        ...
        registry.registerTool({ToolId("park.tree-tool"), {}, "Tree", "Build", {}, ExtensionAudience::Player});
        registry.registerRule({RuleId("park.path-rule"), {}, ExtensionAudience::Player});
    }
};
...
TEST_CASE("editor.v2.runtime_manifest_is_explicit_and_closes_asset_dependencies") {
    ...
    REQUIRE(extensions.load(park).accepted());
    ...
    REQUIRE(assets
                ...
                .accepted());
    REQUIRE(assets.publish(runtimeAsset("tree", "content://Tree")).accepted());
    REQUIRE(assets.publish(runtimeAsset("source", "content://TreeSource")).accepted());
