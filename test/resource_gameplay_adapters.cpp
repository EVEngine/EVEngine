...
public:
    ...
    [[nodiscard]] eve::Result<void> prepare(
        ...
        if (prepared || committed)
            return eve::Result<void>::failure(injectedFailure("effect.reentry"));
        prepared = true;
    ...
    [[nodiscard]] eve::Result<void> commit(
        const eve::transaction::TransactionContext&) override {
        if (!prepared || committed)
            return eve::Result<void>::failure(injectedFailure("effect.commit.state"));
    ...
    [[nodiscard]] eve::Result<void> rollback(
        const eve::transaction::TransactionContext&) override {
        if (committed)
            return eve::Result<void>::failure(injectedFailure("effect.rollback.committed"));
        prepared = false;
        return eve::Result<void>::success();
    }
...
TEST_CASE("resource.gameplay.attributeViewsReuseOneCanonicalAccount") {
    ...
    eve::attributes::StaminaAccountAdapter stamina(
        base, eve::attributes::AttributeResourceKind::Stamina);
...
TEST_CASE("resource.gameplay.cardManaPaymentIsAtomicAndUsesPlayerAccount") {
    ...
    auto paid = eve::card::CardPlayPaymentAdapter::play(*card, definition, playerAccount,
                                                         "card.firebolt.1");
    REQUIRE(paid.ok());
...
TEST_CASE("resource.gameplay.cardInsufficientManaLeavesEffectUnchanged") {
    ...
    auto rejected = eve::card::CardPlayPaymentAdapter::play(*card, definition, playerAccount,
                                                              "card.expensive.1");
    CHECK(!rejected.ok());
    CHECK_EQ(static_cast<int>(card->state()->phase),
             static_cast<int>(eve::card::CardState::Hand));
...
TEST_CASE("resource.gameplay.weaponAmmoUsesInventoryAndPrepareFailureDoesNotCharge") {
    ...
    auto fired = eve::weapon::WeaponActionAdapter::fire(weapon, account, effect,
                                                         "weapon.rifle.1");
    REQUIRE(fired.ok());
    ...
    auto rejected = eve::weapon::WeaponActionAdapter::fire(weapon, account, effect,
                                                            "weapon.rifle.2");
    CHECK(!rejected.ok());
    CHECK_EQ(bag.countItem("ammo"), 4);
    CHECK_EQ(effectValue, 1);
    ...
    auto commitRejected = eve::weapon::WeaponActionAdapter::fire(
        weapon, account, effect, "weapon.rifle.3");
...
TEST_CASE("resource.gameplay.cardPlayCompositionIsOneAtomicCrossWrite") {
    ...
    eve::card::CardContainerAdapter source(
        eve::container::ContainerId("card:atomic-deck"), eve::card::CardContainerKind::Deck, deck);
    eve::card::CardContainerAdapter destination(
        eve::container::ContainerId("card:atomic-hand"), eve::card::CardContainerKind::Hand,
        nullptr, hand, nullptr, eve::container::Capacity::fixed(2));
    eve::attributes::AttributeSet attributes;
    attributes.setBase("mana", 10.0);
