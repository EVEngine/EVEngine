...
TEST_CASE("editor.curve.timeline_is_reversible_sampled_and_serializable") {
    ...
    REQUIRE(start.value); REQUIRE(end.value); CHECK(curve.applyDomainOperation(*start.value).accepted()); CHECK(curve.applyDomainOperation(*end.value).accepted());
    ...
    REQUIRE(black.value); REQUIRE(white.value); CHECK(curve.applyDomainOperation(*black.value).accepted()); CHECK(curve.applyDomainOperation(*white.value).accepted());
    ...
    auto remove = curve.makeDeleteKey(StableId("end")); REQUIRE(remove.value); CHECK(curve.applyDomainOperation(*remove.value).accepted());
    DomainOperation undo = *remove.value; undo.type = remove.value->inverseType; undo.payload = remove.value->inverse;
    CHECK(curve.applyDomainOperation(undo).accepted()); CHECK_EQ(curve.keys().size(), 2U);
    EditorCurveDocument restored("particle-size-color"); CHECK(restored.loadSnapshot(curve.snapshotValue()).accepted());
