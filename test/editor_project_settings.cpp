...
SelectionSnapshot selection(const ProjectSettingsTarget& target) {
    ...
    SelectionItem item; item.target = TargetId(target.targetId()); item.item = StableId(target.targetId());
...
TEST_CASE("editor.settings.secret_references_are_redacted_and_raw_values_rejected") {
    ...
    CHECK_EQ(static_cast<int>(raw.status), static_cast<int>(EditorStatus::Rejected));
    ...
    CHECK(settings.applyDomainOperation(*secret.value).isAccepted());
...
TEST_CASE("editor.settings_changes_are_reversible_restart_aware_and_atomic") {
    ...
    auto change = settings.makeSet(selected, PropertyPath("content.asset-root"), "content",
                                   PropertySetMode::Absolute);
    REQUIRE(change.value); CHECK(settings.applyDomainOperation(*change.value).isAccepted());
    ...
    CHECK(settings.applyDomainOperation(undo).isAccepted());
    ...
    CHECK_EQ(static_cast<int>(settings.loadSnapshot(invalid).status), static_cast<int>(EditorStatus::Rejected));
    CHECK(settings.read(selected, PropertyPath("content.asset-root")).value == EditorValue("assets"));
}
