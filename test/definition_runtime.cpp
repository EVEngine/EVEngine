...
TEST_CASE("definitionRuntime.policiesAndGenerationQualifiedStaleChecks") {
    ...
    auto created = eve::definition::RuntimeInstance<TypedState>::create(
        testId(), reference, eve::Generation(1), TypedState{7, "original"});
    REQUIRE(created.ok());
    auto runtime = std::move(created).takeValue();

    auto wrongGeneration = runtime.checkDefinition(
        eve::definition::DefinitionHandle{reference, eve::Generation(2)});
    CHECK(!wrongGeneration.ok());
    CHECK(static_cast<int>(wrongGeneration.error()->code()) ==
          static_cast<int>(eve::DiagnosticCode::StaleHandle));

    auto kept = runtime.reload(
        eve::definition::DefinitionHandle{reference, eve::Generation(2)},
        eve::definition::ReloadPolicy::KeepInstanceValues, TypedState{99, "new defaults"});
    REQUIRE(kept.ok());
    CHECK(static_cast<int>(kept.value().disposition) ==
    ...
    auto reapplied = runtime.reload(
        eve::definition::DefinitionHandle{reference, eve::Generation(3)},
    ...
    auto rebuilt = runtime.reload(
        eve::definition::DefinitionHandle{reference, eve::Generation(4)},
    ...
    auto failedRebuild = runtime.reload(
        eve::definition::DefinitionHandle{reference, eve::Generation(5)},
        ...
        [](const TypedState&, const eve::definition::InstanceIdentity&,
           const eve::definition::DefinitionHandle&) -> eve::Result<TypedState> {
            return eve::Result<TypedState>::failure(
                eve::Diagnostic::error(eve::DiagnosticCode::Failed, "injected rebuild failure"));
        });
    CHECK(!failedRebuild.ok());
    CHECK_EQ(runtime.identity().definitionGeneration.value(), uint64_t{4});
    CHECK_EQ(runtime.state().value, 43);

    auto rejected = runtime.reload(
        eve::definition::DefinitionHandle{reference, eve::Generation(5)},
    ...
    auto inactiveReject = runtime.reload(
        eve::definition::DefinitionHandle{reference, eve::Generation(5)},
...
TEST_CASE("definitionRuntime.cardAdapterProjectsTypedIdentityAndReloads") {
    ...
    auto inserted = registry.insert(
        "card", "scout", 1,
        R"({"name":"Scout","kind":"creature","cost":2,"attack":3,"health":5,"tags":["unit"]})");
    REQUIRE(inserted.ok());

    auto runtimeResult = eve::card::CardDefinitionRuntime::create(
        registry, testReference("card:scout"), testId(),
...
TEST_CASE("definitionRuntime.effectAdapterKeepsTypedPolicyAndRejectsActiveReload") {
    ...
    REQUIRE(runtimeResult.ok());
    auto runtime = std::move(runtimeResult).takeValue();
    CHECK(static_cast<int>(runtime.state().policy.stackMode) ==
          static_cast<int>(eve::effects::StackMode::Reuse));
    ...
    CHECK(static_cast<int>(instance.policy.stackMode) ==
          static_cast<int>(eve::effects::StackMode::Reuse));
