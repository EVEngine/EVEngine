...
TEST_CASE("editor.social.graph_edges_are_reversible_validated_and_publishable") {
    ...
    for (const SocialEntityRecord& entity : std::vector<SocialEntityRecord>{
        ...
        CHECK(document.applyDomainOperation(*operation.value).accepted());
    }
    ...
    CHECK(document.applyDomainOperation(*set.value).accepted());
    ...
    CHECK(SocialRuntimeApplier().apply(document, &runtime).accepted());
    ...
    CHECK(document.makeDeleteEntity(StableId("unit")).accepted());
}

TEST_CASE("editor.social.snapshot_rejects_dangling_or_duplicate_semantics_atomically") {
    SocialDocumentTarget source("source");
    CHECK(source.applyDomainOperation(*source.makeSetEntity({StableId("a"), "A", "actor"}).value).accepted());
    CHECK(source.applyDomainOperation(*source.makeSetEntity({StableId("b"), "B", "actor"}).value).accepted());
    CHECK(source.applyDomainOperation(*source.makeSetEdge({StableId("likes"), StableId("a"), StableId("b"), "relation", "likes", 0.8}).value).accepted());
