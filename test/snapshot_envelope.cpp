...
SnapshotEnvelope makeEnvelope(std::string type, LogicalId schema, SchemaVersion version,
                              PersistentId instanceId, Value payload,
                              const SnapshotHashProvider& hashProvider) {
    auto result = eve::makeSnapshotEnvelope(std::move(type), std::move(schema), version, instanceId,
                                            eve::Revision(7), SimulationTick(11), std::move(payload),
                                            hashProvider);
    REQUIRE(result.ok());
...
TEST_CASE("snapshot.envelope.canonicalHashRoundTripAndNoWallClock") {
    ...
    const auto envelope = makeEnvelope("test.snapshot", logicalId("test:payload"), SchemaVersion(1),
                                       persistentId("01234567-89ab-4cde-8fab-0123456789ab"),
                                       Value(std::move(payloadObject)), hash);
...
TEST_CASE("snapshot.migration.rejectsUnknownNewVersionAndHashMismatch") {
    ...
    auto registration = chain.add(
        schema, SchemaVersion(0), SchemaVersion(1), [](const Value& payload) -> Result<Value> {
    ...
    auto valid = makeEnvelope("test.snapshot", schema, SchemaVersion(1), PersistentId::nil(),
                              Value(Value::Object{}), hash);
...
TEST_CASE("snapshot.consumers.malformedPayloadAndUnknownVersionLeaveStateUnchanged") {
    ...
    auto malformedAuthorityEnvelope = makeEnvelope(
        "authority.store", logicalId("authority:store"), SchemaVersion(1), authorityId,
        Value(std::move(malformedAuthority)), hash);
    ...
    auto newerAuthorityEnvelope = makeEnvelope(
        "authority.store", logicalId("authority:store"), SchemaVersion(2), authorityId,
        Value(std::move(newerAuthority)), hash);
    ...
    auto malformedDefinitionsEnvelope = makeEnvelope(
        "definitions.registry", logicalId("definitions:registry"), SchemaVersion(1), definitionsId,
