...
public:
    explicit TestPropertyAccess(eve::property_access::PropertySchema schema = {})
        ...
        for (const auto& property : schema_.properties) values_.emplace(property.path, property.defaultValue);
    }
    ...
    eve::Subscription subscribe(ChangeCallback callback) override {
        ...
        return eve::Subscription([weak, id]() {
            if (const auto state = weak.lock())
                std::erase_if(state->entries, [id](const Entry& entry) { return entry.id == id; });
        });
    }
