...
public:
    ...
    eve::Result<void> prepare(const eve::transaction::TransactionContext&) override {
        ...
        if (failPrepare)
            return eve::Result<void>::failure(
                eve::Diagnostic::error(eve::DiagnosticCode::Failed, name_ + " prepare"));
        prepared = true;
    ...
    eve::Result<void> commit(const eve::transaction::TransactionContext&) override {
        ...
        if (failCommit)
            return eve::Result<void>::failure(
                eve::Diagnostic::error(eve::DiagnosticCode::Failed, name_ + " commit"));
        prepared = false;
        committed = true;
        return eve::Result<void>::success();
    }
    ...
    eve::Result<void> rollback(const eve::transaction::TransactionContext&) override {
        ...
        if (failRollback)
            return eve::Result<void>::failure(
                eve::Diagnostic::error(eve::DiagnosticCode::Failed, name_ + " rollback"));
        prepared = false;
        return eve::Result<void>::success();
    }
...
public:
    ...
    eve::Result<std::unique_ptr<PreparedArtifactPublish>> prepare(
        const eve::artifact::PublicationView&) override {
        ++counters_.prepared;
        if (fail_) return eve::Result<std::unique_ptr<PreparedArtifactPublish>>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::Failed, "injected adapter failure"));
        return eve::Result<std::unique_ptr<PreparedArtifactPublish>>::success(
            std::make_unique<Stage>(counters_));
    }
private:
    Counters& counters_;
    bool fail_;
...
TEST_CASE("procgen.artifactPublish.missingCapabilityIsExplicitAndAtomic") {
    ...
    auto generated = generateHexTerrainArtifact(
        params(), id("018f0b7e-6e50-7a10-8c22-2c8f8e3dd101"));
...
TEST_CASE("procgen.artifactPublish.capabilitiesCommitHexAndCastle") {
    ...
    ArtifactStore store;
    ArtifactPublisher publisher(store);
    auto hex = generateHexTerrainArtifact(params(),
        id("018f0b7e-6e50-7a10-8c22-2c8f8e3dd102"));
    REQUIRE(hex.ok());
    auto first = publisher.publish(std::move(hex).takeValue(), {true, true, true});
    REQUIRE(first.ok());
    std::move(first).takeValue();
    auto castle = generateCastleArtifact(params(),
        id("018f0b7e-6e50-7a10-8c22-2c8f8e3dd103"));
    REQUIRE(castle.ok());
...
TEST_CASE("procgen.artifactPublish.adapterFailureRollsBackWithoutStoreState") {
    ...
    eve::cap::provide<ISceneArtifactAdapter>(&scene);
    eve::cap::provide<IGraphicsArtifactAdapter>(&graphics);
    auto generated = generateHexTerrainArtifact(params(),
        id("018f0b7e-6e50-7a10-8c22-2c8f8e3dd104"));
    REQUIRE(generated.ok());
    ArtifactStore store;
    ArtifactPublisher publisher(store);
...
TEST_CASE("procgen.artifactPublish.badDependencyRejectsWholeBatch") {
    auto first = generateHexTerrainArtifact(params(),
        id("018f0b7e-6e50-7a10-8c22-2c8f8e3dd105"));
    auto second = generateCastleArtifact(params(),
        id("018f0b7e-6e50-7a10-8c22-2c8f8e3dd106"));
    REQUIRE(first.ok());
    REQUIRE(second.ok());
...
TEST_CASE("procgen.artifactPublish.buildKeyAndPayloadAreDeterministicAcrossIdentity") {
    auto first = generateHexTerrainArtifact(params(),
        id("018f0b7e-6e50-7a10-8c22-2c8f8e3dd107"));
    auto second = generateHexTerrainArtifact(params(),
        id("018f0b7e-6e50-7a10-8c22-2c8f8e3dd108"));
    REQUIRE(first.ok());
    REQUIRE(second.ok());
    ...
    CHECK_EQ(std::get<MeshData>(aMesh->payload).positions(),
             std::get<MeshData>(bMesh->payload).positions());
    CHECK_EQ(std::get<MeshData>(aMesh->payload).indices(),
             std::get<MeshData>(bMesh->payload).indices());
}
...
TEST_CASE("procgen.artifactPublish.coordinatorFailureInjectionCoversPrepareCommitAndRollback") {
    ...
    auto prepareResult = coordinator.execute(
        eve::transaction::TransactionContext("procgen-prepare-failure"), prepareParticipants);
    ...
    auto commitResult = coordinator.execute(
        eve::transaction::TransactionContext("procgen-commit-failure"), commitParticipants);
    ...
    auto rollbackResult = coordinator.execute(
        eve::transaction::TransactionContext("procgen-rollback-failure"), rollbackParticipants);
