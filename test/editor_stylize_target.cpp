...
void apply(StylizeRecipeTarget&target,EditorResult<DomainOperation> operation){REQUIRE(operation.value);REQUIRE(target.applyDomainOperation(*operation.value).accepted());}
}
...
TEST_CASE("editor.stylize.recipe_supports_dynamic_parameters_order_and_undo") {
 ...
 auto parameter=target.makeSet(selection,PropertyPath("parameter.pixelSize"),12.0,PropertySetMode::Absolute);REQUIRE(parameter.value);REQUIRE(target.applyDomainOperation(*parameter.value).accepted());
 CHECK_EQ(target.passes()[1].overrides.at("pixelSize"),12.0);DomainOperation undo=*parameter.value;undo.payload=parameter.value->inverse;REQUIRE(target.applyDomainOperation(undo).accepted());CHECK(target.passes()[1].overrides.empty());
...
TEST_CASE("editor.stylize.recipe_rejects_mixed_stages_and_loads_atomically") {
 ...
 StylizeRecipeTarget restored("copy");REQUIRE(restored.loadSnapshot(before).accepted());CHECK_EQ(restored.passes()[0].style,std::string("cartoon"));
}
