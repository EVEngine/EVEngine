...
public:
    ...
    eve::Result<LineOfSightResult> query(const TargetLocation& from,
        ...
        if (from.index() == 1)
            return eve::Result<LineOfSightResult>::failure(eve::Diagnostic::error(
                eve::DiagnosticCode::Unsupported, "test LOS does not support grid coordinates"));
...
TEST_CASE("targeting.providerAndResolverApplyGenericConstraints") {
    ...
    REQUIRE(provider.upsert(TargetCandidate{enemy, world2D(3.f, 4.f), TargetDomain::Enemy,
                                            {"unit", "visible"}, {*zone}}).ok());
    REQUIRE(provider.upsert(TargetCandidate{farEnemy, world2D(50.f, 0.f), TargetDomain::Enemy,
                                            {"unit"}, {}}).ok());
    REQUIRE(provider.upsert(TargetCandidate{ally, world2D(2.f, 0.f), TargetDomain::Ally,
                                            {"unit", "visible"}, {}}).ok());
