...
TEST_CASE("editor.scene_import.presets_expand_and_custom_edits_are_reversible"){
 ...
 auto camera=target.makeSet(selection,PropertyPath("importCameras"),true,PropertySetMode::Absolute);REQUIRE(camera.value);REQUIRE(target.applyDomainOperation(*camera.value).isAccepted());CHECK_EQ(target.value().preset,std::string("custom"));DomainOperation undo=*camera.value;undo.payload=camera.value->inverse;REQUIRE(target.applyDomainOperation(undo).isAccepted());CHECK_EQ(target.value().preset,std::string("mobile"));CHECK(!target.value().importCameras);
}
