...
TEST_CASE("editor.voxel_palette.publication_is_atomic_and_reports_variant_ids"){
 VoxelPaletteTarget target("blocks");apply(target,target.makeCreate(cube("stone")));apply(target,target.makeCreate(cube("furnace",true)));VoxelPaletteRuntime runtime;auto result=runtime.publish(target);REQUIRE(result.value);CHECK_EQ(result.value->size(),static_cast<std::size_t>(2));CHECK_EQ((*result.value)[0].baseId,1);CHECK_EQ((*result.value)[1].baseId,2);CHECK_EQ((*result.value)[1].variants,4);REQUIRE(runtime.registry());CHECK_EQ(int(runtime.registry()->variantId("furnace",3)),5);CHECK(runtime.registry()->find("furnace")!=nullptr);
}
TEST_CASE("editor.voxel_palette.invalid_snapshot_does_not_partially_replace"){
 VoxelPaletteTarget target("blocks");apply(target,target.makeCreate(cube("stone")));const auto before=target.snapshotValue();EditorValue invalid=before;auto*root=invalid.getIf<EditorValue::Object>();auto*content=(*root)["content"].getIf<EditorValue::Object>();auto*entries=(*content)["entries"].getIf<EditorValue::Array>();auto*entry=(*entries)[0].getIf<EditorValue::Object>();auto*faces=(*entry)["faces"].getIf<EditorValue::Array>();(*faces)[0]=int64_t{999};CHECK_EQ(static_cast<int>(target.loadSnapshot(invalid).status),static_cast<int>(EditorStatus::Rejected));CHECK_EQ(target.snapshotValue(),before);
}
