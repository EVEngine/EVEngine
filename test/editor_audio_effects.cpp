...
TEST_CASE("editor.audio.effects_are_typed_reversible_ordered_and_assignable") {
    ...
    REQUIRE(first.value);REQUIRE(second.value);CHECK(chain.applyDomainOperation(*first.value).isAccepted());
    CHECK(chain.applyDomainOperation(*second.value).isAccepted());
    auto reorder=chain.makeReorder({StableId("post"),StableId("pre")});REQUIRE(reorder.value);
    CHECK(chain.applyDomainOperation(*reorder.value).isAccepted());CHECK_EQ(chain.effects().front().id.value(),std::string("post"));
    DomainOperation undo=*reorder.value;undo.type=reorder.value->inverseType;undo.payload=reorder.value->inverse;
    CHECK(chain.applyDomainOperation(undo).isAccepted());CHECK_EQ(chain.effects().front().id.value(),std::string("pre"));

    AudioMixerTarget mixer("mixer");auto bus=mixer.makeCreate({ObjectId("music"),ObjectId("master"),"Music"});
    REQUIRE(bus.value);CHECK(mixer.applyDomainOperation(*bus.value).isAccepted());
    auto assign=chain.makeAssignToBus(mixer,ObjectId("music"));REQUIRE(assign.value);
    CHECK(mixer.applyDomainOperation(*assign.value).isAccepted());
...
TEST_CASE("editor.audio.effect_publisher_rejects_stale_revision_and_snapshot_is_atomic") {
    ...
    CHECK(chain.applyDomainOperation(*set.value).isAccepted());Sink sink;AudioEffectChainPublisher publisher;
    CHECK_EQ(static_cast<int>(publisher.publish(chain,chain.revision()-1,sink).status),static_cast<int>(EditorStatus::Conflict));
    CHECK(publisher.publish(chain,chain.revision(),sink).isAccepted());CHECK_EQ(sink.effects.size(),1U);
