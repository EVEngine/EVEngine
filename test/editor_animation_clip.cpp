...
TEST_CASE("editor.animation.clip_timeline_supports_stable_reversible_tracks_events_and_masks") {
    ...
    CHECK(clip.applyDomainOperation(*settings.value).accepted());
    AnimationBoneTrack track{StableId("hips-track"), "mixamorig:Hips", {key("start", 0.0, 0.0), key("end", 2.0, 4.0)}};
    auto setTrack = clip.makeSetTrack(track); REQUIRE(setTrack.value); CHECK(clip.applyDomainOperation(*setTrack.value).accepted());
    auto event = clip.makeSetEvent({StableId("footstep"), 0.5, "footstep", "left"}); REQUIRE(event.value); CHECK(clip.applyDomainOperation(*event.value).accepted());
    auto mask = clip.makeSetMask({"mixamorig:Hips", 0.25}); REQUIRE(mask.value); CHECK(clip.applyDomainOperation(*mask.value).accepted());
    ...
    auto remove = clip.makeDeleteTrack(StableId("hips-track")); REQUIRE(remove.value); CHECK(clip.applyDomainOperation(*remove.value).accepted());
    DomainOperation undo = *remove.value; undo.type = remove.value->inverseType; undo.payload = remove.value->inverse;
    CHECK(clip.applyDomainOperation(undo).accepted()); CHECK_EQ(clip.tracks().size(), 1U);
}

TEST_CASE("editor.animation.clip_snapshot_and_retarget_preview_are_non_destructive") {
    AnimationClipDocumentTarget clip("source");
    auto track = clip.makeSetTrack({StableId("hips"), "mixamorig:Hips", {key("key", 0.0, 1.0)}}); REQUIRE(track.value); CHECK(clip.applyDomainOperation(*track.value).accepted());
    ...
    AnimationClipDocumentTarget restored("source"); CHECK(restored.loadSnapshot(clip.snapshotValue()).accepted()); CHECK_EQ(restored.tracks().size(), 1U);
}
...
TEST_CASE("editor.animation.clip_builds_real_runtime_asset_and_rejects_missing_bones") {
    ...
    auto track = document.makeSetTrack({StableId("root-track"), "root", {key("a", 0.0, 0.0), key("b", 1.0, 2.0)}}); REQUIRE(track.value); CHECK(document.applyDomainOperation(*track.value).accepted());
    auto event = document.makeSetEvent({StableId("event"), 0.25, "step", "right"}); REQUIRE(event.value); CHECK(document.applyDomainOperation(*event.value).accepted());
