...
inline eve::Result<std::uint64_t> runtimeSnapshotUint(const eve::Value& value,
                                                       std::string_view path) {
    const auto text = value.getIf<std::string>();
    if (text == nullptr || text->empty())
        return eve::Result<std::uint64_t>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::ParseError, "runtime snapshot integer must be a decimal string",
    ...
    if (error != std::errc{} || end != text->data() + text->size())
        return eve::Result<std::uint64_t>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::ParseError, "runtime snapshot integer is out of range",
            std::string(path), {}, "common.definitions.snapshot"));
...
inline eve::Result<void> runtimeSnapshotShape(const eve::Value::Object& object) {
    static const std::set<std::string> fields = {
        "active", "definition", "definitionGeneration", "instanceId", "state"};
    for (const auto& [name, unused] : object) {
        (void)unused;
        if (!fields.contains(name))
            return eve::Result<void>::failure(eve::Diagnostic::error(
                eve::DiagnosticCode::ParseError,
                "runtime snapshot payload contains an unknown field", "state." + name, {},
    ...
    for (const auto& name : fields) {
        if (!object.contains(name))
            return eve::Result<void>::failure(eve::Diagnostic::error(
                eve::DiagnosticCode::ParseError,
                "runtime snapshot payload is missing a required field", "state." + name, {},
                "common.definitions.snapshot"));
    }
...
[[nodiscard]] eve::Result<eve::SnapshotEnvelope> snapshotRuntimeInstance(
    const RuntimeInstance<State>& runtime, std::string type, eve::LogicalId schema,
    eve::SchemaVersion schemaVersion, eve::Revision revision, eve::SimulationTick tick,
    const eve::SnapshotHashProvider& hashProvider, const RuntimeStateEncoder<State>& encode) {
    if (!encode)
        return eve::Result<eve::SnapshotEnvelope>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::InvalidArgument, "runtime snapshot encoder is required", "encode",
            {}, "common.definitions.snapshot"));
    ...
    return eve::makeSnapshotEnvelope(std::move(type), std::move(schema), schemaVersion,
                                     runtime.identity().instanceId, revision, tick,
                                     eve::Value(std::move(payload)), hashProvider);
}
...
[[nodiscard]] eve::Result<void> restoreRuntimeInstance(
    RuntimeInstance<State>& runtime, const eve::SnapshotEnvelope& snapshot,
    std::string_view expectedType, const eve::LogicalId& expectedSchema,
    eve::SchemaVersion expectedVersion, const eve::SnapshotHashProvider& hashProvider,
    const RuntimeStateDecoder<State>& decode) {
    if (snapshot.type != expectedType || snapshot.schema != expectedSchema)
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::InvalidArgument, "runtime snapshot type or schema does not match",
    ...
    if (snapshot.schemaVersion != expectedVersion)
        return eve::Result<void>::failure(eve::Diagnostic::error(
            ...
            "runtime snapshot schema version is not supported", "schemaVersion", {},
            "common.definitions.snapshot"));
    auto verified = eve::verifySnapshotEnvelope(snapshot, hashProvider);
    if (!verified) return verified;
    auto metadata = eve::validateSnapshotPayloadMetadata(snapshot.payload, snapshot.revision,
                                                         snapshot.tick);
    if (!metadata) return metadata;
    const auto object = snapshot.payload.getIf<eve::Value::Object>();
    if (object == nullptr) {
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::ParseError, "runtime snapshot payload must be an object", "payload",
            {}, "common.definitions.snapshot"));
    }
    ...
    if (active == nullptr || definitionText == nullptr || instanceText == nullptr)
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::ParseError, "runtime snapshot identity fields have invalid types",
            "payload", {}, "common.definitions.snapshot"));
    const auto instanceId = eve::PersistentId::parse(*instanceText);
    if (!instanceId)
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::ParseError, "runtime snapshot instanceId is invalid",
    ...
    auto generation = detail::runtimeSnapshotUint(object->at("definitionGeneration"),
                                                   "payload.definitionGeneration");
    if (!generation) return eve::Result<void>::failure(generation.status());
    const eve::definition::InstanceIdentity identity{
        *instanceId, std::move(definition).takeValue(),
    ...
    if (identity.definition != runtime.identity().definition)
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::Conflict, "runtime snapshot definition reference does not match",
            "payload.definition", {}, "common.definitions.snapshot"));
    if (identity.definitionGeneration != runtime.identity().definitionGeneration)
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::StaleHandle,
            "runtime snapshot definition generation does not match the destination",
            "payload.definitionGeneration", {}, "common.definitions.snapshot"));
    if (!decode)
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::InvalidArgument, "runtime snapshot decoder is required", "decode",
            {}, "common.definitions.snapshot"));
