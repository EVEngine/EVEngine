...
inline std::ostream& operator<<(std::ostream& stream, Severity severity) {
    return stream << severityName(severity);
}
...
public:
     ...
     */
    Diagnostic(DiagnosticCode code, Severity severity, std::string message,
               std::string path = {}, DiagnosticDetails details = {}, std::string source = {})
        : code_(code), severity_(severity), message_(std::move(message)), path_(std::move(path)),
          details_(std::move(details)), source_(std::move(source)) {}
    ...
    static Diagnostic error(DiagnosticCode code, std::string message, std::string path = {},
        ...
        return Diagnostic(code, Severity::Error, std::move(message), std::move(path),
                          std::move(details), std::move(source));
    }

    /** @brief Construct a warning diagnostic. */
    static Diagnostic warning(DiagnosticCode code, std::string message, std::string path = {},
                              DiagnosticDetails details = {}, std::string source = {}) {
        return Diagnostic(code, Severity::Warning, std::move(message), std::move(path),
                          std::move(details), std::move(source));
    }
    ...
    bool isError() const noexcept {
        return severity_ == Severity::Error || severity_ == Severity::Fatal;
    }
