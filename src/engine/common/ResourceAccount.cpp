...
eve::Result<AccountNonce> allocateAccountNonce() {
    ...
    for (;;) {
        if (value == 0 || value == std::numeric_limits<std::uint64_t>::max())
            return eve::Result<AccountNonce>::failure(
                invariantFailure("resource account nonce exhausted", "account"));
...
eve::Result<ResourceId> ResourceId::parse(std::string_view text) {
    if (text.empty())
        return eve::Result<ResourceId>::failure(
            invalidArgument("resource id must not be empty", "resource"));
    if (text.size() > 128)
        return eve::Result<ResourceId>::failure(
            invalidArgument("resource id is too long", "resource"));
    for (const unsigned char character : text) {
        if (std::iscntrl(character) || std::isspace(character))
            return eve::Result<ResourceId>::failure(
                invalidArgument("resource id must not contain whitespace or control characters",
                                "resource"));
    }
...
eve::Result<Amount> Amount::from(std::int64_t value) {
    if (value < 0)
        return eve::Result<Amount>::failure(
            invalidArgument("resource amount must not be negative", "amount"));
...
eve::Result<Amount> Amount::checkedAdd(Amount other) const {
    if (other.value_ > std::numeric_limits<std::int64_t>::max() - value_)
        return eve::Result<Amount>::failure(
            invariantFailure("resource amount addition overflowed", "amount"));
    return eve::Result<Amount>::success(Amount(value_ + other.value_));
}

eve::Result<ResourceCost> ResourceCost::create(std::string_view resourceId,
    ...
    if (amount.value().isZero())
        return eve::Result<ResourceCost>::failure(
            invalidArgument("resource cost amount must be positive", "amount"));
...
eve::Result<CostSpec> CostSpec::create(std::vector<ResourceCost> items) {
    if (items.empty())
        return eve::Result<CostSpec>::failure(
            invalidArgument("resource cost must contain at least one item", "cost"));
