...
[[nodiscard]] Result<void> failure(DiagnosticCode code, std::string message,
                                   std::string path = {}) {
    return Result<void>::failure(
        Diagnostic::error(code, std::move(message), std::move(path)));
}

[[nodiscard]] const MembershipEntry* findEntry(const ContainerSnapshot& snapshot,
...
[[nodiscard]] bool hasDuplicateMemberships(const ContainerSnapshot& snapshot) {
    ...
    for (const auto& entry : snapshot.entries) {
        if (!entry.membership.object.isValid() || !ids.insert(entry.membership.object).second)
            return true;
        if (!entry.membership.slot.isValid() ||
            !slots.insert(entry.membership.slot.value()).second)
            return true;
...
[[nodiscard]] Result<void> validateSnapshot(const ContainerSnapshot& snapshot,
    ...
    if (!snapshot.id.isValid() || snapshot.id != container.descriptor().id)
        return failure(DiagnosticCode::InvariantViolation,
                       "container snapshot identity does not match adapter");
    if (hasDuplicateMemberships(snapshot))
        return failure(DiagnosticCode::Conflict,
                       "container snapshot contains duplicate or invalid membership");
    ...
    if (!descriptor.capacity.isUnlimited()) {
        for (const auto& entry : snapshot.entries) {
            if (static_cast<std::size_t>(entry.membership.slot.value()) >=
...
[[nodiscard]] Result<ContainerSnapshot> removeEntry(const ContainerSnapshot& original,
    ...
    const auto it = std::find_if(candidate.entries.begin(), candidate.entries.end(),
                                 [&](const MembershipEntry& entry) {
                                     return entry.membership.object == object;
                                 });
    ...
    if (descriptor.ordering != Ordering::ExplicitSlots) {
        if (candidate.entries.size() >
            ...
            return Result<ContainerSnapshot>::failure(Diagnostic::error(
                DiagnosticCode::InvariantViolation, "container has too many indexed memberships"));
        for (std::size_t index = 0; index < candidate.entries.size(); ++index)
            candidate.entries[index].membership.slot =
                SlotIndex(static_cast<std::int32_t>(index));
    }
    return Result<ContainerSnapshot>::success(std::move(candidate));
}

[[nodiscard]] Result<SlotIndex> chooseDestinationSlot(
    const ContainerSnapshot& snapshot, const ContainerDescriptor& descriptor,
    ...
    if (requested) {
        ...
        if (descriptor.ordering == Ordering::ExplicitSlots) {
            if (!descriptor.capacity.isUnlimited() && value >= descriptor.capacity.value())
                return Result<SlotIndex>::failure(Diagnostic::error(
                    DiagnosticCode::Conflict, "destination slot exceeds container capacity"));
            for (const auto& entry : snapshot.entries)
    ...
    if (descriptor.ordering != Ordering::ExplicitSlots) {
        if (snapshot.entries.size() >
            ...
            return Result<SlotIndex>::failure(Diagnostic::error(
                DiagnosticCode::InvariantViolation, "container has too many indexed memberships"));
        return Result<SlotIndex>::success(
            SlotIndex(static_cast<std::int32_t>(snapshot.entries.size())));
    }

    const std::size_t limit = descriptor.capacity.isUnlimited()
    ...
    return Result<SlotIndex>::failure(
        Diagnostic::error(DiagnosticCode::Conflict, "container has no available slot"));
}

[[nodiscard]] Result<ContainerSnapshot> insertEntry(
    ContainerSnapshot base, const ContainerDescriptor& descriptor,
    const MembershipEntry& sourceEntry, std::optional<SlotIndex> requested) {
    auto destinationResult = chooseDestinationSlot(base, descriptor, requested);
    if (!destinationResult)
        return Result<ContainerSnapshot>::failure(destinationResult.status());
...
Result<TransferReceipt> TransferService::transfer(const TransferRequest& request,
    ...
    if (request.source == nullptr || request.destination == nullptr ||
        ...
        return Result<TransferReceipt>::failure(
            Diagnostic::error(DiagnosticCode::InvalidArgument,
                              "transfer requires two containers and a membership object"));
    }
    ...
    if (request.expectedSourceRevision &&
    ...
    if (request.expectedDestinationRevision &&
        *request.expectedDestinationRevision != destinationSnapshot.revision) {
        return Result<TransferReceipt>::failure(
            Diagnostic::error(DiagnosticCode::StaleHandle,
                              "destination container revision is stale"));
    }
    ...
    if (sourceEntry == nullptr) {
        ...
        return Result<TransferReceipt>::failure(Diagnostic::error(
            duplicate ? DiagnosticCode::Conflict : DiagnosticCode::NotFound,
    ...
    if (!sameContainer && request.source->descriptor().id == request.destination->descriptor().id) {
        return Result<TransferReceipt>::failure(Diagnostic::error(
            ...
            "different container adapters must not expose the same container identity"));
    }
        ...
        if (entry.membership.object == request.object && !sameContainer)
            return Result<TransferReceipt>::failure(
                Diagnostic::error(DiagnosticCode::Conflict,
                                  "destination already contains this membership object"));
    ...
    auto accepted = request.destination->validateInsert(
        sourceEntry->object, request.destinationSlot,
        sameContainer ? std::optional<MembershipId>(request.object) : std::nullopt);
    if (!accepted) {
        return Result<TransferReceipt>::failure(accepted.status());
    }

    if (sameContainer && request.destinationSlot &&
        sourceEntry->membership.slot == *request.destinationSlot) {
        TransferReceipt receipt{sourceSnapshot.id, destinationSnapshot.id, request.object,
                                sourceEntry->membership.slot, sourceEntry->membership.slot,
                                sourceSnapshot.revision, destinationSnapshot.revision};
        return Result<TransferReceipt>::success(std::move(receipt),
    ...
    auto sourceCandidateResult = removeEntry(sourceSnapshot, request.source->descriptor(),
                                              request.object);
    if (!sourceCandidateResult)
        return Result<TransferReceipt>::failure(sourceCandidateResult.status());
    ...
    if (sameContainer) {
        auto candidateResult = insertEntry(std::move(sourceCandidate),
                                           request.source->descriptor(), *sourceEntry,
        ...
        if (moved == nullptr)
            return Result<TransferReceipt>::failure(Diagnostic::error(
                DiagnosticCode::InvariantViolation, "same-container candidate lost membership"));
        ...
        if (!destinationRevision)
            return Result<TransferReceipt>::failure(Diagnostic::error(
                DiagnosticCode::InvariantViolation, "destination revision exhausted"));
        auto candidateResult = insertEntry(destinationSnapshot, request.destination->descriptor(),
                                           *sourceEntry, request.destinationSlot);
        if (!candidateResult)
            return Result<TransferReceipt>::failure(candidateResult.status());
        destinationCandidate = std::move(candidateResult).takeValue();
        ...
        if (moved == nullptr)
            return Result<TransferReceipt>::failure(Diagnostic::error(
                DiagnosticCode::InvariantViolation, "destination candidate lost membership"));
        destinationSlot = moved->membership.slot;
    }
    ...
    if (!sourcePreparedResult)
        return Result<TransferReceipt>::failure(sourcePreparedResult.status());
    auto sourcePrepared = std::move(sourcePreparedResult).takeValue();

    if (sameContainer) {
        sourcePrepared->commit();
    } else {
        auto destinationPreparedResult =
            request.destination->prepare(destinationSnapshot, destinationCandidate);
    ...
    if (eventSink) {
        try {
            eventSink(TransferEvent{receipt.source, receipt.destination, receipt.object,
                                    receipt.sourceSlot, receipt.destinationSlot,
                                    receipt.sourceRevision, receipt.destinationRevision});
        } catch (const std::exception&) {
            try {
                std::vector<Diagnostic> diagnostics;
                diagnostics.emplace_back(Diagnostic::warning(
                    DiagnosticCode::CallbackFailure,
