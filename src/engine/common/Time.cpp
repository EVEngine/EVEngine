...
Result<Duration> Duration::fromSeconds(double seconds) {
    if (!std::isfinite(seconds)) return Result<Duration>::failure(invalidTime("duration seconds must be finite"));
...
double Duration::seconds() const noexcept { return static_cast<double>(nanoseconds_) / 1000000000.0; }
...
Result<Duration> MonotonicTimestamp::since(MonotonicTimestamp earlier) const {
    ...
    const std::uint64_t distance =
        static_cast<std::uint64_t>(nanoseconds_) - static_cast<std::uint64_t>(earlier.nanoseconds_);
    if (distance > static_cast<std::uint64_t>(std::numeric_limits<std::int64_t>::max()))
        return Result<Duration>::failure(Diagnostic::error(DiagnosticCode::InvariantViolation,
                                                           "monotonic timestamp distance exceeds Duration range"));
    return Result<Duration>::success(Duration(static_cast<std::int64_t>(distance)));
}

SimulationClock::SimulationClock(ITimeSource& source, Duration fixedStep) : source_(source), fixedStep_(fixedStep) {
...
Result<std::vector<SimulationStep>> SimulationClock::advance(Duration realDelta) {
    ...
    for (std::uint64_t i = 0; i < stepCount; ++i) steps.push_back({SimulationTick(tick_.value() + i + 1), fixedStep_});
