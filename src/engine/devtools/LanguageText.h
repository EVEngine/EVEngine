...
EVENGINE_API_FOUNDATION size_t offsetAt(std::string_view source, Position position) noexcept;

/** @brief Full-document range covering every character in source. */
EVENGINE_API_FOUNDATION Range documentRange(std::string_view source) noexcept;

/** @brief Last 0-based line index (empty source is line 0). */
EVENGINE_API_FOUNDATION size_t lastLineIndex(std::string_view source) noexcept;
 ...
 */
EVENGINE_API_FOUNDATION bool applyIncremental(std::string& source, const Range* range, std::string_view text);

/** @brief Indents EveScript by braces/brackets and trims trailing whitespace. */
EVENGINE_API_FOUNDATION std::string formatEveScript(std::string_view source, const FormatOptions& options = {});

/** @brief Brace, bracket, and block-comment folds that span more than one line. */
EVENGINE_API_FOUNDATION std::vector<FoldingRange> foldingRanges(std::string_view source);
