...
[[nodiscard]] EVENGINE_API_FOUNDATION Result<Config> decodeConfig(const Value& value);
...
[[nodiscard]] EVENGINE_API_FOUNDATION Result<Policy> decodePolicy(const Value& value);
/** @brief Decode version-1 owning replay data; run-time consistency is checked before replay reset. */
[[nodiscard]] EVENGINE_API_FOUNDATION Result<Trace> decodeTrace(const Value& value);
/** @brief Encode a runner-produced policy into owning versioned data; no references retained. */
[[nodiscard]] EVENGINE_API_FOUNDATION Value encodePolicy(const Policy& policy);
/** @brief Encode a runner-produced trace into owning versioned data; seeds use lossless decimal strings. */
[[nodiscard]] EVENGINE_API_FOUNDATION Value encodeTrace(const Trace& trace);
