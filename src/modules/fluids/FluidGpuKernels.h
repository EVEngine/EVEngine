 ...
 *   [31] pad
 *
 * Buffer bindings: 0 pos(vec4), 1 vel(vec4), 2 cellHead(int), 3 cellNext(int),
 * 4 densities(float), 5 sdf(float). 6/7 are reserved for PBF lambdas and
 * screen-space reconstruction data.
 */
...
/** @brief Compute per-particle SPH density (poly6, mass = 1). */
inline const char* kFluidDensity = R"GLSL(
#version 450
layout(local_size_x = 64) in;
layout(set = 0, binding = 0) buffer Pos { vec4 p[]; } pos;
layout(set = 0, binding = 2) buffer CellHead { int h[]; } head;
...
/** @brief Viscosity + gravity + integration + SDF projection (mirror of FluidSimulation::integrate). */
            ...
            for (int oy = -1; oy <= 1; ++oy) {
                for (int ox = -1; ox <= 1; ++ox) {
                    ...
                    for (int j = head.h[cell]; j >= 0; j = next.nx[j]) {
                        ...
                        float r2 = dot(dx, dx);
                        if (r2 < h2) viscAcc += (vel.v[j].xyz - v) * poly6(r2, h);
                    }
                }
            }
        }
