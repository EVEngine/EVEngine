...
int FluidSimulation::spawnDrop(const glm::vec3& center, float radius, int count) {
    ...
    for (int k = 0; k < count && count_ < maxParticles_; ++k) {
        // Deterministic jitter: golden-ratio sphere fill so tests are stable.
        const uint32_t u  = seed * uint32_t(k + 1);
        const float    z  = (float(u % 10000u) / 10000.f) * 2.f - 1.f;
        ...
        // Snap inside particles onto the surface.
...
void FluidSimulation::computeDensities() {
...
void FluidSimulation::integrate(float dt) {
    ...
    for (int i = 0; i < count_; ++i) {
        ...
        // XSPH viscosity: pull velocity toward the local neighborhood mean.
