...
eve::Result<void> staleSolver(const char* path) {
    return eve::Result<void>::failure(eve::Diagnostic::error(
        ...
        "Surface-fluid adapter has no live borrowed solver", path));
}
...
eve::Result<void> applied() {
    return eve::Result<void>::success(eve::Status::success(eve::StatusCode::Applied));
}

eve::Result<void> validateTick(const eve::SimulationStep& step,
                               eve::SimulationTick lastTick, const char* path) {
    ...
    if (!(step.tick > lastTick))
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::PreconditionViolation,
            "Surface-fluid simulation tick must increase strictly", path));
    return applied();
}
...
int FluidSimulationAdapter::particleCount() const noexcept {
    return solver_ ? solver_->particleCount() : 0;
}
...
eve::Result<void> FluidSurfaceConstraintAdapter::build(
...
eve::Result<void> FluidSurfaceConstraintAdapter::setDeformedPositions(
    const std::vector<glm::vec3>& worldPositions) {
    if (!binding_) return staleSolver("fluids.surfaceConstraint.binding");
    if (!binding_->setDeformedPositions(worldPositions))
        return invalidInput("fluids.surfaceConstraint.pose",
                            "Deformed surface vertex count does not match topology");
    return applied();
}

bool FluidSurfaceConstraintAdapter::isValid() const noexcept {
    return binding_ && binding_->isValid();
}

eve::Result<SurfaceSample> FluidSurfaceConstraintAdapter::evaluate(
    const SurfaceLocation& location, eve::Duration poseDelta) const {
    ...
    if (!binding_ || !binding_->isValid())
        return eve::Result<SurfaceSample>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::PreconditionViolation,
            "Cannot evaluate an invalid surface constraint", "fluids.surfaceConstraint.evaluate"));
    if (location.triangle >= static_cast<std::uint32_t>(binding_->triangleCount()))
        return eve::Result<SurfaceSample>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::NotFound,
            "Surface location triangle is outside the bound topology",
            "fluids.surfaceConstraint.evaluate.triangle"));
    return eve::Result<SurfaceSample>::success(
        binding_->evaluate(location, static_cast<float>(poseDelta.seconds())));
}

eve::Result<SurfaceLocation> FluidSurfaceConstraintAdapter::project(
    const glm::vec3& worldPosition, float maxDistance) const {
    if (!binding_ || !binding_->isValid())
        return eve::Result<SurfaceLocation>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::PreconditionViolation,
            "Cannot project against an invalid surface constraint",
            "fluids.surfaceConstraint.project"));
    SurfaceLocation location;
    if (!binding_->project(worldPosition, maxDistance, location))
        return eve::Result<SurfaceLocation>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::NotFound,
            "No surface location satisfies the projection distance",
            "fluids.surfaceConstraint.project"));
    return eve::Result<SurfaceLocation>::success(location);
}

eve::Result<SurfaceWalkResult> FluidSurfaceConstraintAdapter::walk(
    const SurfaceLocation& start, const glm::vec3& displacement, int maxCrossings) const {
    if (!binding_ || !binding_->isValid())
    ...
    if (maxCrossings < 0)
        return eve::Result<SurfaceWalkResult>::failure(eve::Diagnostic::error(
            ...
            "Surface traversal maxCrossings must be non-negative",
            "fluids.surfaceConstraint.walk.maxCrossings"));
    const auto result = binding_->walkAcrossSurface(start, displacement, maxCrossings);
    if (!result.valid)
        return eve::Result<SurfaceWalkResult>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::NotFound,
            "Surface traversal start is outside the bound topology",
            "fluids.surfaceConstraint.walk.start"));
    return eve::Result<SurfaceWalkResult>::success(result);
}

ScreenSpaceSurfaceReconstructionAdapter::ScreenSpaceSurfaceReconstructionAdapter(
    const FluidSurfaceParams& params, bool preferGpu) {
    if (params.width <= 0 || params.height <= 0)
        throw eve::Exception("Surface SSF output dimensions must be positive");
...
eve::Result<void> ScreenSpaceSurfaceReconstructionAdapter::reconstruct(
    std::span<const glm::vec3> positions, float particleRadius) {
...
int ScreenSpaceSurfaceReconstructionAdapter::width() const noexcept {
    return renderer_ ? renderer_->getWidth() : 0;
}

int ScreenSpaceSurfaceReconstructionAdapter::height() const noexcept {
    return renderer_ ? renderer_->getHeight() : 0;
}
...
std::span<const glm::vec3> ScreenSpaceSurfaceReconstructionAdapter::normals() const noexcept {
    return renderer_ ? std::span<const glm::vec3>(renderer_->normals())
                     : std::span<const glm::vec3>{};
}

std::span<const float> ScreenSpaceSurfaceReconstructionAdapter::thickness() const noexcept {
    return renderer_ ? std::span<const float>(renderer_->thickness())
                     : std::span<const float>{};
}

std::span<const std::uint8_t> ScreenSpaceSurfaceReconstructionAdapter::color() const noexcept {
    return renderer_ ? std::span<const std::uint8_t>(renderer_->color())
                     : std::span<const std::uint8_t>{};
}

bool ScreenSpaceSurfaceReconstructionAdapter::usingGpu() const noexcept {
    return renderer_ && renderer_->usingGpu();
}
