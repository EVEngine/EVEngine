...
EditorValue settingsValue(const SurfaceFluidSettings& s) {
    return EditorValue::Object{{"gravity", EditorValue::Array{s.gravityX, s.gravityY, s.gravityZ}},
                               {"friction", s.friction},
                               {"maxSpeed", s.maxSpeed},
                               {"adhesionAcceleration", s.adhesionAcceleration},
                               {"maxCrossings", int64_t{s.maxCrossings}},
                               {"contactAngleDegrees", s.contactAngleDegrees},
                               {"mergeRadiusScale", s.mergeRadiusScale},
                               {"trailDeposition", s.trailDeposition},
                               {"airDrag", s.airDrag},
                               {"reattachDistance", s.reattachDistance},
                               {"diffusion", s.diffusion},
                               {"evaporation", s.evaporation},
                               {"maxWetness", s.maxWetness},
                               {"velocityStretch", s.velocityStretch},
                               {"maxAspectRatio", s.maxAspectRatio},
                               {"surfaceOffset", s.surfaceOffset},
                               {"dryRoughness", s.dryRoughness},
                               {"wetRoughness", s.wetRoughness},
                               {"drySpecular", s.drySpecular},
                               {"wetSpecular", s.wetSpecular},
                               {"wetDarkening", s.wetDarkening},
...
std::vector<EditorDiagnostic> validateSettings(const SurfaceFluidSettings& s) {
    ...
    const double                  finite[]{s.gravityX,
                                           s.gravityY,
                                           s.gravityZ,
                                           s.friction,
                                           s.maxSpeed,
                                           s.adhesionAcceleration,
                                           s.contactAngleDegrees,
                                           s.mergeRadiusScale,
                                           s.trailDeposition,
                                           s.airDrag,
                                           s.reattachDistance,
    ...
    if (std::any_of(std::begin(finite), std::end(finite), [](double v) { return !std::isfinite(v); }))
        out.push_back(diagnostic("editor.surface-fluid.nonfinite", DiagnosticSeverity::Error,
    ...
    if (s.friction < 0 || s.maxSpeed <= 0 || s.adhesionAcceleration < 0 || s.maxCrossings < 1 ||
        ...
        s.diffusion < 0 || s.evaporation < 0 || s.maxWetness <= 0 || s.velocityStretch < 0 || s.maxAspectRatio < 1 ||
        s.surfaceOffset < 0 || s.dryRoughness < 0 || s.dryRoughness > 1 || s.wetRoughness < 0 || s.wetRoughness > 1 ||
        s.drySpecular < 0 || s.drySpecular > 1 || s.wetSpecular < 0 || s.wetSpecular > 1 || s.wetDarkening < 0 ||
        s.wetDarkening > 1 || s.normalStrength < 0 || s.normalStrength > 1)
        out.push_back(diagnostic("editor.surface-fluid.range", DiagnosticSeverity::Error,
                                 "Surface fluid values are outside safe runtime ranges"));
    if (s.wetRoughness > s.dryRoughness)
        out.push_back(diagnostic("editor.surface-fluid.roughness-order", DiagnosticSeverity::Warning,
                                 "Wet roughness exceeds dry roughness"));
    if (s.wetSpecular < s.drySpecular)
        out.push_back(diagnostic("editor.surface-fluid.specular-order", DiagnosticSeverity::Warning,
...
EditorResult<SurfaceFluidSettings> parse(const EditorValue& value) {
    ...
    bool complete  = true;
    complete &= number("friction", s.friction);
    complete &= number("maxSpeed", s.maxSpeed);
    complete &= number("adhesionAcceleration", s.adhesionAcceleration);
    complete &= number("contactAngleDegrees", s.contactAngleDegrees);
    complete &= number("mergeRadiusScale", s.mergeRadiusScale);
    complete &= number("trailDeposition", s.trailDeposition);
    complete &= number("airDrag", s.airDrag);
    complete &= number("reattachDistance", s.reattachDistance);
    complete &= number("diffusion", s.diffusion);
    complete &= number("evaporation", s.evaporation);
    complete &= number("maxWetness", s.maxWetness);
    complete &= number("velocityStretch", s.velocityStretch);
    complete &= number("maxAspectRatio", s.maxAspectRatio);
    complete &= number("surfaceOffset", s.surfaceOffset);
    complete &= number("dryRoughness", s.dryRoughness);
    complete &= number("wetRoughness", s.wetRoughness);
    complete &= number("drySpecular", s.drySpecular);
    complete &= number("wetSpecular", s.wetSpecular);
...
PropertyDescriptor descriptor(const char* path, PropertyType type, EditorValue defaultValue, const char* category,
    ...
    d.displayNameKey = std::string("editor.surface-fluid.") + path;
    d.category       = category;
    d.type           = type;
...
bool SurfaceFluidTarget::matches(const SelectionSnapshot& s) const {
...
PropertySchema SurfaceFluidTarget::schema(const SelectionSnapshot&) const {
    ...
    PropertySchema       s;
    s.typeId     = "fluids.surface";
    s.properties = {
        descriptor("gravity", PropertyType::Vec3, EditorValue::Array{d.gravityX, d.gravityY, d.gravityZ}, "droplet"),
        descriptor("friction", PropertyType::Float, d.friction, "droplet", 0, 1000, true),
        descriptor("maxSpeed", PropertyType::Float, d.maxSpeed, "droplet", 0.0001, 100000, true),
        descriptor("adhesionAcceleration", PropertyType::Float, d.adhesionAcceleration, "droplet", 0, 100000, true),
        descriptor("maxCrossings", PropertyType::Int, int64_t{d.maxCrossings}, "droplet", 1, 1024, true),
        descriptor("contactAngleDegrees", PropertyType::Float, d.contactAngleDegrees, "droplet", 0.001, 179.999, true),
        descriptor("mergeRadiusScale", PropertyType::Float, d.mergeRadiusScale, "droplet", 0.0001, 1000, true),
        descriptor("trailDeposition", PropertyType::Float, d.trailDeposition, "droplet", 0, 1000, true),
        descriptor("airDrag", PropertyType::Float, d.airDrag, "droplet", 0, 1000, true),
        descriptor("reattachDistance", PropertyType::Float, d.reattachDistance, "droplet", 0, 1000, true),
        descriptor("diffusion", PropertyType::Float, d.diffusion, "wetness", 0, 1000, true),
        descriptor("evaporation", PropertyType::Float, d.evaporation, "wetness", 0, 1000, true),
        descriptor("maxWetness", PropertyType::Float, d.maxWetness, "wetness", 0.0001, 1000, true),
        descriptor("velocityStretch", PropertyType::Float, d.velocityStretch, "render", 0, 1000, true),
        descriptor("maxAspectRatio", PropertyType::Float, d.maxAspectRatio, "render", 1, 1000, true),
        descriptor("surfaceOffset", PropertyType::Float, d.surfaceOffset, "render", 0, 1000, true),
        descriptor("dryRoughness", PropertyType::Float, d.dryRoughness, "material", 0, 1, true),
        descriptor("wetRoughness", PropertyType::Float, d.wetRoughness, "material", 0, 1, true),
        descriptor("drySpecular", PropertyType::Float, d.drySpecular, "material", 0, 1, true),
        descriptor("wetSpecular", PropertyType::Float, d.wetSpecular, "material", 0, 1, true),
        descriptor("wetDarkening", PropertyType::Float, d.wetDarkening, "material", 0, 1, true),
        descriptor("normalStrength", PropertyType::Float, d.normalStrength, "material", 0, 1, true)};
    return s;
}
PropertyReadResult SurfaceFluidTarget::read(const SelectionSnapshot& s, const PropertyPath& p) const {
