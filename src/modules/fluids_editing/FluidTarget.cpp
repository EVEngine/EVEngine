...
EditorValue settingsValue(const FluidSimulationSettings& s) {
    return EditorValue::Object{{"maxParticles", int64_t{s.maxParticles}},
                               {"previewParticles", int64_t{s.previewParticles}},
                               {"particleRadius", s.particleRadius},
                               {"supportRadius", s.supportRadius},
                               {"restDensity", s.restDensity},
                               {"gravity", EditorValue::Array{s.gravityX, s.gravityY, s.gravityZ}},
                               {"viscosity", s.viscosity},
                               {"yieldStress", s.yieldStress},
                               {"cohesion", s.cohesion},
                               {"adhesion", s.adhesion},
                               {"damping", s.damping},
                               {"maximumVelocity", s.maximumVelocity},
                               {"iterations", int64_t{s.iterations}},
...
EditorResult<FluidSimulationSettings> parseSettings(const EditorValue& value) {
    ...
    const auto* max          = integer("maxParticles");
    const auto* preview      = integer("previewParticles");
    const auto* radius       = number("particleRadius");
    const auto* support      = number("supportRadius");
    const auto* density      = number("restDensity");
    const auto* gravityValue = field(value, "gravity");
    const auto* gravity      = gravityValue ? gravityValue->getIf<EditorValue::Array>() : nullptr;
    const auto* viscosity    = number("viscosity");
    const auto* yield        = number("yieldStress");
    const auto* cohesion     = number("cohesion");
    const auto* adhesion     = number("adhesion");
    const auto* damping      = number("damping");
    const auto* velocity     = number("maximumVelocity");
    const auto* iterations   = integer("iterations");
...
PropertyDescriptor descriptor(const char* path, PropertyType type, EditorValue value, double minimum = 0.0,
    ...
    result.displayNameKey = std::string("editor.fluid.") + path;
    result.category       = "fluid";
    result.type           = type;
...
bool FluidSimulationTarget::matches(const SelectionSnapshot& selection) const {
...
PropertySchema FluidSimulationTarget::schema(const SelectionSnapshot&) const {
    FluidSimulationSettings d;
    PropertySchema          result;
    result.typeId     = "fluids.simulation";
    result.properties = {
...
EditorResult<DomainOperation> FluidSimulationTarget::makeSet(const SelectionSnapshot& selection,
    ...
    operation.payload     = settingsValue(candidate.value());
    operation.inverse     = settingsValue(settings_);
    operation.hasInverse  = true;
    operation.affectedProperties.push_back(path.value());
    operation.mergeKey = "fluid:" + id_ + ":" + path.value();
...
std::vector<EditorDiagnostic> FluidSimulationTarget::validate() const {
    ...
    if (settings_.supportRadius < settings_.particleRadius * 2.0)
        result.push_back(fluidDiagnostic("editor.fluid.support-radius-small", DiagnosticSeverity::Error,
                                         "Fluid support radius should be at least twice particle radius"));
    if (settings_.supportRadius > settings_.particleRadius * 8.0)
        result.push_back(fluidDiagnostic("editor.fluid.support-radius-large", DiagnosticSeverity::Warning,
                                         "Large support radius can make neighbor search prohibitively expensive"));
    return result;
}
...
FluidSimulationPreview FluidSimulationTarget::previewBudget(std::uint64_t byteBudget,
    ...
    result.estimatedNeighborChecks = static_cast<std::uint64_t>(
        particles * std::min(512.0, ratio * ratio * ratio * 2.0) * settings_.iterations * settings_.pbfIterations);
    result.diagnostics = validate();
    if (result.estimatedBytes > byteBudget)
        result.diagnostics.push_back(fluidDiagnostic("editor.fluid.memory-budget", DiagnosticSeverity::Error,
                                                      "Fluid preview exceeds its memory budget"));
    if (result.estimatedNeighborChecks > neighborBudget)
        result.diagnostics.push_back(fluidDiagnostic("editor.fluid.compute-budget", DiagnosticSeverity::Error,
                                                      "Fluid preview exceeds its estimated neighbor-work budget"));
