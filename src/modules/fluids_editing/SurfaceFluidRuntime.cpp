...
EditorResult<void> SurfaceFluidRuntimeApplier::apply(const SurfaceFluidTarget&         target,
    ...
    if (!simulation || !render || !wetness)
        return eve::editing::failed<void>(
            EditorStatus::Rejected, RuleId("editor.surface-fluid.runtime-services"),
    ...
    for (const auto& diagnostic : target.validate())
        if (diagnostic.severity() == DiagnosticSeverity::Error)
            return eve::editing::failed<void>(EditorStatus::Rejected, RuleId("editor.surface-fluid.runtime-invalid"),
                                              "Surface fluid document is invalid");
    const SurfaceFluidSettings s = target.settings();

    fluids::SurfaceDropletParams droplet;
    droplet.gravity  = {static_cast<float>(s.gravityX), static_cast<float>(s.gravityY), static_cast<float>(s.gravityZ)};
