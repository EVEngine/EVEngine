...
UiSkinDrawPlan UiSkinPreviewPlanner::build(const UiDocumentTarget& document,
    ...
    if (preview.status != EditorStatus::Applied || preview.documentRevision != document.revision()) {
        ...
        error("editor.ui.skin-stale-preview", "UI skin planning requires a current layout preview");
        return result;
    }
    ...
    for (const auto& rectangle : preview.widgets) {
        ...
        if (!widgetResult.ok()) { result.status = EditorStatus::Conflict; return result; }
        ...
        while (widget.content.clip && !ancestor.empty()) {
            ...
            if (!parentWidget.ok() || parentRect == rectangles.end()) break;
        ...
        if (!widget.content.textureAsset.empty()) {
            ...
            if (!metadata.ok() || metadata.value().width <= 0.0 || metadata.value().height <= 0.0) {
                error("editor.ui.texture-unresolved",
        ...
        if (!widget.text.empty()) {
            ...
            if (widget.content.fontAsset.empty() || !font.ok()) {
                error("editor.ui.font-unresolved", "Could not resolve UI font: " + widget.content.fontAsset);
            } else {
