...
void numeric(PropertySchema& schema, const char* path, const char* label, const char* category, double value,
             double minimum, double maximum, double step = 0.01) {
...
PropertySchema environmentSchema() {
    ...
    auto mode = property("environment.mode", "editor.environment.mode", "environment", PropertyType::Enum, "static");
    ...
    schema.properties.push_back(property("environment.ambient-color", "editor.environment.ambient-color", "environment",
                                         PropertyType::Color, EditorValue::Array{0.12, 0.12, 0.14, 1.0}));
    ...
    schema.properties.push_back(
        property("daynight.paused", "editor.environment.paused", "daynight", PropertyType::Bool, false));
    ...
    schema.properties.push_back(
        property("daynight.skybox", "editor.environment.skybox", "daynight", PropertyType::Bool, true));
    auto preset =
        property("weather.preset", "editor.environment.weather-preset", "weather", PropertyType::Enum, "clear");
    ...
    schema.properties.push_back(
        property("weather.lightning", "editor.environment.lightning", "weather", PropertyType::Bool, false));
    ...
    schema.properties.push_back(property("weather.environment-enabled", "editor.environment.weather-owned", "weather",
...
LightingPropertyTargetBase::LightingPropertyTargetBase(std::string id, std::string targetType, PropertySchema schema)
...
void* LightingPropertyTargetBase::queryCapability(const CapabilityId& capability) {
    return capability == CapabilityId("eve.editor.target.lighting-properties") ? static_cast<IPropertyProvider*>(this)
                                                                               : nullptr;
}
...
eve::Result<eve::Revision> LightingPropertyTargetBase::currentRevision(const SelectionSnapshot& selection) const {
...
std::vector<EditorDiagnostic> EnvironmentDocumentTarget::validate() const {
    ...
    if (mode && *mode == "weather" && lightning && *lightning && preset && *preset != "storm")
        diagnostics.push_back(eve::editing::ruleDiagnostic(eve::DiagnosticCode::PreconditionViolation,
            RuleId("editor.environment.lightning-preset"), DiagnosticSeverity::Warning,
    ...
    if (mode && *mode == "daynight" && weatherOwned && *weatherOwned)
        diagnostics.push_back(eve::editing::ruleDiagnostic(eve::DiagnosticCode::Conflict,
            RuleId("editor.environment.multiple-owners"), DiagnosticSeverity::Error,
