...
EditorResult<std::vector<PrefabOverrideRecord>> ScenePrefabService::inspectOverrides(
    ...
    return eve::editing::applied<std::vector<PrefabOverrideRecord>>(std::move(result));
}
...
PrefabDependencyReport ScenePrefabService::inspectDependencies(
    ...
    const auto diagnostic = [&](DiagnosticCode code, const char* rule, DiagnosticSeverity severity,
                                std::string message) {
        report.diagnostics.push_back(eve::editing::ruleDiagnostic(
            code, RuleId(rule), severity, std::move(message)));
    };
    report.rootAsset = prefab.asset; report.rootRevision = prefab.revision;
    if (!resolver || !validatePrefab(prefab).ok()) {
        report.status = EditorStatus::Rejected;
        diagnostic(DiagnosticCode::InvalidArgument, "editor.prefab.invalid-dependency-root",
    ...
    std::function<bool(const PrefabAssetSnapshot&)> visit = [&](const PrefabAssetSnapshot& current) {
        if (visiting.contains(current.asset)) {
            diagnostic(DiagnosticCode::Conflict, "editor.prefab.dependency-cycle",
                       DiagnosticSeverity::Error,
        ...
        for (const PrefabObjectRecord& object : current.objects) {
            ...
            if (!resolved.ok()) {
                diagnostic(DiagnosticCode::NotFound, "editor.prefab.dependency-missing",
                           DiagnosticSeverity::Error,
                           "Nested prefab cannot be resolved: " + object.nestedPrefab.value());
                valid = false; continue;
            }
            if (!validatePrefab(resolved.value()).ok()) {
                diagnostic(DiagnosticCode::PreconditionViolation, "editor.prefab.dependency-invalid",
                           DiagnosticSeverity::Error,
                           "Nested prefab is invalid: " + object.nestedPrefab.value());
                valid = false; continue;
            }
            if (resolved.value().revision != object.nestedRevision)
                diagnostic(DiagnosticCode::Conflict, "editor.prefab.dependency-stale",
