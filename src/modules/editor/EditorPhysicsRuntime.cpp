...
EditorResult<physics::Fixture*> PhysicsColliderRuntimeBuilder::build2D(
    const PhysicsColliderTarget& target, physics::Body* body) const {
    ...
    if (kind == "box")
        fixture = body->newRectangleFixtureAt(static_cast<float>(*(*size)[0].getIf<double>()),
                                              static_cast<float>(*(*size)[1].getIf<double>()),
                                              static_cast<float>(*(*offset)[0].getIf<double>()),
                                              static_cast<float>(*(*offset)[1].getIf<double>()),
                                              density, friction, restitution);
    else if (kind == "circle")
        fixture = body->newCircleFixture(radius, density, friction, restitution);
    else
        return bridgeError<physics::Fixture*>(EditorStatus::Unsupported,
                                              "editor.physics.runtime-2d-asset-shape",
...
EditorResult<physics::Fixture*> PhysicsColliderRuntimeBuilder::build2D(
    const PhysicsColliderTarget& target, physics::Body* body,
    const IPhysicsColliderAssetResolver& assets) const {
    if (!body || target.describe().type != "physics-collider-2d")
        return bridgeError<physics::Fixture*>(EditorStatus::Rejected,
                                              "editor.physics.runtime-2d-input",
                                              "A 2D collider target and body are required");
    const EditorValue snapshot = target.snapshotValue();
    const auto* root = snapshot.getIf<EditorValue::Object>();
    const auto* values = root ? root->at("properties").getIf<EditorValue::Object>() : nullptr;
    if (!values)
        return bridgeError<physics::Fixture*>(EditorStatus::Failed,
                                              "editor.physics.runtime-properties",
                                              "Collider properties are unavailable");
    const std::string kind = *propertyValue<std::string>(*values, "shape.kind");
...
EditorResult<physics::Shape3D*> PhysicsColliderRuntimeBuilder::build3D(
    const PhysicsColliderTarget& target, physics::Body3D* body) const {
    ...
    if (kind == "box")
        shape = body->newBoxShape(static_cast<float>(*(*size)[0].getIf<double>()),
                                  static_cast<float>(*(*size)[1].getIf<double>()),
                                  static_cast<float>(*(*size)[2].getIf<double>()), density, friction,
                                  restitution);
    else if (kind == "sphere")
        ...
        return bridgeError<physics::Shape3D*>(EditorStatus::Unsupported,
                                              "editor.physics.runtime-3d-asset-shape",
...
EditorResult<physics::Shape3D*> PhysicsColliderRuntimeBuilder::build3D(
    const PhysicsColliderTarget& target, physics::Body3D* body,
    const IPhysicsColliderAssetResolver& assets) const {
    if (!body || target.describe().type != "physics-collider-3d")
        return bridgeError<physics::Shape3D*>(EditorStatus::Rejected, "editor.physics.runtime-3d-input",
                                              "A 3D collider target and body are required");
    ...
    const auto* root = snapshot.getIf<EditorValue::Object>();
    const auto* values = root ? root->at("properties").getIf<EditorValue::Object>() : nullptr;
    if (!values) return bridgeError<physics::Shape3D*>(EditorStatus::Failed,
        "editor.physics.runtime-properties", "Collider properties are unavailable");
    const std::string kind = *propertyValue<std::string>(*values, "shape.kind");
    if (kind == "box" || kind == "sphere" || kind == "capsule") return build3D(target, body);
    const std::string asset = *propertyValue<std::string>(*values, "shape.asset");
    auto geometry = assets.resolve(asset, kind);
    if (!geometry.value) {
        EditorResult<physics::Shape3D*> failed; failed.status = geometry.status;
        failed.diagnostics = std::move(geometry.diagnostics); return failed;
    }
    physics::Shape3D* shape = nullptr;
    try {
        const float density = static_cast<float>(*propertyValue<double>(*values, "material.density"));
        ...
        if (kind == "convex-hull")
            ...
            shape = body->newHeightFieldShape(geometry.value->countX, geometry.value->countZ,
                geometry.value->cellSizeX, geometry.value->cellSizeZ, geometry.value->heights,
                geometry.value->minimumHeight, geometry.value->maximumHeight);
    } catch (const std::exception& exception) {
        return bridgeError<physics::Shape3D*>(EditorStatus::Rejected,
            "editor.physics.runtime-complex-shape", exception.what());
    }
    if (!shape) return bridgeError<physics::Shape3D*>(EditorStatus::Failed,
        "editor.physics.runtime-create", "Physics backend failed to create the resolved shape");
...
EditorResult<void> PhysicsCollider3DRuntimeSink::publish(
    const PhysicsColliderTarget& candidate) {
    ...
    if (currentRequiresStatic && desiredBodyType != "static" && desiredBodyType != previousBodyType)
        return bridgeError<void>(EditorStatus::Unsupported,
                                 "editor.physics.live-complex-type-transition",
    ...
    try {
        ...
        return bridgeError<void>(EditorStatus::Rejected, "editor.physics.live-body-type",
                                 exception.what());
    }
...
EditorResult<void> PhysicsCollider2DRuntimeSink::publish(
    const PhysicsColliderTarget& candidate) {
