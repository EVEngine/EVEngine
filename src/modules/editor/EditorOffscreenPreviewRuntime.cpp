...
GraphicsOffscreenPreviewService::GraphicsOffscreenPreviewService(
    graphics::Graphics* graphics, graphics::ICanvasFactory* canvases,
...
EditorResult<OffscreenPreviewArtifact> GraphicsOffscreenPreviewService::render(
    const OffscreenPreviewRequest& request, const DrawCallback& draw,
    int maximumDimension, std::uint64_t maximumPixels) {
    if (!impl_->graphics || !impl_->canvases || !impl_->targets || request.previewId.empty() || !draw ||
        request.sourceRevision == 0 ||
        request.width <= 0 || request.height <= 0 || maximumDimension <= 0 ||
        ...
        return EditorResult<OffscreenPreviewArtifact>::error(EditorStatus::Rejected,
            RuleId("editor.preview.invalid-offscreen-request"),
    ...
    for (double component : request.clearColor)
        if (!std::isfinite(component) || component < 0.0 || component > 1.0)
            return EditorResult<OffscreenPreviewArtifact>::error(EditorStatus::Rejected,
                RuleId("editor.preview.invalid-clear-color"), "Offscreen clear color must be normalized");
    ...
    if (!canvas)
        return EditorResult<OffscreenPreviewArtifact>::error(EditorStatus::Failed,
            RuleId("editor.preview.canvas-allocation"), "Graphics backend could not allocate an offscreen Canvas");
    ...
    canvas->clear(graphics::Color(static_cast<float>(request.clearColor[0]),
                                  static_cast<float>(request.clearColor[1]),
                                  static_cast<float>(request.clearColor[2]),
                                  static_cast<float>(request.clearColor[3])), 0, 1.0);
    EditorResult<void> drawn;
    try {
        drawn = draw(impl_->graphics, canvas);
    } catch (const std::exception& exception) {
        return EditorResult<OffscreenPreviewArtifact>::error(EditorStatus::Failed,
            RuleId("editor.preview.draw-exception"), exception.what());
    } catch (...) {
        return EditorResult<OffscreenPreviewArtifact>::error(EditorStatus::Failed,
    ...
    if (!pixels)
        return EditorResult<OffscreenPreviewArtifact>::error(EditorStatus::Failed,
            RuleId("editor.preview.readback-failed"), "Graphics backend could not read back the preview Canvas");
    OffscreenPreviewArtifact metadata;
    metadata.previewId = request.previewId; metadata.sourceRevision = request.sourceRevision;
    metadata.width = request.width; metadata.height = request.height;
    metadata.handle = "preview://" + request.previewId.value() + "/" +
                      std::to_string(request.sourceRevision) + "/" + std::to_string(++impl_->sequence);
    impl_->artifacts.insert_or_assign(metadata.handle, Impl::Artifact{metadata, std::move(pixels)});
    return EditorResult<OffscreenPreviewArtifact>::applied(std::move(metadata));
}

EditorResult<const image::ImageData*> GraphicsOffscreenPreviewService::image(
    const std::string& handle, Revision currentRevision) const {
    const auto found = impl_->artifacts.find(handle);
    if (found == impl_->artifacts.end())
        return EditorResult<const image::ImageData*>::error(EditorStatus::NotFound,
            RuleId("editor.preview.artifact-not-found"), "Offscreen preview artifact was not found");
    if (found->second.metadata.sourceRevision != currentRevision)
        return EditorResult<const image::ImageData*>::error(EditorStatus::Conflict,
            RuleId("editor.preview.stale-artifact"), "Offscreen preview artifact belongs to an older source revision");
    return EditorResult<const image::ImageData*>::applied(found->second.pixels.get());
}

EditorResult<void> GraphicsOffscreenPreviewService::release(const std::string& handle) {
    if (!impl_->artifacts.erase(handle))
        return EditorResult<void>::error(EditorStatus::NotFound, RuleId("editor.preview.artifact-not-found"),
