...
EditorResult<void>DecalRuntimeBinding::publish(const DecalDocumentTarget&document){
 ...
 EditorResult<void>assetFailure=EditorResult<void>::applied();auto*albedo=resolve(document,assets_,"texture.albedo",assetFailure);if(!assetFailure.isAccepted())return assetFailure;auto*normal=resolve(document,assets_,"texture.normal",assetFailure);if(!assetFailure.isAccepted())return assetFailure;auto*params=resolve(document,assets_,"texture.params",assetFailure);if(!assetFailure.isAccepted())return assetFailure;
...
EditorResult<void>DecalPublishingTarget::commitDomainState(std::unique_ptr<IDomainOperationTarget>candidate){auto*typed=dynamic_cast<DecalPublishingTarget*>(candidate.get());if(!typed||typed->targetId()!=targetId()||typed->sink_!=sink_||!typed->staging_)return fail<void>(EditorStatus::Conflict,"editor.decal.publish-candidate","Decal publishing candidate mismatch");if(!sink_)return fail<void>(EditorStatus::Rejected,"editor.decal.publish-sink","Decal publishing target requires a runtime sink");auto published=sink_->publish(typed->document_);if(!published.isAccepted())return published;document_=typed->document_;return EditorResult<void>::applied();}
