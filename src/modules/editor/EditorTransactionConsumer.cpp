...
eve::Result<EditorTransactionRecord> EditorTransactionConsumer::commitAttempt(
    ...
    if (retryAttempt && impl_->state != EditorCommitState::FailedRetryable)
        return reject(eve::DiagnosticCode::PreconditionViolation, "editor transaction is not awaiting retry",
                      "editor.transaction.retry");
    ...
    if (!pending.operations.empty() && !impl_->authority)
        return reject(eve::DiagnosticCode::Unsupported, "authority commit requires an injected authority",
                      "editor.authority");
    ...
    if (!retryAttempt) {
        ...
        for (auto* participant : additional) pending.additionalNames.emplace_back(participant->name());
    } else {
        if (additional.size() != pending.additionalNames.size())
            return reject(eve::DiagnosticCode::Conflict, "retry must provide the same additional participant set",
                          "editor.transaction.retry");
        for (std::size_t i = 0; i < additional.size(); ++i) {
    ...
    if (!pending.operations.empty()) {
        authorityParticipant =
            std::make_unique<AuthorityParticipant>(*impl_->authority, pending.specification, pending.operations);
    ...
    if (retryAttempt) attemptId += ".retry." + std::to_string(pending.attemptSequence);
...
eve::Result<void> EditorTransactionConsumer::rollback() { return discard(); }
...
eve::Result<EditorTransactionRecord> EditorTransactionConsumer::undo() {
    ...
    if (history.authorityReceipt && impl_->authority) {
        authorityParticipant = std::make_unique<AuthorityParticipant>(*impl_->authority, history.specification,
                                                                      history.operations, *history.authorityReceipt);
        participants.push_back(authorityParticipant.get());
    }
    if (history.commands) {
        commandParticipant =
            std::make_unique<CommandParticipant>(*history.commands, CommandParticipant::InitialPhase::Committed);
        participants.push_back(commandParticipant.get());
    }
    if (participants.empty()) {
        impl_->undo.push_back(std::move(history));
...
eve::Result<EditorTransactionRecord> EditorTransactionConsumer::redo() {
    ...
    specification.id = TransactionId(specification.id.value() + ".redo." + std::to_string(++impl_->redoSequence));
    std::unique_ptr<AuthorityParticipant>              authorityParticipant;
    std::unique_ptr<CommandParticipant>                commandParticipant;
    std::vector<transaction::ITransactionParticipant*> participants;
    if (!history.operations.empty()) {
        authorityParticipant =
            std::make_unique<AuthorityParticipant>(*impl_->authority, specification, history.operations);
        participants.push_back(authorityParticipant.get());
    }
    if (history.commands) {
