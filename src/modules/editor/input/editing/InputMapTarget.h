...
class EVENGINE_API_BACKENDS InputMapTarget final:public virtual IEditableTarget,public IDomainOperationTarget,public IDomainOperationTargetStaging,public IPropertyProvider{
...
class EVENGINE_API_BACKENDS InputMapEvaluator{public:EditorResult<std::map<std::string,double>>evaluate(const InputMapTarget&,const std::vector<InputControlSample>&)const;};
/** @brief Captures the first intentional control movement for a rebind UI. */
class EVENGINE_API_BACKENDS InputBindingCapture{public:void begin(std::string device={});void cancel();bool active()const{return active_;}std::optional<InputControlSample>feed(InputControlSample);private:bool active_=false;std::string device_;};
} // namespace eve::input_editing
