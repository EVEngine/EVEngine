...
EditorResult<OffscreenPreviewArtifact>StylizeOffscreenPreviewService::render(const StylizeRecipeTarget&document,graphics::Texture*source,const StableId&previewId,int width,int height)const{
 ...
 StylizeRecipeRuntime candidate;auto published=candidate.publish(document,graphics_);if(!published.accepted()){EditorResult<OffscreenPreviewArtifact>failed;failed.status=published.status;failed.diagnostics=std::move(published.diagnostics);return failed;}
