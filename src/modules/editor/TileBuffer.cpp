...
bool TileBuffer::inBounds(int x, int y) const {
    return x >= 0 && y >= 0 && x < width_ && y < height_;
}

void TileBuffer::setGid(int x, int y, int gid) {
    if (!inBounds(x, y)) throw Exception("TileBuffer::setGid: out of bounds");
    gids_[static_cast<size_t>(index(x, y))] = gid;
}

int TileBuffer::getGid(int x, int y) const {
    if (!inBounds(x, y)) throw Exception("TileBuffer::getGid: out of bounds");
