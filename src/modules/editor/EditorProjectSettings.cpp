...
DomainOperation operation(const std::string& target, const PropertyPath& path,
                          EditorValue value, EditorValue inverse) {
...
TargetDescriptor ProjectSettingsTarget::describe() const {
    return {TargetId(id_), "project-settings", revision_, false,
            {CapabilityId("eve.editor.target.project-settings")}};
}

void* ProjectSettingsTarget::queryCapability(const CapabilityId& capability) {
    return capability == CapabilityId("eve.editor.target.project-settings")
               ? static_cast<IPropertyProvider*>(this) : nullptr;
}

const ProjectSettingDescriptor* ProjectSettingsTarget::descriptor(const PropertyPath& path) const {
    const auto found = std::find_if(schema_.settings.begin(), schema_.settings.end(),
...
EditorResult<void> ProjectSettingsTarget::applyDomainOperation(const DomainOperation& domainOperation) {
    ...
    if (values_.at(*path) == *value) { EditorResult<void> result; result.status = EditorStatus::NoOp; return result; }
    values_[*path] = *value;
...
eve::Result<eve::Revision> ProjectSettingsTarget::currentRevision(const SelectionSnapshot& selection) const {
    if (!selectionMatches(selection)) return eve::Result<eve::Revision>::failure(eve::Diagnostic::error(
        eve::DiagnosticCode::InvalidArgument, "Selection does not match settings target"));
...
PropertySchema ProjectSettingsTarget::schema(const SelectionSnapshot&) const {
    PropertySchema result; result.typeId = schema_.typeId; result.version = schema_.version;
    for (const auto& setting : schema_.settings) result.properties.push_back(setting.property);
    return result;
}

PropertyReadResult ProjectSettingsTarget::read(const SelectionSnapshot& selection,
                                                const PropertyPath& path) const {
    if (!selectionMatches(selection)) return {PropertyReadState::Error, {},
        {{RuleId("editor.settings.selection"), DiagnosticSeverity::Error, "Selection does not match settings target"}}};
    ...
    if (setting && setting->sensitive) {
        ...
        if (!reference) return {PropertyReadState::Error, {},
            {{RuleId("editor.settings.invalid-sensitive-schema"), DiagnosticSeverity::Error,
              "Sensitive setting schema must use a string value"}}};
        if (!reference->empty()) return {PropertyReadState::Value, EditorValue("secret://redacted"), {}};
    }
    return {PropertyReadState::Value, found->second, {}};
}

EditorResult<DomainOperation> ProjectSettingsTarget::makeSet(
    const SelectionSnapshot& selection, const PropertyPath& path,
    const EditorValue& value, PropertySetMode mode) const {
    if (!selectionMatches(selection)) return settingsError<DomainOperation>(EditorStatus::Rejected,
        "editor.settings.selection", "Selection does not match settings target");
    const auto* setting = descriptor(path);
    if (!setting) return settingsError<DomainOperation>(EditorStatus::Unsupported,
    ...
    if (setting->sensitive) {
        ...
        if (!reference || (!reference->empty() && !reference->starts_with("secret://")))
            return settingsError<DomainOperation>(EditorStatus::Rejected, "editor.settings.raw-secret",
                "Sensitive settings accept an empty value or secret:// reference only");
    }
    return EditorResult<DomainOperation>::applied(operation(id_, path, candidate, values_.at(path.value())));
}

EditorResult<DomainOperation> ProjectSettingsTarget::makeReset(
...
std::vector<PropertyPath> ProjectSettingsTarget::pendingRestart() const {
    ...
    for (const auto& [path, dirty] : restartDirty_) if (dirty) result.emplace_back(path);
    return result;
}
...
EditorValue ProjectSettingsTarget::snapshotValue() const {
    ...
    return EditorValue::Object{{"schema", schema_.typeId}, {"schemaVersion", int64_t{schema_.version}},
                               {"values", std::move(values)}};
}

EditorResult<void> ProjectSettingsTarget::loadSnapshot(const EditorValue& snapshot) {
    const auto* schemaEntry = field(snapshot, "schema"); const auto* versionEntry = field(snapshot, "schemaVersion");
    ...
    values_ = std::move(candidate.values_); restartDirty_.clear(); ++revision_;
    dirty_.include(0, 0); return EditorResult<void>::applied();
}

ProjectSettingsSchema defaultProjectSettingsSchema() {
    ProjectSettingsSchema result; result.typeId = "engine.project-settings";
    result.settings = {
        ...
        {property("plugins.allow-unsigned", PropertyType::Bool, false, PropertyFlag::Dangerous), "plugins", true, false},
