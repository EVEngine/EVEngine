...
EditorResult<void> AnimationClipDocumentTarget::loadSnapshot(const EditorValue& snapshot) {
    ...
    if (!settingsOp.value || !candidate.applyDomainOperation(*settingsOp.value).accepted()) return clipError<void>(EditorStatus::Rejected, "editor.animation.invalid-settings", "Clip snapshot settings are invalid");
    for (const auto& entry : *tracks) { auto parsed = parseTrack(entry); if (!parsed.value) return clipError<void>(EditorStatus::Rejected, "editor.animation.invalid-track", "Clip snapshot contains invalid track"); auto op = candidate.makeSetTrack(*parsed.value); if (!op.value || !candidate.applyDomainOperation(*op.value).accepted()) return clipError<void>(EditorStatus::Rejected, "editor.animation.invalid-track", "Clip snapshot track cannot be applied"); }
    for (const auto& entry : *events) { auto parsed = parseEvent(entry); if (!parsed.value) return clipError<void>(EditorStatus::Rejected, "editor.animation.invalid-event", "Clip snapshot contains invalid event"); auto op = candidate.makeSetEvent(*parsed.value); if (!op.value || !candidate.applyDomainOperation(*op.value).accepted()) return clipError<void>(EditorStatus::Rejected, "editor.animation.invalid-event", "Clip snapshot event cannot be applied"); }
    for (const auto& entry : *masks) { const auto* bone = stringField(entry, "bone"); const auto* weight = numberField(entry, "weight"); if (!bone || !weight) return clipError<void>(EditorStatus::Rejected, "editor.animation.invalid-mask", "Clip snapshot contains invalid mask"); auto op = candidate.makeSetMask({*bone, *weight}); if (!op.value || !candidate.applyDomainOperation(*op.value).accepted()) return clipError<void>(EditorStatus::Rejected, "editor.animation.invalid-mask", "Clip snapshot mask cannot be applied"); }
