...
PropertyDescriptor prop(const char* p, PropertyType t, EditorValue d, const char* c) {
    PropertyDescriptor x;
    x.path           = PropertyPath(p);
    x.displayNameKey = std::string("editor.scene-import.") + p;
    x.category       = c;
    x.type           = t;
    x.defaultValue   = std::move(d);
    x.flags          = PropertyFlag::EditorOnly;
    return x;
}
bool errors(const std::vector<EditorDiagnostic>& d) {
    return std::any_of(d.begin(), d.end(), [](const auto& x) { return x.severity() == DiagnosticSeverity::Error; });
}
}  // namespace
...
bool SceneImportTarget::matches(const SelectionSnapshot& s) const {
    return s.items.size() == 1 && s.items[0].target == TargetId(id_) && s.items[0].type == "scene-import";
}
eve::Result<eve::Revision> SceneImportTarget::currentRevision(const SelectionSnapshot& s) const {
    if (!matches(s))
...
PropertySchema SceneImportTarget::schema(const SelectionSnapshot&) const {
    PropertySchema s;
    s.typeId                         = "scene-import";
    s.properties                     = {prop("source", PropertyType::AssetRef, "", "source"),
                                        prop("preset", PropertyType::Enum, "balanced", "source"),
                                        prop("triangulate", PropertyType::Bool, true, "geometry"),
                                        prop("generateNormals", PropertyType::Bool, true, "geometry"),
                                        prop("joinVertices", PropertyType::Bool, true, "geometry"),
                                        prop("flipUvs", PropertyType::Bool, true, "geometry"),
                                        prop("improveCache", PropertyType::Bool, true, "geometry"),
                                        prop("sharedMeshes", PropertyType::Bool, true, "runtime"),
                                        prop("mipmaps", PropertyType::Bool, true, "runtime"),
                                        prop("importLights", PropertyType::Bool, true, "content"),
                                        prop("importCameras", PropertyType::Bool, false, "content"),
                                        prop("importAnimations", PropertyType::Bool, true, "content")};
    s.properties[0].assetTypeFilters = {"model3d", "scene"};
    s.properties[1].enumItems        = {"quality", "balanced", "mobile", "raw", "custom"};
    return s;
}
EditorValue SceneImportTarget::contentValue() const {
    return EditorValue::Object{{"source", value_.sourceAsset},
                               {"preset", value_.preset},
                               {"triangulate", value_.triangulate},
                               {"generateNormals", value_.generateNormals},
                               {"joinVertices", value_.joinVertices},
                               {"flipUvs", value_.flipUvs},
                               {"improveCache", value_.improveCache},
                               {"sharedMeshes", value_.sharedMeshes},
                               {"mipmaps", value_.mipmaps},
                               {"importLights", value_.importLights},
                               {"importCameras", value_.importCameras},
                               {"importAnimations", value_.importAnimations}};
}
PropertyReadResult SceneImportTarget::read(const SelectionSnapshot& s, const PropertyPath& p) const {
    if (!matches(s)) return {};
    const auto content = contentValue();
...
EditorResult<DomainOperation> SceneImportTarget::makeSet(const SelectionSnapshot& s, const PropertyPath& p,
    ...
    object[p.value()]   = v;
    if (p == PropertyPath("preset")) {
        const auto& name = *v.getIf<std::string>();
        if (name != "custom") {
            const bool raw = name == "raw", mobile = name == "mobile";
            object["triangulate"]      = true;
            object["generateNormals"]  = !raw;
            object["joinVertices"]     = !raw;
            object["flipUvs"]          = !raw;
            object["improveCache"]     = !raw;
            object["sharedMeshes"]     = !raw;
            object["mipmaps"]          = !raw;
            object["importLights"]     = !raw && !mobile;
            object["importCameras"]    = false;
            object["importAnimations"] = !raw && !mobile;
        }
    } else
...
std::vector<EditorDiagnostic> SceneImportTarget::validate() const {
    ...
    if (value_.sourceAsset.empty())
        d.push_back(eve::editing::ruleDiagnostic(eve::DiagnosticCode::InvalidArgument,
            RuleId("editor.scene-import.source"), DiagnosticSeverity::Error, "Scene import requires a source asset"));
    const auto presets = std::vector<std::string>{"quality", "balanced", "mobile", "raw", "custom"};
    if (std::find(presets.begin(), presets.end(), value_.preset) == presets.end())
        d.push_back(eve::editing::ruleDiagnostic(eve::DiagnosticCode::InvalidArgument,
            RuleId("editor.scene-import.preset"), DiagnosticSeverity::Error, "Unknown scene import preset"));
    if (!value_.triangulate)
        d.push_back(eve::editing::ruleDiagnostic(eve::DiagnosticCode::Unsupported,
            RuleId("editor.scene-import.non-triangles"), DiagnosticSeverity::Warning,
            "Disabled triangulation may produce meshes unsupported by render backends"));
    if (!value_.generateNormals)
        d.push_back(eve::editing::ruleDiagnostic(eve::DiagnosticCode::PreconditionViolation,
            RuleId("editor.scene-import.normals"), DiagnosticSeverity::Warning,
...
EditorResult<void> SceneImportTarget::applyDomainOperation(const DomainOperation& op) {
    ...
    bool*       dst[]  = {&v.triangulate,  &v.generateNormals, &v.joinVertices, &v.flipUvs,       &v.improveCache,
                          &v.sharedMeshes, &v.mipmaps,         &v.importLights, &v.importCameras, &v.importAnimations};
    const char* keys[] = {"triangulate",  "generateNormals", "joinVertices", "flipUvs",       "improveCache",
                          "sharedMeshes", "mipmaps",         "importLights", "importCameras", "importAnimations"};
    for (int i = 0; i < 10; ++i) {
