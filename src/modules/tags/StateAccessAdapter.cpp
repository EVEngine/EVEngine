...
bool isTagKey(std::string_view key) { return key.starts_with("tag.") || key.starts_with("tag:"); }
...
std::optional<eve::Value> TagStoreStateAdapter::value(std::string_view subject, std::string_view key) const {
    if (subject.empty() || key.empty() || !isTagKey(key)) return std::nullopt;
    return eve::Value(store_.hasTag(std::string(subject), tagName(key)));
}

std::optional<bool> TagStoreStateAdapter::hasTag(std::string_view subject, std::string_view tag) const {
    if (subject.empty() || tag.empty()) return std::nullopt;
    return store_.hasTag(std::string(subject), std::string(tag));
}

std::optional<eve::Value> TagStoreStateAdapter::state(std::string_view subject, std::string_view key) const {
    return value(subject, key);
}

eve::Result<eve::MutationReceipt> TagStoreStateAdapter::apply(std::span<const eve::StateMutation> mutations,
    ...
    for (std::size_t index = 0; index < mutations.size(); ++index) {
        ...
        if (mutation.subject.empty() || mutation.key.empty())
            return failure(eve::DiagnosticCode::InvalidArgument, "tag mutation requires subject and tag",
                           "mutations[" + std::to_string(index) + "]");
        if ((mutation.kind == eve::MutationKind::Set) && (!isTagKey(mutation.key) || !mutation.value.isBool()))
            return failure(eve::DiagnosticCode::InvalidArgument, "tag Set requires a tag.* key and boolean value",
                           "mutations[" + std::to_string(index) + "]");
        if (mutation.kind == eve::MutationKind::Set || mutation.kind == eve::MutationKind::AddTag ||
            mutation.kind == eve::MutationKind::RemoveTag)
            continue;
        return failure(eve::DiagnosticCode::Unsupported, "TagStore supports Set, AddTag, and RemoveTag only",
                       "mutations[" + std::to_string(index) + "]");
    }
    ...
    for (const auto& mutation : mutations) {
        ...
        switch (mutation.kind) {
            case eve::MutationKind::Set:
                ...
                break;
            case eve::MutationKind::AddTag: didChange = candidate.addTag(mutation.subject, tag); break;
            case eve::MutationKind::RemoveTag: didChange = candidate.removeTag(mutation.subject, tag); break;
            case eve::MutationKind::Remove:
            case eve::MutationKind::AddNumber: break;
        }
