...
std::uint64_t checksum(const image::ImageData& image) {
    ...
    for (std::size_t i = 0; i < image.getSize(); ++i) {
        hash ^= bytes[i];
        hash *= 1099511628211ull;
    }
    return hash;
}
SpriteStackLayerArtifact artifact(const image::ImageData& image, int index) {
    std::size_t covered = 0;
    for (int y = 0; y < image.getHeight(); ++y)
        for (int x = 0; x < image.getWidth(); ++x)
            if (image.getPixel(x, y).a > 0.f) ++covered;
    const std::size_t pixels = static_cast<std::size_t>(image.getWidth()) * image.getHeight();
    return {index, image.getWidth(), image.getHeight(), checksum(image),
            pixels ? static_cast<double>(covered) / pixels : 0.0};
}
}  // namespace
...
EditorResult<std::vector<SpriteStackLayerArtifact>> SpriteStackBakeRuntime::bake(
    ...
    try {
        if (document.value().sourceKind == "primitive")
    ...
    artifacts.reserve(raw.size());
    for (std::size_t i = 0; i < raw.size(); ++i) {
        candidate.emplace_back(raw[i]);
        artifacts.push_back(artifact(*raw[i], static_cast<int>(i)));
    }
    layers_ = std::move(candidate);
    stack_.reset();
    value_             = document.value();
...
EditorResult<spritestack::SpriteStack2D*> SpriteStackBakeRuntime::publish(graphics::Graphics* graphics,
    ...
    try {
        ...
        for (std::size_t i = 0; i < layers_.size(); ++i)
            candidate->setLayerImage(graphics, layers_[i].get(), static_cast<int>(i));
        candidate->setThickness(value_.layerOffset);
        candidate->setSize(value_.displayWidth, value_.displayHeight);
        candidate->setShadowEnabled(value_.shadow);
