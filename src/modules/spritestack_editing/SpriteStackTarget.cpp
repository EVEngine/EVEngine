...
PropertyDescriptor prop(std::string path, PropertyType type, EditorValue value, const char* category,
                        double minimum = -1e9, double maximum = 1e9) {
    PropertyDescriptor d;
    d.path            = PropertyPath(path);
    d.displayNameKey  = "editor.spritestack." + path;
    d.category        = category;
    d.type            = type;
...
SpriteStackDocumentTarget::SpriteStackDocumentTarget(std::string id) : id_(std::move(id)) {}
...
void* SpriteStackDocumentTarget::queryCapability(const CapabilityId& c) {
    return c == CapabilityId("eve.editor.target.spritestack-properties") ? static_cast<IPropertyProvider*>(this)
                                                                         : nullptr;
}
bool SpriteStackDocumentTarget::matches(const SelectionSnapshot& s) const {
    return s.items.size() == 1 && s.items.front().target == TargetId(id_) && s.items.front().item.value() == id_;
}
eve::Result<eve::Revision> SpriteStackDocumentTarget::currentRevision(const SelectionSnapshot& s) const {
    if (!matches(s))
...
PropertySchema SpriteStackDocumentTarget::schema(const SelectionSnapshot&) const {
    PropertySchema s;
    s.typeId     = "spritestack.asset";
    s.properties = {prop("source.kind", PropertyType::Enum, "primitive", "source"),
                    prop("source.value", PropertyType::AssetRef, "box", "source"),
                    prop("bake.layers", PropertyType::Int, int64_t{16}, "bake", 1, 512),
                    prop("bake.width", PropertyType::Int, int64_t{128}, "bake", 1, 4096),
                    prop("bake.height", PropertyType::Int, int64_t{128}, "bake", 1, 4096),
                    prop("bake.axis", PropertyType::Enum, "y", "bake"),
                    prop("bake.thickness", PropertyType::Float, 0.0, "bake", 0, 100000),
                    prop("bake.padding", PropertyType::Float, .04, "bake", 0, .49),
                    prop("bake.shade", PropertyType::Bool, true, "bake"),
                    prop("bake.tint", PropertyType::Color, EditorValue::Array{1.0, 1.0, 1.0, 1.0}, "bake", 0, 1),
                    prop("display.size", PropertyType::Vec2, EditorValue::Array{64.0, 64.0}, "display", .01, 100000),
                    prop("display.layerOffset", PropertyType::Float, 1.0, "display", .001, 100000),
                    prop("display.shadow", PropertyType::Bool, true, "display"),
                    prop("display.shadowOpacity", PropertyType::Float, .3, "display", 0, 1),
                    prop("display.outlineWidth", PropertyType::Float, 0.0, "display", 0, 10000)};
    s.properties[0].enumItems        = {"primitive", "model"};
    s.properties[1].assetTypeFilters = {"model3d"};
    s.properties[5].enumItems        = {"x", "y", "z"};
    return s;
}
EditorValue SpriteStackDocumentTarget::contentValue() const {
    const auto& v = value_;
    return EditorValue::Object{{"sourceKind", v.sourceKind},
                               {"source", v.source},
                               {"layers", int64_t{v.bake.layerCount}},
                               {"width", int64_t{v.bake.imageW}},
                               {"height", int64_t{v.bake.imageH}},
                               {"axis", v.bake.axis},
                               {"thickness", v.bake.thickness},
                               {"padding", v.bake.padding},
                               {"shade", v.bake.shade},
                               {"tint", EditorValue::Array{v.bake.tintR, v.bake.tintG, v.bake.tintB, 1.0}},
                               {"displaySize", EditorValue::Array{v.displayWidth, v.displayHeight}},
                               {"layerOffset", v.layerOffset},
                               {"shadow", v.shadow},
                               {"shadowOpacity", v.shadowOpacity},
                               {"outlineWidth", v.outlineWidth}};
}
PropertyReadResult SpriteStackDocumentTarget::read(const SelectionSnapshot& s, const PropertyPath& p) const {
    if (!matches(s) || !schema(s).find(p)) return {};
    const auto& v = value_;
    EditorValue value;
    if (p == PropertyPath("source.kind"))
        value = v.sourceKind;
    else if (p == PropertyPath("source.value"))
        value = v.source;
    else if (p == PropertyPath("bake.layers"))
        value = int64_t{v.bake.layerCount};
    else if (p == PropertyPath("bake.width"))
        value = int64_t{v.bake.imageW};
    else if (p == PropertyPath("bake.height"))
        value = int64_t{v.bake.imageH};
    else if (p == PropertyPath("bake.axis"))
        value = v.bake.axis;
    else if (p == PropertyPath("bake.thickness"))
        value = v.bake.thickness;
    else if (p == PropertyPath("bake.padding"))
        value = v.bake.padding;
    else if (p == PropertyPath("bake.shade"))
        value = v.bake.shade;
    else if (p == PropertyPath("bake.tint"))
        value = EditorValue::Array{v.bake.tintR, v.bake.tintG, v.bake.tintB, 1.0};
    else if (p == PropertyPath("display.size"))
        value = EditorValue::Array{v.displayWidth, v.displayHeight};
    else if (p == PropertyPath("display.layerOffset"))
        value = v.layerOffset;
    else if (p == PropertyPath("display.shadow"))
        value = v.shadow;
    else if (p == PropertyPath("display.shadowOpacity"))
...
EditorResult<DomainOperation> SpriteStackDocumentTarget::replacement(EditorValue content,
    ...
    op.hasInverse  = true;
    op.affectedProperties.push_back(property);
...
EditorResult<DomainOperation> SpriteStackDocumentTarget::makeSet(const SelectionSnapshot& s, const PropertyPath& p,
    ...
    if (p == PropertyPath("source.kind"))
        v.sourceKind = *value.getIf<std::string>();
    else if (p == PropertyPath("source.value"))
        v.source = *value.getIf<std::string>();
    else if (p == PropertyPath("bake.layers"))
        v.bake.layerCount = static_cast<int>(*value.getIf<int64_t>());
    else if (p == PropertyPath("bake.width"))
        v.bake.imageW = static_cast<int>(*value.getIf<int64_t>());
    else if (p == PropertyPath("bake.height"))
        v.bake.imageH = static_cast<int>(*value.getIf<int64_t>());
    else if (p == PropertyPath("bake.axis"))
        v.bake.axis = *value.getIf<std::string>();
    else if (p == PropertyPath("bake.thickness"))
        v.bake.thickness = static_cast<float>(*value.getIf<double>());
    else if (p == PropertyPath("bake.padding"))
        v.bake.padding = static_cast<float>(*value.getIf<double>());
    else if (p == PropertyPath("bake.shade"))
        v.bake.shade = *value.getIf<bool>();
    else if (p == PropertyPath("bake.tint")) {
        const auto& a = *value.getIf<EditorValue::Array>();
        v.bake.tintR  = static_cast<float>(*a[0].getIf<double>());
        v.bake.tintG  = static_cast<float>(*a[1].getIf<double>());
        v.bake.tintB  = static_cast<float>(*a[2].getIf<double>());
    } else if (p == PropertyPath("display.size")) {
        const auto& a   = *value.getIf<EditorValue::Array>();
        v.displayWidth  = static_cast<float>(*a[0].getIf<double>());
        v.displayHeight = static_cast<float>(*a[1].getIf<double>());
    } else if (p == PropertyPath("display.layerOffset"))
        v.layerOffset = static_cast<float>(*value.getIf<double>());
    else if (p == PropertyPath("display.shadow"))
        v.shadow = *value.getIf<bool>();
    else if (p == PropertyPath("display.shadowOpacity"))
        v.shadowOpacity = static_cast<float>(*value.getIf<double>());
    else
...
std::vector<EditorDiagnostic> SpriteStackDocumentTarget::validate() const {
    ...
    const auto&                   v = value_;
    if (v.sourceKind != "primitive" && v.sourceKind != "model")
        d.push_back(eve::editing::ruleDiagnostic(eve::DiagnosticCode::InvalidArgument,
                     RuleId("editor.spritestack.source-kind"), DiagnosticSeverity::Error,
                     "SpriteStack source kind must be primitive or model"));
    if (v.source.empty())
        d.push_back(eve::editing::ruleDiagnostic(eve::DiagnosticCode::InvalidArgument,
            RuleId("editor.spritestack.source"), DiagnosticSeverity::Error, "SpriteStack source must not be empty"));
    if (v.sourceKind == "primitive" && v.source != "box" && v.source != "sphere" && v.source != "cylinder" &&
        v.source != "cone")
        d.push_back(eve::editing::ruleDiagnostic(eve::DiagnosticCode::InvalidArgument,
            RuleId("editor.spritestack.primitive"), DiagnosticSeverity::Error, "Unknown SpriteStack primitive"));
    const bool axisInvalid = v.bake.axis != "x" && v.bake.axis != "y" && v.bake.axis != "z";
    if (v.bake.layerCount < 1 || v.bake.layerCount > 512 || v.bake.imageW < 1 || v.bake.imageW > 4096 ||
        v.bake.imageH < 1 || v.bake.imageH > 4096 || axisInvalid || !std::isfinite(v.bake.thickness) ||
        v.bake.thickness < 0 || !std::isfinite(v.bake.padding) || v.bake.padding < 0 || v.bake.padding >= .5f)
        d.push_back(eve::editing::ruleDiagnostic(eve::DiagnosticCode::InvalidArgument,
            RuleId("editor.spritestack.bake-range"), DiagnosticSeverity::Error,
            "SpriteStack bake settings are invalid"));
    const std::uint64_t bytes = static_cast<std::uint64_t>(std::max(0, v.bake.layerCount)) *
                                std::max(0, v.bake.imageW) * std::max(0, v.bake.imageH) * 4ull;
    if (bytes > 512ull * 1024 * 1024)
        d.push_back(eve::editing::ruleDiagnostic(eve::DiagnosticCode::PreconditionViolation,
            RuleId("editor.spritestack.memory"), DiagnosticSeverity::Error,
            "SpriteStack output exceeds the 512 MiB decoded budget"));
    else if (bytes > 128ull * 1024 * 1024)
        d.push_back(eve::editing::ruleDiagnostic(eve::DiagnosticCode::PreconditionViolation,
            RuleId("editor.spritestack.memory-warning"), DiagnosticSeverity::Warning,
            "SpriteStack output exceeds 128 MiB decoded"));
    if (!std::isfinite(v.displayWidth) || v.displayWidth <= 0 || !std::isfinite(v.displayHeight) ||
        v.displayHeight <= 0 || !std::isfinite(v.layerOffset) || v.layerOffset <= 0 ||
        !std::isfinite(v.shadowOpacity) || v.shadowOpacity < 0 || v.shadowOpacity > 1 ||
        !std::isfinite(v.outlineWidth) || v.outlineWidth < 0)
        d.push_back(eve::editing::ruleDiagnostic(eve::DiagnosticCode::InvalidArgument,
            RuleId("editor.spritestack.display"), DiagnosticSeverity::Error,
...
EditorResult<void> SpriteStackDocumentTarget::applyDomainOperation(const DomainOperation& op) {
    ...
    const auto *          sKind = field(op.payload, "sourceKind"), *source = field(op.payload, "source"),
               ...
               *shadow = field(op.payload, "shadow");
    const auto* sk     = sKind ? sKind->getIf<std::string>() : nullptr;
    const auto* sv     = source ? source->getIf<std::string>() : nullptr;
    const auto* av     = axis ? axis->getIf<std::string>() : nullptr;
    ...
    v.bake.shade            = *sh;
    v.shadow                = *sd;
    int*        ints[]      = {&v.bake.layerCount, &v.bake.imageW, &v.bake.imageH};
    const char* intFields[] = {"layers", "width", "height"};
    for (int i = 0; i < 3; ++i) {
    ...
    const char* floatFields[] = {"thickness", "padding", "layerOffset", "shadowOpacity", "outlineWidth"};
    for (int i = 0; i < 5; ++i) {
