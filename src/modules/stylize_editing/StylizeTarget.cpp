...
EditorValue passValue(const StylizePassValue& p) {
    EditorValue::Object overrides;
    for (const auto& [name, value] : p.overrides) overrides[name] = value;
    return EditorValue::Object{{"id", p.id.value()},
                               {"style", p.style},
                               {"enabled", p.enabled},
...
EditorResult<StylizePassValue> parsePass(const EditorValue& v) {
    ...
    const auto* style     = stylev ? stylev->getIf<std::string>() : nullptr;
    const auto* enabled   = enabledv ? enabledv->getIf<bool>() : nullptr;
    const auto* priority  = priorityv ? priorityv->getIf<int64_t>() : nullptr;
...
bool hasError(const std::vector<EditorDiagnostic>& d) {
    return std::any_of(d.begin(), d.end(), [](const auto& v) { return v.severity() == DiagnosticSeverity::Error; });
}
PropertyDescriptor property(const std::string& path, PropertyType type, EditorValue value, const char* category) {
    PropertyDescriptor d;
    d.path           = PropertyPath(path);
    d.displayNameKey = "editor.stylize." + path;
    d.category       = category;
    d.type           = type;
    d.flags          = PropertyFlag::Runtime;
    d.defaultValue   = std::move(value);
    if (path == "pass.priority") {
        d.numeric.minimum = -100000;
        d.numeric.maximum = 100000;
    }
    return d;
}
}  // namespace
...
bool StylizeRecipeTarget::matches(const SelectionSnapshot& s) const {
    if (s.items.empty()) return false;
    for (const auto& i : s.items)
        if (i.target != TargetId(id_) || !passes_.contains(ObjectId(i.item.value()))) return false;
    return true;
}
eve::Result<eve::Revision> StylizeRecipeTarget::currentRevision(const SelectionSnapshot& s) const {
    if (!matches(s))
...
PropertySchema StylizeRecipeTarget::schema(const SelectionSnapshot& s) const {
    ...
    schema.typeId     = "stylize.pass";
    schema.properties = {property("pass.enabled", PropertyType::Bool, true, "pass"),
                         property("pass.priority", PropertyType::Int, int64_t{0}, "pass")};
    if (!matches(s)) return schema;
    const std::string style = passes_.at(ObjectId(s.items.front().item.value())).style;
    for (const auto& i : s.items)
        if (passes_.at(ObjectId(i.item.value())).style != style) return schema;
    for (int index = 0; index < stylize::styleParamCount(style); ++index) {
        const auto* d = stylize::styleParameterAt(style, index);
        auto p = property("parameter." + std::string(d->id), PropertyType::Float, static_cast<double>(d->defaultValue),
                          "parameters");
        p.numeric.minimum = d->minValue;
        p.numeric.maximum = d->maxValue;
        schema.properties.push_back(std::move(p));
    }
    return schema;
}
PropertyReadResult StylizeRecipeTarget::read(const SelectionSnapshot& s, const PropertyPath& path) const {
    if (!matches(s) || !schema(s).find(path)) return {};
    std::optional<EditorValue> common;
    for (const auto& i : s.items) {
        const auto& p = passes_.at(ObjectId(i.item.value()));
        EditorValue value;
        if (path == PropertyPath("pass.enabled"))
            value = p.enabled;
        else if (path == PropertyPath("pass.priority"))
            value = int64_t{p.priority};
        else {
            const std::string name  = path.value().substr(std::string("parameter.").size());
            auto              found = p.overrides.find(name);
            const auto*       d     = stylize::findStyleParameter(p.style, name);
            value                   = found == p.overrides.end() ? EditorValue(static_cast<double>(d->defaultValue))
                                                                 : EditorValue(found->second);
        }
        if (!common)
            common = value;
...
EditorValue StylizeRecipeTarget::contentValue() const {
    EditorValue::Array array;
...
EditorResult<DomainOperation> StylizeRecipeTarget::replacement(EditorValue payload,
    ...
    op.hasInverse  = true;
    if (!propertyName.empty()) op.affectedProperties.push_back(propertyName);
...
EditorResult<DomainOperation> StylizeRecipeTarget::makeSet(const SelectionSnapshot& s, const PropertyPath& path,
    ...
    for (const auto& i : s.items) {
        ...
        if (path == PropertyPath("pass.enabled"))
            p.enabled = *value.getIf<bool>();
        else if (path == PropertyPath("pass.priority"))
            p.priority = static_cast<int>(*value.getIf<int64_t>());
        else
...
EditorResult<DomainOperation> StylizeRecipeTarget::makeReset(const SelectionSnapshot& s,
    ...
    if (path.value().starts_with("parameter.")) {
        auto candidate = *this;
        for (const auto& i : s.items)
            candidate.passes_[ObjectId(i.item.value())].overrides.erase(path.value().substr(10));
        return replacement(candidate.contentValue(), path.value());
    }
...
EditorResult<DomainOperation> StylizeRecipeTarget::makeMove(const ObjectId& id, std::size_t index) const {
    ...
    for (std::size_t i = 0; i < candidate.order_.size(); ++i)
...
std::vector<StylizePassValue> StylizeRecipeTarget::passes() const {
    std::vector<StylizePassValue> result;
    for (const auto& id : order_) result.push_back(passes_.at(id));
    return result;
}
std::vector<EditorDiagnostic> StylizeRecipeTarget::validate() const {
    std::vector<EditorDiagnostic> d;
    std::string                   stage;
    for (const auto& id : order_) {
        const auto& p          = passes_.at(id);
        const auto* definition = stylize::findStyleDefinition(p.style);
        if (!definition) {
            d.push_back(eve::editing::ruleDiagnostic(eve::DiagnosticCode::InvalidArgument,
                RuleId("editor.stylize.unknown-style"), DiagnosticSeverity::Error, "Unknown style: " + p.style));
            continue;
        }
        if (!p.enabled) continue;
        const std::string current = graphics::postEffectStageName(definition->stage);
        if (stage.empty())
            stage = current;
        else if (stage != current)
            d.push_back(eve::editing::ruleDiagnostic(eve::DiagnosticCode::PreconditionViolation,
                RuleId("editor.stylize.mixed-stage"), DiagnosticSeverity::Error,
                "Enabled recipe passes must share one pipeline stage"));
        if (!definition->post)
            d.push_back(eve::editing::ruleDiagnostic(eve::DiagnosticCode::PreconditionViolation,
                RuleId("editor.stylize.mesh-in-recipe"), DiagnosticSeverity::Error,
                "Post recipe cannot contain a mesh-only style: " + p.style));
    }
    if (order_.empty())
        d.push_back(eve::editing::ruleDiagnostic(eve::DiagnosticCode::PreconditionViolation,
            RuleId("editor.stylize.empty"), DiagnosticSeverity::Warning, "Stylize recipe has no passes"));
    return d;
}
