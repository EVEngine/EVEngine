...
EditorResult<void> StylizeRecipeRuntime::publish(const StylizeRecipeTarget& document, graphics::Graphics* graphics) {
    ...
    try {
        for (const auto& pass : document.passes()) {
            ...
            auto instance = std::make_unique<stylize::StyleInstance>(pass.style);
            instance->setPriority(pass.priority);
            for (const auto& [name, value] : pass.overrides) instance->setFloat(name, static_cast<float>(value));
            recipe->add(instance.get());
...
EditorResult<void> StylizeRecipeRuntime::apply(graphics::Graphics* graphics, graphics::Texture* source,
    ...
    try {
        if (instances_.empty()) {
            ...
            graphics->drawTexturedRect(source, 0, 0, static_cast<float>(destination->getWidth()),
                                       static_cast<float>(destination->getHeight()), graphics::Color(1, 1, 1, 1));
