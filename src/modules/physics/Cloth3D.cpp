...
int64_t pairKey(int lo, int hi) { return (int64_t(lo) << 32) | int64_t(hi); }
...
float signedAngle(const V3& a, const V3& b, const V3& u) { return std::atan2(dot(cross(a, b), u), dot(a, b)); }
...
float closestPointSegments(const V3& p1, const V3& p2, const V3& q1, const V3& q2, V3& cp1, V3& cp2) {
...
float triangleDistance(const V3* A, const V3* B, int featA[3], int featB[3], int& countA, int& countB, V3& cpA,
                       V3& cpB) {
    ...
    for (const auto& ea : aEdges) {
        for (const auto& eb : bEdges) {
            V3          cp1;
            V3          cp2;
            const float d = closestPointSegments(A[ea[0]], A[ea[1]], B[eb[0]], B[eb[1]], cp1, cp2);
            if (d < best) {
                best     = d;
                cpA      = cp1;
                cpB      = cp2;
...
Cloth3D::Cloth3D(int cols, int rows, float spacing, float originX, float originY, float originZ)
    : cols_(cols), rows_(rows), spacing_(spacing), originX_(originX), originY_(originY), originZ_(originZ) {
    auto model = ClothModel::grid(cols, rows, spacing, originX, originY, originZ);
    if (!model)
        throw Exception("%s", model.error() ? model.error()->message().c_str() : "Cloth3D grid model creation failed");
    initializeFromModel(model.value());
}

Cloth3D::Cloth3D(const ClothModel& model) { initializeFromModel(model); }
...
bool Cloth3D::validIndex(int index) const { return index >= 0 && index < getParticleCount(); }
...
void Cloth3D::setStiffness(float stiffness) { stiffness_ = std::clamp(stiffness, 0.f, 1.f); }

void Cloth3D::setStretchCompliance(float compliance) {
    stretchCompliance_ = std::isfinite(compliance) ? std::max(0.f, compliance) : 0.f;
}

void Cloth3D::setShearCompliance(float compliance) {
    shearCompliance_ = std::isfinite(compliance) ? std::max(0.f, compliance) : 0.f;
}

void Cloth3D::setBendCompliance(float compliance) {
    bendCompliance_ = std::isfinite(compliance) ? std::max(0.f, compliance) : 0.f;
}

void Cloth3D::setIterations(int iterations) { iterations_ = std::max(1, iterations); }

void Cloth3D::setDamping(float damping) { damping_ = std::clamp(damping, 0.f, 1.f); }

void Cloth3D::setParticleSize(float size) { particleSize_ = std::max(0.01f, size); }

void Cloth3D::setParticleMass(float mass) { particleMass_ = std::max(1e-4f, mass); }

void Cloth3D::initializeFromModel(const ClothModel& model) {
    cols_    = model.gridCols();
    rows_    = model.gridRows();
    spacing_ = model.gridSpacing();
    if (!model.particles().empty()) {
        originX_ = model.particles().front().x;
        originY_ = model.particles().front().y;
        originZ_ = model.particles().front().z;
    }
    particles_.clear();
    particles_.reserve(model.particles().size());
    for (const ClothModelParticle& source : model.particles()) {
        Particle particle;
        particle.x = particle.px = source.x;
        particle.y = particle.py = source.y;
        particle.z = particle.pz = source.z;
        particle.inverseMass      = source.inverseMass;
        particle.pinned          = source.inverseMass == 0.f;
        particles_.push_back(particle);
    }
    restParticles_ = particles_;

    links_.clear();
    links_.reserve(model.distanceConstraints().size());
    for (const ClothModelDistanceConstraint& source : model.distanceConstraints())
        links_.push_back({source.a, source.b, source.restLength, source.kind, 0.f});

    triangles_.clear();
    triangles_.reserve(model.triangles().size());
    for (const ClothModelTriangle& source : model.triangles())
        triangles_.push_back({{source.vertices[0], source.vertices[1], source.vertices[2]}});

    foldPairs_.clear();
    foldPairs_.reserve(model.foldConstraints().size());
    for (const ClothModelFoldConstraint& source : model.foldConstraints())
        foldPairs_.push_back({source.a, source.b, source.oppositeA, source.oppositeB});
    tethers_.clear();
    tethers_.reserve(model.tetherConstraints().size());
    for (const ClothModelTetherConstraint& source : model.tetherConstraints())
        tethers_.push_back({source.particle, source.anchor, source.maxLength, 0.f});
    restVolume_ = model.restVolume();
    buildLinkKeys();
}

void Cloth3D::setWindVelocity(float vx, float vy, float vz) {
    windVelocityX_ = vx;
    windVelocityY_ = vy;
    windVelocityZ_ = vz;
}

float Cloth3D::getWindVelocityX() const { return windVelocityX_; }

float Cloth3D::getWindVelocityY() const { return windVelocityY_; }

float Cloth3D::getWindVelocityZ() const { return windVelocityZ_; }

void Cloth3D::setAerodynamics(float airDensity, float dragCoefficient, float liftCoefficient) {
    airDensity_      = std::max(0.f, airDensity);
    dragCoefficient_ = std::max(0.f, dragCoefficient);
    liftCoefficient_ = std::max(0.f, liftCoefficient);
}

float Cloth3D::getAirDensity() const { return airDensity_; }

float Cloth3D::getDragCoefficient() const { return dragCoefficient_; }

float Cloth3D::getLiftCoefficient() const { return liftCoefficient_; }

void Cloth3D::setSelfCollision(bool on) { selfCollision_ = on; }

void Cloth3D::setFoldStiffness(float k) { foldStiffness_ = std::clamp(k, 0.f, 1.f); }

void Cloth3D::setMaxFoldAngle(float degrees) { maxFoldAngle_ = std::clamp(degrees, 0.f, 180.f) * kPi / 180.f; }
...
void Cloth3D::setCollisionMaterial(float friction, float restitution) {
    collisionFriction_    = std::isfinite(friction) ? std::clamp(friction, 0.f, 1.f) : 0.f;
    collisionRestitution_ = std::isfinite(restitution) ? std::clamp(restitution, 0.f, 1.f) : 0.f;
}

void Cloth3D::setCollisionFilter(uint64_t categoryBits, uint64_t maskBits) {
    collisionCategoryBits_ = categoryBits;
    collisionMaskBits_     = maskBits;
}
...
void Cloth3D::unpin(int index) {
    ...
    particle.inverseMass = 1.f;
}

void Cloth3D::pinTopRow() {
    for (int c = 0; c < cols_; ++c) pin(c);
}
...
void Cloth3D::setParticleInverseMass(int index, float inverseMass) {
    if (!validIndex(index)) throw Exception("Cloth3D.setParticleInverseMass: index out of range");
    Particle& particle = particles_[static_cast<size_t>(index)];
    particle.inverseMass = std::isfinite(inverseMass) ? std::max(0.f, inverseMass) : 0.f;
    particle.pinned      = particle.inverseMass == 0.f;
}

float Cloth3D::getParticleInverseMass(int index) const {
    return validIndex(index) ? particles_[static_cast<size_t>(index)].inverseMass : 0.f;
}
...
int64_t Cloth3D::cellKey(int cx, int cy, int cz) const {
    ...
    return int64_t(uint32_t(cx) & 0x1fffffu) | (int64_t(uint32_t(cy) & 0x1fffffu) << 21) |
...
void Cloth3D::integrate(float dt) {
    ...
    std::vector<float> aerodynamicAcceleration;
    if (airDensity_ > 0.f && (dragCoefficient_ > 0.f || liftCoefficient_ > 0.f)) {
        aerodynamicAcceleration.assign(particles_.size() * 3, 0.f);
        accumulateAerodynamicAcceleration(dt, aerodynamicAcceleration);
    }

    for (size_t particleIndex = 0; particleIndex < particles_.size(); ++particleIndex) {
        ...
        p.py              = p.y;
        p.pz              = p.z;
        const float aeroX = aerodynamicAcceleration.empty() ? 0.f : aerodynamicAcceleration[particleIndex * 3];
        const float aeroY = aerodynamicAcceleration.empty() ? 0.f : aerodynamicAcceleration[particleIndex * 3 + 1];
        const float aeroZ = aerodynamicAcceleration.empty() ? 0.f : aerodynamicAcceleration[particleIndex * 3 + 2];
        p.x += vx + (ax + aeroX) * dt * dt;
        p.y += vy + (ay + aeroY) * dt * dt;
        p.z += vz + (az + aeroZ) * dt * dt;
        if (hasInteract) {
...
void Cloth3D::accumulateAerodynamicAcceleration(float dt, std::vector<float>& accelerations) const {
    const float invDt   = dt > 1e-6f ? 1.f / dt : 0.f;
    const float invMass = 1.f / particleMass_;
    for (const Tri& tri : triangles_) {
        const Particle& a = particles_[static_cast<size_t>(tri.v[0])];
        const Particle& b = particles_[static_cast<size_t>(tri.v[1])];
        const Particle& c = particles_[static_cast<size_t>(tri.v[2])];

        const V3    ab{b.x - a.x, b.y - a.y, b.z - a.z};
        const V3    ac{c.x - a.x, c.y - a.y, c.z - a.z};
        const V3    areaNormal = cross(ab, ac);
        const float twiceArea  = vlen(areaNormal);
        if (twiceArea < 1e-8f) continue;
        V3 normal = mul(1.f / twiceArea, areaNormal);

        const V3    clothVelocity{((a.x - a.px) + (b.x - b.px) + (c.x - c.px)) * (invDt / 3.f),
                                  ((a.y - a.py) + (b.y - b.py) + (c.y - c.py)) * (invDt / 3.f),
                                  ((a.z - a.pz) + (b.z - b.pz) + (c.z - c.pz)) * (invDt / 3.f)};
        const V3    relativeWind{windVelocityX_ - clothVelocity.x, windVelocityY_ - clothVelocity.y,
                                 windVelocityZ_ - clothVelocity.z};
        const float speed = vlen(relativeWind);
        if (speed < 1e-5f) continue;
        const V3 windDirection = mul(1.f / speed, relativeWind);
        float    incidence     = dot(normal, windDirection);
        if (incidence < 0.f) {
            normal    = mul(-1.f, normal);
            incidence = -incidence;
        }
        if (incidence < 1e-5f) continue;

        const float dynamicPressureArea = 0.5f * airDensity_ * speed * speed * (twiceArea * 0.5f);
        const V3    drag                = mul(dragCoefficient_ * incidence * dynamicPressureArea, windDirection);
        V3          liftDirection       = sub(normal, mul(incidence, windDirection));
        const float liftLength          = vlen(liftDirection);
        if (liftLength > 1e-6f)
            liftDirection = mul(1.f / liftLength, liftDirection);
        else
            liftDirection = V3{0.f, 0.f, 0.f};
        const V3 lift         = mul(liftCoefficient_ * incidence * dynamicPressureArea, liftDirection);
        const V3 acceleration = mul(invMass / 3.f, add(drag, lift));

        for (int vertex : tri.v) {
            if (particles_[static_cast<size_t>(vertex)].pinned) continue;
            accelerations[static_cast<size_t>(vertex) * 3] += acceleration.x;
            accelerations[static_cast<size_t>(vertex) * 3 + 1] += acceleration.y;
            accelerations[static_cast<size_t>(vertex) * 3 + 2] += acceleration.z;
        }
    }
}

void Cloth3D::solveConstraints(float dt) {
    for (Link& link : links_) link.lambda = 0.f;
    for (Tether& tether : tethers_) tether.lambda = 0.f;
    for (SkinConstraint& skin : skinConstraints_) skin.lambda = 0.f;
    for (Attachment& attachment : attachments_) attachment.lambda = 0.f;
    volumeLambda_            = 0.f;
    const float invDtSquared = dt > 1e-8f ? 1.f / (dt * dt) : 0.f;
    for (int iter = 0; iter < iterations_; ++iter) {
        for (Link& link : links_) {
            ...
            float compliance = stretchCompliance_;
            if (link.kind == ClothConstraintKind::Shear)
                compliance = shearCompliance_;
            else if (link.kind == ClothConstraintKind::Bend)
                compliance = bendCompliance_;
            if (compliance > 0.f) {
                const float weightA     = a.pinned ? 0.f : a.inverseMass;
                const float weightB     = b.pinned ? 0.f : b.inverseMass;
                const float alpha       = compliance * invDtSquared;
                const float deltaLambda = (-(dist - link.rest) - alpha * link.lambda) / (weightA + weightB + alpha);
                link.lambda += deltaLambda;
                const float nx = dx / dist;
                const float ny = dy / dist;
                const float nz = dz / dist;
                a.x -= weightA * nx * deltaLambda;
                a.y -= weightA * ny * deltaLambda;
                a.z -= weightA * nz * deltaLambda;
                b.x += weightB * nx * deltaLambda;
                b.y += weightB * ny * deltaLambda;
                b.z += weightB * nz * deltaLambda;
                continue;
            }
        ...
        solveVolumeConstraint(dt);
        solveSkinConstraints(dt);
        solveAttachments(dt);
    ...
    const auto insertTri = [&](int ti) {
        ...
        for (int cz = z0; cz <= z1; ++cz)
            for (int cy = y0; cy <= y1; ++cy)
                for (int cx = x0; cx <= x1; ++cx) triHash[triCellKey(cx, cy, cz)].push_back(ti);
    };
        ...
        for (int cz = z0; cz <= z1; ++cz)
            for (int cy = y0; cy <= y1; ++cy)
                for (int cx = x0; cx <= x1; ++cx) {
                    ...
                    if (it != triHash.end()) cells.insert(cells.end(), it->second.begin(), it->second.end());
                }
        for (int j : cells) {
            if (j <= i) continue;
            ...
            const V3    A[3] = {pos[static_cast<size_t>(triI.v[0])], pos[static_cast<size_t>(triI.v[1])],
                                pos[static_cast<size_t>(triI.v[2])]};
            const V3    B[3] = {pos[static_cast<size_t>(triJ.v[0])], pos[static_cast<size_t>(triJ.v[1])],
...
void Cloth3D::collideWorld(float dt) {
    ...
    for (int i = 0; i < getParticleCount(); ++i) {
        ...
        if (p.pinned) continue;
        if (world_->pointProbeFiltered(p.x, p.y, p.z, particleSize_, &contact, collisionCategoryBits_,
                                       collisionMaskBits_) != ClothProbeStatus::Hit ||
            !contact.hit)
            continue;
        ...
        float      vbz      = 0.f;
        float      bodyMass = 0.f;
        const bool dynamic  = contact.body != nullptr && contact.body->getType() == "dynamic";
        if (dynamic) {
        ...
        const float vn = (vpx - vbx) * contact.nx + (vpy - vby) * contact.ny + (vpz - vbz) * contact.nz;
        if (vn < 0.f) {
            const float restitution = std::max(collisionRestitution_, contact.restitution);
            const float friction = std::clamp(std::sqrt(collisionFriction_ * std::max(0.f, contact.friction)), 0.f, 1.f);
            const float relativeX = vpx - vbx;
            const float relativeY = vpy - vby;
            const float relativeZ = vpz - vbz;
            const float tangentX = relativeX - vn * contact.nx;
            const float tangentY = relativeY - vn * contact.ny;
            const float tangentZ = relativeZ - vn * contact.nz;
            const float nextNormal = -vn * restitution;
            const float nextVx = vbx + tangentX * (1.f - friction) + contact.nx * nextNormal;
            const float nextVy = vby + tangentY * (1.f - friction) + contact.ny * nextNormal;
            const float nextVz = vbz + tangentZ * (1.f - friction) + contact.nz * nextNormal;
            p.px = p.x - nextVx * dt;
            p.py = p.y - nextVy * dt;
            p.pz = p.z - nextVz * dt;
            ...
            if (dynamic && bodyMass > 0.f) {
                contact.body->applyLinearImpulse(-contact.nx * j, -contact.ny * j, -contact.nz * j);
            }
        }
    }
}
...
void Cloth3D::collideBounds() {
    ...
    const float bounce = collisionRestitution_;
...
void Cloth3D::updateSubsteps(float dt, int substeps) {
    if (destroyed_ || substeps < 1) return;

    const float h = dt / float(substeps);
    for (int s = 0; s < substeps; ++s) {
        integrate(h);
        applyAutomaticTearing();
        solveConstraints(h);
        solveFoldConstraint();
        solveSelfCollision();
...
eve::Result<void> Cloth3D::step(const eve::SimulationStep& stepValue, const SimulationSettings& settings) {
    if (destroyed_)
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::PreconditionViolation, "Cannot step a destroyed cloth", "physics.cloth3d.step"));
    ...
    try {
        ...
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::Failed, std::string("Cloth3D step failed: ") + error.what(), "physics.cloth3d.step"));
    } catch (...) {
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::Failed, "Cloth3D step failed with an unknown exception", "physics.cloth3d.step"));
    }
    observation_ = std::move(next).takeValue();
    return eve::Result<void>::success(eve::Status::success(eve::StatusCode::Applied));
}

eve::Result<void> Cloth3D::restoreObservation(const SimulationObservation& observation) {
    auto valid = detail::validateSimulationObservation(observation, "physics.cloth3d.restoreObservation");
    if (!valid) return valid;
    if (destroyed_)
        return eve::Result<void>::failure(eve::Diagnostic::error(eve::DiagnosticCode::PreconditionViolation,
                                                                 "Cannot restore a destroyed cloth",
...
void Cloth3D::draw(graphics::Graphics* gfx) {
    ...
    if (!mesh_ || meshVertexCount_ != vertexCount || meshIndexCount_ != indexCount) {
        ...
        mesh_ = gfx->newMeshFromArrays(pos.data(), nrm.data(), uv.data(), vertexCount, indices.data(), indexCount);
    ...
    if (mesh_) {
        gfx->drawMesh(mesh_, glm::mat4(1.f), nullptr, Color(colorR_, colorG_, colorB_, colorA_));
    }
}
