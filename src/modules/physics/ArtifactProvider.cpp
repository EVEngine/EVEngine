...
bool readBounds(const eve::Value::Object& object, eve::artifact::Bounds& output) {
    ...
    if (!bounds || !readFloat(*bounds, "minX", output.minX) ||
        !readFloat(*bounds, "minY", output.minY) || !readFloat(*bounds, "minZ", output.minZ) ||
        !readFloat(*bounds, "maxX", output.maxX) || !readFloat(*bounds, "maxY", output.maxY) ||
        !readFloat(*bounds, "maxZ", output.maxZ))
        return false;
    output.valid = true;
...
bool validGeometry(const std::vector<float>& vertices,
                   const std::vector<std::uint32_t>& indices) {
    if (vertices.empty() || vertices.size() % 3u != 0u || indices.empty() ||
        indices.size() % 3u != 0u || vertices.size() / 3u >
            static_cast<std::size_t>(std::numeric_limits<std::int32_t>::max()))
        return false;
    for (const float value : vertices)
        if (!std::isfinite(value)) return false;
    for (const auto index : indices)
        if (index >= vertices.size() / 3u || index >
...
eve::Result<std::unique_ptr<eve::artifact::PreparedPublication>> prepareFailure(
    eve::DiagnosticCode code, std::string message) {
...
public:
    PhysicsArtifactStage(PhysicsArtifactProvider& owner, PhysicsArtifactCollider collider,
                         std::unique_ptr<World3D> world, std::unique_ptr<Body3D> body,
                         std::unique_ptr<Shape3D> shape)
        : owner_(&owner), collider_(std::move(collider)), world_(std::move(world)),
          body_(std::move(body)), shape_(std::move(shape)) {}

    ~PhysicsArtifactStage() override { rollback(); }

    void commit() noexcept override {
        if (!owner_) return;
        owner_->commit(std::move(collider_), std::move(world_), std::move(body_),
                       std::move(shape_));
        owner_ = nullptr;
    }
...
eve::Result<std::unique_ptr<eve::artifact::PreparedPublication>> PhysicsArtifactProvider::prepare(
    ...
    if (failPrepare_)
        return prepareFailure(eve::DiagnosticCode::Failed,
                              "physics artifact prepare failure was injected");
    if (publication.schemaVersion != 1 || publication.id.isNil() || publication.buildKey.empty())
        return prepareFailure(publication.schemaVersion != 1 ? eve::DiagnosticCode::UnknownVersion
    ...
    if (find(publication.id) != nullptr)
        return prepareFailure(eve::DiagnosticCode::Conflict,
                              "physics artifact identity is already published");

    const eve::artifact::PartView* colliderPart = nullptr;
    for (const auto& part : publication.parts) {
        if (part.kind == eve::artifact::PartKind::Collider &&
            (part.role == "collider" || colliderPart == nullptr))
            colliderPart = &part;
    }
    if (!colliderPart)
    ...
    if (colliderPart->schemaVersion != 1)
        return prepareFailure(eve::DiagnosticCode::UnknownVersion,
                              "physics collider schema version is unsupported");
    ...
    try {
        ...
        if (nextIndex_ == PhysicsArtifactHandle::invalidIndex)
            return prepareFailure(eve::DiagnosticCode::Failed,
                                  "physics artifact handle index exhausted");
        ...
        if (!body)
            return prepareFailure(eve::DiagnosticCode::Failed,
                                  "Box3D did not create the generated collider body");
        ...
        if (!body || !shape)
            return prepareFailure(eve::DiagnosticCode::Failed,
                                  "Box3D did not create the generated collider");
        auto stage = std::make_unique<PhysicsArtifactStage>(*this, std::move(collider),
                                                            std::move(world), std::move(body),
                                                            std::move(shape));
        ...
        return prepareFailure(eve::DiagnosticCode::Failed,
                              "Box3D collider construction failed");
    }
}

void PhysicsArtifactProvider::commit(PhysicsArtifactCollider collider,
                                     std::unique_ptr<World3D> world,
                                     std::unique_ptr<Body3D> body,
...
void PhysicsArtifactProvider::release(PhysicsArtifactCollider& collider,
                                      std::unique_ptr<World3D>& world,
                                      std::unique_ptr<Body3D>& body,
                                      std::unique_ptr<Shape3D>& shape) noexcept {
...
PhysicsArtifactRayHit PhysicsArtifactProvider::rayCast(eve::PersistentId id, float ox, float oy,
                                                        float oz, float dx, float dy, float dz) const noexcept {
    ...
    if (!runtime.world || !runtime.body || !runtime.shape || !runtime.world->isValid() ||
        !runtime.body->isValid() || !runtime.shape->isValid())
        return result;
    if (!std::isfinite(ox) || !std::isfinite(oy) || !std::isfinite(oz) || !std::isfinite(dx) ||
...
bool PhysicsArtifactProvider::isBox3DBacked(eve::PersistentId id) const noexcept {
    ...
    const auto& runtime = *found;
    return runtime.world && runtime.body && runtime.shape && runtime.world->isValid() &&
           runtime.body->isValid() && runtime.shape->isValid() &&
           runtime.shape->getKind() == "triangleMesh";
}
...
eve::Result<eve::Value> PhysicsArtifactProvider::snapshotState() const {
    try {
        ...
        return eve::Result<eve::Value>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::Failed, "physics artifact snapshot allocation failed"));
    }
}
...
eve::Result<void> PhysicsArtifactProvider::restoreState(const eve::Value& state) {
    ...
    if (!object || !providerName || *providerName != "physics.box3d-collider" ||
        !hasSupportedVersion(*object) || !colliders)
        return restoreFailure(eve::DiagnosticCode::ParseError,
                              "invalid physics artifact provider state");
    ...
    try {
        ...
        for (const eve::Value& encoded : *colliders) {
            ...
            if (!item || !readString(*item, "artifactId", idText) ||
                !readString(*item, "buildKey", buildKey) || !readString(*item, "shape", shape) ||
                !readString(*item, "backend", backend) || !readU64(*item, "handle", packed) ||
                !readBounds(*item, collider.bounds) ||
                !readArray(member(*item, "vertices"), collider.vertices) ||
                !readArray(member(*item, "indices"), collider.indices))
                return restoreFailure(eve::DiagnosticCode::ParseError,
                                      "invalid physics artifact collider state");
            const auto parsedId = eve::PersistentId::parse(idText);
            const auto persistedHandle = PhysicsArtifactHandle::fromPacked(packed);
            if (!parsedId || parsedId->isNil() || persistedHandle.isInvalid() ||
                backend != "box3d" || shape != "triangle_mesh" ||
            ...
            if (!body)
                return restoreFailure(eve::DiagnosticCode::Failed,
                                      "Box3D did not recreate the restored collider body");
            const auto mesh = signedIndices(collider.indices);
            auto shapeObject =
                std::unique_ptr<Shape3D>(body->newTriangleMeshShape(collider.vertices, mesh));
            if (!body || !shapeObject)
                return restoreFailure(eve::DiagnosticCode::Failed,
        ...
        return restoreFailure(eve::DiagnosticCode::Failed,
                              std::string("Box3D restore failed: ") + error.what());
    } catch (...) {
