...
eve::Diagnostic staleLinkDiagnostic(const char* path) {
    return eve::Diagnostic::error(eve::DiagnosticCode::StaleHandle,
                                  "Physics link does not resolve to a live object owned by this world", path);
}

template <typename Link, typename Handle>
eve::Result<Link> attachLink(PhysicsWorldHandle world, Handle handle, const char* path, const char* message) {
...
eve::Result<PhysicsLink> PhysicsLink::attach(PhysicsWorldHandle world, PhysicsBodyHandle body) {
    if (world.isInvalid() || body.isInvalid()) {
        return eve::Result<PhysicsLink>::failure(
            invalidLinkDiagnostic("physics.link", "PhysicsLink requires valid world and body handles"));
    }
...
eve::Result<PhysicsShapeLink> PhysicsShapeLink::attach(PhysicsWorldHandle world, PhysicsShapeHandle shape) {
    return attachLink<PhysicsShapeLink>(world, shape, "physics.shapeLink",
...
eve::Result<PhysicsShapeLink> PhysicsShapeLink::fromShape(const Shape3D& shape) {
    ...
    if (!shape.isValid() || !body || !world || !world->isValid())
        return eve::Result<PhysicsShapeLink>::failure(staleLinkDiagnostic("physics.shapeLink.shape"));
...
eve::Result<Shape3D*> PhysicsShapeLink::resolve(World3D& world) const {
    ...
    if (!resolved) return eve::Result<Shape3D*>::failure(staleLinkDiagnostic("physics.shapeLink.resolve"));
    return eve::Result<Shape3D*>::success(resolved);
}

eve::Result<PhysicsJointLink> PhysicsJointLink::attach(PhysicsWorldHandle world, PhysicsJointHandle joint) {
    return attachLink<PhysicsJointLink>(world, joint, "physics.jointLink",
...
eve::Result<PhysicsJointLink> PhysicsJointLink::fromJoint(const Joint3D& joint) {
    ...
    if (!joint.isValid() || !world || !world->isValid())
        return eve::Result<PhysicsJointLink>::failure(staleLinkDiagnostic("physics.jointLink.joint"));
...
eve::Result<Joint3D*> PhysicsJointLink::resolve(World3D& world) const {
    ...
    if (!resolved) return eve::Result<Joint3D*>::failure(staleLinkDiagnostic("physics.jointLink.resolve"));
