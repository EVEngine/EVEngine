...
eve::Result<SoftBodyModel> cookSoftBodyModel(const eve::asset::CanonicalMeshData& mesh,
    ...
    if (definition.surfaceSampling != SoftBodySurfaceSampling::Vertices ||
        ...
        return eve::Result<SoftBodyModel>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::Unsupported,
                                   "only vertex surface sampling without volume voxels is implemented", "sampling"));
    ...
    if (mesh.indices.empty() || mesh.indices.size() % 3 != 0)
        return eve::Result<SoftBodyModel>::failure(eve::Diagnostic::error(
    ...
    for (std::size_t index = 0; index < vertexCount; ++index) {
        ...
        if (!std::isfinite(x) || !std::isfinite(y) || !std::isfinite(z))
            return eve::Result<SoftBodyModel>::failure(eve::Diagnostic::error(
                eve::DiagnosticCode::InvalidArgument, "mesh position must be finite", "mesh.positions"));
    ...
    for (std::size_t triangle = 0; triangle < mesh.indices.size(); triangle += 3) {
        ...
        if (a >= vertexCount || b >= vertexCount || c >= vertexCount)
            return eve::Result<SoftBodyModel>::failure(eve::Diagnostic::error(
                eve::DiagnosticCode::InvalidArgument, "mesh triangle index exceeds vertex count", "mesh.indices"));
        if (a == b || b == c || a == c)
            return eve::Result<SoftBodyModel>::failure(eve::Diagnostic::error(
                eve::DiagnosticCode::InvalidArgument, "mesh contains a degenerate index triangle", "mesh.indices"));
