...
bool hasExactFields(const eve::Value::Object& object,
...
eve::Result<std::string> readString(const eve::Value::Object& object,
                                    std::string_view name) {
    const eve::Value* value = field(object, name);
    if (!value || !value->isString())
        return failure<std::string>(eve::DiagnosticCode::ParseError,
                                    "snapshot field must be a string",
                                    std::string(name));
    return eve::Result<std::string>::success(value->asString());
}

eve::Result<std::uint64_t> readUint64(const eve::Value::Object& object,
                                      std::string_view name) {
    ...
    if (value.empty() || error != std::errc{} || end != value.data() + value.size())
        return failure<std::uint64_t>(eve::DiagnosticCode::ParseError,
                                      "snapshot integer is not a uint64 decimal string",
                                      std::string(name));
    return eve::Result<std::uint64_t>::success(output);
}

eve::Result<std::int64_t> readInt64(const eve::Value::Object& object,
                                    std::string_view name) {
    auto text = readString(object, name);
    ...
    const auto [end, error] =
        std::from_chars(value.data(), value.data() + value.size(), output);
    if (value.empty() || error != std::errc{} || end != value.data() + value.size())
        return failure<std::int64_t>(eve::DiagnosticCode::ParseError,
                                     "snapshot integer is not an int64 decimal string",
                                     std::string(name));
    return eve::Result<std::int64_t>::success(output);
}

eve::Result<double> readNumber(const eve::Value::Object& object,
                               std::string_view name) {
    const eve::Value* value = field(object, name);
    if (!value || !value->isNumeric())
        return failure<double>(eve::DiagnosticCode::ParseError,
                               "snapshot field must be numeric", std::string(name));
    const double result = value->isDouble() ? value->asDouble()
                                            : static_cast<double>(value->asInt());
    if (!std::isfinite(result))
        return failure<double>(eve::DiagnosticCode::InvalidArgument,
                               "snapshot numeric field must be finite",
                               std::string(name));
    return eve::Result<double>::success(result);
}

eve::Result<float> readFloat(const eve::Value::Object& object,
                             std::string_view name) {
    ...
    if (number.value() < -std::numeric_limits<float>::max() ||
        number.value() > std::numeric_limits<float>::max())
        return failure<float>(eve::DiagnosticCode::InvalidArgument,
                              "snapshot numeric field is outside float range",
                              std::string(name));
    return eve::Result<float>::success(static_cast<float>(number.value()));
}

eve::Result<bool> readBool(const eve::Value::Object& object,
                           std::string_view name) {
    const eve::Value* value = field(object, name);
    if (!value || !value->isBool())
        return failure<bool>(eve::DiagnosticCode::ParseError,
                             "snapshot field must be boolean", std::string(name));
...
eve::Result<eve::LogicalId> snapshotSchema(std::string_view text) {
    ...
    if (!parsed)
        return failure<eve::LogicalId>(eve::DiagnosticCode::InvariantViolation,
                                       "physics snapshot schema constant is invalid",
                                       "schema");
    return eve::Result<eve::LogicalId>::success(*parsed);
}

eve::Result<void> checkEnvelope(const eve::SnapshotEnvelope& snapshot,
    ...
    if (snapshot.type != type || snapshot.schema.format() != schema)
        return eve::Result<void>::failure(eve::Diagnostic::error(
            ...
            "snapshot type or schema does not belong to this physics world",
            "snapshot.type"));
    if (snapshot.schemaVersion.value() != kSnapshotVersion)
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::UnknownVersion,
            "physics world snapshot schema version is not supported",
...
eve::Result<ObservationState> parseObservation(const eve::Value::Object& object,
    ...
    if (duration.value() < 0 || revision.value() != stepCount.value() ||
        tick.value() != snapshot.tick.value() ||
        stepCount.value() != snapshot.revision.value()) {
        return failure<ObservationState>(
            eve::DiagnosticCode::Conflict,
    ...
    auto valid = detail::validateSimulationObservation(
        result.value, "physics.snapshot.observation");
...
bool validBodyType(const std::string& type) {
    return type == "static" || type == "kinematic" || type == "dynamic";
}

eve::Result<Body2DState> parseBody2D(const eve::Value& value) {
    const auto* object = value.getIf<eve::Value::Object>();
    if (!object || !hasExactFields(*object,
                                   {"active", "angle", "awake", "bullet",
                                    "fixedRotation", "id", "type", "vx", "vy",
                                    "angularVelocity", "x", "y"}))
        return failure<Body2DState>(eve::DiagnosticCode::ParseError,
                                    "2D physics snapshot body has unknown or missing fields",
                                    "payload.bodies");
    Body2DState state;
    auto id = readUint64(*object, "id");
    if (!id || id.value() > static_cast<std::uint64_t>(std::numeric_limits<int>::max()))
        return failure<Body2DState>(eve::DiagnosticCode::ParseError,
                                    "2D physics snapshot body id is invalid", "payload.bodies.id");
    state.id = static_cast<int>(id.value());
    auto type = readString(*object, "type");
    if (!type || !validBodyType(type.value()))
        return failure<Body2DState>(eve::DiagnosticCode::InvalidArgument,
                                    "2D physics snapshot body type is invalid", "payload.bodies.type");
    state.type = std::move(type).takeValue();
    auto x = readFloat(*object, "x"); if (!x) return eve::Result<Body2DState>::failure(x.status()); state.x = x.value();
    auto y = readFloat(*object, "y"); if (!y) return eve::Result<Body2DState>::failure(y.status()); state.y = y.value();
    auto angle = readFloat(*object, "angle"); if (!angle) return eve::Result<Body2DState>::failure(angle.status()); state.angle = angle.value();
    auto vx = readFloat(*object, "vx"); if (!vx) return eve::Result<Body2DState>::failure(vx.status()); state.vx = vx.value();
    auto vy = readFloat(*object, "vy"); if (!vy) return eve::Result<Body2DState>::failure(vy.status()); state.vy = vy.value();
    auto angular = readFloat(*object, "angularVelocity"); if (!angular) return eve::Result<Body2DState>::failure(angular.status()); state.angularVelocity = angular.value();
    auto active = readBool(*object, "active"); if (!active) return eve::Result<Body2DState>::failure(active.status()); state.active = active.value();
    auto bullet = readBool(*object, "bullet"); if (!bullet) return eve::Result<Body2DState>::failure(bullet.status()); state.bullet = bullet.value();
    auto awake = readBool(*object, "awake"); if (!awake) return eve::Result<Body2DState>::failure(awake.status()); state.awake = awake.value();
    auto fixed = readBool(*object, "fixedRotation"); if (!fixed) return eve::Result<Body2DState>::failure(fixed.status()); state.fixedRotation = fixed.value();
...
eve::Result<Body3DState> parseBody3D(const eve::Value& value) {
    const auto* object = value.getIf<eve::Value::Object>();
    if (!object || !hasExactFields(*object,
                                   {"active", "awake", "bullet", "fixedRotation", "id",
                                    "type", "x", "y", "z", "qx", "qy", "qz", "qw",
                                    "vx", "vy", "vz", "wx", "wy", "wz"}))
        return failure<Body3DState>(eve::DiagnosticCode::ParseError,
                                    "3D physics snapshot body has unknown or missing fields",
                                    "payload.bodies");
    Body3DState state;
    auto id = readUint64(*object, "id");
    if (!id || id.value() > static_cast<std::uint64_t>(std::numeric_limits<int>::max()))
        return failure<Body3DState>(eve::DiagnosticCode::ParseError,
                                    "3D physics snapshot body id is invalid", "payload.bodies.id");
    state.id = static_cast<int>(id.value());
    auto type = readString(*object, "type");
    if (!type || !validBodyType(type.value()))
        return failure<Body3DState>(eve::DiagnosticCode::InvalidArgument,
                                    "3D physics snapshot body type is invalid", "payload.bodies.type");
    state.type = std::move(type).takeValue();
#define EV_READ_BODY3D_FLOAT(name) \
    do { auto value = readFloat(*object, #name); \
         if (!value) return eve::Result<Body3DState>::failure(value.status()); \
         state.name = value.value(); } while (false)
    EV_READ_BODY3D_FLOAT(x); EV_READ_BODY3D_FLOAT(y); EV_READ_BODY3D_FLOAT(z);
    EV_READ_BODY3D_FLOAT(qx); EV_READ_BODY3D_FLOAT(qy); EV_READ_BODY3D_FLOAT(qz);
    EV_READ_BODY3D_FLOAT(qw); EV_READ_BODY3D_FLOAT(vx); EV_READ_BODY3D_FLOAT(vy);
    EV_READ_BODY3D_FLOAT(vz); EV_READ_BODY3D_FLOAT(wx); EV_READ_BODY3D_FLOAT(wy);
...
#undef EV_READ_BODY3D_FLOAT
    ...
    if (!(quaternionLengthSquared > 1e-16))
        return failure<Body3DState>(eve::DiagnosticCode::InvalidArgument,
                                    "3D physics snapshot rotation must be non-zero",
                                    "payload.bodies.rotation");
#define EV_READ_BODY3D_BOOL(name) \
    do { auto value = readBool(*object, #name); \
         if (!value) return eve::Result<Body3DState>::failure(value.status()); \
         state.name = value.value(); } while (false)
    EV_READ_BODY3D_BOOL(active); EV_READ_BODY3D_BOOL(bullet);
    EV_READ_BODY3D_BOOL(awake); EV_READ_BODY3D_BOOL(fixedRotation);
...
eve::Value bodyValue(const Body3D& body) {
    ...
    object.emplace("id", eve::Value(std::to_string(body.getId())));
    object.emplace("type", eve::Value(body.getType()));
    object.emplace("x", eve::Value(body.getX())); object.emplace("y", eve::Value(body.getY()));
    object.emplace("z", eve::Value(body.getZ())); object.emplace("qx", eve::Value(body.getRotX()));
    object.emplace("qy", eve::Value(body.getRotY())); object.emplace("qz", eve::Value(body.getRotZ()));
    object.emplace("qw", eve::Value(body.getRotW()));
    object.emplace("vx", eve::Value(body.getLinearVelocityX())); object.emplace("vy", eve::Value(body.getLinearVelocityY()));
    object.emplace("vz", eve::Value(body.getLinearVelocityZ())); object.emplace("wx", eve::Value(body.getAngularVelocityX()));
    object.emplace("wy", eve::Value(body.getAngularVelocityY())); object.emplace("wz", eve::Value(body.getAngularVelocityZ()));
    return eve::Value(std::move(object));
}

eve::Value::Object observationFields(const SimulationObservation& observation,
    ...
    object.emplace("simulatedDurationNs",
                   eve::Value(std::to_string(observation.simulatedDuration.nanoseconds())));
...
eve::Value make2DPayload(const World& world, const std::vector<Body*>& bodies) {
    ...
    std::sort(sortedBodies.begin(), sortedBodies.end(), [](const Body* left, const Body* right) {
...
eve::Value make3DPayload(const World3D& world, const std::vector<Body3D*>& bodies) {
    ...
    std::sort(sortedBodies.begin(), sortedBodies.end(), [](const Body3D* left, const Body3D* right) {
        return left->getId() < right->getId();
    });
    eve::Value::Array values;
    values.reserve(sortedBodies.size());
...
eve::Result<void> matchBodyIds(const std::vector<BodyState>& states,
                               std::set<int>& ids) {
    for (const BodyState& state : states) {
        if (!ids.insert(state.id).second)
            return eve::Result<void>::failure(eve::Diagnostic::error(
                eve::DiagnosticCode::Conflict,
                "physics snapshot contains a duplicate body id", "payload.bodies.id"));
    }
    return eve::Result<void>::success(eve::Status::success(eve::StatusCode::Applied));
}
...
eve::Result<std::vector<Body2DState>> parseBodies2D(const eve::Value::Object& object) {
    ...
    if (!array) return failure<std::vector<Body2DState>>(eve::DiagnosticCode::ParseError,
                                                        "physics snapshot bodies must be an array",
                                                        "payload.bodies");
...
eve::Result<std::vector<Body3DState>> parseBodies3D(const eve::Value::Object& object) {
    const eve::Value* value = field(object, "bodies");
    const auto* array = value ? value->getIf<eve::Value::Array>() : nullptr;
    if (!array) return failure<std::vector<Body3DState>>(eve::DiagnosticCode::ParseError,
                                                        "physics snapshot bodies must be an array",
                                                        "payload.bodies");
...
eve::Result<eve::SnapshotEnvelope> World::snapshot(
    const eve::SnapshotHashProvider& hashProvider) const {
    if (!isValid() || !simulation_)
        return failure<eve::SnapshotEnvelope>(
...
eve::Result<void> World::restore(const eve::SnapshotEnvelope& snapshotValue,
    ...
    if (!isValid() || !simulation_)
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::PreconditionViolation,
            "Cannot restore a destroyed or uninitialized physics world",
    ...
    if (!object || !hasExactFields(*object,
                                   {"bodies", "gravityX", "gravityY", "meter", "tick",
                                    "revision", "stepCount", "simulatedDurationNs",
                                    "lastDeltaSeconds"}))
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::ParseError,
            "2D physics snapshot payload has unknown or missing fields",
            "payload"));
    ...
    if (meter.value() <= 0.f) return failure<void>(eve::DiagnosticCode::InvalidArgument,
                                                    "physics snapshot meter must be positive",
                                                    "payload.meter");

    std::set<int> ids;
    for (const Body* body : bodies_) if (body && body->isValid()) ids.insert(body->getId());
    std::set<int> snapshotIds;
    auto matched = matchBodyIds(bodies.value(), snapshotIds);
    if (!matched) return matched;
    if (ids != snapshotIds) return failure<void>(eve::DiagnosticCode::Conflict,
                                                  "physics snapshot body ids do not match the live world",
                                                  "payload.bodies");
    std::vector<std::pair<Body*, Body2DState>> targets;
    targets.reserve(bodies.value().size());
    for (const Body2DState& state : bodies.value()) {
        Body* target = nullptr;
        for (Body* candidate : bodies_) if (candidate && candidate->getId() == state.id) target = candidate;
        if (!target) return failure<void>(eve::DiagnosticCode::Conflict,
                                          "physics snapshot body target disappeared during validation",
                                          "payload.bodies");
    ...
    for (const auto& target : targets) {
        ...
        backup.push_back({body.getId(), body.getType(), body.getX(), body.getY(), body.getAngle(),
                          body.getLinearVelocityX(), body.getLinearVelocityY(),
                          body.getAngularVelocity(), body.isActive(), body.isBullet(),
    ...
    try {
        ...
        for (const auto& target : targets) {
            Body& body = *target.first; const Body2DState& state = target.second;
            body.setType(state.type); body.setFixedRotation(state.fixedRotation);
            body.setBullet(state.bullet); body.setActive(state.active);
            body.setPosition(state.x, state.y); body.setAngle(state.angle);
            body.setLinearVelocity(state.vx, state.vy); body.setAngularVelocity(state.angularVelocity);
            body.setAwake(state.awake);
        }
    } catch (const std::exception& error) {
        try {
            setMeter(previousMeter); setGravity(previousGravityX, previousGravityY);
            for (const Body2DState& state : backup) {
                Body* body = nullptr;
                for (Body* candidate : bodies_) if (candidate && candidate->getId() == state.id) body = candidate;
                if (!body) continue;
                body->setType(state.type); body->setFixedRotation(state.fixedRotation);
                body->setBullet(state.bullet); body->setActive(state.active);
                body->setPosition(state.x, state.y); body->setAngle(state.angle);
...
eve::Result<eve::SnapshotEnvelope> World3D::snapshot(
    const eve::SnapshotHashProvider& hashProvider) const {
    if (!isValid() || !simulation_)
        return failure<eve::SnapshotEnvelope>(
            eve::DiagnosticCode::PreconditionViolation,
    ...
    return eve::makeSnapshotEnvelope(
        std::string(kWorld3DType), std::move(schema).takeValue(),
        eve::SchemaVersion(kSnapshotVersion), eve::PersistentId::nil(),
        eve::Revision(observation.stepCount), simulationTick(),
        make3DPayload(*this, bodies),
        hashProvider);
}
...
eve::Result<void> World3D::restore(const eve::SnapshotEnvelope& snapshotValue,
    ...
    if (!isValid() || !simulation_)
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::PreconditionViolation,
            "Cannot restore a destroyed or uninitialized 3D physics world",
    ...
    const auto* object = snapshotValue.payload.getIf<eve::Value::Object>();
    if (!object || !hasExactFields(*object,
                                   {"bodies", "gravityX", "gravityY", "gravityZ", "tick",
                                    "revision", "stepCount", "simulatedDurationNs",
                                    "lastDeltaSeconds"}))
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::ParseError,
            "3D physics snapshot payload has unknown or missing fields", "payload"));
    auto observation = parseObservation(*object, snapshotValue);
    if (!observation) return eve::Result<void>::failure(observation.status());
    ...
    std::set<int> ids;
    for (const Body3D* body : bodies_) if (body && body->isValid()) ids.insert(body->getId());
    std::set<int> snapshotIds;
    auto matched = matchBodyIds(bodies.value(), snapshotIds);
    if (!matched) return matched;
    if (ids != snapshotIds) return failure<void>(eve::DiagnosticCode::Conflict,
    ...
    for (const Body3DState& state : bodies.value()) {
        ...
        for (Body3D* candidate : bodies_) if (candidate && candidate->getId() == state.id) target = candidate;
        if (!target) return failure<void>(eve::DiagnosticCode::Conflict,
                                          "physics snapshot body target disappeared during validation",
                                          "payload.bodies");
        targets.emplace_back(target, state);
    ...
    for (const auto& target : targets) {
        ...
        backup.push_back({body.getId(), body.getType(), body.getX(), body.getY(), body.getZ(),
                          body.getRotX(), body.getRotY(), body.getRotZ(), body.getRotW(),
                          body.getLinearVelocityX(), body.getLinearVelocityY(), body.getLinearVelocityZ(),
                          body.getAngularVelocityX(), body.getAngularVelocityY(), body.getAngularVelocityZ(),
    ...
    try {
        ...
        for (const auto& target : targets) {
            Body3D& body = *target.first; const Body3DState& state = target.second;
            body.setType(state.type); body.setFixedRotation(state.fixedRotation);
            body.setBullet(state.bullet); body.setActive(state.active);
            ...
            body.setAngularVelocity(state.wx, state.wy, state.wz); body.setAwake(state.awake);
        }
    } catch (const std::exception& error) {
        try {
            ...
            for (const Body3DState& state : backup) {
                Body3D* body = nullptr;
                for (Body3D* candidate : bodies_) if (candidate && candidate->getId() == state.id) body = candidate;
                if (!body) continue;
                body->setType(state.type); body->setFixedRotation(state.fixedRotation);
                body->setBullet(state.bullet); body->setActive(state.active);
                body->setPosition(state.x, state.y, state.z);
