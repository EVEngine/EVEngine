...
eve::Result<void> TargetingLineOfSightAdapter::addWorld(World3D* world) {
    if (!world)
        return failure<void>(eve::DiagnosticCode::InvalidArgument, "Physics LOS cannot register a null World3D");
    if (!world->isValid())
        return failure<void>(eve::DiagnosticCode::NotFound, "Physics LOS cannot register an invalid World3D");
    ...
    for (auto it = worlds_.begin(); it != worlds_.end();) {
        ...
        return failure<void>(eve::DiagnosticCode::Conflict, "Physics LOS already has a different active World3D");
    }
...
eve::Result<void> TargetingLineOfSightAdapter::removeWorld(World3D* world) {
    if (!world) return failure<void>(eve::DiagnosticCode::InvalidArgument, "Physics LOS cannot remove a null World3D");
...
eve::Result<sensing::LineOfSightResult> TargetingLineOfSightAdapter::query(const sensing::TargetLocation& from,
    ...
    for (const RegisteredWorld& entry : worlds_) {
        ...
        if (activeWorld)
            return failure<sensing::LineOfSightResult>(eve::DiagnosticCode::Conflict,
                                                       "Physics LOS has more than one valid active World3D");
        activeWorld    = world;
        activeLifetime = std::move(lifetime);
    }
    if (!activeWorld)
        return failure<sensing::LineOfSightResult>(eve::DiagnosticCode::Unsupported,
