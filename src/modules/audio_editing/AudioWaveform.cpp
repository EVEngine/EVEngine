...
AudioWaveformResult AudioWaveformService::generate(const AudioWaveformRequest& request,
                                                   const EditorAudioPcm& pcm,
                                                   int maxPixels,
    ...
    result.sampleRate = pcm.sampleRate; result.channels = pcm.channels;
    if (request.asset.empty() || request.pixelWidth <= 0 || request.pixelWidth > maxPixels ||
        maxPixels <= 0 || pcm.sampleRate <= 0 || pcm.channels <= 0 || pcm.channels > 32 ||
        pcm.samples.empty() || pcm.samples.size() > maxSamples ||
        pcm.samples.size() % static_cast<std::size_t>(pcm.channels) != 0) {
        result.status = EditorStatus::Rejected;
        result.diagnostics.push_back({RuleId("editor.audio.invalid-waveform-input"), DiagnosticSeverity::Error,
    ...
    for (float sample : pcm.samples)
        if (!std::isfinite(sample)) {
            result.status = EditorStatus::Rejected;
            result.diagnostics.push_back({RuleId("editor.audio.nonfinite-pcm"), DiagnosticSeverity::Error,
    ...
    for (int bucket = 0; bucket < buckets; ++bucket) {
        ...
        for (int channel = 0; channel < pcm.channels; ++channel) {
            ...
            for (std::size_t frame = begin; frame < end; ++frame) {
                ...
                minimum = std::min(minimum, sample); maximum = std::max(maximum, sample);
                squares += static_cast<double>(sample) * sample;
            }
            result.envelopes[static_cast<std::size_t>(channel)].push_back({
                static_cast<double>(begin) / pcm.sampleRate, static_cast<double>(end) / pcm.sampleRate,
                minimum, maximum, static_cast<float>(std::sqrt(squares / static_cast<double>(end - begin)))});
        }
    }
    if (request.loopEnd > 0.0 || request.loopStart > 0.0) {
        if (!std::isfinite(request.loopStart) || !std::isfinite(request.loopEnd) ||
            request.loopStart < 0.0 || request.loopEnd <= request.loopStart || request.loopEnd > result.duration) {
            result.status = EditorStatus::Rejected;
            result.diagnostics.push_back({RuleId("editor.audio.invalid-loop-range"), DiagnosticSeverity::Error,
        ...
        const std::size_t startFrame = std::min(frameCount - 1, static_cast<std::size_t>(request.loopStart * pcm.sampleRate));
        const std::size_t endFrame = std::min(frameCount - 1, static_cast<std::size_t>(request.loopEnd * pcm.sampleRate));
        ...
        if (maximumDelta > 0.1)
            result.diagnostics.push_back({RuleId("editor.audio.loop-seam-discontinuity"), DiagnosticSeverity::Warning,
