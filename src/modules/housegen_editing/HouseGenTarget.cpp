...
std::string directionName(housegen::SocketDirection d) {
    switch (d) {
        case housegen::SocketDirection::North: return "north";
        case housegen::SocketDirection::East: return "east";
        case housegen::SocketDirection::South: return "south";
        case housegen::SocketDirection::West: return "west";
        case housegen::SocketDirection::Up: return "up";
        case housegen::SocketDirection::Down: return "down";
    }
    return "north";
}
bool direction(const std::string& s, housegen::SocketDirection& d) {
    if (s == "north")
        d = housegen::SocketDirection::North;
    else if (s == "east")
        d = housegen::SocketDirection::East;
    else if (s == "south")
        d = housegen::SocketDirection::South;
    else if (s == "west")
        d = housegen::SocketDirection::West;
    else if (s == "up")
        d = housegen::SocketDirection::Up;
    else if (s == "down")
        d = housegen::SocketDirection::Down;
    else
        return false;
    return true;
}
EditorValue strings(const std::vector<std::string>& values) {
    EditorValue::Array out;
    for (const auto& v : values) out.push_back(v);
    return out;
}
EditorValue integers(const std::vector<int>& values) {
    EditorValue::Array out;
    for (int v : values) out.push_back(int64_t{v});
    return out;
}
bool readStrings(const EditorValue* value, std::vector<std::string>& out) {
...
bool readIntegers(const EditorValue* value, std::vector<int>& out) {
    const auto* array = value ? value->getIf<EditorValue::Array>() : nullptr;
    if (!array) return false;
    out.clear();
    for (const auto& v : *array) {
        const auto* n = v.getIf<int64_t>();
        if (!n) return false;
        out.push_back(static_cast<int>(*n));
    }
    return true;
}
EditorValue materialValue(const housegen::HouseMaterialOverride& m) {
    return EditorValue::Object{
        {"hasBaseColor", m.hasBaseColor},
        {"baseColor", EditorValue::Array{m.baseColorR, m.baseColorG, m.baseColorB, m.baseColorA}},
        {"baseColorTexture", m.baseColorTexture},
        {"normalTexture", m.normalTexture},
        {"heightTexture", m.heightTexture},
        {"hasMetallic", m.hasMetallic},
        {"metallic", m.metallic},
        {"hasRoughness", m.hasRoughness},
        {"roughness", m.roughness},
        {"parallaxScale", m.parallaxScale},
        {"parallaxMinLayers", m.parallaxMinLayers},
        {"parallaxMaxLayers", m.parallaxMaxLayers},
        {"cellBombScale", m.cellBombScale},
        {"cellBombStrength", m.cellBombStrength},
        {"cellBombRotation", m.cellBombRotation}};
}
EditorValue componentValue(const HouseKitComponentValue& value) {
    const auto&        c = value.component;
    EditorValue::Array sockets;
    for (const auto& s : c.sockets)
        sockets.push_back(EditorValue::Object{
            {"direction", directionName(s.direction)}, {"type", s.type}, {"accepts", strings(s.accepts)}});
    return EditorValue::Object{{"editorId", value.id.value()},
                               {"id", c.id},
                               {"model", c.modelPath},
                               {"category", c.category},
                               {"width", int64_t{c.width}},
                               {"depth", int64_t{c.depth}},
                               {"height", int64_t{c.height}},
                               {"weight", int64_t{c.weight}},
                               {"rotations", integers(c.rotations)},
                               {"tags", strings(c.tags)},
                               {"sockets", std::move(sockets)},
                               {"material", materialValue(c.material)}};
}
bool number(const EditorValue* value, float& out) {
    const auto* n = value ? value->getIf<double>() : nullptr;
    if (!n || !std::isfinite(*n)) return false;
...
EditorResult<HouseKitComponentValue> parseComponent(const EditorValue& value) {
    ...
    const auto* editorIds  = editorId ? editorId->getIf<std::string>() : nullptr;
    ...
    out.component.category  = *categories;
    int*        values[] = {&out.component.width, &out.component.depth, &out.component.height, &out.component.weight};
    const char* fields[] = {"width", "depth", "height", "weight"};
    for (int i = 0; i < 4; ++i) {
    ...
    for (const auto& socketValue : *socketArray) {
        ...
        const auto*           ds = d ? d->getIf<std::string>() : nullptr;
    ...
    const auto* hbcb       = hbc ? hbc->getIf<bool>() : nullptr;
    const auto* hmb        = hm ? hm->getIf<bool>() : nullptr;
    const auto* hrb        = hr ? hr->getIf<bool>() : nullptr;
    ...
    float*      scalars[]      = {&m.metallic,          &m.roughness,     &m.parallaxScale,    &m.parallaxMinLayers,
                                  &m.parallaxMaxLayers, &m.cellBombScale, &m.cellBombStrength, &m.cellBombRotation};
    const char* scalarFields[] = {"metallic",          "roughness",     "parallaxScale",    "parallaxMinLayers",
...
EditorValue requestValue(const housegen::HouseRequest& r) {
    return EditorValue::Object{{"seed", int64_t{r.seed}},
                               {"width", int64_t{r.width}},
                               {"depth", int64_t{r.depth}},
                               {"floors", int64_t{r.floors}},
...
PropertyDescriptor prop(std::string path, PropertyType type, EditorValue defaultValue, double minimum = -1e9,
    ...
    d.displayNameKey  = "editor.housegen." + path;
...
bool HouseGenDocumentTarget::matches(const SelectionSnapshot& s) const {
    if (s.items.empty()) return false;
    for (const auto& item : s.items)
        if (item.target != TargetId(id_) || std::none_of(components_.begin(), components_.end(), [&](const auto& c) {
                return c.id.value() == item.item.value();
            }))
            return false;
    return true;
}
eve::Result<eve::Revision> HouseGenDocumentTarget::currentRevision(const SelectionSnapshot& s) const {
    if (!matches(s))
...
PropertySchema HouseGenDocumentTarget::schema(const SelectionSnapshot&) const {
    PropertySchema s;
    s.typeId                         = "housegen.component";
    s.properties                     = {prop("component.model", PropertyType::AssetRef, ""),
                                        prop("component.category", PropertyType::Enum, "wall"),
                                        prop("component.width", PropertyType::Int, int64_t{1}, 1, 128),
                                        prop("component.depth", PropertyType::Int, int64_t{1}, 1, 128),
                                        prop("component.height", PropertyType::Int, int64_t{1}, 1, 128),
                                        prop("component.weight", PropertyType::Int, int64_t{1}, 1, 100000),
                                        prop("material.metallic", PropertyType::Float, 0.0, 0, 1),
                                        prop("material.roughness", PropertyType::Float, 1.0, 0, 1)};
    s.properties[0].assetTypeFilters = {"model3d"};
    s.properties[1].enumItems        = {"foundation", "floor", "wall", "door", "roof"};
    return s;
}
PropertyReadResult HouseGenDocumentTarget::read(const SelectionSnapshot& s, const PropertyPath& path) const {
    if (!matches(s) || !schema(s).find(path)) return {};
    std::optional<EditorValue> common;
    for (const auto& item : s.items) {
        const auto& c = std::find_if(components_.begin(), components_.end(), [&](const auto& v) {
                            return v.id.value() == item.item.value();
                        })->component;
        EditorValue value;
        if (path == PropertyPath("component.model"))
            value = c.modelPath;
        else if (path == PropertyPath("component.category"))
            value = c.category;
        else if (path == PropertyPath("component.width"))
            value = int64_t{c.width};
        else if (path == PropertyPath("component.depth"))
            value = int64_t{c.depth};
        else if (path == PropertyPath("component.height"))
            value = int64_t{c.height};
        else if (path == PropertyPath("component.weight"))
            value = int64_t{c.weight};
        else if (path == PropertyPath("material.metallic"))
            value = static_cast<double>(c.material.metallic);
        else
            value = static_cast<double>(c.material.roughness);
        if (!common)
            common = value;
...
EditorValue HouseGenDocumentTarget::contentValue() const {
    EditorValue::Array components;
...
EditorResult<DomainOperation> HouseGenDocumentTarget::makeSet(const SelectionSnapshot& s, const PropertyPath& path,
    ...
    for (const auto& item : s.items) {
        ...
        if (path == PropertyPath("component.model"))
            c.modelPath = *value.getIf<std::string>();
        else if (path == PropertyPath("component.category"))
            c.category = *value.getIf<std::string>();
        else if (path == PropertyPath("component.width"))
            c.width = static_cast<int>(*value.getIf<int64_t>());
        else if (path == PropertyPath("component.depth"))
            c.depth = static_cast<int>(*value.getIf<int64_t>());
        else if (path == PropertyPath("component.height"))
            c.height = static_cast<int>(*value.getIf<int64_t>());
        else if (path == PropertyPath("component.weight"))
            c.weight = static_cast<int>(*value.getIf<int64_t>());
        else if (path == PropertyPath("material.metallic")) {
            c.material.hasMetallic = true;
            c.material.metallic    = static_cast<float>(*value.getIf<double>());
        } else {
            c.material.hasRoughness = true;
            c.material.roughness    = static_cast<float>(*value.getIf<double>());
        }
    }
...
std::vector<EditorDiagnostic> HouseGenDocumentTarget::validate() const {
    ...
    if (components_.size() > 2048)
        out.push_back(eve::editing::ruleDiagnostic(eve::DiagnosticCode::PreconditionViolation,
            RuleId("editor.housegen.component-budget"), DiagnosticSeverity::Error,
            "House kit exceeds 2048 components"));
    housegen::HouseComponentLibrary library;
    std::set<std::string>           editorIds;
    std::set<std::string>           categories;
    for (const auto& c : components_) {
        if (c.id.empty() || !editorIds.insert(c.id.value()).second)
            out.push_back(eve::editing::ruleDiagnostic(eve::DiagnosticCode::Conflict,
                RuleId("editor.housegen.editor-id"), DiagnosticSeverity::Error,
                "House editor component IDs must be unique"));
        auto registered = library.registerComponent(c.component);
        if (!registered.ok())
            out.push_back(eve::editing::ruleDiagnostic(eve::DiagnosticCode::InvalidArgument,
                RuleId("editor.housegen.component"), DiagnosticSeverity::Error,
                "Invalid or duplicate runtime component: " + c.component.id));
        categories.insert(c.component.category);
    }
    for (const char* required : {"foundation", "floor", "wall", "door", "roof"})
        if (!categories.contains(required))
            out.push_back(eve::editing::ruleDiagnostic(eve::DiagnosticCode::PreconditionViolation,
                RuleId("editor.housegen.category"), DiagnosticSeverity::Warning,
                "House kit is not generation-ready; missing category: " + std::string(required)));
    const bool footprint = request_.footprint == "auto" || request_.footprint == "rectangle" ||
                           request_.footprint == "l_shape" || request_.footprint == "t_shape";
    const bool roof =
        request_.roof == "auto" || request_.roof == "gable" || request_.roof == "flat" || request_.roof == "shed";
    const bool entrance = request_.entrance == "auto" || request_.entrance == "north" || request_.entrance == "east" ||
                          request_.entrance == "south" || request_.entrance == "west";
    if (request_.width < 2 || request_.width > 256 || request_.depth < 2 || request_.depth > 256 ||
        request_.floors < 1 || request_.floors > 64 || !std::isfinite(request_.moduleSize) ||
        request_.moduleSize <= 0 || request_.moduleSize > 10000 || !std::isfinite(request_.floorHeight) ||
        request_.floorHeight <= 0 || request_.floorHeight > 10000 || request_.maxAttempts < 1 ||
        request_.maxAttempts > 10000 || !footprint || !roof || !entrance)
        out.push_back(eve::editing::ruleDiagnostic(eve::DiagnosticCode::InvalidArgument,
            RuleId("editor.housegen.request-range"), DiagnosticSeverity::Error,
            "House generation request exceeds supported values or budgets"));
    if (request_.requiredRooms.size() > 256)
        out.push_back(eve::editing::ruleDiagnostic(eve::DiagnosticCode::PreconditionViolation,
            RuleId("editor.housegen.room-budget"), DiagnosticSeverity::Error,
