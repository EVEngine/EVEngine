...
#include <array>
#include <cmath>
namespace eve::system { namespace {
...
Result<void> PcgFrameRateManager::configure(int target,float interval,int minimum,int maximum,int current){
 if(target<=0||!std::isfinite(interval)||interval<=0||minimum<0||maximum>=int(k.size())||minimum>maximum||current<minimum||current>maximum)return bad<void>("invalid frame-rate manager configuration","system.pcgFrameRate.configure");
...
Result<int> PcgFrameRateManager::update(float dt,float scale){
 if(!std::isfinite(dt)||dt<=0||!std::isfinite(scale)||scale<0)return bad<int>("dt must be positive and timeScale non-negative","system.pcgFrameRate.update");
...
Result<void> PcgFrameRateManager::selectManualQuality(int q){if(q<min_||q>max_)return bad<void>("quality is outside configured range","system.pcgFrameRate.quality");changed_=q!=quality_;quality_=q;automatic_=false;return Result<void>::success();}
