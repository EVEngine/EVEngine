...
const char* attributeOperationName(AttributeOperation operation) noexcept {
    ...
    case AttributeOperation::AdditivePercent:
        return "additive_percent";
    case AttributeOperation::MultiplicativePercent:
        return "multiplicative_percent";
...
eve::Result<ParsedAttributeOperation> parseAttributeOperation(std::string_view operation,
                                                              double value) {
...
void AttributeOperationRegistry::unregisterOperation(const std::string& policyId) {
    operations_.erase(policyId);
}
...
double AttributeOperationRegistry::apply(const std::string& policyId, double current,
                                         double value) const {
...
void AttributeOperationRegistry::unregisterOp(const std::string& policyId) {
    unregisterOperation(policyId);
}

AttributeModifier::AttributeModifier(ModifierId modifierId, SourceId sourceId,
                                     std::string_view operationName, double modifierValue,
                                     ModifierPriority priorityValue)
    : id(std::move(modifierId)), source(std::move(sourceId)), value(modifierValue),
...
double computeAttributeValue(const AttributeValue& attribute,
                             const AttributeOperationRegistry* customOperations) {
    double additive = 0.0;
    ...
    for (const auto& modifier : attribute.modifiers) {
        ...
        case AttributeOperation::AdditivePercent:
            ...
            break;
        case AttributeOperation::MultiplicativePercent:
            multiplicativePercent *= 1.0 + modifier.value;
            break;
        case AttributeOperation::Override:
        case AttributeOperation::ClampMin:
        case AttributeOperation::ClampMax:
...
AttributeSet::AttributeSet(const AttributeSet& other)
    : subject_(other.subject_), values_(other.values_), nextSequence_(other.nextSequence_),
...
AttributeSet::AttributeSet(AttributeSet&& other) noexcept
    : subject_(std::move(other.subject_)), values_(std::move(other.values_)),
      nextSequence_(other.nextSequence_), nextModifierId_(other.nextModifierId_) {}
...
bool AttributeSet::isValidOperation(const AttributeModifier& modifier) noexcept {
    ...
    case AttributeOperation::ClampMax:
        return true;
    case AttributeOperation::Custom:
        return !modifier.policyId.empty();
    }
    return false;
}
...
eve::Result<ModifierId> AttributeSet::addModifier(AttributeModifier modifier) {
    ...
    if (!std::isfinite(modifier.value))
        return eve::Result<ModifierId>::failure(
            invalidArgument("attribute modifier value must be finite", "value"));
    ...
    state.dirty = true;
    orderDirty_ = true;
    return eve::Result<ModifierId>::success(modifierId,
                                            eve::Status::success(eve::StatusCode::Applied));
}

bool AttributeSet::addModifier(const std::string& id, const std::string& attribute,
                               const std::string& source, const std::string& operation,
                               double value, ModifierPriority priority) {
    if (id.empty()) return false;
    auto parsed = parseAttributeOperation(operation, value);
    if (!parsed.ok()) return false;
    AttributeModifier modifier{id, attribute, source, parsed.value().operation, parsed.value().value,
                               priority};
...
eve::Result<void> AttributeSet::removeModifier(const ModifierId& id) {
    if (id.empty())
        return eve::Result<void>::failure(invalidArgument("modifier id must not be empty", "id"));
    ...
    return eve::Result<void>::failure(
        eve::Status::failure(eve::StatusCode::NotFound,
                             eve::Diagnostic::error(eve::DiagnosticCode::NotFound,
                                                    "attribute modifier was not found", "id")));
}
...
eve::Result<void> AttributeSet::removeModifier(const AttributeId& attribute, const ModifierId& id) {
    ...
    const auto modifierIt = std::find_if(modifiers.begin(), modifiers.end(),
                                         [&](const auto& modifier) { return modifier.id == id; });
...
eve::Result<int> AttributeSet::removeBySource(const SourceId& source, const AttributeId& attribute) {
    if (source.empty())
        return eve::Result<int>::failure(invalidArgument("source id must not be empty", "source"));
    int removed = 0;
    for (auto& [attributeId, state] : values_) {
        ...
        state.modifiers.erase(
            std::remove_if(state.modifiers.begin(), state.modifiers.end(),
...
void AttributeSet::rebuildOrder() const {
    ...
    std::sort(order_.begin(), order_.end(), [](const auto* left, const auto* right) {
        return left->sequence < right->sequence;
    });
    orderDirty_ = false;
}
