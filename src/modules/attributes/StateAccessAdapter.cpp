...
std::optional<eve::Value> AttributeSetStateAdapter::value(std::string_view subject,
...
std::optional<eve::Value> AttributeSetStateAdapter::attribute(std::string_view subject,
                                                              std::string_view key) const {
    return value(subject, key);
}

std::optional<eve::Value> AttributeSetStateAdapter::state(std::string_view subject,
                                                          std::string_view key) const {
    return value(subject, key);
}

eve::Result<eve::MutationReceipt> AttributeSetStateAdapter::apply(
    ...
    for (std::size_t index = 0; index < mutations.size(); ++index) {
        ...
        if (mutation.kind != eve::MutationKind::Set && mutation.kind != eve::MutationKind::AddNumber)
            return failure(eve::DiagnosticCode::Unsupported,
                           "AttributeSet supports Set and AddNumber only",
                           "mutations[" + std::to_string(index) + "]");
        double ignored = 0.0;
        if (!numeric(mutation.value, ignored))
            return failure(eve::DiagnosticCode::InvalidArgument,
                           "attribute mutation value must be a finite number",
    ...
    for (const auto& mutation : mutations) {
        const double amount = mutation.value.isInt64() ? static_cast<double>(mutation.value.asInt())
                                                       : mutation.value.asDouble();
