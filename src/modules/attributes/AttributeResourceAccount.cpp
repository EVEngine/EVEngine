...
AttributeResourceAccountAdapter::AttributeResourceAccountAdapter(
    AttributeSetResourceAccount& account, AttributeResourceKind kind) noexcept
    : account_(account), kind_(kind) {}

const char* AttributeResourceAccountAdapter::resourceName(
...
eve::Result<eve::resource::CostSpec> AttributeResourceAccountAdapter::makeCost(
    AttributeResourceKind kind, std::int64_t amount) {
    const char* name = resourceName(kind);
    if (name == nullptr)
        return invalid<eve::resource::CostSpec>(
            "attribute resource kind is invalid", "resource.kind");
    return eve::resource::CostSpec::single(name, amount);
}

eve::Result<void> AttributeResourceAccountAdapter::validateCost(
    const eve::resource::CostSpec& cost) const {
    const char* name = resourceName(kind_);
    if (name == nullptr)
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::InvalidArgument, "attribute resource kind is invalid",
            "resource.kind"));
    if (!cost.isValid())
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::InvalidArgument,
            "attribute resource view requires a validated CostSpec", "cost"));
...
eve::Result<eve::resource::Reservation> AttributeResourceAccountAdapter::reserve(
...
eve::Result<eve::resource::Receipt> AttributeResourceAccountAdapter::debit(
    const eve::resource::CostSpec& cost) {
    auto valid = validateCost(cost);
...
eve::Result<eve::resource::Receipt> AttributeResourceAccountAdapter::credit(
    const eve::resource::CostSpec& cost) {
    auto valid = validateCost(cost);
    if (!valid) return eve::Result<eve::resource::Receipt>::failure(valid.status());
...
eve::Result<eve::resource::Receipt> AttributeResourceAccountAdapter::commit(
    ...
    if (name == nullptr || !reservation.isValid() || reservation.cost.size() != 1 ||
        ...
        return eve::Result<eve::resource::Receipt>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::InvalidArgument,
            "named attribute resource view received a foreign reservation cost",
            "reservation.cost"));
    return account_.commit(reservation);
}

eve::Result<void> AttributeResourceAccountAdapter::rollback(
    const eve::resource::Reservation& reservation) {
    const char* name = resourceName(kind_);
    if (name == nullptr || !reservation.isValid() || reservation.cost.size() != 1 ||
        reservation.cost.items().front().resource.value() != name)
