...
EditorValue floats(const float* v, int n) {
    EditorValue::Array a;
    for (int i = 0; i < n; ++i) a.push_back(v[i]);
    return a;
}
bool readFloats(const EditorValue* v, float* out, int n) {
...
EditorValue layerValue(const AvatarLayerValue& v) {
    return EditorValue::Object{{"id", v.id.value()},
                               {"name", v.name},
                               {"texture", v.textureAsset},
                               {"z", int64_t{v.zIndex}},
                               {"visible", v.visible},
                               {"offset", floats(v.offset.data(), 2)},
                               {"size", floats(v.size.data(), 2)},
                               {"color", floats(v.color.data(), 4)}};
}
EditorValue parameterValue(const AvatarParameterValue& v) {
    return EditorValue::Object{{"id", v.id.value()},   {"name", v.name},       {"default", v.defaultValue},
                               {"minimum", v.minimum}, {"maximum", v.maximum}, {"value", v.value}};
}
EditorValue expressionValue(const AvatarExpressionValue& v) {
    EditorValue::Object channels;
...
EditorResult<AvatarLayerValue> parseLayer(const EditorValue& v) {
    ...
    const auto* textures = texture ? texture->getIf<std::string>() : nullptr;
    const auto* zs       = z ? z->getIf<int64_t>() : nullptr;
    const auto* vis      = visible ? visible->getIf<bool>() : nullptr;
...
EditorResult<AvatarParameterValue> parseParameter(const EditorValue& v) {
    ...
    const char* fields[] = {"default", "minimum", "maximum", "value"};
    for (int i = 0; i < 4; ++i) {
...
PropertyDescriptor prop(std::string path, PropertyType type, EditorValue value, const char* category, double min = -1e9,
                        double max = 1e9) {
    PropertyDescriptor d;
    d.path            = PropertyPath(path);
    d.displayNameKey  = "editor.avatar." + path;
    d.category        = category;
    d.type            = type;
...
bool AvatarDocumentTarget::matches(const SelectionSnapshot& s) const {
    ...
    const std::string type = s.items.front().type;
    for (const auto& i : s.items) {
        if (i.target != TargetId(id_) || i.type != type) return false;
        if (type == "avatar.layer" &&
            std::none_of(layers_.begin(), layers_.end(), [&](const auto& v) { return v.id.value() == i.item.value(); }))
            return false;
        if (type == "avatar.parameter" && std::none_of(parameters_.begin(), parameters_.end(),
                                                       [&](const auto& v) { return v.id.value() == i.item.value(); }))
            return false;
    }
    return type == "avatar.layer" || type == "avatar.parameter";
}
eve::Result<eve::Revision> AvatarDocumentTarget::currentRevision(const SelectionSnapshot& s) const {
    if (!matches(s))
...
PropertySchema AvatarDocumentTarget::schema(const SelectionSnapshot& s) const {
    ...
    if (!s.items.empty() && s.items.front().type == "avatar.parameter") {
        schema.typeId     = "avatar.parameter";
        schema.properties = {prop("parameter.default", PropertyType::Float, 0.0, "parameter"),
                             prop("parameter.minimum", PropertyType::Float, 0.0, "parameter"),
                             prop("parameter.maximum", PropertyType::Float, 1.0, "parameter"),
                             prop("parameter.value", PropertyType::Float, 0.0, "parameter")};
    } else {
        schema.typeId     = "avatar.layer";
        schema.properties = {
            prop("layer.texture", PropertyType::AssetRef, "", "layer"),
            prop("layer.z", PropertyType::Int, int64_t{0}, "layer", -100000, 100000),
            prop("layer.visible", PropertyType::Bool, true, "layer"),
            prop("layer.offset", PropertyType::Vec2, EditorValue::Array{0.0, 0.0}, "layer"),
            prop("layer.size", PropertyType::Vec2, EditorValue::Array{64.0, 64.0}, "layer", .01, 100000),
            prop("layer.color", PropertyType::Color, EditorValue::Array{1.0, 1.0, 1.0, 1.0}, "layer", 0, 1)};
        schema.properties[0].assetTypeFilters = {"image", "texture"};
    }
    return schema;
}
PropertyReadResult AvatarDocumentTarget::read(const SelectionSnapshot& s, const PropertyPath& p) const {
    if (!matches(s) || !schema(s).find(p)) return {};
    std::optional<EditorValue> common;
    for (const auto& i : s.items) {
        EditorValue value;
        if (i.type == "avatar.layer") {
            const auto& v = *std::find_if(layers_.begin(), layers_.end(),
                                          [&](const auto& x) { return x.id.value() == i.item.value(); });
            if (p == PropertyPath("layer.texture"))
                value = v.textureAsset;
            else if (p == PropertyPath("layer.z"))
                value = int64_t{v.zIndex};
            else if (p == PropertyPath("layer.visible"))
                value = v.visible;
            else if (p == PropertyPath("layer.offset"))
                value = floats(v.offset.data(), 2);
            else if (p == PropertyPath("layer.size"))
                value = floats(v.size.data(), 2);
            else
                value = floats(v.color.data(), 4);
        } else {
            const auto& v = *std::find_if(parameters_.begin(), parameters_.end(),
                                          [&](const auto& x) { return x.id.value() == i.item.value(); });
            if (p == PropertyPath("parameter.default"))
                value = v.defaultValue;
            else if (p == PropertyPath("parameter.minimum"))
                value = v.minimum;
            else if (p == PropertyPath("parameter.maximum"))
                value = v.maximum;
            else
                value = v.value;
        }
...
EditorValue AvatarDocumentTarget::contentValue() const {
    EditorValue::Array layers, parameters, expressions;
    for (const auto& v : layers_) layers.push_back(layerValue(v));
    for (const auto& v : parameters_) parameters.push_back(parameterValue(v));
    for (const auto& v : expressions_) expressions.push_back(expressionValue(v));
    return EditorValue::Object{{"kind", kind_},
                               {"source", sourceAsset_},
...
EditorResult<DomainOperation> AvatarDocumentTarget::makeSet(const SelectionSnapshot& s, const PropertyPath& p,
    ...
    for (const auto& i : s.items)
        if (i.type == "avatar.layer") {
            auto& v = *std::find_if(candidate.layers_.begin(), candidate.layers_.end(),
                                    [&](const auto& x) { return x.id.value() == i.item.value(); });
            if (p == PropertyPath("layer.texture"))
                v.textureAsset = *value.getIf<std::string>();
            else if (p == PropertyPath("layer.z"))
                v.zIndex = static_cast<int>(*value.getIf<int64_t>());
            else if (p == PropertyPath("layer.visible"))
                v.visible = *value.getIf<bool>();
            else if (p == PropertyPath("layer.offset"))
                readFloats(&value, v.offset.data(), 2);
            else if (p == PropertyPath("layer.size"))
                readFloats(&value, v.size.data(), 2);
            else
                readFloats(&value, v.color.data(), 4);
        } else {
            auto&       v = *std::find_if(candidate.parameters_.begin(), candidate.parameters_.end(),
                                          [&](const auto& x) { return x.id.value() == i.item.value(); });
            const float n = static_cast<float>(*value.getIf<double>());
            if (p == PropertyPath("parameter.default"))
                v.defaultValue = n;
            else if (p == PropertyPath("parameter.minimum"))
                v.minimum = n;
...
std::vector<EditorDiagnostic> AvatarDocumentTarget::validate() const {
    std::vector<EditorDiagnostic> d;
    if (kind_ != "image" && kind_ != "live2d" && kind_ != "vroid")
        d.push_back(diagnostic("editor.avatar.kind", "Avatar kind must be image, live2d or vroid"));
    if (kind_ != "image" && sourceAsset_.empty())
        d.push_back(diagnostic("editor.avatar.source", "Live2D/VRoid Avatar requires a source asset"));
    if (kind_ != "image" && !layers_.empty())
        d.push_back(diagnostic("editor.avatar.backend-layers",
                               "Image layers are supported only by image Avatar assets"));
    if (layers_.size() > 512 || parameters_.size() > 2048 || expressions_.size() > 1024)
        d.push_back(diagnostic("editor.avatar.budget", "Avatar asset exceeds layer, parameter or expression budgets"));
    std::set<std::string> ids, names, channels;
    for (const auto& v : layers_) {
        if (v.id.empty() || v.name.empty() || !ids.insert(v.id.value()).second || !names.insert(v.name).second)
            d.push_back(diagnostic("editor.avatar.layer-identity", "Avatar layer IDs and names must be unique"));
        channels.insert(v.name);
        const bool vectorsFinite =
            std::all_of(v.offset.begin(), v.offset.end(), [](float n) { return std::isfinite(n); }) &&
            std::all_of(v.size.begin(), v.size.end(), [](float n) { return std::isfinite(n); });
        if (v.textureAsset.empty() || v.zIndex < -100000 || v.zIndex > 100000 || !vectorsFinite || v.size[0] <= 0 ||
            v.size[1] <= 0 ||
            !std::all_of(v.color.begin(), v.color.end(), [](float n) { return std::isfinite(n) && n >= 0 && n <= 1; }))
            d.push_back(diagnostic("editor.avatar.layer-range", "Avatar layer values are invalid"));
    }
    for (const auto& v : parameters_) {
        if (v.id.empty() || v.name.empty() || !ids.insert(v.id.value()).second || !names.insert(v.name).second)
            d.push_back(diagnostic("editor.avatar.parameter-identity",
                                   "Avatar parameter IDs and names must be unique"));
        channels.insert(v.name);
        if (!std::isfinite(v.minimum) || !std::isfinite(v.maximum) || !std::isfinite(v.defaultValue) ||
            !std::isfinite(v.value) || v.minimum > v.maximum || v.defaultValue < v.minimum ||
            v.defaultValue > v.maximum || v.value < v.minimum || v.value > v.maximum)
            d.push_back(diagnostic("editor.avatar.parameter-range", "Avatar parameter range/default/value is invalid"));
    }
    for (const auto& e : expressions_) {
        if (e.id.empty() || e.name.empty() || !ids.insert(e.id.value()).second || !names.insert(e.name).second)
            d.push_back(diagnostic("editor.avatar.expression-identity",
                                   "Avatar expression IDs and names must be unique"));
        if (e.channels.size() > 2048)
            d.push_back(diagnostic("editor.avatar.expression-budget", "Avatar expression exceeds 2048 channels"));
        for (const auto& [name, value] : e.channels)
            if (!channels.contains(name) || !std::isfinite(value))
                d.push_back(diagnostic("editor.avatar.expression-channel",
                                       "Avatar expression references an unknown or invalid channel"));
    }
    return d;
}
