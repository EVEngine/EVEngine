...
EditorResult<MapLayerRecord> parseLayer(const EditorValue& value) {
    ...
    if (!id || id->empty() || !name || name->empty() || !kind || !kinds.contains(*kind) || !visible || !locked ||
        ...
        return eve::editing::failed<MapLayerRecord>(EditorStatus::Rejected, RuleId("editor.map.invalid-layer"),
...
EditorResult<MapSplinePointRecord> parsePoint(const EditorValue& value) {
    ...
    if (!id || id->empty() || !x || !y || !z || !width || *width <= 0.0 || !std::isfinite(*x) || !std::isfinite(*y) ||
        ...
        return eve::editing::failed<MapSplinePointRecord>(
            EditorStatus::Rejected, RuleId("editor.map.invalid-spline-point"),
...
EditorResult<MapRoadRecord> parseRoad(const EditorValue& value) {
    ...
    if (!id || id->empty() || !layer || layer->empty() || !name || name->empty() || !material || !closed || !points ||
        points->size() < 2)
        return eve::editing::failed<MapRoadRecord>(EditorStatus::Rejected, RuleId("editor.map.invalid-road"),
    ...
    for (const EditorValue& pointValueEntry : *points) {
        ...
        if (!point.ok() || !pointIds.insert(point.value().id).second)
            return eve::editing::failed<MapRoadRecord>(EditorStatus::Rejected,
                                                       RuleId("editor.map.duplicate-spline-point"),
                                                       "Road spline point ids must be unique");
        result.points.push_back(std::move(point).value());
    }
    if (result.closed && result.points.size() < 3)
        return eve::editing::failed<MapRoadRecord>(EditorStatus::Rejected, RuleId("editor.map.closed-road-point-count"),
...
EditorResult<MapPlacementRecord> parsePlacement(const EditorValue& value) {
    ...
    if (!id || id->empty() || !layer || layer->empty() || !asset || asset->empty() || !x || !y || !z || !rx || !ry ||
        ...
        return eve::editing::failed<MapPlacementRecord>(
            EditorStatus::Rejected, RuleId("editor.map.invalid-placement"),
            "Placement requires ids, asset, complete TRS and positive scale");
    for (const double* component : {x, y, z, rx, ry, rz, sx, sy, sz})
        if (!std::isfinite(*component))
            return eve::editing::failed<MapPlacementRecord>(
                EditorStatus::Rejected, RuleId("editor.map.nonfinite-placement"), "Placement transform must be finite");
...
EditorResult<void> MapDocumentTarget::applyDomainOperation(const DomainOperation& domainOperation) {
    ...
    if (domainOperation.type == "map.layer.create.v1") {
        ...
        if (!layer.ok())
            return eve::editing::failed<void>(layer.code(), RuleId("editor.map.invalid-layer"),
                                              "Layer payload is invalid");
        if (layers_.contains(layer.value().id))
            return eve::editing::failed<void>(EditorStatus::Conflict, RuleId("editor.map.layer-exists"),
                                              "Map layer already exists");
        for (const auto& [id, current] : layers_) {
            static_cast<void>(id);
            if (current.order == layer.value().order)
                return eve::editing::failed<void>(EditorStatus::Conflict, RuleId("editor.map.layer-order-conflict"),
        ...
        if (!layer.ok() || !layers_.contains(layer.value().id))
            return eve::editing::failed<void>(EditorStatus::NotFound, RuleId("editor.map.layer-not-found"),
                                              "Map layer was not found");
        if (layers_.at(layer.value().id).locked)
            return eve::editing::failed<void>(EditorStatus::Rejected, RuleId("editor.map.layer-locked"),
                                              "Map layer is locked");
        for (const auto& [id, road] : roads_) {
            static_cast<void>(id);
            if (road.layer == layer.value().id)
                return eve::editing::failed<void>(EditorStatus::Rejected, RuleId("editor.map.layer-not-empty"),
        ...
        if (layers_.at(layer.value().id).kind != layer.value().kind) {
            for (const auto& [id, road] : roads_) {
                static_cast<void>(id);
                if (road.layer == layer.value().id)
                    return eve::editing::failed<void>(EditorStatus::Rejected, RuleId("editor.map.layer-kind-in-use"),
        ...
        if (!road.ok())
            return eve::editing::failed<void>(road.code(), RuleId("editor.map.invalid-road"),
                                              "Road payload is invalid");
        const auto layer = layers_.find(road.value().layer);
        if (layer == layers_.end() || layer->second.kind != "road")
            return eve::editing::failed<void>(EditorStatus::Rejected, RuleId("editor.map.road-layer-required"),
                                              "Road must reference a road layer");
        if (layer->second.locked)
            return eve::editing::failed<void>(EditorStatus::Rejected, RuleId("editor.map.layer-locked"),
                                              "Road layer is locked");
        ...
        if (!road.ok() || !roads_.contains(road.value().id))
            return eve::editing::failed<void>(EditorStatus::NotFound, RuleId("editor.map.road-not-found"),
                                              "Road was not found");
        ...
        if (!placement.ok())
            return eve::editing::failed<void>(placement.code(), RuleId("editor.map.invalid-placement"),
                                              "Placement payload is invalid");
        const auto layer = layers_.find(placement.value().layer);
        if (layer == layers_.end() || layer->second.kind != "object")
            return eve::editing::failed<void>(EditorStatus::Rejected, RuleId("editor.map.object-layer-required"),
                                              "Placement must reference an object layer");
        if (layer->second.locked)
            return eve::editing::failed<void>(EditorStatus::Rejected, RuleId("editor.map.layer-locked"),
                                              "Object layer is locked");
        ...
        if (!placement.ok() || !placements_.contains(placement.value().id))
            return eve::editing::failed<void>(EditorStatus::NotFound, RuleId("editor.map.placement-not-found"),
        ...
        return eve::editing::failed<void>(EditorStatus::Unsupported, RuleId("editor.map.operation-unsupported"),
...
EditorResult<DomainOperation> MapDocumentTarget::makeCreateLayer(const MapLayerRecord& layer) const {
    ...
    if (layers_.contains(layer.id))
        return eve::editing::failed<DomainOperation>(EditorStatus::Conflict, RuleId("editor.map.layer-exists"),
                                                     "Map layer already exists");
    for (const auto& [id, current] : layers_) {
        static_cast<void>(id);
        if (current.order == layer.order)
            return eve::editing::failed<DomainOperation>(EditorStatus::Conflict,
                                                         RuleId("editor.map.layer-order-conflict"),
                                                         "Map layer visual order is already occupied");
    }
...
EditorResult<DomainOperation> MapDocumentTarget::makeDeleteLayer(const StableId& layer) const {
    ...
    if (found->second.locked)
        return eve::editing::failed<DomainOperation>(EditorStatus::Rejected, RuleId("editor.map.layer-locked"),
                                                     "Map layer is locked");
    for (const auto& [id, road] : roads_) {
        static_cast<void>(id);
        if (road.layer == layer)
            return eve::editing::failed<DomainOperation>(EditorStatus::Rejected, RuleId("editor.map.layer-not-empty"),
        ...
        for (const auto& [id, road] : roads_) {
            ...
            if (road.layer == layer.id)
                return eve::editing::failed<DomainOperation>(EditorStatus::Rejected,
                                                             RuleId("editor.map.layer-kind-in-use"),
                                                             "Cannot change kind of a layer containing roads");
        }
        for (const auto& [id, placement] : placements_) {
            static_cast<void>(id);
...
EditorResult<DomainOperation> MapDocumentTarget::makeSetRoad(const MapRoadRecord& road) const {
    ...
    if (layer == layers_.end() || layer->second.kind != "road")
        return eve::editing::failed<DomainOperation>(EditorStatus::Rejected, RuleId("editor.map.road-layer-required"),
                                                     "Road must reference a road layer");
    if (layer->second.locked)
        return eve::editing::failed<DomainOperation>(EditorStatus::Rejected, RuleId("editor.map.layer-locked"),
                                                     "Road layer is locked");
...
EditorResult<DomainOperation> MapDocumentTarget::makeSetPlacement(const MapPlacementRecord& placement) const {
    ...
    if (layer == layers_.end() || layer->second.kind != "object")
        return eve::editing::failed<DomainOperation>(EditorStatus::Rejected, RuleId("editor.map.object-layer-required"),
                                                     "Placement must reference an object layer");
    if (layer->second.locked)
        return eve::editing::failed<DomainOperation>(EditorStatus::Rejected, RuleId("editor.map.layer-locked"),
                                                     "Object layer is locked");
...
EditorResult<void> MapDocumentTarget::loadSnapshot(const EditorValue& snapshot) {
    ...
    if (!version || *version != 1 || !layers || !roads || !placements)
        return eve::editing::failed<void>(EditorStatus::Unsupported, RuleId("editor.map.invalid-snapshot"),
    ...
    for (const EditorValue& value : *layers) {
        ...
        if (!layer.ok())
            return eve::editing::failed<void>(EditorStatus::Rejected, RuleId("editor.map.invalid-layer"),
                                              "Invalid layer");
        auto create = candidate.makeCreateLayer(layer.value());
        if (!create.ok() || !candidate.applyDomainOperation(create.value()).ok())
            return eve::editing::failed<void>(EditorStatus::Rejected, RuleId("editor.map.invalid-layer-set"),
                                              "Map snapshot contains conflicting layers");
    }
    for (const EditorValue& value : *roads) {
        auto road = parseRoad(value);
        if (!road.ok())
            return eve::editing::failed<void>(EditorStatus::Rejected, RuleId("editor.map.invalid-road"),
                                              "Invalid road");
        auto set = candidate.makeSetRoad(road.value());
        if (!set.ok() || !candidate.applyDomainOperation(set.value()).ok())
            return eve::editing::failed<void>(EditorStatus::Rejected, RuleId("editor.map.invalid-road-set"),
    ...
    for (const EditorValue& value : *placements) {
        ...
        if (!placement.ok())
            return eve::editing::failed<void>(EditorStatus::Rejected, RuleId("editor.map.invalid-placement"),
                                              "Invalid placement");
        auto set = candidate.makeSetPlacement(placement.value());
        if (!set.ok() || !candidate.applyDomainOperation(set.value()).ok())
            return eve::editing::failed<void>(EditorStatus::Rejected, RuleId("editor.map.invalid-placement-set"),
