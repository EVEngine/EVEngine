...
eve::Result<std::unique_ptr<eve::artifact::PreparedPublication>> MapArtifactProvider::prepare(
    ...
    if (failPrepare_)
        return eve::Result<std::unique_ptr<eve::artifact::PreparedPublication>>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::Failed, "map artifact prepare failure was injected"));
    if (publication.id.isNil() || publication.buildKey.empty())
        return eve::Result<std::unique_ptr<eve::artifact::PreparedPublication>>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::InvalidArgument, "map publication requires identity and build key"));
    if (find(publication.id) != nullptr)
        return eve::Result<std::unique_ptr<eve::artifact::PreparedPublication>>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::Conflict, "map artifact identity is already published"));

    const eve::artifact::PartView* topology = nullptr;
    for (const auto& part : publication.parts) {
        if (part.kind == eve::artifact::PartKind::Grid && (part.role == "topology" || topology == nullptr))
            topology = &part;
    }
    if (!topology || topology->width <= 0 || topology->height <= 0 ||
        topology->cells.size() !=
            static_cast<std::size_t>(topology->width) * static_cast<std::size_t>(topology->height))
        return eve::Result<std::unique_ptr<eve::artifact::PreparedPublication>>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::PreconditionViolation, "map publication requires a complete grid topology"));
    if (nextIndex_ == MapArtifactHandle::invalidIndex)
        return eve::Result<std::unique_ptr<eve::artifact::PreparedPublication>>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::Failed, "map artifact handle index exhausted"));
    ...
    try {
        ...
        return eve::Result<std::unique_ptr<eve::artifact::PreparedPublication>>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::Failed, "map artifact prepare allocation failed"));
    }
}
...
std::optional<std::uint32_t> MapArtifactProvider::cell(eve::PersistentId id, std::int32_t x,
    ...
    return record
        ->cells[static_cast<std::size_t>(y) * static_cast<std::size_t>(record->width) + static_cast<std::size_t>(x)];
}
...
eve::Result<eve::Value> MapArtifactProvider::snapshotState() const {
    try {
        ...
        return eve::Result<eve::Value>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::Failed, "map artifact snapshot allocation failed"));
    }
}
...
eve::Result<void> MapArtifactProvider::restoreState(const eve::Value& state) {
    if (!records_.empty())
        return eve::Result<void>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::Conflict, "map artifact restore requires an empty registry"));
    ...
    if (!object || !providerName || *providerName != "map.cpu-registry" || !hasSupportedVersion(*object) || !records)
        return eve::Result<void>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "invalid map artifact provider state"));
    ...
    try {
        ...
        for (const eve::Value& encoded : *records) {
            ...
            if (!item || !readString(*item, "artifactId", idText) || !readString(*item, "buildKey", buildKey) ||
                !readU64(*item, "handle", packed) || !readI64(*item, "width", width) ||
                !readI64(*item, "height", height) || width <= 0 || height <= 0 || !cells)
                return eve::Result<void>::failure(
                    eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "invalid map artifact record"));
            ...
            if (!parsedId || parsedId->isNil() || handle.isInvalid() ||
                width > std::numeric_limits<std::int32_t>::max() || height > std::numeric_limits<std::int32_t>::max() ||
            ...
            for (const eve::Value& value : *cells) {
                ...
                if (!cell || *cell < 0 || *cell > std::numeric_limits<std::uint32_t>::max())
                    return eve::Result<void>::failure(
                        eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "invalid map topology cell"));
        ...
        return eve::Result<void>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::Failed, "map artifact restore allocation failed"));
    }
...
void registerMapArtifactProvider() { eve::cap::provide<eve::artifact::IMapArtifactAdapter>(&mapArtifactProvider()); }
