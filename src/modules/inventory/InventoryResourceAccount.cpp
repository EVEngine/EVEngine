...
eve::Status foreignReservation() {
    return eve::Status::failure(
        ...
        eve::Diagnostic::error(eve::DiagnosticCode::Conflict,
                               "resource reservation belongs to a different inventory account", "reservation.account"));
}

eve::Status terminalReservation() {
    return eve::Status::failure(
        eve::StatusCode::Conflict,
        eve::Diagnostic::error(eve::DiagnosticCode::Conflict, "inventory resource reservation is no longer active",
                               "reservation"));
}
...
eve::Status unknownReservation() {
    return eve::Status::failure(
        ...
        eve::Diagnostic::error(eve::DiagnosticCode::NotFound,
                               "inventory resource reservation was not issued by this account", "reservation"));
}
...
eve::Result<T> exceptionFailure(std::string_view operation, const std::exception& exception) {
    return failure<T>(eve::DiagnosticCode::Failed,
                      std::string(operation) + " failed while staging an inventory candidate: " + exception.what(),
                      "inventory.account");
}
...
eve::Result<T> unknownExceptionFailure(std::string_view operation) {
    return failure<T>(eve::DiagnosticCode::Failed,
                      std::string(operation) + " failed while staging an inventory candidate", "inventory.account");
}
...
eve::Result<std::int64_t> InventoryResourceAccount::balanceOf(const ResourceId& resource) const {
    ...
    if (quantity < 0)
        return failure<std::int64_t>(eve::DiagnosticCode::InvariantViolation,
                                     "inventory item quantity must not be negative", resource.value());
    return eve::Result<std::int64_t>::success(static_cast<std::int64_t>(quantity));
}

eve::Result<std::int64_t> InventoryResourceAccount::activeReservationsFor(const ResourceId& resource) const {
    std::int64_t total = 0;
...
eve::Result<Reservation> InventoryResourceAccount::reserve(const CostSpec& cost) {
    ...
    if (!inserted)
        return failure<Reservation>(eve::DiagnosticCode::Conflict, "inventory resource reservation id was already used",
                                    "reservation");
            ...
            if (debit) {
                ...
                if (!balance || balance.value() < amount) {
                    ...
                    return failure<void>(eve::DiagnosticCode::PreconditionViolation,
                                         "inventory debit would make item quantity negative", item.resource.value());
                }
                if (InventorySystem::removeItem(&bag_, item.resource.value(), amount) != amount) {
                    restore();
                    return failure<void>(eve::DiagnosticCode::Failed, "inventory debit was not applied in full",
                                         item.resource.value());
                }
            } else {
                std::string reason;
                ...
                if (InventorySystem::addItem(&bag_, item.resource.value(), amount) != amount) {
                    restore();
                    return failure<void>(eve::DiagnosticCode::PreconditionViolation,
                                         "inventory credit was not applied in full", item.resource.value());
                }
            }
        }
...
eve::Result<Receipt> InventoryResourceAccount::commit(const Reservation& reservation) {
    ...
    if (!reservation.isValid())
        return failure<Receipt>(eve::DiagnosticCode::InvalidArgument, "inventory resource reservation is invalid",
                                "reservation");
...
eve::Result<void> InventoryResourceAccount::rollback(const Reservation& reservation) {
    ...
    if (!reservation.isValid())
        return failure<void>(eve::DiagnosticCode::InvalidArgument, "inventory resource reservation is invalid",
                             "reservation");
...
eve::Result<CostSpec> ItemCostAdapter::itemCost(std::string_view itemId, std::int64_t quantity) {
    return CostSpec::single(itemId, quantity);
}

eve::Result<CostSpec> ItemCostAdapter::ammoCost(std::string_view ammoItemId, std::int64_t quantity) {
