...
[[nodiscard]] bool hasId(const Bag& bag, const eve::container::MembershipId& id) {
    return std::any_of(bag.slots().begin(), bag.slots().end(), [&](const ItemStack& stack) {
        return !stack.empty() &&
               eve::container::MembershipId("inventory:" + std::to_string(stack.instanceId)) == id;
    });
}
...
[[nodiscard]] bool sameLayout(const eve::container::ContainerSnapshot& lhs,
    ...
    if (lhs.id != rhs.id || lhs.revision != rhs.revision || lhs.entries.size() != rhs.entries.size())
        return false;
    for (std::size_t index = 0; index < lhs.entries.size(); ++index) {
        const auto& left = lhs.entries[index];
        const auto& right = rhs.entries[index];
        if (left.membership.object != right.membership.object ||
            left.membership.slot != right.membership.slot ||
...
class InventoryContainerAdapter::PreparedState final
    : public eve::container::IContainer::PreparedState {
public:
    PreparedState(InventoryContainerAdapter& owner, std::vector<ItemStack> staged,
...
InventoryContainerAdapter::InventoryContainerAdapter(eve::container::ContainerId id, Bag* bag,
    ...
    : descriptor_{std::move(id),
      ...
      kind_(InventoryContainerKind::Bag), bag_(bag) {}

InventoryContainerAdapter::InventoryContainerAdapter(eve::container::ContainerId id,
                                                     EquipmentSet* equipment)
    : descriptor_{std::move(id),
                  eve::container::Capacity::fixed(
                      equipment ? static_cast<std::size_t>(equipment->getSlotCount()) : 0u),
                  eve::container::Ordering::ExplicitSlots, eve::container::Filter{}},
      kind_(InventoryContainerKind::Equipment), equipment_(equipment) {}
...
eve::container::ContainerObject InventoryContainerAdapter::describe(const ItemStack& stack,
...
eve::Result<eve::container::ContainerSnapshot> InventoryContainerAdapter::snapshot() const {
    if (!descriptor_.id.isValid() ||
        (kind_ == InventoryContainerKind::Bag ? bag_ == nullptr : equipment_ == nullptr))
        return eve::Result<eve::container::ContainerSnapshot>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument,
                                   "inventory container adapter is not bound"));
    eve::container::ContainerSnapshot snapshot{descriptor_.id, revision_, {}};
    std::unordered_set<eve::container::MembershipId> ids;
    const int count = kind_ == InventoryContainerKind::Bag ? bag_->getSlotCount()
                                                            : equipment_->getSlotCount();
    for (int index = 0; index < count; ++index) {
        ...
        if (!descriptor_.capacity.isUnlimited() &&
            static_cast<std::size_t>(index) >= descriptor_.capacity.value())
            return eve::Result<eve::container::ContainerSnapshot>::failure(
                eve::Diagnostic::error(eve::DiagnosticCode::InvariantViolation,
                                       "inventory membership exceeds configured capacity"));
        if (stack->instanceId <= 0)
            return eve::Result<eve::container::ContainerSnapshot>::failure(
                eve::Diagnostic::error(eve::DiagnosticCode::InvariantViolation,
                                       "inventory stack has no instance identity"));
        const auto id = objectId(*stack);
        if (!ids.insert(id).second)
            return eve::Result<eve::container::ContainerSnapshot>::failure(
                eve::Diagnostic::error(eve::DiagnosticCode::Conflict,
                                       "inventory container contains duplicate instance identity"));
        snapshot.entries.push_back({{id, slot, eve::Generation(1)},
                                     describe(*stack, slotName(slot))});
    }
    if (!descriptor_.capacity.isUnlimited() &&
        snapshot.entries.size() > descriptor_.capacity.value())
        return eve::Result<eve::container::ContainerSnapshot>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::InvariantViolation,
                                   "inventory container exceeds configured capacity"));
    return eve::Result<eve::container::ContainerSnapshot>::success(std::move(snapshot));
}

eve::Result<void> InventoryContainerAdapter::validateObject(
    const InventoryContainerObject& object) const {
...
eve::Result<void> InventoryContainerAdapter::validateInsert(
    ...
    if (object.type != payload->stack.itemId ||
        object.quantity != static_cast<std::uint32_t>(payload->stack.quantity))
    ...
    if (definition == nullptr)
        return error(eve::DiagnosticCode::NotFound, "inventory item definition was not found");
    if (definition->maxStack <= 0 || payload->stack.quantity > definition->maxStack)
        return error(eve::DiagnosticCode::PreconditionViolation,
                     "inventory stack exceeds item definition maxStack");
    ...
    if (kind_ == InventoryContainerKind::Bag) {
        ...
        const std::size_t slotLimit = descriptor_.capacity.isUnlimited()
                                          ? slotCount
                                          : std::min(slotCount, descriptor_.capacity.value());
        ...
        if (destination) {
            if (!destination->isValid() ||
                static_cast<std::size_t>(destination->value()) >= slotLimit)
            ...
            const bool hasEmptySlot = std::any_of(
                bag_->slots().begin(), bag_->slots().begin() + static_cast<std::ptrdiff_t>(slotLimit),
    ...
    if (!destination || !destination->isValid() ||
        static_cast<std::size_t>(destination->value()) >=
            static_cast<std::size_t>(equipment_->getSlotCount()))
        return error(eve::DiagnosticCode::PreconditionViolation,
    ...
    if (current != nullptr && !current->empty() &&
        (!ignoredObject || object.id != objectId(*current)))
    ...
    if (!definition->equipSlot.empty() && definition->equipSlot != target)
        return error(eve::DiagnosticCode::PreconditionViolation,
                     "inventory item does not fit equipment slot");
    ...
    if (allowed != nullptr && !allowed->empty()) {
        ...
        if (!accepted)
            return error(eve::DiagnosticCode::PreconditionViolation,
                         "equipment slot rejected inventory tags");
    }
    return eve::Result<void>::success();
}

eve::Result<std::unique_ptr<eve::container::IContainer::PreparedState>>
InventoryContainerAdapter::prepare(const eve::container::ContainerSnapshot& expected,
    ...
    if (!bound || !descriptor_.id.isValid())
        return eve::Result<std::unique_ptr<eve::container::IContainer::PreparedState>>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument,
                                   "inventory adapter is not bound"));
    if (expected.id != descriptor_.id || candidate.id != descriptor_.id)
        return eve::Result<std::unique_ptr<eve::container::IContainer::PreparedState>>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument,
                                   "inventory prepare targets another container"));
    ...
    if (!sameLayout(currentResult.value(), expected))
        return eve::Result<std::unique_ptr<eve::container::IContainer::PreparedState>>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::StaleHandle,
                                   "inventory prepare expected snapshot is stale"));
    const auto nextRevision = expected.revision.incremented();
    if (!nextRevision || candidate.revision != *nextRevision)
        return eve::Result<std::unique_ptr<eve::container::IContainer::PreparedState>>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::InvariantViolation,
                                   "inventory candidate revision is not the next revision"));
    const int slotCount = kind_ == InventoryContainerKind::Bag ? bag_->getSlotCount()
                                                                : equipment_->getSlotCount();
    if (!descriptor_.capacity.isUnlimited() &&
        candidate.entries.size() > descriptor_.capacity.value())
        return eve::Result<std::unique_ptr<eve::container::IContainer::PreparedState>>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::Conflict,
                                   "inventory candidate exceeds container capacity"));
    ...
    for (const auto& entry : candidate.entries) {
        if (!entry.membership.object.isValid() || !entry.membership.slot.isValid() ||
            ...
            !ids.insert(entry.membership.object).second ||
        ...
        if (payload == nullptr)
            return eve::Result<std::unique_ptr<eve::container::IContainer::PreparedState>>::failure(
                eve::Diagnostic::error(eve::DiagnosticCode::StaleHandle,
                                       "inventory candidate payload is stale"));
        auto validObject = validateObject(*payload);
        if (!validObject)
            return eve::Result<std::unique_ptr<eve::container::IContainer::PreparedState>>::failure(
                validObject.status());
        if (entry.object.type != payload->stack.itemId ||
            ...
            return eve::Result<std::unique_ptr<eve::container::IContainer::PreparedState>>::failure(
                eve::Diagnostic::error(eve::DiagnosticCode::StaleHandle,
                                       "inventory candidate object facts are stale"));
