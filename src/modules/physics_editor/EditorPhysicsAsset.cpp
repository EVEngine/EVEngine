...
EditorResult<PhysicsColliderAssetGeometry> AssetDatabasePhysicsColliderResolver::resolve(
    ...
    if (!found.ok()) found = database_->findByUri(reference);
    if (!found.ok()) return error(EditorStatus::NotFound, "editor.physics.collider-asset-not-found",
    ...
    return eve::editing::applied<PhysicsColliderAssetGeometry>(std::move(result));
}
