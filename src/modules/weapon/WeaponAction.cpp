...
void put(eve::Value::Object& object, const char* key, int value) { object.emplace(key, eve::Value(value)); }

}  // namespace

eve::Result<std::optional<eve::resource::CostSpec>> WeaponActionAdapter::resourceCost(const WeaponDefinition& weapon) {
...
eve::Result<eve::transaction::TransactionReceipt> executeActionFire(
    ...
    eve::transaction::ITransactionParticipant& effect, eve::SimulationTick tick, std::string transactionId) {
    auto definition = WeaponActionAdapter::makeDefinition(weapon, attack);
    if (!definition) return eve::Result<eve::transaction::TransactionReceipt>::failure(definition.status());
    auto request = WeaponActionAdapter::makeRequest(weapon, attack, std::nullopt, tick);
    if (!request) return eve::Result<eve::transaction::TransactionReceipt>::failure(request.status());
    ...
    if (!submitted) return eve::Result<eve::transaction::TransactionReceipt>::failure(submitted.status());
    ...
    auto total = submittedExecution->definition().timing.windup.tryAdd(submittedExecution->definition().timing.active);
    if (!total) return eve::Result<eve::transaction::TransactionReceipt>::failure(total.status());
    total = std::move(total).takeValue().tryAdd(submittedExecution->definition().timing.recover);
    if (!total) return eve::Result<eve::transaction::TransactionReceipt>::failure(total.status());

    auto advanced = runtime.advance(executionId, tick, std::move(total).takeValue());
    if (!advanced) return eve::Result<eve::transaction::TransactionReceipt>::failure(advanced.status());
    ...
    return eve::Result<eve::transaction::TransactionReceipt>::success(*execution->transactionReceipt(),
                                                                      advanced.status());
}
...
eve::Result<action::ActionDefinition> WeaponActionAdapter::makeDefinition(const WeaponDefinition& weapon,
    ...
    definition.targetingMode  = request.hasTarget ? action::TargetingMode::Explicit : action::TargetingMode::None;
...
eve::Result<action::ActionRequest> WeaponActionAdapter::makeRequest(const WeaponDefinition&          weapon,
                                                                    const AttackRequest&             attack,
