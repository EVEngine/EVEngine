...
template <class T>
eve::Result<T> definitionsBindingFailure(eve::DiagnosticCode code, std::string message,
                                          std::string path = {}) {
    return eve::Result<T>::failure(eve::Diagnostic::error(
        code, std::move(message), std::move(path), {}, "definitions.squirrel"));
}
...
const eve::SnapshotMigrationChain& definitionsMigrations() {
    static const eve::SnapshotMigrationChain chain = [] {
        ...
        const auto registration = result.add(
            definitionsSchema(), eve::SchemaVersion(0), eve::SchemaVersion(1),
...
eve::Result<Definition> makeDefinition(const std::string& type, const std::string& id, int version,
    ...
    if (type.empty() || id.empty())
        return failure<Definition>(eve::DiagnosticCode::InvalidArgument,
                                   "definition type and id must not be empty");
    ...
    if (eve::schema::SchemaRegistry::resolve(type, version)) {
        ...
        if (!errors.empty()) {
            ...
            return eve::Result<Definition>::failure(eve::Diagnostic::error(
                ...
                eve::DiagnosticDetails{{"schemaId", type},
...
DefinitionEvent DefinitionRegistry::projectEvent(const Storage::Event& event) {
    ...
    result.name = event.operation == eve::RegistryOperation::Remove ? "definition_removed"
                                                                      : "definition_reloaded";
...
DefinitionRegistry::Storage::Handle DefinitionRegistry::storageHandle(const DefinitionHandle& handle) {
    const auto& logical = handle.reference.id();
    return Storage::Handle{Key{std::string(logical.namespaceName()), std::string(logical.name())},
                           handle.generation};
}

void DefinitionRegistry::clearEventProjection() const { eventProjection_.clear(); }

eve::Result<DefinitionHandle> DefinitionRegistry::insert(
...
eve::Result<DefinitionHandle> DefinitionRegistry::replace(
    const std::string& type, const std::string& id, int version, const std::string& json) {
    auto definition = makeDefinition(type, id, version, json);
    if (!definition.ok()) {
        return eve::Result<DefinitionHandle>::failure(definition.status());
...
eve::ResultRef<const Definition> DefinitionRegistry::resolve(
...
eve::ResultRef<const Definition> DefinitionRegistry::resolveHandle(
...
eve::Result<DefinitionHandle> DefinitionRegistry::handle(
    const std::string& type, const std::string& id) const {
...
eve::Result<eve::Generation> DefinitionRegistry::generationOf(
    const std::string& type, const std::string& id) const {
...
eve::Result<void> DefinitionRegistry::restoreJson(const std::string& json) {
    ...
    for (std::size_t index = 0; index < generationArray->size(); ++index) {
        ...
        if (!item || !readString(*item, "type", type) || !readString(*item, "id", id) || type.empty() || id.empty() ||
            ...
            !candidate.entries.emplace(Key{type, id}, Storage::Entry{
    ...
    for (std::size_t index = 0; index < eventArray->size(); ++index) {
        ...
        if (!item || !readString(*item, "name", name) || !readString(*item, "type", type) ||
            ...
            (name != "definition_reloaded" && name != "definition_removed") || type.empty() || id.empty() ||
            !versionField || !readPositiveSchemaVersion(*versionField, versionValue) || !sequenceField ||
            !readUint64String(*sequenceField, sequence) || sequence == 0 ||
        ...
        event.operation = removed ? eve::RegistryOperation::Remove
                                  : (generation == 1 ? eve::RegistryOperation::Insert
...
eve::Result<eve::SnapshotEnvelope> DefinitionRegistry::snapshot(
...
eve::Result<void> DefinitionRegistry::restoreSnapshot(
...
eve::Result<std::string> DefinitionRegistry::snapshotEnvelopeJson(
    const eve::SnapshotHashProvider& hashProvider) const {
...
eve::Result<void> DefinitionRegistry::restoreSnapshotJson(
...
void Definitions::expose(ssq::Table& table) {
    ...
    auto registry = table.addClass<DefinitionRegistry>(
        ...
        false);
    registry.addFunc("insert", [vm](DefinitionRegistry* value, const std::string& type,
                                       const std::string& id, int version, const std::string& json) {
        if (!value)
            return eve::script::projectResult(
                vm, definitionsBindingFailure<DefinitionHandle>(
                        eve::DiagnosticCode::InvalidArgument,
                        "definition registry must not be null", "registry"),
                [](DefinitionHandle&& handle) { return definitionHandleProjection(handle); });
        return eve::script::projectResult(
            vm, value->insert(type, id, version, json),
            [](DefinitionHandle&& handle) { return definitionHandleProjection(handle); });
    });
    registry.addFunc("replace", [vm](DefinitionRegistry* value, const std::string& type,
                                        const std::string& id, int version, const std::string& json) {
        if (!value)
            return eve::script::projectResult(
                vm, definitionsBindingFailure<DefinitionHandle>(
                ...
                [](DefinitionHandle&& handle) { return definitionHandleProjection(handle); });
        return eve::script::projectResult(
            vm, value->replace(type, id, version, json),
            [](DefinitionHandle&& handle) { return definitionHandleProjection(handle); });
    });
    registry.addFunc("remove", [vm](DefinitionRegistry* value, const std::string& type,
                                       const std::string& id) {
        if (!value)
            return eve::script::projectResult(
                vm, definitionsBindingFailure<DefinitionHandle>(
                ...
                [](DefinitionHandle&& handle) { return definitionHandleProjection(handle); });
        return eve::script::projectResult(
            vm, value->remove(type, id),
            [](DefinitionHandle&& handle) { return definitionHandleProjection(handle); });
    });
    registry.addFunc("resolve", [vm](DefinitionRegistry* value, const std::string& type,
                                       const std::string& id) {
        if (!value)
            return eve::script::projectResult(
                vm, definitionsBindingFailure<std::reference_wrapper<const Definition>>(
                        eve::DiagnosticCode::InvalidArgument,
                        "definition registry must not be null", "registry"),
        ...
        return eve::script::projectResult(
            vm, value->resolve(type, id),
            [](std::reference_wrapper<const Definition>&& definition) {
                return definitionProjection(definition.get());
            });
    });
    registry.addFunc("resolveHandle", [vm](DefinitionRegistry* value,
        ...
        if (generation <= 0)
            return eve::script::projectResult(
                vm, definitionsBindingFailure<std::reference_wrapper<const Definition>>(
                        eve::DiagnosticCode::InvalidArgument,
                        "definition generation must be positive", "generation"),
                [](std::reference_wrapper<const Definition>&& definition) {
                    return definitionProjection(definition.get());
                });
        return eve::script::projectResult(
            vm, value->resolveHandle(DefinitionHandle{
                    value->reference(type, id), eve::Generation(static_cast<std::uint64_t>(generation))}),
            [](std::reference_wrapper<const Definition>&& definition) {
                return definitionProjection(definition.get());
            });
    });
    ...
    registry.addFunc("handle", [vm](DefinitionRegistry* value, const std::string& type,
                                             const std::string& id) {
        if (!value)
            return eve::script::projectResult(
                vm, definitionsBindingFailure<DefinitionHandle>(
                ...
                [](DefinitionHandle&& handle) { return definitionHandleProjection(handle); });
        return eve::script::projectResult(
            vm, value->handle(type, id),
            [](DefinitionHandle&& handle) { return definitionHandleProjection(handle); });
    });
    registry.addFunc("generationOf", [vm](DefinitionRegistry* value, const std::string& type,
                                            const std::string& id) {
        if (!value)
            return eve::script::projectResult(
                vm, definitionsBindingFailure<eve::Generation>(
                        eve::DiagnosticCode::InvalidArgument,
                        "definition registry must not be null", "registry"),
