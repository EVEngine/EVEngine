...
eve::Result<const eve::Value::Object*> object(const eve::Value& value, std::string path,
    ...
    if (!result || result->size() != fields.size())
        return eve::Result<const eve::Value::Object*>::failure(eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "replay object has unknown or missing fields", std::move(path), {},
                                                           "pixelworld.replay"));
...
eve::Result<std::uint64_t> decimal(const eve::Value& value, std::string path) {
    ...
    if (!text) return eve::Result<std::uint64_t>::failure(eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "expected decimal string", path, {},
                                                           "pixelworld.replay"));
    std::uint64_t result = 0;
    const auto [end, error] = std::from_chars(text->data(), text->data() + text->size(), result);
    if (error != std::errc{} || end != text->data() + text->size())
        return eve::Result<std::uint64_t>::failure(eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "invalid decimal string", std::move(path), {},
                                                           "pixelworld.replay"));
...
eve::Result<int> integer(const eve::Value& value, std::string path) {
    ...
    if (!number || *number < std::numeric_limits<int>::min() || *number > std::numeric_limits<int>::max())
        return eve::Result<int>::failure(eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "expected in-range integer", std::move(path), {},
                                                           "pixelworld.replay"));
...
eve::Result<eve::pixelworld::PixelEditCommand> parseCommand(const eve::Value& value, const std::string& path) {
    ...
    if (!centerX || !centerY || !kind || !material || !radius || !sequence || !strength || !temperature)
        return eve::Result<eve::pixelworld::PixelEditCommand>::failure(eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "invalid replay command", path, {},
                                                           "pixelworld.replay"));
    if (kind.value() < 0 || kind.value() > static_cast<int>(eve::pixelworld::PixelEditKind::Explosion) ||
        ...
        return eve::Result<eve::pixelworld::PixelEditCommand>::failure(eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "replay command enum or scalar is out of range", path, {},
                                                           "pixelworld.replay"));
...
eve::Result<void> PixelReplayLog::restoreSnapshot(
    ...
    if (snapshot.type != kReplayType || snapshot.schema != replaySchema())
        return eve::Result<void>::failure(eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument, "replay snapshot type or schema does not match", "schema", {},
                                                           "pixelworld.replay"));
    if (snapshot.schemaVersion.value() < 1 || snapshot.schemaVersion.value() > kReplaySchemaVersion)
        return eve::Result<void>::failure(eve::Diagnostic::error(eve::DiagnosticCode::UnknownVersion, "unsupported replay snapshot schema version", "schemaVersion", {},
                                                           "pixelworld.replay"));
    ...
    if (!encodedEntries || !encodedCheckpoints || encodedEntries->size() > kMaximumReplayEntries ||
        ...
        return eve::Result<void>::failure(eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "replay arrays are invalid or exceed decode budget", "payload", {},
                                                           "pixelworld.replay"));
    ...
    for (std::size_t index = 0; index < encodedEntries->size(); ++index) {
        ...
        if (!tick || !command)
            return eve::Result<void>::failure(eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "invalid replay entry", path, {},
                                                           "pixelworld.replay"));
    ...
    for (std::size_t index = 0; index < encodedCheckpoints->size(); ++index) {
        ...
        if (!tick || !revision || !worldDigest || tick.value() == 0 || tick.value() <= previousCheckpointTick)
            return eve::Result<void>::failure(eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "invalid replay checkpoint", path, {},
                                                           "pixelworld.replay"));
        PixelReplayCheckpoint checkpoint;
        ...
        if (!versionOne) {
            ...
            if (!chunks || chunks->size() > kMaximumReplayChunkDigests - totalChunks)
                return eve::Result<void>::failure(eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "replay Chunk digests exceed decode budget", path + ".chunks", {},
                                                           "pixelworld.replay"));
            ...
            for (std::size_t chunkIndex = 0; chunkIndex < chunks->size(); ++chunkIndex) {
                ...
                if (!x || !y || !digest)
                    return eve::Result<void>::failure(eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "invalid replay Chunk digest", chunkPath, {},
                                                           "pixelworld.replay"));
                ...
                if (!checkpoint.chunks.empty()) {
                    ...
                    if (decoded.y < prior.y || (decoded.y == prior.y && decoded.x <= prior.x))
                        return eve::Result<void>::failure(eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "replay Chunk digests must be unique canonical order", chunkPath, {},
                                                           "pixelworld.replay"));
                }
    ...
    if (snapshot.revision.value() != candidate.entries_.size() + candidate.checkpoints_.size() ||
        ...
        return eve::Result<void>::failure(eve::Diagnostic::error(eve::DiagnosticCode::Conflict, "replay payload count or Tick disagrees with envelope metadata", "payload", {},
                                                           "pixelworld.replay"));
