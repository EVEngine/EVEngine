...
eve::Result<eve::SnapshotEnvelope> archiveChunkBatch(
    ...
    if (batch.chunks.size() > kMaximumArchiveChunks)
        return eve::Result<eve::SnapshotEnvelope>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument, "Chunk archive exceeds record budget",
                                   "chunks", {}, "pixelworld.chunk-archive"));
    ...
    for (const auto& chunk : batch.chunks) {
        ...
        if (previous && !(previous.value() < key))
            return eve::Result<eve::SnapshotEnvelope>::failure(eve::Diagnostic::error(
                eve::DiagnosticCode::InvalidArgument, "Chunks must be unique canonical y/x order", "chunks", {},
                "pixelworld.chunk-archive"));
        previous = key;
        if (chunk.revision == 0 || chunk.revision > batch.sourceRevision ||
            ...
            return eve::Result<eve::SnapshotEnvelope>::failure(
                eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument, "invalid Chunk revision or cell shape",
                                       "chunks", {}, "pixelworld.chunk-archive"));
...
eve::Result<eve::pixelworld::PixelChunkBatch> decodeChunkBatchArchive(
    ...
    if (snapshot.type != kArchiveType || snapshot.schema != archiveSchema())
        return eve::Result<eve::pixelworld::PixelChunkBatch>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument, "Chunk archive type or schema does not match",
                                   "schema", {}, "pixelworld.chunk-archive"));
    if (snapshot.schemaVersion.value() < 1 || snapshot.schemaVersion.value() > kArchiveVersion)
        return eve::Result<eve::pixelworld::PixelChunkBatch>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::UnknownVersion, "unsupported Chunk archive version",
                                   "schemaVersion", {}, "pixelworld.chunk-archive"));
    ...
    if (!catalog || !seed || !sequence || !records || records->size() > kMaximumArchiveChunks)
        return eve::Result<eve::pixelworld::PixelChunkBatch>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "invalid Chunk archive payload", "payload", {},
                                   "pixelworld.chunk-archive"));
    ...
    if (!versionOne && !versionTwo) {
        ...
        if (!fullResync)
            return eve::Result<eve::pixelworld::PixelChunkBatch>::failure(
                eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "invalid fullResync flag", "payload.fullResync",
                                       {}, "pixelworld.chunk-archive"));
    ...
    for (std::size_t index = 0; index < records->size(); ++index) {
        ...
        if (previous && !(previous.value() < key))
            return eve::Result<eve::pixelworld::PixelChunkBatch>::failure(
                eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "Chunk records are not canonical", path, {},
                                       "pixelworld.chunk-archive"));
        previous = key;
        ...
        if ((!*removed && !currentCells && !legacyCells) || (*removed && decodedSize.value() != 0) ||
            ...
            return eve::Result<eve::pixelworld::PixelChunkBatch>::failure(
                eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "Chunk decoded size exceeds budget", path, {},
                                       "pixelworld.chunk-archive"));
        ...
        if (*removed && (codec != PixelChunkArchiveCodec::None || !stored.value().empty()))
            return eve::Result<eve::pixelworld::PixelChunkBatch>::failure(eve::Diagnostic::error(
                eve::DiagnosticCode::ParseError, "removed Chunk must have empty uncompressed payload", path, {},
                "pixelworld.chunk-archive"));
        ...
        if (!expectedHash || !actualHash || actualHash.value() != *expectedHash)
            return eve::Result<eve::pixelworld::PixelChunkBatch>::failure(
                eve::Diagnostic::error(eve::DiagnosticCode::HashMismatch, "decoded Chunk content hash does not match",
