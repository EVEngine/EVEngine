...
eve::Result<std::vector<std::uint8_t>> unhex(const eve::Value& value, std::size_t maximumBytes,
    ...
    if (!text || text->size() % 2 != 0 || text->size() / 2 > maximumBytes)
        ...
        eve::DiagnosticCode::ParseError, "invalid or oversized hexadecimal payload", std::move(path), {}, "pixelworld.chunk-archive"));
    ...
    for (std::size_t index = 0; index < result.size(); ++index) {
        ...
        if (high < 0 || low < 0)
            return eve::Result<std::vector<std::uint8_t>>::failure(eve::Diagnostic::error(
        eve::DiagnosticCode::ParseError, "invalid hexadecimal digit", std::move(path), {}, "pixelworld.chunk-archive"));
...
eve::Result<const eve::Value::Object*> object(const eve::Value& value, std::string path,
    ...
    if (!result || result->size() != fields.size())
        ...
        eve::DiagnosticCode::ParseError, "archive object has unknown or missing fields", std::move(path), {}, "pixelworld.chunk-archive"));
...
eve::Result<std::uint64_t> decimal(const eve::Value& value, std::string path) {
    const auto* text = value.getIf<std::string>();
    if (!text) return eve::Result<std::uint64_t>::failure(eve::Diagnostic::error(
        eve::DiagnosticCode::ParseError, "expected decimal string", path, {}, "pixelworld.chunk-archive"));
    std::uint64_t result = 0;
    const auto [end, error] = std::from_chars(text->data(), text->data() + text->size(), result);
    if (error != std::errc{} || end != text->data() + text->size())
        return eve::Result<std::uint64_t>::failure(eve::Diagnostic::error(
        eve::DiagnosticCode::ParseError, "invalid decimal string", std::move(path), {}, "pixelworld.chunk-archive"));
...
eve::Result<int> integer(const eve::Value& value, std::string path) {
    ...
    if (!number || *number < std::numeric_limits<int>::min() || *number > std::numeric_limits<int>::max())
        ...
        eve::DiagnosticCode::ParseError, "expected in-range integer", std::move(path), {}, "pixelworld.chunk-archive"));
...
eve::Result<eve::ContentId> digest(const std::vector<std::uint8_t>& decoded,
    ...
    if (!hashProvider)
        ...
        eve::DiagnosticCode::Unsupported, "archive hash provider is required", {}, {}, "pixelworld.chunk-archive"));
...
eve::Result<eve::SnapshotEnvelope> archiveChunkBatch(
    ...
    if (codec != PixelChunkArchiveCodec::None && codec != PixelChunkArchiveCodec::Zstd)
        ...
        eve::DiagnosticCode::Unsupported, "unknown Chunk archive codec", {}, {}, "pixelworld.chunk-archive"));
    if (batch.chunks.size() > kMaximumArchiveChunks)
        return eve::Result<eve::SnapshotEnvelope>::failure(eve::Diagnostic::error(
        eve::DiagnosticCode::InvalidArgument, "Chunk archive exceeds record budget", "chunks", {}, "pixelworld.chunk-archive"));
    ...
    for (const auto& chunk : batch.chunks) {
        ...
        if (previous && !(previous.value() < key))
            return eve::Result<eve::SnapshotEnvelope>::failure(eve::Diagnostic::error(
        eve::DiagnosticCode::InvalidArgument, "Chunks must be unique canonical y/x order", "chunks", {}, "pixelworld.chunk-archive"));
        previous = key;
        if (chunk.revision == 0 || chunk.revision > batch.sourceRevision ||
            ...
            return eve::Result<eve::SnapshotEnvelope>::failure(eve::Diagnostic::error(
        eve::DiagnosticCode::InvalidArgument, "invalid Chunk revision or cell shape", "chunks", {}, "pixelworld.chunk-archive"));
...
eve::Result<eve::pixelworld::PixelChunkBatch> decodeChunkBatchArchive(
    ...
    if (snapshot.type != kArchiveType || snapshot.schema != archiveSchema())
        return eve::Result<eve::pixelworld::PixelChunkBatch>::failure(eve::Diagnostic::error(
        eve::DiagnosticCode::InvalidArgument, "Chunk archive type or schema does not match", "schema", {}, "pixelworld.chunk-archive"));
    if (snapshot.schemaVersion.value() < 1 || snapshot.schemaVersion.value() > kArchiveVersion)
        return eve::Result<eve::pixelworld::PixelChunkBatch>::failure(eve::Diagnostic::error(
        eve::DiagnosticCode::UnknownVersion, "unsupported Chunk archive version", "schemaVersion", {}, "pixelworld.chunk-archive"));
    ...
    if (!catalog || !seed || !sequence || !records || records->size() > kMaximumArchiveChunks)
        return eve::Result<eve::pixelworld::PixelChunkBatch>::failure(eve::Diagnostic::error(
        eve::DiagnosticCode::ParseError, "invalid Chunk archive payload", "payload", {}, "pixelworld.chunk-archive"));
    ...
    if (!versionOne && !versionTwo) {
        ...
        if (!fullResync)
            return eve::Result<eve::pixelworld::PixelChunkBatch>::failure(eve::Diagnostic::error(
        eve::DiagnosticCode::ParseError, "invalid fullResync flag", "payload.fullResync", {}, "pixelworld.chunk-archive"));
    ...
    for (std::size_t index = 0; index < records->size(); ++index) {
        ...
        if (!x || !y || !revision || !decodedSize || !removed || !codecValue || !hashText ||
            ...
            return eve::Result<eve::pixelworld::PixelChunkBatch>::failure(eve::Diagnostic::error(
        eve::DiagnosticCode::ParseError, "invalid Chunk archive record", path, {}, "pixelworld.chunk-archive"));
        const std::pair<int, int> key{y.value(), x.value()};
        if (previous && !(previous.value() < key))
            return eve::Result<eve::pixelworld::PixelChunkBatch>::failure(eve::Diagnostic::error(
        eve::DiagnosticCode::ParseError, "Chunk records are not canonical", path, {}, "pixelworld.chunk-archive"));
        previous = key;
        ...
        if ((!*removed && !currentCells && !legacyCells) || (*removed && decodedSize.value() != 0) ||
            ...
            return eve::Result<eve::pixelworld::PixelChunkBatch>::failure(eve::Diagnostic::error(
        eve::DiagnosticCode::ParseError, "Chunk decoded size exceeds budget", path, {}, "pixelworld.chunk-archive"));
        ...
        if (*removed && (codec != PixelChunkArchiveCodec::None || !stored.value().empty()))
            return eve::Result<eve::pixelworld::PixelChunkBatch>::failure(eve::Diagnostic::error(
        eve::DiagnosticCode::ParseError, "removed Chunk must have empty uncompressed payload", path, {}, "pixelworld.chunk-archive"));
        ...
        if (!expectedHash || !actualHash || actualHash.value() != *expectedHash)
            return eve::Result<eve::pixelworld::PixelChunkBatch>::failure(eve::Diagnostic::error(
        eve::DiagnosticCode::HashMismatch, "decoded Chunk content hash does not match", path, {}, "pixelworld.chunk-archive"));
