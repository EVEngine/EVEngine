...
void* UiDocumentTarget::queryCapability(const CapabilityId& capability) {
    if (capability == IUiDocumentEditTarget::editorCapabilityId()) return static_cast<IUiDocumentEditTarget*>(this);
    if (capability == CapabilityId("eve.editor.target.ui-properties")) return static_cast<IPropertyProvider*>(this);
    return nullptr;
}
...
EditorResult<DomainOperation> UiDocumentTarget::makeSetStyle(const ObjectId& id, const UiStyleValue& style) const {
    ...
    if (style.marginLeft < 0.0 || style.marginTop < 0.0 || style.marginRight < 0.0 || style.marginBottom < 0.0 ||
        style.paddingLeft < 0.0 || style.paddingTop < 0.0 || style.paddingRight < 0.0 || style.paddingBottom < 0.0 ||
        style.cornerRadius < 0.0 || style.gap < 0.0 || style.flexGrow < 0.0 || style.tintR < 0.0 || style.tintR > 1.0 ||
...
EditorResult<DomainOperation> UiDocumentTarget::makeSetContent(const ObjectId&       id,
    ...
    if (!std::isfinite(content.fontSize) || content.fontSize <= 0.0 || content.fontSize > 512.0 ||
        std::any_of(std::begin(colors), std::end(colors),
...
std::vector<EditorDiagnostic> UiDocumentTarget::validate() const {
    ...
    for (const auto& [id, value] : widgets_) {
        ...
        if (value.layout.width == 0.0 || value.layout.height == 0.0)
            diagnostics.push_back(uiDiagnostic("editor.ui.zero-size-widget", DiagnosticSeverity::Warning,
        ...
        if (value.style.cornerRadius * 2.0 > std::min(value.layout.width, value.layout.height) &&
            ...
            diagnostics.push_back(uiDiagnostic("editor.ui.excessive-corner-radius", DiagnosticSeverity::Warning,
                                               "Corner radius exceeds half of widget size: " + id.value()));
        if (!value.text.empty() && value.content.fontAsset.empty())
            diagnostics.push_back(uiDiagnostic("editor.ui.missing-font", DiagnosticSeverity::Warning,
                                               "Text widget has no font asset: " + id.value()));
    }
    if (!widgets_.empty() && roots == 0)
        diagnostics.push_back(uiDiagnostic("editor.ui.missing-root", DiagnosticSeverity::Error,
                                           "UI document has no root widget"));
    if (roots > 1)
        diagnostics.push_back(uiDiagnostic("editor.ui.multiple-roots", DiagnosticSeverity::Warning,
