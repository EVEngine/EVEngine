...
UiPreviewSnapshot UiDocumentPreviewService::build(const UiDocumentTarget& document, double width, double height) const {
    ...
    if (!std::isfinite(width) || !std::isfinite(height) || width <= 0.0 || height <= 0.0) {
        result.diagnostics.push_back(eve::editing::ruleDiagnostic(
            eve::DiagnosticCode::InvalidArgument, RuleId("editor.ui.invalid-preview-viewport"),
            DiagnosticSeverity::Error, "UI preview viewport must have finite positive dimensions"));
        return result;
    }
    ...
    visit = [&](const ObjectId& id, const UiPreviewWidget& parent, int depth) {
        ...
        const double            contentX = parent.x + widget.style.marginLeft + (depth == 0 ? 0.0 : 0.0);
        const double            contentY = parent.y + widget.style.marginTop;
        const double parentWidth  = std::max(0.0, parent.width - widget.style.marginLeft - widget.style.marginRight);
        const double parentHeight = std::max(0.0, parent.height - widget.style.marginTop - widget.style.marginBottom);
        UiPreviewWidget preview;
        preview.id     = widget.id;
        preview.parent = widget.parent;
        preview.depth  = depth;
        preview.width  = widget.layout.width;
        preview.height = widget.layout.height;
        preview.x =
            contentX + parentWidth * widget.layout.anchorX + widget.layout.x - preview.width * widget.layout.pivotX;
        preview.y =
            contentY + parentHeight * widget.layout.anchorY + widget.layout.y - preview.height * widget.layout.pivotY;
        preview.visible = widget.visible && parent.visible;
        computed[id]    = preview;
        result.widgets.push_back(preview);
        UiPreviewWidget childParent = preview;
        childParent.x += widget.style.paddingLeft;
        childParent.y += widget.style.paddingTop;
...
EditorResult<ObjectId> UiDocumentPreviewService::pick(const UiPreviewSnapshot& preview, double x, double y) const {
    ...
    for (const UiPreviewWidget& widget : preview.widgets) {
        if (!widget.visible || x < widget.x || y < widget.y || x > widget.x + widget.width ||
            ...
            continue;
        if (!best || widget.depth > best->depth || (widget.depth == best->depth && widget.id > best->id))
            best = &widget;
    }
    if (!best)
        return eve::editing::failed<ObjectId>(EditorStatus::NotFound, RuleId("editor.ui.pick-miss"),
                                              "No visible widget contains the preview point");
...
EditorResult<EditorValue> UiDocumentPreviewService::anchorGizmo(const UiDocumentTarget&  document,
    ...
    if (preview.documentRevision != document.revision())
        return eve::editing::failed<EditorValue>(EditorStatus::Conflict, RuleId("editor.ui.stale-preview"),
                                                 "UI preview revision no longer matches the document");
    auto widget = document.widget(id);
    if (!widget.ok())
        return eve::editing::failed<EditorValue>(EditorStatus::NotFound, RuleId("editor.ui.widget-not-found"),
    ...
    if (!widget.value().parent.empty()) {
        const auto parent =
            std::find_if(preview.widgets.begin(), preview.widgets.end(),
                         [&](const UiPreviewWidget& value) { return value.id == widget.value().parent; });
        if (parent == preview.widgets.end())
            return eve::editing::failed<EditorValue>(EditorStatus::NotFound,
                                                     RuleId("editor.ui.preview-parent-not-found"),
                                                     "Parent is absent from UI preview");
        ...
        parentW           = std::max(
            0.0, parent->width - parentWidget.value().style.paddingLeft - parentWidget.value().style.paddingRight);
        parentH = std::max(
            0.0, parent->height - parentWidget.value().style.paddingTop - parentWidget.value().style.paddingBottom);
    }
    ...
    if (previewWidget == preview.widgets.end())
        return eve::editing::failed<EditorValue>(EditorStatus::NotFound, RuleId("editor.ui.preview-widget-not-found"),
                                                 "Widget is absent from UI preview");
    ...
    return eve::editing::applied<EditorValue>(
        EditorValue::Object{{"anchor", EditorValue::Array{anchorX, anchorY}},
