#include "audio/editing/AudioEffects.h"
...
EditorValue effectValue(const AudioEffectRecord& effect) {
    return EditorValue::Object{{"id", effect.id.value()},
                               {"type", effect.type},
                               {"bypass", effect.bypass},
                               {"mix", effect.mix},
                               {"parameters", effect.parameters}};
}
bool numberIn(const EditorValue::Object& parameters, const char* key, double minimum, double maximum) {
...
EditorResult<AudioEffectRecord> parseEffect(const EditorValue& value) {
    ...
    const auto* type       = typeEntry ? typeEntry->getIf<std::string>() : nullptr;
    const auto* bypass     = bypassEntry ? bypassEntry->getIf<bool>() : nullptr;
    const auto* mix        = mixEntry ? mixEntry->getIf<double>() : nullptr;
    const auto* parameters = parametersEntry ? parametersEntry->getIf<EditorValue::Object>() : nullptr;
    static const std::set<std::string> types{"gain", "low-pass", "high-pass", "delay", "reverb", "compressor"};
    bool valid = id && !id->empty() && type && types.contains(*type) && bypass && mix && std::isfinite(*mix) &&
                 *mix >= 0.0 && *mix <= 1.0 && parameters;
    if (valid && *type == "gain")
        valid = numberIn(*parameters, "decibels", -96.0, 24.0);
    else if (valid && (*type == "low-pass" || *type == "high-pass"))
        valid = numberIn(*parameters, "cutoff", 20.0, 24000.0) && numberIn(*parameters, "resonance", 0.1, 20.0);
    else if (valid && *type == "delay")
        valid = numberIn(*parameters, "seconds", 0.001, 5.0) && numberIn(*parameters, "feedback", 0.0, 0.99);
    else if (valid && *type == "reverb")
        valid = numberIn(*parameters, "room-size", 0.0, 1.0) && numberIn(*parameters, "damping", 0.0, 1.0);
...
DomainOperation operation(const char* type, const char* inverse, const std::string& target, EditorValue payload,
                          EditorValue undo, const StableId& id = {}) {
    DomainOperation result;
    result.type        = type;
    result.inverseType = inverse;
    result.target      = TargetId(target);
    result.payload     = std::move(payload);
    result.inverse     = std::move(undo);
    result.hasInverse  = true;
    if (!id.empty()) result.affectedObjects.push_back({TargetId(target), id.value(), 0});
    return result;
}
}  // namespace
...
std::vector<AudioEffectRecord> AudioEffectChainTarget::effects() const {
    std::vector<AudioEffectRecord> result;
...
std::vector<EditorDiagnostic> AudioEffectChainTarget::validate() const {
    std::vector<EditorDiagnostic> result;
    if (order_.size() > 16)
        result.push_back(eve::editing::ruleDiagnostic(
            eve::DiagnosticCode::PreconditionViolation, RuleId("editor.audio.effect-budget"),
            DiagnosticSeverity::Error, "Effect chain exceeds the 16-stage safety budget"));
    int reverbs = 0;
    for (const auto& id : order_)
        if (effects_.at(id).type == "reverb") ++reverbs;
    if (reverbs > 2)
        result.push_back(eve::editing::ruleDiagnostic(
            eve::DiagnosticCode::PreconditionViolation, RuleId("editor.audio.reverb-budget"),
            DiagnosticSeverity::Warning, "More than two reverbs may be expensive"));
    return result;
}
EditorValue AudioEffectChainTarget::snapshotValue() const {
    EditorValue::Array effects;
