...
EditorValue layerValue(const MapLayerRecord& layer) {
    return EditorValue::Object{{"id", layer.id.value()}, {"name", layer.name}, {"kind", layer.kind},
                               ...
                               {"opacity", layer.opacity}, {"order", int64_t{layer.order}}};
}
...
EditorResult<MapLayerRecord> parseLayer(const EditorValue& value) {
    ...
    return EditorResult<MapLayerRecord>::applied(
...
EditorResult<MapSplinePointRecord> parsePoint(const EditorValue& value) {
    ...
    if (!id || id->empty() || !x || !y || !z || !width || *width <= 0.0 || !std::isfinite(*x) ||
        !std::isfinite(*y) || !std::isfinite(*z) || !std::isfinite(*width))
    ...
    return EditorResult<MapSplinePointRecord>::applied({StableId(*id), *x, *y, *z, *width});
}
...
EditorValue roadValue(const MapRoadRecord& road) {
    ...
    return EditorValue::Object{{"id", road.id.value()}, {"layer", road.layer.value()}, {"name", road.name},
                               {"materialAsset", road.materialAsset}, {"closed", road.closed},
...
EditorResult<MapRoadRecord> parseRoad(const EditorValue& value) {
    ...
    if (!id || id->empty() || !layer || layer->empty() || !name || name->empty() || !material || !closed ||
    ...
    for (const EditorValue& pointValueEntry : *points) {
        ...
        if (!point.value || !pointIds.insert(point.value->id).second)
    ...
    return EditorResult<MapRoadRecord>::applied(std::move(result));
}

EditorValue placementValue(const MapPlacementRecord& placement) {
    return EditorValue::Object{{"id", placement.id.value()}, {"layer", placement.layer.value()},
                               {"asset", placement.asset}, {"x", placement.x}, {"y", placement.y},
                               {"z", placement.z}, {"rotationX", placement.rotationX},
                               {"rotationY", placement.rotationY}, {"rotationZ", placement.rotationZ},
...
EditorResult<MapPlacementRecord> parsePlacement(const EditorValue& value) {
    ...
    if (!id || id->empty() || !layer || layer->empty() || !asset || asset->empty() || !x || !y || !z || !rx ||
        !ry || !rz || !sx || !sy || !sz || *sx <= 0.0 || *sy <= 0.0 || *sz <= 0.0)
    ...
    return EditorResult<MapPlacementRecord>::applied({StableId(*id), StableId(*layer), *asset, *x, *y, *z,
                                                       *rx, *ry, *rz, *sx, *sy, *sz});
}

DomainOperation operation(std::string type, std::string inverseType, const std::string& target,
...
void* MapDocumentTarget::queryCapability(const CapabilityId& capability) {
    return capability == IMapStructureEditTarget::editorCapabilityId()
               ? static_cast<IMapStructureEditTarget*>(this)
               : nullptr;
}
...
EditorResult<void> MapDocumentTarget::applyDomainOperation(const DomainOperation& domainOperation) {
    ...
    if (domainOperation.type == "map.layer.create.v1") {
        ...
        if (!layer.value) return mapError<void>(layer.status, "editor.map.invalid-layer", "Layer payload is invalid");
        ...
        layers_.emplace(layer.value->id, std::move(*layer.value));
    } else if (domainOperation.type == "map.layer.delete.v1") {
        auto layer = parseLayer(domainOperation.payload);
        if (!layer.value || !layers_.contains(layer.value->id))
        ...
        layers_[layer.value->id] = std::move(*layer.value);
    } else if (domainOperation.type == "map.road.set.v1") {
        auto road = parseRoad(domainOperation.payload);
        if (!road.value) return mapError<void>(road.status, "editor.map.invalid-road", "Road payload is invalid");
        ...
        roads_[road.value->id] = std::move(*road.value);
    } else if (domainOperation.type == "map.road.delete.v1") {
        auto road = parseRoad(domainOperation.payload);
        if (!road.value || !roads_.contains(road.value->id))
        ...
        if (!placement.value)
            return mapError<void>(placement.status, "editor.map.invalid-placement", "Placement payload is invalid");
        ...
        placements_[placement.value->id] = std::move(*placement.value);
    } else if (domainOperation.type == "map.placement.delete.v1") {
        auto placement = parsePlacement(domainOperation.payload);
        if (!placement.value || !placements_.contains(placement.value->id))
            return mapError<void>(EditorStatus::NotFound, "editor.map.placement-not-found",
...
EditorResult<DomainOperation> MapDocumentTarget::makeCreateLayer(const MapLayerRecord& layer) const {
    ...
    if (!parsed.value) return mapError<DomainOperation>(parsed.status, "editor.map.invalid-layer", "Layer is invalid");
    ...
    return EditorResult<DomainOperation>::applied(operation("map.layer.create.v1", "map.layer.delete.v1", id_,
                                                            layerValue(layer), layerValue(layer), layer.id));
}
...
EditorResult<DomainOperation> MapDocumentTarget::makeDeleteLayer(const StableId& layer) const {
    ...
    if (found == layers_.end())
        return mapError<DomainOperation>(EditorStatus::NotFound, "editor.map.layer-not-found", "Map layer was not found");
    ...
    return EditorResult<DomainOperation>::applied(operation("map.layer.delete.v1", "map.layer.create.v1", id_,
                                                            layerValue(found->second), layerValue(found->second), layer));
}
...
EditorResult<DomainOperation> MapDocumentTarget::makeSetLayer(const MapLayerRecord& layer) const {
    ...
    return EditorResult<DomainOperation>::applied(operation("map.layer.set.v1", "map.layer.set.v1", id_,
                                                            layerValue(layer), layerValue(found->second), layer.id));
}
...
EditorResult<DomainOperation> MapDocumentTarget::makeSetRoad(const MapRoadRecord& road) const {
    ...
    if (!parsed.value) return mapError<DomainOperation>(parsed.status, "editor.map.invalid-road", "Road is invalid");
    ...
    return EditorResult<DomainOperation>::applied(operation("map.road.set.v1", inverseType, id_, roadValue(road),
                                                            inverse, road.id));
}
...
EditorResult<DomainOperation> MapDocumentTarget::makeDeleteRoad(const StableId& road) const {
    ...
    if (layers_.at(found->second.layer).locked)
        return mapError<DomainOperation>(EditorStatus::Rejected, "editor.map.layer-locked", "Road layer is locked");
    return EditorResult<DomainOperation>::applied(operation("map.road.delete.v1", "map.road.set.v1", id_,
...
EditorResult<DomainOperation> MapDocumentTarget::makeSetPlacement(const MapPlacementRecord& placement) const {
    ...
    if (!parsed.value)
        return mapError<DomainOperation>(parsed.status, "editor.map.invalid-placement", "Placement is invalid");
    ...
    return EditorResult<DomainOperation>::applied(operation("map.placement.set.v1", inverseType, id_,
                                                            placementValue(placement), inverse, placement.id));
}
...
EditorResult<DomainOperation> MapDocumentTarget::makeDeletePlacement(const StableId& placement) const {
    ...
    if (layers_.at(found->second.layer).locked)
        return mapError<DomainOperation>(EditorStatus::Rejected, "editor.map.layer-locked", "Object layer is locked");
    return EditorResult<DomainOperation>::applied(operation("map.placement.delete.v1", "map.placement.set.v1", id_,
                                                            placementValue(found->second), placementValue(found->second),
                                                            placement));
}
...
std::vector<EditorDiagnostic> MapDocumentTarget::validate() const {
    ...
    for (const auto& [id, layer] : layers_) {
        ...
        if (!orders.insert(layer.order).second)
            diagnostics.push_back({RuleId("editor.map.duplicate-layer-order"), DiagnosticSeverity::Error,
                                   "Multiple map layers share visual order " + std::to_string(layer.order)});
    }
    for (const auto& [id, road] : roads_) {
        const auto layer = layers_.find(road.layer);
        if (layer == layers_.end() || layer->second.kind != "road")
            diagnostics.push_back({RuleId("editor.map.invalid-road-layer"), DiagnosticSeverity::Error,
                                   "Road references a missing or non-road layer: " + id.value()});
        ...
        if (lengthSquared <= 1e-12)
            diagnostics.push_back({RuleId("editor.map.zero-length-road"), DiagnosticSeverity::Error,
                                   "Road has no geometric length: " + id.value()});
        if (road.materialAsset.empty())
            diagnostics.push_back({RuleId("editor.map.road-material-missing"), DiagnosticSeverity::Warning,
                                   "Road has no material asset: " + id.value()});
    }
    for (const auto& [id, placement] : placements_) {
        const auto layer = layers_.find(placement.layer);
        if (layer == layers_.end() || layer->second.kind != "object")
            diagnostics.push_back({RuleId("editor.map.invalid-placement-layer"), DiagnosticSeverity::Error,
                                   "Placement references a missing or non-object layer: " + id.value()});
    }
    return diagnostics;
}
...
MapRoadPreviewResult MapDocumentTarget::previewRoad(const StableId& roadId, int triangleBudget) const {
    ...
    if (found == roads_.end()) {
        ...
        result.diagnostics.push_back({RuleId("editor.map.road-not-found"), DiagnosticSeverity::Error,
                                      "Road preview target was not found"});
        return result;
    }
    if (triangleBudget <= 0) {
        result.status = EditorStatus::Rejected;
        result.diagnostics.push_back({RuleId("editor.map.invalid-preview-budget"), DiagnosticSeverity::Error,
                                      "Road preview triangle budget must be positive"});
        return result;
    }
    ...
    if (triangleCount > triangleBudget) {
        result.status = EditorStatus::Rejected;
        result.diagnostics.push_back({RuleId("editor.map.road-preview-budget"), DiagnosticSeverity::Error,
                                      "Road strip exceeds preview triangle budget"});
        return result;
    }
    ...
    for (size_t index = 0; index < road.points.size(); ++index) {
        ...
        const MapSplinePointRecord& previous = road.points[index == 0 ? (road.closed ? road.points.size() - 1 : 0)
                                                                     : index - 1];
        const MapSplinePointRecord& next = road.points[index + 1 == road.points.size()
                                                           ? (road.closed ? 0 : road.points.size() - 1)
                                                           : index + 1];
        ...
        if (length <= 1e-9) {
            ...
            result.diagnostics.push_back({RuleId("editor.map.degenerate-road-tangent"),
        ...
        vertices.emplace_back(EditorValue::Array{point.x - tangentZ * halfWidth, point.y,
                                                  point.z + tangentX * halfWidth});
        vertices.emplace_back(EditorValue::Array{point.x + tangentZ * halfWidth, point.y,
                                                  point.z - tangentX * halfWidth});
        if (index > 0) {
    ...
    result.mesh = EditorValue::Object{{"primitive", "triangles"}, {"vertices", std::move(vertices)},
...
EditorValue MapDocumentTarget::snapshotValue() const {
    ...
    return EditorValue::Object{{"schemaVersion", int64_t{1}}, {"layers", std::move(layers)},
                               {"roads", std::move(roads)}, {"placements", std::move(placements)}};
}
