...
EditorResult<void> MapRoadMeshPublisher::publish(const MapDocumentTarget& document, const StableId& road,
    ...
    if (expectedRevision != document.revision())
        return eve::editing::failed<void>(EditorStatus::Conflict, RuleId("editor.map.stale-road-preview"),
    ...
    if (preview.status != EditorStatus::Applied)
        return eve::editing::failed<void>(
            preview.status,
            preview.diagnostics.empty() ? RuleId("editor.map.road-preview-failed")
                                        : eve::editing::diagnosticRule(preview.diagnostics.front()),
            preview.diagnostics.empty() ? "Road preview failed" : preview.diagnostics.front().message());
    auto published = sink.publishRoad(document.targetId().value(), road, preview.documentRevision, preview.mesh);
    if (!published.ok()) return published;
    if (document.revision() != expectedRevision)
        return eve::editing::failed<void>(EditorStatus::Conflict, RuleId("editor.map.road-changed-during-publish"),
                                          "Map road changed while its generated mesh was being published");
    return eve::editing::applied<void>();
}

MapObjectImportPlan MapObjectImporter::plan(const MapDocumentTarget& document, const StableId& layer,
    ...
    if (layer.empty() || idPrefix.empty()) {
        ...
        result.diagnostics.push_back(eve::editing::ruleDiagnostic(
            eve::DiagnosticCode::InvalidArgument, RuleId("editor.map.invalid-import-target"),
            DiagnosticSeverity::Error, "Map object import requires a target layer and stable id prefix"));
        return result;
    }
    ...
    for (const auto& record : records) {
        if (record.sourceId.empty() || record.asset.empty() || !sourceIds.insert(record.sourceId).second) {
            ...
            result.diagnostics.push_back(eve::editing::ruleDiagnostic(
                eve::DiagnosticCode::Conflict, RuleId("editor.map.invalid-import-object"), DiagnosticSeverity::Error,
        ...
        if (existing.contains(id)) {
            result.status = EditorStatus::Conflict;
            result.diagnostics.push_back(eve::editing::ruleDiagnostic(
                eve::DiagnosticCode::Conflict, RuleId("editor.map.import-id-conflict"), DiagnosticSeverity::Error,
                "Imported placement id already exists: " + id.value()));
            return result;
        }
        MapPlacementRecord placement{id,
                                     layer,
                                     record.asset,
                                     record.x,
                                     record.y,
        ...
        if (!operation.ok()) {
            result.status = operation.code();
            result.diagnostics.insert(result.diagnostics.end(), operation.diagnostics().begin(),
