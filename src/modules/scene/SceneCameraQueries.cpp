...
std::string Scene::pickScreenAt(const std::string &hostName, graphics::Camera3D *cam,
    ...
    if (!ray) return {};
    return pickRayAt(hostName, ray->origin.x, ray->origin.y, ray->origin.z,
...
Result<int> Scene::applyPcgTerrainCullingAt(const std::string &hostName,
                                            graphics::Camera3D *cam, float viewW,
                                            float viewH, const std::string &tag) {
    SceneHost *host = resolveHost(hostName);
    if (!host || !cam || !std::isfinite(viewW) || !std::isfinite(viewH) || viewW <= 0.f ||
        viewH <= 0.f || tag.empty())
        return Result<int>::failure(Diagnostic::error(
            DiagnosticCode::InvalidArgument,
            "Pcg terrain culling requires host, camera, positive viewport and tag",
            "scene.pcgTerrainCulling"));
    auto *projection = eve::cap::query<ISceneCameraProjection>();
    if (!projection)
        return Result<int>::failure(Diagnostic::error(
            DiagnosticCode::Unsupported,
            "Pcg terrain culling needs the graphics camera projection capability",
            "scene.pcgTerrainCulling"));
    const auto clip = projection->clipForViewport(*cam, viewW, viewH);
    if (!clip)
        return Result<int>::failure(Diagnostic::error(
    ...
    host->walkDepthFirst([&](SceneHost *, int, SceneNode &node) {
        if (!node.hasBounds ||
            std::find(node.tags.begin(), node.tags.end(), tag) == node.tags.end())
            return;
        const bool visible =
            aabbIntersectsFrustum(clip->clip, clip->inverse, worldBoundsOf(node));
