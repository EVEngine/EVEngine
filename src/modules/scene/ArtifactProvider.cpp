...
bool readBounds(const eve::Value::Object& object, eve::artifact::Bounds& bounds) {
    ...
    return value && readFloat(*value, "minX", bounds.minX) &&
           readFloat(*value, "minY", bounds.minY) && readFloat(*value, "minZ", bounds.minZ) &&
           readFloat(*value, "maxX", bounds.maxX) && readFloat(*value, "maxY", bounds.maxY) &&
           readFloat(*value, "maxZ", bounds.maxZ) &&
...
eve::Result<std::unique_ptr<eve::artifact::PreparedPublication>> SceneArtifactProvider::prepare(
    ...
    for (const auto& part : publication.parts) {
        if (part.id.isNil() || part.role.empty() || !validBounds(part.bounds))
            return eve::Result<std::unique_ptr<eve::artifact::PreparedPublication>>::failure(
                eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument,
                                       "scene artifact part has invalid identity, role or bounds"));
    ...
    try {
        ...
        return eve::Result<std::unique_ptr<eve::artifact::PreparedPublication>>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::Failed,
                                   "scene artifact prepare allocation failed"));
    }
}
...
eve::Result<void> SceneArtifactProvider::restoreState(const eve::Value& state) {
    if (!records_.empty())
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::Conflict, "scene artifact restore requires an empty registry"));
    ...
    if (!object || !providerName || *providerName != "scene.cpu-registry" ||
        !hasSupportedVersion(*object) || !array)
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::ParseError, "invalid scene artifact provider state"));
    ...
    try {
        ...
        for (const eve::Value& encoded : *array) {
            ...
            if (!item || !readString(*item, "artifactId", idText) ||
                !readString(*item, "buildKey", buildKey) ||
                !readUint64String(*item, "handle", packed))
                return eve::Result<void>::failure(eve::Diagnostic::error(
                    eve::DiagnosticCode::ParseError, "invalid scene artifact record"));
            ...
            if (!parsedId || parsedId->isNil() || handle.isInvalid() || !nodes ||
                ...
                return eve::Result<void>::failure(eve::Diagnostic::error(
                    eve::DiagnosticCode::ParseError, "invalid scene artifact identity or handle"));
            ...
            for (const eve::Value& encodedNode : *nodes) {
                ...
                if (!node || !readString(*node, "role", role) || !boundsObject)
                    return eve::Result<void>::failure(eve::Diagnostic::error(
                        eve::DiagnosticCode::ParseError, "invalid scene artifact node"));
                eve::artifact::Bounds bounds;
                if (!readBounds(*node, bounds))
                    return eve::Result<void>::failure(eve::Diagnostic::error(
                        eve::DiagnosticCode::ParseError, "invalid scene artifact node bounds"));
                record.nodes.push_back({std::move(role), bounds});
            }
            next = std::max(next, handle.index() == std::numeric_limits<std::uint32_t>::max()
                                      ? handle.index()
                                      : handle.index() + 1u);
        ...
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::Failed, "scene artifact restore allocation failed"));
    }
