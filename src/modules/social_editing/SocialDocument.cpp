...
EditorResult<SocialEntityRecord> parseEntity(const EditorValue& value) {
    ...
    const auto* id       = i ? i->getIf<std::string>() : nullptr;
...
EditorResult<SocialEdgeRecord> parseEdge(const EditorValue& value) {
    ...
    const auto* w      = field(value, "weight");
    const auto* weight = w ? w->getIf<double>() : nullptr;
    const auto *id = str("id"), *source = str("source"), *target = str("target"), *kind = str("kind"),
               *type = str("type");
    static const std::set<std::string> kinds{"owner", "controller", "assignment", "relation"};
...
DomainOperation op(const char* t, const char* inverse, const std::string& target, EditorValue payload, EditorValue undo,
                   const StableId& id) {
    DomainOperation r;
    r.type        = t;
    r.inverseType = inverse;
    r.target      = TargetId(target);
    r.payload     = std::move(payload);
    r.inverse     = std::move(undo);
    r.hasInverse  = true;
    r.affectedObjects.push_back({TargetId(target), id.value(), 0});
    return r;
}
template <class T>
std::vector<T> values(const std::map<StableId, T>& m) {
    std::vector<T> r;
...
std::vector<EditorDiagnostic> SocialDocumentTarget::validate() const {
    std::vector<EditorDiagnostic>                                      r;
    std::set<std::tuple<StableId, StableId, std::string, std::string>> unique;
    std::map<StableId, int>                                            owners, controllers;
    for (const auto& [id, e] : edges_) {
        if (!entities_.contains(e.source) || !entities_.contains(e.target))
            r.push_back(eve::editing::ruleDiagnostic(
                eve::DiagnosticCode::NotFound, RuleId("editor.social.missing-endpoint"),
                DiagnosticSeverity::Error, "Edge " + id.value() + " has a missing endpoint"));
        if (!unique.emplace(e.source, e.target, e.kind, e.type).second)
            r.push_back(eve::editing::ruleDiagnostic(
                eve::DiagnosticCode::Conflict, RuleId("editor.social.duplicate-edge"),
                DiagnosticSeverity::Error, "Duplicate social edge semantics"));
        if (e.kind == "owner" && ++owners[e.source] > 1)
            r.push_back(eve::editing::ruleDiagnostic(
                eve::DiagnosticCode::Conflict, RuleId("editor.social.multiple-owners"),
                DiagnosticSeverity::Error, "Entity has multiple owners"));
        if (e.kind == "controller" && ++controllers[e.source] > 1)
            r.push_back(eve::editing::ruleDiagnostic(
                eve::DiagnosticCode::Conflict, RuleId("editor.social.multiple-controllers"),
                DiagnosticSeverity::Error, "Entity has multiple controllers"));
    }
    return r;
}
EditorValue SocialDocumentTarget::snapshotValue() const {
    EditorValue::Array entities, edges;
    for (const auto& [id, v] : entities_) {
        (void)id;
        entities.push_back(entityValue(v));
    }
    for (const auto& [id, v] : edges_) {
        (void)id;
        edges.push_back(edgeValue(v));
    }
    return EditorValue::Object{
