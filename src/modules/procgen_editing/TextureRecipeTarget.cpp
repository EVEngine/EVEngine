...
EditorValue defaultValue(const procgen::ParamDescriptor& p) {
    try {
        switch (p.kind) {
            case procgen::ParamKind::Integer: return int64_t{std::stoll(p.defaultValue)};
            case procgen::ParamKind::Float: return std::stod(p.defaultValue);
            case procgen::ParamKind::Boolean: return p.defaultValue == "1" || p.defaultValue == "true";
            case procgen::ParamKind::String:
            case procgen::ParamKind::Choice: return p.defaultValue;
        }
    } catch (...) {
        return {};
    }
...
PropertyType propertyType(procgen::ParamKind k) {
    switch (k) {
        case procgen::ParamKind::Integer: return PropertyType::Int;
        case procgen::ParamKind::Float: return PropertyType::Float;
        case procgen::ParamKind::Boolean: return PropertyType::Bool;
        case procgen::ParamKind::Choice: return PropertyType::Enum;
        case procgen::ParamKind::String: return PropertyType::String;
    }
    return PropertyType::String;
}
bool hasErrors(const std::vector<EditorDiagnostic>& d) {
    for (const auto& x : d)
        if (x.severity() == DiagnosticSeverity::Error) return true;
    return false;
}
}  // namespace
TextureRecipeTarget::TextureRecipeTarget(std::string id, std::string recipe)
...
void* TextureRecipeTarget::queryCapability(const CapabilityId& c) {
    return c == CapabilityId("eve.editor.target.texture-recipe-properties") ? static_cast<IPropertyProvider*>(this)
                                                                            : nullptr;
}
bool TextureRecipeTarget::matches(const SelectionSnapshot& s) const {
    return s.items.size() == 1 && s.items[0].target == TargetId(id_) && s.items[0].type == "texture-recipe";
}
eve::Result<eve::Revision> TextureRecipeTarget::currentRevision(const SelectionSnapshot& s) const {
    if (!matches(s))
...
PropertySchema TextureRecipeTarget::schema(const SelectionSnapshot&) const {
    PropertySchema out;
    out.typeId    = "texture-recipe";
    auto&       r = procgen::TextureRecipeRegistry::instance();
    const auto* d = r.descriptor(recipe_);
    if (!d) return out;
    for (const auto& p : d->params) {
        PropertyDescriptor x;
        x.path           = PropertyPath("param." + p.key);
        x.displayNameKey = p.displayName;
        x.descriptionKey = p.description;
        x.category       = p.category;
        x.type           = propertyType(p.kind);
        x.defaultValue   = defaultValue(p);
        x.flags          = PropertyFlag::Runtime;
        x.enumItems      = p.choices;
        if (p.hasMinimum) x.numeric.minimum = p.minimum;
        if (p.hasMaximum) x.numeric.maximum = p.maximum;
        x.numeric.step = p.step;
        out.properties.push_back(std::move(x));
    }
    return out;
}
PropertyReadResult TextureRecipeTarget::read(const SelectionSnapshot& s, const PropertyPath& p) const {
    if (!matches(s) || p.value().rfind("param.", 0) != 0) return {};
    auto i = values_.find(p.value().substr(6));
    return i == values_.end() ? PropertyReadResult{} : PropertyReadResult{PropertyReadState::Value, i->second, {}};
}
...
EditorResult<DomainOperation> TextureRecipeTarget::makeSet(const SelectionSnapshot& s,
    ...
    op.payload     = EditorValue::Object{{"recipe", recipe_}, {"values", std::move(values)}};
...
std::vector<EditorDiagnostic> TextureRecipeTarget::validate() const {
    ...
    auto&                         r    = procgen::TextureRecipeRegistry::instance();
    const auto*                   desc = r.descriptor(recipe_);
    if (!desc) {
        d.push_back(diagnostic("editor.texture-recipe.unknown", DiagnosticSeverity::Error,
                               "Texture recipe is not registered"));
        return d;
    }
    const auto s = schema({});
    for (const auto& p : desc->params) {
        auto i = values_.find(p.key);
        auto x = s.find(PropertyPath("param." + p.key));
        if (i == values_.end() || !x || !validatePropertyValue(*x, i->second).ok())
            d.push_back(diagnostic("editor.texture-recipe.parameter", DiagnosticSeverity::Error,
                                   "Invalid texture parameter: " + p.key));
    }
    if (values_.size() != desc->params.size())
        d.push_back(diagnostic("editor.texture-recipe.extra", DiagnosticSeverity::Error,
...
EditorResult<TextureRecipePreviewArtifact> TextureRecipePreviewRuntime::generate(
    ...
    for (const auto& [key, v] : target.values()) {
        if (auto* x = v.getIf<int64_t>())
            params.setInt(key, static_cast<int>(*x));
        else if (auto* x = v.getIf<double>())
            params.setFloat(key, static_cast<float>(*x));
        else if (auto* x = v.getIf<bool>())
            params.setBool(key, *x);
        else if (auto* x = v.getIf<std::string>())
            params.setString(key, *x);
    }
    std::string error;
    ...
    TextureRecipePreviewArtifact artifact{target.revision(), candidate->getWidth(), candidate->getHeight(), hash};
    image_             = std::move(candidate);
    revision_          = target.revision();
    return eve::editing::applied<TextureRecipePreviewArtifact>(artifact, diagnostics);
}
}  // namespace eve::procgen_editing
