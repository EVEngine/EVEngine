...
SpatialBounds intersectionBounds(const SpatialBounds& a, const SpatialBounds& b) {
    ...
    SpatialBounds result{std::max(a.minX, b.minX),
                         std::max(a.minY, b.minY),
                         std::max(a.minZ, b.minZ),
                         std::min(a.maxX, b.maxX),
                         std::min(a.maxY, b.maxY),
                         std::min(a.maxZ, b.maxZ),
                         true};
    if (result.minX > result.maxX || result.minY > result.maxY || result.minZ > result.maxZ) return {};
    return result;
}

float pointSegmentDistanceSquared(float px, float py, float pz, const ProcgenPoint& a, const ProcgenPoint& b) {
    const float vx = b.x - a.x;
    const float vy = b.y - a.y;
    const float vz = b.z - a.z;
    const float l2 = vx * vx + vy * vy + vz * vz;
    const float t  = l2 > 0.f ? std::clamp(((px - a.x) * vx + (py - a.y) * vy + (pz - a.z) * vz) / l2, 0.f, 1.f) : 0.f;
...
bool triangleHeightAtXZ(const std::vector<float>& positions, std::uint32_t ia, std::uint32_t ib, std::uint32_t ic,
                        float x, float z, float& y, float& nx, float& ny, float& nz) {
    const size_t a = size_t(ia) * 3;
    const size_t b = size_t(ib) * 3;
    const size_t c = size_t(ic) * 3;
    if (a + 2 >= positions.size() || b + 2 >= positions.size() || c + 2 >= positions.size()) return false;
...
bool closestMeshHeight(const MeshBuild& mesh, float x, float z, float referenceY, float& y, float& nx, float& ny,
                       float& nz) {
    bool found = false;
    ...
    for (size_t i = 0; i + 2 < indices.size(); i += 3) {
        ...
        if (!triangleHeightAtXZ(positions, indices[i], indices[i + 1], indices[i + 2], x, z, candidateY, candidateNx,
                                candidateNy, candidateNz))
            continue;
...
SpatialData SpatialData::box(float minX, float minY, float minZ, float maxX, float maxY, float maxZ) {
    SpatialData result;
    result.kind_   = Kind::Box;
    result.bounds_ = {std::min(minX, maxX),
                      std::min(minY, maxY),
                      std::min(minZ, maxZ),
                      std::max(minX, maxX),
                      std::max(minY, maxY),
...
SpatialData SpatialData::sphere(float x, float y, float z, float radius) {
    ...
    result.bounds_  = {x - radius, y - radius, z - radius, x + radius, y + radius, z + radius, true};
    return result;
}
...
SpatialData SpatialData::heightfield(const Heightmap& heightmap, float originX, float originZ, float cellSize,
                                     float heightScale) {
    SpatialData result;
    ...
    result.bounds_ = {originX,   minHeight,
                      originZ,   originX + float(heightmap.getWidth() - 1) * cellSize,
                      maxHeight, originZ + float(heightmap.getHeight() - 1) * cellSize,
                      true};
    return result;
}

SpatialData SpatialData::textureMask(const Heightmap& values, float originX, float originZ, float cellSize,
    ...
    result.bounds_    = {originX,
                         std::min(minY, maxY),
                         originZ,
                      originX + float(values.getWidth() - 1) * cellSize,
                      std::max(minY, maxY),
                         originZ + float(values.getHeight() - 1) * cellSize,
                         true};
    return result;
}
...
SpatialData SpatialData::meshSurface(const MeshBuild& mesh, float tolerance) {
    SpatialData result;
    if (mesh.positions().size() < 9 || mesh.indices().size() < 3 || tolerance < 0.f) return result;
...
bool SpatialData::contains(float x, float y, float z) const {
    switch (kind_) {
        case Kind::Points:
            ...
            return false;
        case Kind::Box:
            return x >= bounds_.minX && x <= bounds_.maxX && y >= bounds_.minY && y <= bounds_.maxY &&
        ...
        case Kind::Polygon: return y >= bounds_.minY && y <= bounds_.maxY && pointInPolygon(x, z, points_.points());
        case Kind::Spline:
            for (size_t i = 1; i < points_.points().size(); ++i)
                if (pointSegmentDistanceSquared(x, y, z, points_.points()[i - 1], points_.points()[i]) <=
                    radius_ * radius_)
                    return true;
            return false;
        case Kind::Heightfield: {
            if (x < bounds_.minX || x > bounds_.maxX || z < bounds_.minZ || z > bounds_.maxZ) return false;
        ...
        case Kind::TextureMask: {
            if (x < bounds_.minX || x > bounds_.maxX || y < bounds_.minY || y > bounds_.maxY || z < bounds_.minZ ||
                z > bounds_.maxZ)
                return false;
            const float value = heightmap_->sampleBilinear((x - originX_) / cellSize_, (z - originZ_) / cellSize_);
            return value >= minValue_ && value <= maxValue_;
        }
        case Kind::MeshSurface: {
            float surfaceY = 0.f, nx = 0.f, ny = 1.f, nz = 0.f;
            return closestMeshHeight(*mesh_, x, z, y, surfaceY, nx, ny, nz) && std::abs(y - surfaceY) <= radius_;
        }
...
PointSet SpatialData::sample(float spacing, uint32_t seed, float jitter) const {
    ...
    if (kind_ == Kind::Heightfield || kind_ == Kind::MeshSurface) {
        for (float z = bounds_.minZ; z <= bounds_.maxZ + spacing * 0.001f; z += spacing) {
            for (float x = bounds_.minX; x <= bounds_.maxX + spacing * 0.001f; x += spacing) {
                ...
                const float    sx =
                    std::clamp(x + (unitFloat(pointSeed) * 2.f - 1.f) * extent, bounds_.minX, bounds_.maxX);
                const float sz = std::clamp(z + (unitFloat(pointSeed ^ 0x02e5be93u) * 2.f - 1.f) * extent, bounds_.minZ,
                                            bounds_.maxZ);
    ...
    for (float z = bounds_.minZ; z <= bounds_.maxZ + spacing * 0.001f; z += spacing) {
        for (float y = bounds_.minY; y <= bounds_.maxY + spacing * 0.001f; y += spacing) {
            for (float x = bounds_.minX; x <= bounds_.maxX + spacing * 0.001f; x += spacing) {
                const uint32_t pointSeed = mixSpatialSeed(seed ^ index++);
                const float    sx = bounds_.maxX > bounds_.minX ? x + (unitFloat(pointSeed) * 2.f - 1.f) * extent : x;
                const float    sy =
                    bounds_.maxY > bounds_.minY ? y + (unitFloat(pointSeed ^ 0x68bc21ebu) * 2.f - 1.f) * extent : y;
                const float sz =
                    bounds_.maxZ > bounds_.minZ ? z + (unitFloat(pointSeed ^ 0x02e5be93u) * 2.f - 1.f) * extent : z;
...
PointSet SpatialData::filter(const PointSet& input, bool invert) const {
    ...
    for (size_t index = 0; index < input.points().size(); ++index) {
        ...
        if (contains(point.x, point.y, point.z) != invert)
            std::move(output.appendPointFrom(input, index)).expect("SpatialData filter attribute schema");
    }
    return output;
}
...
PointSet SpatialData::project(const PointSet& input) const {
    ...
    for (size_t index = 0; index < output.points().size(); ++index) {
        ...
        if (point.x < bounds_.minX || point.x > bounds_.maxX || point.z < bounds_.minZ || point.z > bounds_.maxZ)
            continue;
