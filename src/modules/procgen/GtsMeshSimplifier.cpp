...
Result<int> simplifyGtsMesh(MeshBuild& output, const MeshBuild& source, float quality,
    ...
    if (!valid(source) || !std::isfinite(quality) || quality < 0 || quality > 1 || o.maxIterationCount <= 0 ||
        ...
        return Result<int>::failure(Diagnostic::error(DiagnosticCode::InvalidArgument,
                                                      "invalid GTS mesh simplification input or options",
                                                      "procgen.gtsMeshSimplify"));
    std::vector<V> vs;
        ...
        for (int k = 0; k < 12; ++k)
            if (!std::isfinite(p[k]))
                return Result<int>::failure(Diagnostic::error(
                    DiagnosticCode::InvalidArgument, "mesh attributes must be finite", "procgen.gtsMeshSimplify"));
        vs.push_back(v);
    }
    std::vector<T> ts;
    for (int i = 0; i < source.getIndexCount(); i += 3) {
        ...
        if (a < 0 || b < 0 || c < 0 || a >= int(vs.size()) || b >= int(vs.size()) || c >= int(vs.size()) || a == b ||
            b == c || a == c)
            return Result<int>::failure(Diagnostic::error(DiagnosticCode::InvalidArgument,
                                                          "mesh triangles must be valid and non-degenerate",
                                                          "procgen.gtsMeshSimplify"));
    ...
    for (auto& t : ts) {
        ...
        if (l < 1e-12)
            return Result<int>::failure(Diagnostic::error(
                DiagnosticCode::InvalidArgument, "mesh contains a zero-area triangle", "procgen.gtsMeshSimplify"));
        nx /= l;
        ny /= l;
        nz /= l;
    ...
    if (next.empty())
        return Result<int>::failure(Diagnostic::error(
            DiagnosticCode::InvalidArgument, "simplification removed the complete mesh", "procgen.gtsMeshSimplify"));
