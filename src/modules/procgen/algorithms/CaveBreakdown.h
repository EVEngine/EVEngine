 ...
 */
EVENGINE_API_DOMAINS CaveBreakdownSet createCaveBreakdown(const std::vector<CaveBreakdownChamber>& chambers,
                                                          int eventCount, float strength, uint32_t seed);

/** @brief Carve the event's shallow ceiling detachment scars from a cave SDF. */
EVENGINE_API_DOMAINS float carveCaveBreakdownScars(float x, float y, float z, float current,
                                                   const CaveBreakdownSet& breakdown);

/** @brief Union the event's landed breakdown blocks into a cave SDF. */
EVENGINE_API_DOMAINS float addCaveBreakdownBlocks(float x, float y, float z, float current,
                                                  const CaveBreakdownSet& breakdown);

/** @brief Test whether a point lies on a generated breakdown block surface. */
EVENGINE_API_DOMAINS bool isCaveBreakdownBlockSurface(float x, float y, float z, float tolerance,
                                                      const CaveBreakdownSet& breakdown);
