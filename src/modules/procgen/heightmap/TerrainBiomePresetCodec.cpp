...
template <class T>
Result<T> malformedPreset(const char* message) {
...
Result<std::string> TerrainBiomePreset::snapshotJson() const {
    if (entries_.size() > kMaximumSpawnerEntries)
        return malformedPreset<std::string>("terrain.biomePreset.snapshot: too many spawner entries");
    Value::Array entries;
    entries.reserve(entries_.size());
    for (const auto& entry : entries_) {
        if (entry.entryId.empty() || entry.entryId.size() > std::numeric_limits<std::uint32_t>::max())
            return malformedPreset<std::string>("terrain.biomePreset.snapshot: invalid entry ID");
    ...
    if (encoded.value().size() > kMaximumPresetJsonBytes)
        return malformedPreset<std::string>("terrain.biomePreset.snapshot: JSON exceeds size limit");
    return encoded;
}

Result<void> TerrainBiomePreset::restoreJson(const std::string& json) {
    if (json.size() > kMaximumPresetJsonBytes)
        return malformedPreset<void>("terrain.biomePreset.restore: JSON exceeds size limit");
    ...
    if (!root || root->size() != 3 || !root->contains("schema") || !root->contains("version") ||
        ...
        return malformedPreset<void>("terrain.biomePreset.restore: missing or unknown root fields");
    ...
    if (!schema || *schema != "eve.procgen.terrain-biome-preset" || !version ||
        ...
        return malformedPreset<void>("terrain.biomePreset.restore: unsupported schema, version or entry count");
    ...
    for (const auto& encodedEntry : *entries) {
        ...
        if (!entry || entry->size() != expectedFields || !entry->contains("entryId") ||
            ...
            return malformedPreset<void>("terrain.biomePreset.restore: missing or unknown entry fields");
        ...
        if (!entryId || entryId->empty() || !activeInBiome || !activeInStamper || !planJson ||
            ...
            return malformedPreset<void>("terrain.biomePreset.restore: invalid entry values");
