...
physics::Body3D* resolve(const ColliderLink& link) {
    auto* world = physics::World3D::findWorld(link.world);
    return world ? world->findBody(link.body) : nullptr;
}
int destroy(std::vector<ColliderLink>& links) {
    int count = 0;
    for (const auto& link : links)
        if (auto* world = physics::World3D::findWorld(link.world))
            if (auto* body = world->findBody(link.body)) {
...
Result<std::uint64_t> GtsTerrainColliderRuntime::replace(const procgen::GtsTerrainLodSet& lods, physics::World3D& world,
    ...
    try {
        for (int tileIndex = 0; tileIndex < lods.getTileCount(); ++tileIndex) {
            ...
            for (auto index : mesh.indices()) {
                if (index > static_cast<std::uint32_t>(std::numeric_limits<std::int32_t>::max()))
                    throw eve::Exception("mesh index exceeds physics range");
                indices.push_back(static_cast<std::int32_t>(index));
            }
            try {
                if (!body->newTriangleMeshShape(mesh.positions(), indices))
                    throw eve::Exception("shape creation returned null");
            } catch (...) {
                world.destroyBody(body);
                candidate[static_cast<std::size_t>(tileIndex)] = {};
                throw;
            }
        }
        ...
        return Result<std::uint64_t>::failure(
            Diagnostic::error(DiagnosticCode::Failed, error.what(), "procgen.physics.gtsTerrainCollider"));
    }
    destroy(impl_->tiles);
    impl_->tiles = std::move(candidate);
    ++impl_->revision;
    if (!impl_->revision) ++impl_->revision;
    return Result<std::uint64_t>::success(impl_->revision);
}
Result<int> GtsTerrainColliderRuntime::clear() {
    int count = destroy(impl_->tiles);
    ++impl_->revision;
    if (!impl_->revision) ++impl_->revision;
    return Result<int>::success(count);
}
physics::Body3D* GtsTerrainColliderRuntime::getBody(int index) const {
    return index >= 0 && index < static_cast<int>(impl_->tiles.size())
               ? resolve(impl_->tiles[static_cast<std::size_t>(index)])
               : nullptr;
}
