...
#######################################################################################*/
			...
			// See DrawJagged.hpp — upstream omits drawOperatorArray; required for Apple Clang.
			...
			constexpr Return_Var_ drawOperatorArray(Matrix_& matrix_, const Index_Size max_x_,
				...
				return static_cast<const Derived*>(this)->drawNormal(
					::dtl::utility::makeWrapper<Matrix_Var_>(matrix_, max_x_, max_y_),
					Random_Engine_(DTL_RANDOM_ENGINE.get()), function_);
			}
