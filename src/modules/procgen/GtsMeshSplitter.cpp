...
template<class Distance> std::vector<Vertex> clip(const std::vector<Vertex>& in,Distance distance) {
    ...
    Vertex previous=in.back(); float dp=distance(previous); bool previousInside=dp>=0.f;
    for(const auto& current:in) {
        const float dc=distance(current); const bool currentInside=dc>=0.f;
        ...
        previous=current; dp=dc; previousInside=currentInside;
    }
    return out;
}
...
std::unique_ptr<MeshBuild> GtsMeshSplitResult::copyTileMesh(int index) const {
    const auto* tile=tileAt(index); return tile?std::make_unique<MeshBuild>(tile->mesh):nullptr;
}
float GtsMeshSplitResult::getTileOffsetX(int index) const noexcept { const auto* tile=tileAt(index);return tile?tile->offsetX:0.f; }
float GtsMeshSplitResult::getTileOffsetZ(int index) const noexcept { const auto* tile=tileAt(index);return tile?tile->offsetZ:0.f; }
...
Result<GtsMeshSplitResult> splitGtsMesh(const MeshBuild& source,int xSplits,int zSplits,GtsMeshPivot pivot) {
    ...
    if(vertexCount<3||indexCount<3||indexCount%3!=0||xSplits<0||zSplits<0||xSplits>255||zSplits>255||
       static_cast<int>(pivot)<0||static_cast<int>(pivot)>2)
    ...
    for(int i=0;i<vertexCount;++i) {
        Vertex v{source.getPositionX(i),source.getPositionY(i),source.getPositionZ(i),
                 source.getNormalX(i),source.getNormalY(i),source.getNormalZ(i),source.getUvU(i),source.getUvV(i),
    ...
    for(int i=0;i<indexCount;++i)if(source.getIndex(i)<0||source.getIndex(i)>=vertexCount)
    ...
    GtsMeshSplitResult result; result.columns_=xSplits+1;result.rows_=zSplits+1;
    ...
    for(int row=0;row<result.rows_;++row)for(int column=0;column<result.columns_;++column) {
        const float loX=minX+column*cellX,hiX=column+1==result.columns_?maxX:minX+(column+1)*cellX;
        ...
        if(pivot==GtsMeshPivot::MinimumXZ){pivotX=loX;pivotZ=loZ;}
        else if(pivot==GtsMeshPivot::CenterXZ){pivotX=(loX+hiX)*.5f;pivotZ=(loZ+hiZ)*.5f;}
        tile.offsetX=pivotX;tile.offsetZ=pivotZ;
        std::vector<float> tileColors;
        for(int i=0;i<indexCount;i+=3) {
            std::vector<Vertex> polygon; polygon.reserve(7);
            for(int k=0;k<3;++k){const int n=source.getIndex(i+k);polygon.push_back({
                source.getPositionX(n),source.getPositionY(n),source.getPositionZ(n),source.getNormalX(n),
                source.getNormalY(n),source.getNormalZ(n),source.getUvU(n),source.getUvV(n),
                source.getColor(n,0),source.getColor(n,1),source.getColor(n,2),source.getColor(n,3)});}
            ...
            if(group>=0)tile.mesh.setActiveGroup(source.getGroupName(group));
        ...
        if(!tileColors.empty())tile.mesh.setVertexColors(std::move(tileColors)).ignore("validated clipped color stream");
    }
...
Result<void> splitGtsMeshInto(GtsMeshSplitResult& output,const MeshBuild& source,int xSplits,int zSplits,GtsMeshPivot pivot) {
    auto result=splitGtsMesh(source,xSplits,zSplits,pivot);if(!result.ok())return Result<void>::failure(result.status());
    output=std::move(result).value();return Result<void>::success();
}
}
