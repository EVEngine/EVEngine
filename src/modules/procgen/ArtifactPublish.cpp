...
void appendPartView(const ArtifactPart& source, PublicationBuffer& buffer) {
    ...
    std::visit([&view, &buffer](const auto& payload) {
...
PublicationBuffer makePublicationBuffer(const GeneratedArtifact& artifact) {
    ...
    buffer.view.parts = std::span<const eve::artifact::PartView>(buffer.parts.data(),
                                                                  buffer.parts.size());
    return buffer;
}

eve::Result<void> participantFailure(eve::DiagnosticCode code, std::string message,
                                     std::string path = {}) {
...
public:
    ...
    [[nodiscard]] eve::Result<void> prepare(
            ...
            try {
                ...
                return eve::Result<std::unique_ptr<eve::artifact::PreparedPublication>>::failure(
                    eve::Diagnostic::error(
                        ...
                        std::string("artifact provider prepare threw: ") + error.what(),
                        name_ + ".prepare"));
            } catch (...) {
                return eve::Result<std::unique_ptr<eve::artifact::PreparedPublication>>::failure(
                    eve::Diagnostic::error(eve::DiagnosticCode::Failed,
                                           "artifact provider prepare threw", name_ + ".prepare"));
            }
        }();
        ...
        if (!stage_)
            return participantFailure(eve::DiagnosticCode::InvariantViolation,
                                      "artifact provider returned a null prepared stage",
                                      name_ + ".prepare");
    ...
    [[nodiscard]] eve::Result<void> compensate(
        ...
        if (phase_ != Phase::Committed)
            return participantFailure(eve::DiagnosticCode::PreconditionViolation,
                                       "artifact provider participant is not committed",
                                       name_ + ".compensate");
        return participantFailure(eve::DiagnosticCode::Unsupported,
                                   "artifact provider publication has no inverse compensation",
                                   name_ + ".compensate");
    }
...
public:
    ...
    [[nodiscard]] eve::Result<void> commit(
        ...
        if (phase_ != Phase::Prepared || !stage_)
            return participantFailure(eve::DiagnosticCode::PreconditionViolation,
                                       "artifact store participant is not prepared",
                                       "procgen.artifact-store.commit");
        stage_->commit();
        stage_.reset();
        phase_ = Phase::Committed;
        return eve::Result<void>::success(eve::Status::success(eve::StatusCode::Applied));
    ...
    [[nodiscard]] eve::Result<void> rollback(
        ...
        if (phase_ != Phase::Prepared)
            return participantFailure(eve::DiagnosticCode::PreconditionViolation,
                                       "artifact store participant is not prepared",
                                       "procgen.artifact-store.rollback");
        stage_->rollback();
        stage_.reset();
        phase_ = Phase::RolledBack;
        return eve::Result<void>::success(eve::Status::success(eve::StatusCode::Applied));
...
public:
    ProviderRestoreParticipant(std::string name, eve::artifact::ProviderContract& provider,
    ...
    [[nodiscard]] eve::Result<void> compensate(
        ...
        if (phase_ != Phase::Committed)
            return participantFailure(eve::DiagnosticCode::PreconditionViolation,
                                       "artifact restore participant is not committed",
                                       name_ + ".compensate");
...
public:
    StoreRestoreParticipant(ArtifactStore& store, const eve::Value& state)
        : store_(&store), state_(&state) {}
...
eve::transaction::TransactionContext publicationContext(const eve::PersistentId& id,
...
eve::Result<ArtifactPublishReceipt> ArtifactPublisher::publish(
    GeneratedArtifact artifact, ArtifactPublishOptions options) {
    PublicationBuffer publication;
    try {
        publication = makePublicationBuffer(artifact);
    } catch (const std::exception& error) {
        return eve::Result<ArtifactPublishReceipt>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::Failed,
            std::string("failed to materialize artifact publication view: ") + error.what()));
    } catch (...) {
        return eve::Result<ArtifactPublishReceipt>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::Failed, "failed to materialize artifact publication view"));
    }
    ...
    auto scene = addProviderParticipant<ISceneArtifactAdapter>(
        options.scene, "scene", publication.view, owners, participants);
    if (!scene.ok()) return eve::Result<ArtifactPublishReceipt>::failure(scene.status());
    auto map = addProviderParticipant<IMapArtifactAdapter>(
        options.map, "map", publication.view, owners, participants);
    if (!map.ok()) return eve::Result<ArtifactPublishReceipt>::failure(map.status());
    auto graphics = addProviderParticipant<IGraphicsArtifactAdapter>(
        options.graphics, "graphics", publication.view, owners, participants);
    if (!graphics.ok()) return eve::Result<ArtifactPublishReceipt>::failure(graphics.status());
    auto physics = addProviderParticipant<IPhysicsArtifactAdapter>(
        options.physics, "physics", publication.view, owners, participants);
    ...
    try {
        ...
        return eve::Result<ArtifactPublishReceipt>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::Failed, "failed to allocate artifact store participant"));
    }
    ...
    if (!coordinated.ok())
        return eve::Result<ArtifactPublishReceipt>::failure(coordinated.status());
...
eve::Result<eve::SnapshotEnvelope> ArtifactPublisher::snapshot(
    const ArtifactSnapshotContext& context) const {
    ...
    if (!schema)
        return eve::Result<eve::SnapshotEnvelope>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::InvariantViolation, "artifact snapshot schema id is invalid"));
    ...
    const auto appendProvider = [&providers](const char* name,
                                              eve::artifact::ProviderContract* provider)
        -> eve::Result<void> {
    ...
    return eve::makeSnapshotEnvelope("eve.artifact.publication", *schema, eve::SchemaVersion(1),
                                     context.instanceId, context.revision, context.tick,
                                     eve::Value(std::move(payload)), context.hashProvider);
}

eve::Result<void> ArtifactPublisher::restore(
    ...
    if (!schema || snapshot.type != "eve.artifact.publication" || snapshot.schema != *schema ||
        ...
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::UnknownVersion, "unsupported artifact publication snapshot"));
    if (store_.size() != 0)
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::Conflict, "artifact snapshot restore requires an empty store"));
    ...
    const auto* providerStates = providerStateValue
                                     ? providerStateValue->getIf<eve::Value::Object>()
                                     : nullptr;
    ...
    eve::artifact::IGraphicsArtifactAdapter* graphics =
        eve::cap::query<eve::artifact::IGraphicsArtifactAdapter>();
    eve::artifact::IPhysicsArtifactAdapter* physics =
        eve::cap::query<eve::artifact::IPhysicsArtifactAdapter>();
    if ((scene && !scene->emptyState()) || (map && !map->emptyState()) ||
        (graphics && !graphics->emptyState()) || (physics && !physics->emptyState()))
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::Conflict, "artifact snapshot restore requires empty providers"));
    ...
    for (const auto& entry : *providerStates) {
        if (entry.first != "scene" && entry.first != "map" && entry.first != "graphics" &&
            entry.first != "physics")
            return eve::Result<void>::failure(eve::Diagnostic::error(
                eve::DiagnosticCode::ParseError, "unknown artifact publication provider state"));
    }
    ...
    try {
        ...
        participants.push_back(owners.back().get());
        const auto addRestore = [&owners, &participants](const char* name,
                                                         eve::artifact::ProviderContract* provider,
                                                         const eve::Value* state) {
        ...
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::Failed, "failed to allocate artifact restore participants"));
    }
    ...
    if (transactionReceipt.state != eve::transaction::CoordinatorState::Committed)
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::InvariantViolation,
            "artifact restore coordinator returned a non-committed receipt"));
    return eve::Result<void>::success(eve::Status::success(eve::StatusCode::Applied));
}
