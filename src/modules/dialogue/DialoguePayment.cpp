...
eve::Result<std::int64_t> positiveInteger(const eve::Value& value, std::string_view path) {
    if (!value.isInt64())
        return failure<std::int64_t>(eve::DiagnosticCode::InvalidArgument,
                                     "dialogue payment amount must be an Int64", std::string(path));
    const std::int64_t amount = value.asInt();
    if (amount <= 0)
        return failure<std::int64_t>(eve::DiagnosticCode::InvalidArgument,
                                     "dialogue payment amount must be positive", std::string(path));
...
eve::Result<std::vector<AccountCost>> buildCosts(const DialogueAccountBindings& bindings,
                                                 const PaymentSpec& payment) {
    ...
    for (const auto& item : raw) {
        ...
        if (item.account == nullptr)
            return failure<std::vector<AccountCost>>(eve::DiagnosticCode::Unsupported,
                                                     std::string("dialogue ") + item.path +
                                                         " payment has no account adapter",
                                                     item.path);
        ...
        auto found = std::find_if(result.begin(), result.end(), [&](const AccountCost& candidate) {
            return candidate.account == item.account;
        });
        if (found == result.end()) {
...
eve::Result<void> PaymentSpec::validate() const {
    if (money && *money <= 0)
        return failureVoid(eve::DiagnosticCode::InvalidArgument,
                           "dialogue money payment must be positive", "money");
    if (reputation && *reputation <= 0)
        return failureVoid(eve::DiagnosticCode::InvalidArgument,
                           "dialogue reputation payment must be positive", "reputation");
    return eve::Result<void>::success();
}

eve::Result<PaymentSpec> PaymentSpec::fromValue(const eve::Value& value) {
    if (!value.isObject())
        return failure<PaymentSpec>(eve::DiagnosticCode::InvalidArgument,
    ...
    for (const auto& key : value.keys()) {
        ...
        if (key == "money") {
            ...
            return failure<PaymentSpec>(eve::DiagnosticCode::InvalidArgument,
                                        "unknown dialogue payment field", "payment." + key);
        }
    }
...
DialogueStateMutationParticipant::DialogueStateMutationParticipant(
    eve::IStateMutation& provider, std::span<const eve::StateMutation> mutations)
    : provider_(provider), mutations_(mutations.begin(), mutations.end()) {}

eve::Result<void> DialogueStateMutationParticipant::prepare(
    const eve::transaction::TransactionContext& context) {
    if (context.transactionId().empty())
        return failureVoid(eve::DiagnosticCode::InvalidArgument,
                           "dialogue state transaction requires a transaction id", "transactionId");
    if (phase_ != Phase::Idle)
        return lifecycleFailure("dialogue state participant is not idle");
    phase_ = Phase::Prepared;
    return eve::Result<void>::success(eve::Status::success(eve::StatusCode::Applied));
}

eve::Result<void> DialogueStateMutationParticipant::commit(
    const eve::transaction::TransactionContext& context) {
    if (phase_ != Phase::Prepared)
        return lifecycleFailure("dialogue state participant has no prepared stage");
...
eve::Result<void> DialogueStateMutationParticipant::rollback(
    const eve::transaction::TransactionContext&) {
    if (phase_ != Phase::Prepared)
...
eve::Result<void> DialogueStateMutationParticipant::compensate(
    const eve::transaction::TransactionContext&) {
    return failureVoid(eve::DiagnosticCode::Unsupported,
                       "dialogue state mutation has no generic inverse; it must be last",
                       "transaction.compensation");
}

DialoguePaymentAdapter::DialoguePaymentAdapter(DialogueAccountBindings bindings)
    : bindings_(std::move(bindings)) {}

void DialoguePaymentAdapter::setBindings(DialogueAccountBindings bindings) {
    bindings_ = std::move(bindings);
}
...
eve::Result<eve::transaction::TransactionReceipt> DialoguePaymentAdapter::execute(
    ...
    if (transactionId.empty())
        return failure<eve::transaction::TransactionReceipt>(
            eve::DiagnosticCode::InvalidArgument, "dialogue payment transaction id must not be empty",
            "transactionId");

    for (std::size_t index = 0; index < effects.size(); ++index) {
        if (effects[index] == nullptr)
            return failure<eve::transaction::TransactionReceipt>(
                eve::DiagnosticCode::InvalidArgument, "dialogue payment effect must not be null",
                "effects[" + std::to_string(index) + "]");
        for (std::size_t prior = 0; prior < index; ++prior) {
            if (effects[prior] == effects[index])
                return failure<eve::transaction::TransactionReceipt>(
                    eve::DiagnosticCode::Conflict, "dialogue payment effects must be unique",
                    "effects[" + std::to_string(index) + "]");
        }
    }
    ...
    if (participants.empty())
        return failure<eve::transaction::TransactionReceipt>(
            eve::DiagnosticCode::InvalidArgument, "dialogue payment has no account or effect", "payment");
