...
eve::Result<void> dialogueFailure(eve::DiagnosticCode code, std::string message,
                                  std::string path = {}) {
...
public:
    ...
    [[nodiscard]] eve::Result<void> prepare(
        ...
        if (phase_ != Phase::Idle)
            return dialogueFailure(eve::DiagnosticCode::Conflict, "dialogue choice is not idle", "transaction.lifecycle");
    ...
    [[nodiscard]] eve::Result<void> rollback(
        const eve::transaction::TransactionContext&) override {
        if (phase_ != Phase::Prepared)
            return dialogueFailure(eve::DiagnosticCode::Conflict,
                                   "dialogue choice has no prepared stage to roll back", "transaction.lifecycle");
    ...
    [[nodiscard]] eve::Result<void> compensate(
        ...
        if (phase_ != Phase::Committed)
            return dialogueFailure(eve::DiagnosticCode::Conflict,
                                   "dialogue choice is not committed", "transaction.lifecycle");
        std::string error;
...
void DialogueFlow::configureIntegration(IntegrationConfig config) {
    ...
    if (configuredConditionEvaluator_) {
        ...
        runner_.setConditionEvaluator([this](const eve::Value& specification) {
            return stateContext_.evaluate(specification);
        });
    }

    if (operationRequestHandler_ || gameplayActionHandler_ || commandParticipantFactory_ || stateMutationProvider_) {
        runner_.setCommandRequestDispatcher(
            [this](const CommandRequest& request) { return dispatchCommand(request); });
    } else {
...
eve::Result<eve::MutationReceipt> DialogueFlow::applyStateMutations(
...
bool DialogueFlow::applyDocument(ConversationDocument* document) {
    ...
    if (!lintConversationWorkspace(candidate, "authoring", candidateDiagnostics)) {
        diagnostics_ = std::move(candidateDiagnostics);
        failureMessage_   = diagnostics_.empty() ? "conversation document validation failed" : diagnostics_.front().message;
        return false;
    }
    assets_ = std::move(candidate);
    diagnostics_.clear();
...
CommandResponse DialogueFlow::dispatchCommand(const CommandRequest& request) {
    ...
    CommandRequestHandler* legacy = request.kind == CommandRequestKind::Operation
                                        ? &operationRequestHandler_
                                        : &gameplayActionHandler_;
    ...
    if (!request.stateMutations.empty()) {
        ...
        auto state = std::make_unique<DialogueStateMutationParticipant>(*stateMutationProvider_,
                                                                          request.stateMutations);
        effects.push_back(state.get());
...
void DialogueFlow::expose(ssq::Class& cls) {
    ...
    cls.addFunc("select", [vm = cls.getHandle()](DialogueFlow* value,
                                                   const std::string& routeId) {
        if (!value)
            return eve::script::projectResult(
                vm, dialogueFailure(eve::DiagnosticCode::InvalidArgument,
