...
void GpuBuffer::uploadBytes(const void *src, uint64_t nbytes, uint64_t dstOffset) {
...
void GpuBuffer::writeData(data::ByteData *data, int dstOffset) {
    if (!data) return;
    uploadBytes(data->getData(), data->getSize(), uint64_t(dstOffset < 0 ? 0 : dstOffset));
}

data::ByteData *GpuBuffer::readData(int srcOffset, int size) {
...
void GpuBuffer::writeFloat32(int floatIndex, float value) {
    if (floatIndex < 0) return;
    uploadBytes(&value, sizeof(float), uint64_t(floatIndex) * sizeof(float));
}

float GpuBuffer::readFloat32(int floatIndex) {
