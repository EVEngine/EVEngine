...
void ComputeShader::flushDescriptors(vkb::Device &device) {
    ...
    for (int i = 0; i < kMaxBindings; ++i) {
        GpuBuffer *b = bindings_[size_t(i)];
        infos[size_t(i)].buffer = (b && b->buffer_) ? b->buffer_ : dummyBuffer_;
        infos[size_t(i)].offset = 0;
        infos[size_t(i)].range = (b && b->buffer_) ? b->size_ : vk::DeviceSize(4);
