...
Result<std::unique_ptr<EvpackShader>> EvpackShader::load(const asset::EvpackResourceReader& reader,
    ...
    if (graphics.getBackendName() != "vulkan" || capabilities.graphics != graphics.getBackendName())
        return Result<std::unique_ptr<EvpackShader>>::failure(
            Diagnostic::error(DiagnosticCode::Unsupported, "Shader backend mismatch", {}, {}, "asset.shader.runtime"));
    ...
    try {
        ...
        if (!impl.shader)
            return Result<std::unique_ptr<EvpackShader>>::failure(Diagnostic::error(
                DiagnosticCode::Failed, "Shader upload returned no allocation", {}, {}, "asset.shader.runtime"));
        for (const auto& parameter : impl.definition.parameters) {
            switch (parameter.defaults.size()) {
                ...
                default:
                    return Result<std::unique_ptr<EvpackShader>>::failure(Diagnostic::error(
                        DiagnosticCode::InvalidArgument, "Invalid parameter width", {}, {}, "asset.shader.runtime"));
            }
        ...
        return Result<std::unique_ptr<EvpackShader>>::failure(
            Diagnostic::error(DiagnosticCode::Failed, error.what(), {}, {}, "asset.shader.runtime"));
    }
}
Result<void> EvpackShader::setParameter(std::string_view name, std::span<const float> values) try {
    if (!impl_->shader)
        return Result<void>::failure(
            Diagnostic::error(DiagnosticCode::StaleHandle, "Shader lease is released", {}, {}, "asset.shader.runtime"));
    ...
    if (it == impl_->definition.parameters.end() || it->defaults.size() != values.size() ||
        ...
        return Result<void>::failure(Diagnostic::error(DiagnosticCode::InvalidArgument,
                                                       "Unknown parameter, wrong width or non-finite value", {}, {},
                                                       "asset.shader.runtime"));
...
Result<void> EvpackShader::drawMesh(graphics::Mesh& mesh, const std::array<float, 16>& transform,
    ...
    if (impl_->definition.interface != asset::ShaderAssetInterface::Mesh3D || !finite(transform) || !finite(tint))
        return Result<void>::failure(Diagnostic::error(
            DiagnosticCode::InvalidArgument, "Invalid mesh draw or shader kind", {}, {}, "asset.shader.runtime"));
...
Result<void> EvpackShader::drawSprite(OptionalRef<graphics::Texture> texture, const std::array<float, 4>& rect,
    ...
    if (impl_->definition.interface != asset::ShaderAssetInterface::Sprite2D || !finite(rect) || !finite(tint))
        return Result<void>::failure(Diagnostic::error(
            DiagnosticCode::InvalidArgument, "Invalid sprite draw or shader kind", {}, {}, "asset.shader.runtime"));
...
Result<void> EvpackShader::release() try {
    ...
    if (!impl_->graphics.releaseShader(impl_->shader))
        return Result<void>::failure(Diagnostic::error(DiagnosticCode::Failed, "Graphics rejected shader release", {},
                                                       {}, "asset.shader.runtime"));
