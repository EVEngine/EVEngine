...
Result<graphics::Mesh*> GraphicsMeshFactoryAdapter::uploadMeshColored(
    ...
    if (!mesh)
        return Result<graphics::Mesh*>::failure(
            Diagnostic::error(DiagnosticCode::Failed, "graphics backend rejected colored canonical mesh upload", {}, {},
                              "asset.graphics.adapter"));
    return Result<graphics::Mesh*>::success(mesh);
}
...
Result<void> GraphicsMeshFactoryAdapter::releaseMesh(graphics::Mesh* mesh) {
    if (!mesh)
        return Result<void>::failure(Diagnostic::error(
            DiagnosticCode::InvalidArgument, "cannot release a null graphics mesh", {}, {}, "asset.graphics.adapter"));
    if (!factory_.releaseMesh(mesh))
        return Result<void>::failure(Diagnostic::error(DiagnosticCode::Failed, "graphics backend rejected mesh release",
                                                       {}, {}, "asset.graphics.adapter"));
...
Result<void> GraphicsMeshFactoryAdapter::setMeshTexcoords(graphics::Mesh* mesh, std::uint32_t set,
    ...
    if (!mesh)
        return Result<void>::failure(
            Diagnostic::error(DiagnosticCode::InvalidArgument, "null mesh", {}, {}, "asset.graphics.adapter"));
