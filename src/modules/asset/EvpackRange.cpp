...
Result<std::uint64_t> FileEvpackRangeSource::size() const {
    ...
    if (ec)
        return Result<std::uint64_t>::failure(Diagnostic::error(DiagnosticCode::NotFound,
                                                                "cannot read runtime package size: " + ec.message(),
                                                                path_.string(), {}, "asset.evpack.range"));
...
Result<std::vector<std::uint8_t>> FileEvpackRangeSource::read(std::uint64_t offset,
    ...
    if (size > std::numeric_limits<std::size_t>::max() ||
        ...
        return Result<std::vector<std::uint8_t>>::failure(Diagnostic::error(DiagnosticCode::InvalidArgument,
                                                                            "file range cannot be represented",
                                                                            path_.string(), {}, "asset.evpack.range"));
    std::ifstream input(path_, std::ios::binary);
    if (!input)
        return Result<std::vector<std::uint8_t>>::failure(Diagnostic::error(
            DiagnosticCode::NotFound, "cannot open runtime package", path_.string(), {}, "asset.evpack.range"));
    input.seekg(static_cast<std::streamoff>(offset));
    if (!input)
        return Result<std::vector<std::uint8_t>>::failure(Diagnostic::error(
            DiagnosticCode::Failed, "cannot seek runtime package", path_.string(), {}, "asset.evpack.range"));
    std::vector<std::uint8_t> bytes(static_cast<std::size_t>(size));
    if (!bytes.empty()) input.read(reinterpret_cast<char*>(bytes.data()), static_cast<std::streamsize>(bytes.size()));
    if (!input || static_cast<std::uint64_t>(input.gcount()) != size)
        return Result<std::vector<std::uint8_t>>::failure(Diagnostic::error(
            DiagnosticCode::Failed, "runtime package range is truncated", path_.string(), {}, "asset.evpack.range"));
...
Result<EvpackRangeMount> prepareEvpackRangeMount(std::shared_ptr<const EvpackRangeSource> source,
    ...
    if (!source)
        return Result<EvpackRangeMount>::failure(Diagnostic::error(
            DiagnosticCode::InvalidArgument, "range source is required", {}, {}, "asset.evpack.range"));
    ...
    if (sourceSize.value() < 128 || sourceSize.value() > limits.maximumPackageBytes)
        return Result<EvpackRangeMount>::failure(Diagnostic::error(DiagnosticCode::InvalidArgument,
    ...
    if (header.value().size() != 128)
        return Result<EvpackRangeMount>::failure(Diagnostic::error(
            DiagnosticCode::Failed, "range source returned a short header", {}, {}, "asset.evpack.range"));
    ...
    if (tocOffset > sourceSize.value() || tocSize > sourceSize.value() - tocOffset ||
        ...
        return Result<EvpackRangeMount>::failure(Diagnostic::error(
            DiagnosticCode::ParseError, "runtime package metadata extent is invalid", {}, {}, "asset.evpack.range"));
    ...
    if (signatureSize != 0) {
        if (signatureOffset > sourceSize.value() || signatureSize > sourceSize.value() - signatureOffset)
            return Result<EvpackRangeMount>::failure(Diagnostic::error(DiagnosticCode::ParseError,
    ...
    if (metadata.value().size() != metadataSize)
        return Result<EvpackRangeMount>::failure(Diagnostic::error(
            DiagnosticCode::Failed, "range source returned a short metadata prefix", {}, {}, "asset.evpack.range"));
...
Result<std::vector<std::uint8_t>> EvpackRangeMount::readChunk(std::size_t index) const {
    if (index >= index_.chunks().size())
        return Result<std::vector<std::uint8_t>>::failure(Diagnostic::error(
            DiagnosticCode::InvalidArgument, "chunk index is outside the TOC", {}, {}, "asset.evpack.range"));
    const auto& chunk = index_.chunks()[index];
    if (chunk.storedSize > limits_.maximumChunkBytes)
        return Result<std::vector<std::uint8_t>>::failure(Diagnostic::error(DiagnosticCode::InvalidArgument,
                                                                            "range chunk exceeds configured budget",
                                                                            chunk.type, {}, "asset.evpack.range"));
    ...
    if (bytes.value().size() != chunk.storedSize)
        return Result<std::vector<std::uint8_t>>::failure(Diagnostic::error(
            DiagnosticCode::Failed, "range source returned a short chunk", chunk.type, {}, "asset.evpack.range"));
    ...
    if (sha256(decoded.value()) != chunk.contentHash)
        return Result<std::vector<std::uint8_t>>::failure(Diagnostic::error(
            DiagnosticCode::HashMismatch, "range chunk hash does not match", chunk.type, {}, "asset.evpack.range"));
    return decoded;
}
