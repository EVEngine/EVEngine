...
Result<PreparedAssetImport> prepareUnityNativeMesh(const UnityProjectImportRequest& request,
    ...
    if (text.find("\nMesh:") == std::string::npos && text.find("\nMesh:\r") == std::string::npos)
        return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::Unsupported, "native asset is not a text Mesh", source.path, {}, "asset.import"));
    try {
        ...
        if ((version != 9 && version != 11) || number(text, "m_MeshCompression") != 0 ||
            ...
            return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::Unsupported, "requires uncompressed static Mesh version 9 or 11", source.path, {}, "asset.import"));
        ...
        if (shapes != std::string::npos) {
            ...
            if (section.find("vertices: []") == std::string::npos || section.find("shapes: []") == std::string::npos)
                return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::Unsupported, "Mesh blend shapes require animation conversion", source.path, {}, "asset.import"));
        }
        ...
        for (std::sregex_iterator it(text.begin(), text.end(), channel), end; it != end; ++it) {
            ...
            if (c.dimension && (c.stream != 0 || c.format != 0 || c.dimension > 4 || c.offset > 256))
                return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::Unsupported, "Mesh requires interleaved float32 vertex channels", source.path, {}, "asset.import"));
        ...
        if (channels[12].dimension || channels[13].dimension)
            return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::Unsupported, "Mesh skin channels require animation conversion", source.path, {}, "asset.import"));
        ...
        if (number(text, "size") != 0)
            return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::Unsupported, "external Mesh stream unsupported", source.path, {}, "asset.import"));
        ...
        for (std::sregex_iterator it(text.begin(), text.end(), sub), end; it != end; ++it) {
            ...
            if (topology != 0)
                return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::Unsupported, "non-triangle Mesh topology", source.path, {}, "asset.import"));
        ...
        return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::ParseError, e.what(), source.path, {}, "asset.import"));
    }
}
}  // namespace eve::asset_import
