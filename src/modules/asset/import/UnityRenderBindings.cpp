...
Result<double> property(const std::string& text, const std::string& name, double defaultValue) {
    ...
    if (!parsed || !parsed.value().isNumeric())
        return Result<double>::failure(Diagnostic::error(DiagnosticCode::ParseError, "invalid Unity material scalar", name, {}, "asset.import"));
    const auto number = parsed.value().isInt64() ? double(parsed.value().asInt()) : parsed.value().asDouble();
    if (!std::isfinite(number))
        return Result<double>::failure(Diagnostic::error(DiagnosticCode::ParseError, "nonfinite material scalar", name, {}, "asset.import"));
...
Result<PreparedAssetImport> prepareUnityMaterial(const UnityProjectImportRequest& request,
    ...
    if ((mode.value() != 0 && mode.value() != 2 && mode.value() != 3) || metallic.value() < 0 || metallic.value() > 1 ||
        ...
        return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::Unsupported, "Standard opaque/fade/transparent materials require normalized factors", source.path, {}, "asset.import"));
    ...
    if (auto value = match(text, R"(- _Color: *(\{[^\r\n]+\}))")) {
        ...
        if (!parsed || !parsed.value().isObject())
            return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::ParseError, "invalid material color", source.path, {}, "asset.import"));
        unsigned i = 0;
        for (const auto key : {"r", "g", "b", "a"}) {
            ...
            if (entry == object.end() || !entry->second.isNumeric())
                return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::ParseError, "invalid material color component", source.path, {}, "asset.import"));
    ...
    if (auto main = match(text, R"(- _MainTex:([\s\S]*?)(?:\n    - |\n    m_Floats:))")) {
        ...
        if (guid) {
            if (mode.value() == 3)
                return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::Unsupported, "premultiplied Standard texture alpha requires a shader conversion", source.path, {}, "asset.import"));
            if (!match(*main, R"(m_Scale: *\{x: *(1), y: *1\})") || !match(*main, R"(m_Offset: *\{x: *(0), y: *0\})"))
                return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::Unsupported, "material texture transform is unsupported", source.path, {}, "asset.import"));
...
Result<void> bindUnityRenderers(const UnityProjectImportRequest& request, const UnitySourceIndex& index,
    ...
    for (const auto& source : index.assets) {
        ...
        if (entry == out.entries.end())
            return Result<void>::failure(Diagnostic::error(DiagnosticCode::NotFound, "scene definition absent", source.path, {}, "asset.import"));
        const auto&                     bytes = request.files.at(source.path);
        const std::string               text(bytes.begin(), bytes.end());
        ...
        for (const auto& doc : docs) {
            ...
            if (!target.emplace(*game, doc).second)
                return Result<void>::failure(Diagnostic::error(DiagnosticCode::Conflict, "duplicate Transform or MeshFilter on GameObject", source.path, {}, "asset.import"));
        }
        ...
        if (!object) return Result<void>::failure(Diagnostic::error(DiagnosticCode::ParseError, "invalid scene definition", source.path, {}, "asset.import"));
        auto* nodes = object->at("nodes").getIf<Value::Array>();
        if (!nodes) return Result<void>::failure(Diagnostic::error(DiagnosticCode::ParseError, "invalid scene nodes", source.path, {}, "asset.import"));
        ...
        for (const auto& doc : docs) {
            ...
            for (std::size_t slot = 0; slot < count; ++slot) {
                ...
                if (materialAsset == out.manifest.assets.end())
                    return Result<void>::failure(Diagnostic::error(DiagnosticCode::TypeMismatch, "renderer material definition is absent", source.path, {}, "asset.import"));
                ...
                if (split) {
                    if (nextNode == std::numeric_limits<std::int64_t>::max() ||
                        ...
                        return Result<void>::failure(Diagnostic::error(DiagnosticCode::InvalidArgument, "submesh node budget exceeded", source.path, {}, "asset.import"));
                    ++nextNode;
