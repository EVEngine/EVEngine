...
Result<float> sample(const VegetationPresetImage& image, const VegetationTextureChannelRecipe& recipe, float u,
    ...
    if (recipe.selector == "GET_RED")
        ...
        return Result<float>::failure(Diagnostic::error(DiagnosticCode::Unsupported,
                                                        "unsupported texture channel selector: " + recipe.selector, {},
                                                        {}, "asset.import.vegetation-preset.texture"));
    if (recipe.action == "ACTION_ONE_MINUS")
        value = 1.f - value;
    else if (!recipe.action.empty())
        return Result<float>::failure(Diagnostic::error(DiagnosticCode::Unsupported,
                                                        "unsupported texture channel action: " + recipe.action, {}, {},
                                                        "asset.import.vegetation-preset.texture"));
    ...
    for (std::size_t k = 0; k < mesh.indices.size(); k += 3) {
        ...
        if (a >= count || b >= count || c >= count)
            return Result<void>::failure(Diagnostic::error(DiagnosticCode::InvalidArgument,
                                                           "normal-space mesh index out of range", {}, {},
                                                           "asset.import.vegetation-preset.texture"));
        ...
        if (!std::isfinite(ax) || !std::isfinite(ay) || !std::isfinite(bx) || !std::isfinite(by) ||
            ...
            return Result<void>::failure(Diagnostic::error(DiagnosticCode::InvalidArgument,
                                                           "normal-space mesh contains nonfinite UVs", {}, {},
                                                           "asset.import.vegetation-preset.texture"));
        ...
        if (rasterWork > 512ull * 1024ull * 1024ull)
            return Result<void>::failure(Diagnostic::error(DiagnosticCode::InvalidArgument,
                                                           "normal-space raster work budget exceeded", {}, {},
                                                           "asset.import.vegetation-preset.texture"));
        ...
        if (binReferences > 32ull * 1024ull * 1024ull)
            return Result<void>::failure(Diagnostic::error(DiagnosticCode::InvalidArgument,
                                                           "normal-space triangle-bin budget exceeded", {}, {},
                                                           "asset.import.vegetation-preset.texture"));
...
Result<std::map<std::string, VegetationPresetImage>> executeVegetationTexturePacks(
    ...
    if (!maximumPixels)
        return Result<std::map<std::string, VegetationPresetImage>>::failure(
            Diagnostic::error(DiagnosticCode::InvalidArgument, "texture pixel budget must be positive", {}, {},
                              "asset.import.vegetation-preset.texture"));
    try {
        ...
        for (const auto& recipe : candidate.texturePacks) {
            if (recipe.targetProperty.empty())
                return Result<std::map<std::string, VegetationPresetImage>>::failure(
                    Diagnostic::error(DiagnosticCode::ParseError, "texture recipe has no target property", {}, {},
                                      "asset.import.vegetation-preset.texture"));
            if (recipe.transformSpace == "OBJECT_TO_TANGENT" || recipe.transformSpace == "TANGENT_TO_OBJECT")
                return Result<std::map<std::string, VegetationPresetImage>>::failure(Diagnostic::error(
            ...
            for (const auto& instruction : recipe.channels) {
                ...
                if (!valid(found->second, maximumPixels))
                    return Result<std::map<std::string, VegetationPresetImage>>::failure(
                        Diagnostic::error(DiagnosticCode::InvalidArgument, "invalid source texture image", {}, {},
                                          "asset.import.vegetation-preset.texture"));
            ...
            if (count > maximumPixels || count > SIZE_MAX / 4)
                return Result<std::map<std::string, VegetationPresetImage>>::failure(
                    Diagnostic::error(DiagnosticCode::InvalidArgument, "texture recipe exceeds pixel budget", {}, {},
                                      "asset.import.vegetation-preset.texture"));
        ...
        return Result<std::map<std::string, VegetationPresetImage>>::failure(
            Diagnostic::error(DiagnosticCode::Failed, "texture packing allocation failed", {}, {},
                              "asset.import.vegetation-preset.texture"));
    }
}
...
Result<std::map<std::string, VegetationPresetImage>> executeVegetationTexturePacks(
    ...
    try {
        ...
        return Result<std::map<std::string, VegetationPresetImage>>::failure(
            Diagnostic::error(DiagnosticCode::Failed, "normal-space texture conversion allocation failed", {}, {},
                              "asset.import.vegetation-preset.texture"));
    }
}
}  // namespace eve::asset_import
