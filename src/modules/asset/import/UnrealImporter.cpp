...
Result<std::string> requiredString(const Value::Object& object, std::string_view key, std::string path) {
    ...
    if (!value || !value->isString() || value->asString().empty())
        return Result<std::string>::failure(Diagnostic::error(DiagnosticCode::ParseError, "Unreal adapter field must be a non-empty string", std::move(path), {}, "asset.import"));
...
Result<std::uint32_t> requiredUint32(const Value::Object& object, std::string_view key, std::string path) {
    ...
    if (!value || !value->isInt64() || value->asInt() <= 0 ||
        ...
        return Result<std::uint32_t>::failure(Diagnostic::error(DiagnosticCode::ParseError, "Unreal adapter field must be a positive uint32", std::move(path), {}, "asset.import"));
...
Result<float> number(const Value& value, std::string path) {
    ...
    else return Result<float>::failure(Diagnostic::error(DiagnosticCode::ParseError, "Unreal numeric field is invalid", std::move(path), {}, "asset.import"));
    if (!std::isfinite(result) || result < -std::numeric_limits<float>::max() ||
        result > std::numeric_limits<float>::max())
        return Result<float>::failure(Diagnostic::error(DiagnosticCode::ParseError, "Unreal numeric field is non-finite", std::move(path), {}, "asset.import"));
...
Result<std::array<float, Count>> numberArray(const Value* value, std::string path) {
    ...
    if (!array || array->size() != Count)
        return Result<std::array<float, Count>>::failure(Diagnostic::error(DiagnosticCode::ParseError, "Unreal vector field has the wrong shape", std::move(path), {}, "asset.import"));
...
Result<PreparedAssetImport> prepareUnrealM4Import(const UnrealProjectImportRequest& request) {
    if (!safeProjectPath(request.descriptorPath, request.limits))
        return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::InvalidArgument, "Unreal adapter descriptor path is unsafe", {}, {}, "asset.import"));
    ...
    if (descriptorFile == request.files.end() || descriptorFile->second.size() > request.limits.maximumSourceBytes)
        return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::NotFound, "Unreal adapter descriptor was not supplied", request.descriptorPath, {}, "asset.import"));
    ...
    for (const auto& [path, bytes] : request.files) {
        if (!safeProjectPath(path, request.limits) || bytes.size() > request.limits.maximumSourceBytes ||
            ...
            return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::InvalidArgument, "Unreal source path or total budget is invalid", path, {}, "asset.import"));
    ...
    if (!root) return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::ParseError, "Unreal adapter descriptor root must be an object", {}, {}, "asset.import"));
    auto schema = requiredString(*root, "schema", "$.schema");
    auto version = requiredUint32(*root, "schemaVersion", "$.schemaVersion");
    if (!schema || !version) return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::ParseError, "Unreal descriptor envelope is invalid", {}, {}, "asset.import"));
    if (schema.value() != "eve.unreal-landscape-import" || version.value() != 1)
        return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::UnknownVersion, "unsupported Unreal adapter descriptor schema/version", {}, {}, "asset.import"));
    ...
    if (!landscape) return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::ParseError, "Unreal descriptor landscape is required", {}, {}, "asset.import"));
    ...
    if (!sourceWidth || !sourceHeight || !heightmap || !scale)
        return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::ParseError, "Unreal Landscape fields are invalid", {}, {}, "asset.import"));
    if (sourceWidth.value() < 2 || sourceHeight.value() < 2 || scale.value()[0] <= 0 ||
        ...
        return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::InvalidArgument, "Unreal Landscape dimensions/scale/path are invalid", {}, {}, "asset.import"));
    ...
    if (heightFile == request.files.end() || sampleCount > request.limits.maximumDecodedBytes / sizeof(float) ||
        ...
        return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::ParseError, "Unreal Landscape R16 byte count is invalid", heightmap.value(), {}, "asset.import"));
    ...
    if (layers) for (std::size_t index = 0; index < layers->size(); ++index) {
        ...
        if (!layer) return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::ParseError, "M4 layer must be an object", {}, {}, "asset.import"));
        ...
        if (const Value* diffuse = member(*layer, "diffuse")) {
            if (!diffuse->isString() || !safeProjectPath(diffuse->asString(), request.limits))
                return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::InvalidArgument, "M4 diffuse path is invalid", {}, {}, "asset.import"));
        ...
        if (const Value* normal = member(*layer, "normal")) {
            if (!normal->isString() || !safeProjectPath(normal->asString(), request.limits))
                return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::InvalidArgument, "M4 normal path is invalid", {}, {}, "asset.import"));
        ...
        if (const Value* weight = member(*layer, "weight")) {
            if (!weight->isString() || !safeProjectPath(weight->asString(), request.limits))
                return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::InvalidArgument, "M4 weight path is invalid", {}, {}, "asset.import"));
        ...
        if (const Value* tile = member(*layer, "tileSizeCm")) {
            ...
            if (!parsedTile || parsedTile.value() <= 0) return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::InvalidArgument, "M4 tile size is invalid", {}, {}, "asset.import"));
    ...
    if (grass) for (std::size_t index = 0; index < grass->size(); ++index) {
        ...
        if (!rule) return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::ParseError, "M4 grass rule must be an object", {}, {}, "asset.import"));
        ...
        if (!id || !prototype || !layer || !densityValue)
            return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::ParseError, "M4 grass fields are incomplete", {}, {}, "asset.import"));
        ...
        if (const Value* seed = member(*rule, "seed")) {
            if (!seed->isInt64() || seed->asInt() < 0)
                return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::ParseError, "M4 grass seed is invalid", {}, {}, "asset.import"));
    ...
    if (instances) for (const auto& value : *instances) {
        ...
        if (!instance) return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::ParseError, "UE instance must be an object", {}, {}, "asset.import"));
        ...
        if (!prototype || !position || !rotation || !scaleValue)
            return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::ParseError, "UE instance transform is invalid", {}, {}, "asset.import"));
    ...
    if (const Value* unsupportedValue = member(*root, "unsupported")) {
        ...
        if (!unsupported) return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::ParseError, "unsupported must be an array", {}, {}, "asset.import"));
        for (const auto& feature : *unsupported) {
            if (!feature.isString()) return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::ParseError, "unsupported feature must be a string", {}, {}, "asset.import"));
