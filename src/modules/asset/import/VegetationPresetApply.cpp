...
Result<double> number(std::string_view text, const VegetationPresetCommand& command) {
    ...
    if (errno != 0 || end != token.c_str() + token.size() || !std::isfinite(value))
        return Result<double>::failure(Diagnostic::error(DiagnosticCode::ParseError,
                                                         "preset numeric argument is invalid", command.sourcePath, {},
...
Result<std::array<double, 4>> vector(const VegetationPresetCommand& command) {
    if (command.arguments.size() != 5)
        return Result<std::array<double, 4>>::failure(
            Diagnostic::error(DiagnosticCode::ParseError, "preset vector requires property and four values",
                              command.sourcePath, {}, "asset.import.vegetation-preset.apply"));
    ...
    if (command.operation == "SET_FLOAT") {
        if (a.size() != 2)
            return Result<void>::failure(Diagnostic::error(DiagnosticCode::ParseError,
                                                           "SET_FLOAT requires property and value", command.sourcePath,
                                                           {}, "asset.import.vegetation-preset.apply"));
        ...
        if (a.size() != 2 && !ignoredLegacyFloatArgument)
            return Result<void>::failure(
                Diagnostic::error(DiagnosticCode::ParseError, "COPY operation requires source and destination",
                                  command.sourcePath, {}, "asset.import.vegetation-preset.apply"));
        ...
        if (a.size() != 2)
            return Result<void>::failure(
                Diagnostic::error(DiagnosticCode::ParseError, "COPY_ST_AS_VECTOR requires source and destination",
                                  command.sourcePath, {}, "asset.import.vegetation-preset.apply"));
        ...
        if (a.size() != 2)
            return Result<void>::failure(
                Diagnostic::error(DiagnosticCode::ParseError, "float-to-vector copy requires source and destination",
                                  command.sourcePath, {}, "asset.import.vegetation-preset.apply"));
        static constexpr std::string_view axes = "XYZW";
        const auto                        axis = axes.find(command.operation.back());
        if (axis == std::string_view::npos)
            return Result<void>::failure(Diagnostic::error(DiagnosticCode::ParseError,
                                                           "float-to-vector axis is invalid", command.sourcePath, {},
                                                           "asset.import.vegetation-preset.apply"));
        if (auto v = out.materialFloats.find(a[0]); v != out.materialFloats.end())
        ...
        if (a.size() != 2)
            return Result<void>::failure(
                Diagnostic::error(DiagnosticCode::ParseError, "vector-to-float copy requires source and destination",
                                  command.sourcePath, {}, "asset.import.vegetation-preset.apply"));
        static constexpr std::string_view axes = "XYZW";
        const auto                        axis = axes.find(command.operation[12]);
        if (axis == std::string_view::npos)
            return Result<void>::failure(Diagnostic::error(DiagnosticCode::ParseError,
                                                           "vector-to-float axis is invalid", command.sourcePath, {},
                                                           "asset.import.vegetation-preset.apply"));
        if (auto v = out.materialVectors.find(a[0]); v != out.materialVectors.end())
            out.materialFloats[a[1]] = v->second[axis];
    } else if (command.operation == "SET_SHADER") {
        if (a.size() != 1)
            return Result<void>::failure(Diagnostic::error(DiagnosticCode::ParseError, "SET_SHADER requires one alias",
                                                           command.sourcePath, {},
                                                           "asset.import.vegetation-preset.apply"));
        ...
        if (a.empty())
            return Result<void>::failure(Diagnostic::error(DiagnosticCode::ParseError,
                                                           "SET_SHADER_BY_NAME requires a name", command.sourcePath, {},
                                                           "asset.import.vegetation-preset.apply"));
        out.materialShader = join(a);
    } else if (command.operation == "SET_SHADER_BY_LIGHTING") {
        if (a.size() != 1)
            return Result<void>::failure(
                Diagnostic::error(DiagnosticCode::ParseError, "SET_SHADER_BY_LIGHTING requires one mode",
                                  command.sourcePath, {}, "asset.import.vegetation-preset.apply"));
        out.materialLighting = a[0];
    } else if (command.operation == "ENABLE_KEYWORD" || command.operation == "LOCK_PROP") {
        if (a.size() != 1)
            return Result<void>::failure(
                Diagnostic::error(DiagnosticCode::ParseError, "keyword/property command requires one name",
                                  command.sourcePath, {}, "asset.import.vegetation-preset.apply"));
        ...
        if (!a.empty())
            return Result<void>::failure(Diagnostic::error(DiagnosticCode::ParseError,
                                                           "instancing command takes no arguments", command.sourcePath,
                                                           {}, "asset.import.vegetation-preset.apply"));
        out.materialInstancing = command.operation == "ENABLE_INSTANCING";
    } else if (command.operation == "COPY_TEX_FIRST_VALID") {
        if (a.size() != 1)
            return Result<void>::failure(
                Diagnostic::error(DiagnosticCode::ParseError, "COPY_TEX_FIRST_VALID requires a destination",
                                  command.sourcePath, {}, "asset.import.vegetation-preset.apply"));
        out.firstValidTextureDestinations.insert(a[0]);
    } else {
        return Result<void>::failure(Diagnostic::error(DiagnosticCode::ParseError,
                                                       "unknown Material preset operation: " + command.operation,
                                                       command.sourcePath, {}, "asset.import.vegetation-preset.apply"));
    }
        ...
        if (command.domain == "Material") {
            ...
            if (!meshOperation(command.operation) || command.arguments.empty())
                return Result<VegetationConversionCandidate>::failure(
                    Diagnostic::error(DiagnosticCode::ParseError, "Mesh rule is malformed", command.sourcePath, {},
                                      "asset.import.vegetation-preset.apply"));
            ...
            if (!utilities.contains(command.operation) || !command.arguments.empty())
                return Result<VegetationConversionCandidate>::failure(
                    Diagnostic::error(DiagnosticCode::ParseError, "Utility command takes no arguments",
                                      command.sourcePath, {}, "asset.import.vegetation-preset.apply"));
            ...
            if (out.texturePacks.empty())
                return Result<VegetationConversionCandidate>::failure(
                    Diagnostic::error(DiagnosticCode::ParseError, "Texture command requires START_TEXTURE_PACKING",
                                      command.sourcePath, {}, "asset.import.vegetation-preset.apply"));
            ...
            if (command.operation == "PropName" || command.operation == "ImportType" ||
                ...
                if (command.arguments.size() != 1)
                    return Result<VegetationConversionCandidate>::failure(
                        Diagnostic::error(DiagnosticCode::ParseError, "Texture property command requires one argument",
                                          command.sourcePath, {}, "asset.import.vegetation-preset.apply"));
                ...
                if (found == channels.end() ||
                    ...
                    return Result<VegetationConversionCandidate>::failure(
                        Diagnostic::error(DiagnosticCode::ParseError, "Texture channel command is malformed",
                                          command.sourcePath, {}, "asset.import.vegetation-preset.apply"));
            ...
            if (!command.operation.starts_with("SHADER_") || command.arguments.empty())
                return Result<VegetationConversionCandidate>::failure(
                    Diagnostic::error(DiagnosticCode::ParseError, "Shader alias is malformed", command.sourcePath, {},
                                      "asset.import.vegetation-preset.apply"));
            ...
            return Result<VegetationConversionCandidate>::failure(
                Diagnostic::error(DiagnosticCode::ParseError, "unknown preset command domain: " + command.domain,
                                  command.sourcePath, {}, "asset.import.vegetation-preset.apply"));
        }
    }
