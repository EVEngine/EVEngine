...
Result<std::string> texturePath(const UnityProjectImportRequest& request, const std::string& guid) {
    ...
    for (const auto& [path, bytes] : request.files)
        if (path.ends_with(".meta")) {
            ...
            if (!found.empty())
                return Result<std::string>::failure(Diagnostic::error(DiagnosticCode::Conflict, "duplicate texture GUID", guid, {}, "asset.import"));
            found = path.substr(0, path.size() - 5);
        }
    if (found.empty() || !request.files.contains(found))
        return Result<std::string>::failure(Diagnostic::error(DiagnosticCode::NotFound, "TVE texture source is absent", guid, {}, "asset.import"));
...
Result<Value::Object> textureSampler(const UnityProjectImportRequest& request, const std::string& path) {
    ...
    for (const auto wrap : {u, v}) {
        ...
        if (wrap == 3)
            return Result<Value::Object>::failure(Diagnostic::error(DiagnosticCode::Unsupported, "texture MirrorOnce addressing is not supported by the canonical sampler", path, {}, "asset.import"));
    }
...
Result<PreparedAssetImport> normalImage(const UnityProjectImportRequest& request, const std::string& path,
    ...
    for (const auto& entry : image.value().entries) {
        ...
        if (mipmaps && importerScalar(meta, "mipMapMode", 0) != 0)
            return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::Unsupported, "normal Kaiser mips are not implemented", path, {}, "asset.import"));
        ...
        if (targetWidth != width || targetHeight != height) {
            ...
            for (auto it = std::sregex_iterator(meta.begin(), meta.end(), algorithms); it != std::sregex_iterator();
                ...
                if (selected && *selected != int(algorithm))
                    return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::Unsupported, "platform-specific normal resize algorithms require target-specific image variants", path, {}, "asset.import"));
            ...
            if (uint64_t(targetWidth) * targetHeight > request.limits.maximumDecodedBytes / sizeof(double) / 3)
                return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::InvalidArgument, "resized normal RGB staging exceeds budget", path, {}, "asset.import"));
        ...
        do {
            ...
            if (request.limits.maximumDecodedBytes < 28 || total > request.limits.maximumDecodedBytes - 28 ||
                ...
                return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::InvalidArgument, "normal mip chain exceeds budget", path, {}, "asset.import"));
            total += bytes;
            ++levels;
        ...
        if (total > request.limits.maximumSourceBytes ||
            ...
            return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::InvalidArgument, "normal source or channel staging exceeds budget", path, {}, "asset.import"));
    ...
    return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::NotFound, "normal image definition absent", path, {}, "asset.import"));
}
}  // namespace
...
Result<PreparedAssetImport> prepareUnityVegetationMaterial(const UnityProjectImportRequest& request,
    ...
    try {
        ...
        if (!shader ||
            ...
            return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::Unsupported, "shader is not an admitted TVE 12.6.0 Plant/Prop material", source.path, {}, "asset.import"));
        auto unit = [](double v) {
        ...
        if ((mode != 0 && mode != 1) || cull < 0 || cull > 2 || cull != std::floor(cull) ||
            ...
            return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::InvalidArgument, "TVE render mode, alpha clipping, or cull mode is invalid", source.path, {}, "asset.import"));
        ...
        return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::ParseError, "invalid TVE material saved properties", source.path, {}, "asset.import"));
    }
}
}  // namespace eve::asset_import
