...
Result<void> mergeImport(PreparedAssetImport& output, PreparedAssetImport input, const UnitySourceAsset& source,
    ...
    if (input.manifest.assets.size() > limits.maximumAssets ||
        ...
        return Result<void>::failure(Diagnostic::error(DiagnosticCode::InvalidArgument, "Unity imported asset count exceeds budget", {}, {}, "asset.import"));
...
Result<PreparedAssetImport> importSource(const UnityProjectImportRequest& request, const UnitySourceAsset& source) {
    if (source.guid.empty())
        return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::Unsupported, "a .meta GUID is required for stable collection asset identity", source.path, {}, "asset.import"));
    ...
    return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::Unsupported, "source retained; this Unity resource type does not yet have a canonical converter", source.path, {}, "asset.import"));
}
}  // namespace
...
Result<PreparedAssetImport> prepareUnityCollection(const UnityProjectImportRequest& request,
    ...
    for (const auto& source : index.assets) {
        ...
        if (expanded.value().bytes.size() > request.limits.maximumDecodedBytes ||
            ...
            return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::InvalidArgument, "aggregate Prefab expansion budget exceeded", {}, {}, "asset.import"));
    ...
    if (out.manifest.assets.empty())
        return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::Unsupported, "Unity collection has no convertible assets; use asset scan to inspect all resource types", {}, {}, "asset.import"));
    ...
    for (const auto& entry : out.entries) {
        if (entry.bytes.size() > request.limits.maximumDecodedBytes ||
            ...
            return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::InvalidArgument, "Unity prepared output exceeds byte budget", {}, {}, "asset.import"));
    ...
    if (out.entries.back().bytes.size() > request.limits.maximumDecodedBytes ||
        ...
        return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::InvalidArgument, "Unity import report exceeds prepared output budget", {}, {}, "asset.import"));
