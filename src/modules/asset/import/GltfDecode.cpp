...
Result<Document> parseDocument(const GltfImportRequest& request) {
    if (request.documentBytes.empty() || request.documentBytes.size() > request.limits.maximumSourceBytes)
        return Result<Document>::failure(Diagnostic::error(
            DiagnosticCode::InvalidArgument, "glTF document size is outside limits", {}, {}, "asset.import"));
    std::string               json;
    ...
    if (bytes.size() >= 12 && little32(bytes, 0) == 0x46546c67) {
        if (little32(bytes, 4) != 2 || little32(bytes, 8) != bytes.size() || bytes.size() < 20)
            return Result<Document>::failure(
                Diagnostic::error(DiagnosticCode::ParseError, "GLB header is invalid", {}, {}, "asset.import"));
        ...
        if (jsonType != 0x4e4f534a || jsonSize > bytes.size() - cursor)
            return Result<Document>::failure(
                Diagnostic::error(DiagnosticCode::ParseError, "GLB JSON chunk is invalid", {}, {}, "asset.import"));
        ...
        if (cursor < bytes.size()) {
            if (bytes.size() - cursor < 8)
                return Result<Document>::failure(Diagnostic::error(
                    DiagnosticCode::ParseError, "GLB trailing chunk is truncated", {}, {}, "asset.import"));
            ...
            if (binaryType != 0x004e4942 || binarySize > bytes.size() - cursor || cursor + binarySize != bytes.size())
                return Result<Document>::failure(
                    Diagnostic::error(DiagnosticCode::ParseError, "GLB BIN chunk is invalid", {}, {}, "asset.import"));
...
Result<std::vector<std::span<const std::uint8_t>>> resolveBuffers(const Value::Object& root, const Document& document,
    ...
    if (!buffers || buffers->empty())
        return Result<std::vector<std::span<const std::uint8_t>>>::failure(Diagnostic::error(
            DiagnosticCode::ParseError, "glTF buffers are required", "$.buffers", {}, "asset.import"));
    ...
    for (std::size_t index = 0; index < buffers->size(); ++index) {
        ...
        if (!object)
            return Result<std::vector<std::span<const std::uint8_t>>>::failure(
                Diagnostic::error(DiagnosticCode::ParseError, "glTF buffer must be an object", {}, {}, "asset.import"));
        auto declared =
        ...
        if (uriValue) {
            if (!uriValue->isString() || !safeUri(uriValue->asString(), request.limits))
                return Result<std::vector<std::span<const std::uint8_t>>>::failure(
                    Diagnostic::error(DiagnosticCode::Unsupported, "external glTF buffer URI is unsafe or unsupported",
                                      {}, {}, "asset.import"));
            const auto found = request.externalResources.find(uriValue->asString());
            if (found == request.externalResources.end())
                return Result<std::vector<std::span<const std::uint8_t>>>::failure(
                    Diagnostic::error(DiagnosticCode::NotFound, "external glTF buffer was not supplied",
                                      uriValue->asString(), {}, "asset.import"));
            data = found->second;
        } else {
            if (index != 0 || document.binaryChunk.empty())
                return Result<std::vector<std::span<const std::uint8_t>>>::failure(
                    Diagnostic::error(DiagnosticCode::ParseError, "buffer without URI requires the first GLB BIN chunk",
                                      {}, {}, "asset.import"));
            data = document.binaryChunk;
        }
        if (declared.value() > data.size())
            return Result<std::vector<std::span<const std::uint8_t>>>::failure(Diagnostic::error(
                DiagnosticCode::ParseError, "glTF buffer is shorter than byteLength", {}, {}, "asset.import"));
        if (declared.value() > request.limits.maximumSourceBytes ||
            totalBytes > request.limits.maximumSourceBytes - declared.value())
            return Result<std::vector<std::span<const std::uint8_t>>>::failure(Diagnostic::error(
                DiagnosticCode::InvalidArgument, "glTF buffer budget is exceeded", {}, {}, "asset.import"));
...
Result<Accessor> accessorAt(const Value::Object& root, const std::vector<std::span<const std::uint8_t>>& buffers,
    ...
    if (!accessors || !views || accessorIndex >= accessors->size())
        return Result<Accessor>::failure(
            Diagnostic::error(DiagnosticCode::ParseError, "glTF accessor index is invalid", {}, {}, "asset.import"));
    const auto* object = (*accessors)[accessorIndex].getIf<Value::Object>();
    if (!object || member(*object, "sparse"))
        return Result<Accessor>::failure(Diagnostic::error(
            DiagnosticCode::Unsupported, "sparse or malformed glTF accessor is unsupported", {}, {}, "asset.import"));
    ...
    if (!viewIndex || !count || !component || !accessorOffset)
        return Result<Accessor>::failure(
            Diagnostic::error(DiagnosticCode::ParseError, "glTF accessor fields are invalid", {}, {}, "asset.import"));
    if (viewIndex.value() >= views->size() || count.value() == 0 ||
        ...
        return Result<Accessor>::failure(
            Diagnostic::error(DiagnosticCode::InvalidArgument, "glTF accessor exceeds limits", {}, {}, "asset.import"));
    const Value* typeValue = member(*object, "type");
    if (!typeValue || !typeValue->isString())
        return Result<Accessor>::failure(
            Diagnostic::error(DiagnosticCode::ParseError, "glTF accessor type is missing", {}, {}, "asset.import"));
    ...
    if (typeValue->asString() == "SCALAR")
        ...
        return Result<Accessor>::failure(Diagnostic::error(
            DiagnosticCode::Unsupported, "glTF accessor shape is unsupported", {}, {}, "asset.import"));
    const auto* view = (*views)[viewIndex.value()].getIf<Value::Object>();
    if (!view)
        return Result<Accessor>::failure(
            Diagnostic::error(DiagnosticCode::ParseError, "glTF bufferView is malformed", {}, {}, "asset.import"));
    ...
    if (!bufferIndex || !viewOffset || !viewLength || !byteStride || bufferIndex.value() >= buffers.size())
        return Result<Accessor>::failure(Diagnostic::error(
            DiagnosticCode::ParseError, "glTF bufferView fields are invalid", {}, {}, "asset.import"));
    ...
    if (elementSize == 0 || stride < elementSize || stride > 252 ||
        ...
        return Result<Accessor>::failure(
            Diagnostic::error(DiagnosticCode::ParseError, "glTF accessor layout is invalid", {}, {}, "asset.import"));
    const std::uint64_t required = (count.value() - 1) * stride + elementSize;
    if (required > viewLength.value() - accessorOffset.value())
        return Result<Accessor>::failure(
            Diagnostic::error(DiagnosticCode::ParseError, "glTF accessor range is truncated", {}, {}, "asset.import"));
    const auto* normalized = member(*object, "normalized");
    if (normalized && !normalized->isBool())
        return Result<Accessor>::failure(Diagnostic::error(
            DiagnosticCode::ParseError, "glTF normalized flag must be boolean", {}, {}, "asset.import"));
...
Result<float> readFloat(const Accessor& accessor, std::uint32_t element, std::uint32_t component) {
    if (accessor.componentType != 5126 || accessor.normalized || element >= accessor.count ||
        ...
        return Result<float>::failure(Diagnostic::error(
            DiagnosticCode::Unsupported, "mesh attribute must use FLOAT components", {}, {}, "asset.import"));
    ...
    if (!std::isfinite(value))
        return Result<float>::failure(Diagnostic::error(
            DiagnosticCode::ParseError, "mesh attribute contains non-finite values", {}, {}, "asset.import"));
...
Result<std::uint32_t> readIndex(const Accessor& accessor, std::uint32_t element) {
    if (accessor.components != 1)
        return Result<std::uint32_t>::failure(
            Diagnostic::error(DiagnosticCode::ParseError, "index accessor must be SCALAR", {}, {}, "asset.import"));
    ...
    return Result<std::uint32_t>::failure(
        Diagnostic::error(DiagnosticCode::Unsupported, "indices must use UNSIGNED_BYTE, UNSIGNED_SHORT or UNSIGNED_INT",
                          {}, {}, "asset.import"));
}
