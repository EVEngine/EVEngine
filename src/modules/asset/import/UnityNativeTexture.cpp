...
Result<PreparedAssetImport> prepareUnityNativeTexture(const UnityProjectImportRequest& request,
    ...
    if (text.find("\nTexture2D:") == std::string_view::npos)
        return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::Unsupported, "native asset is not a text Texture2D", source.path, {}, "asset.import"));
    try {
        ...
        if (number(text, "m_TextureFormat") != 12 || number(text, "m_TextureDimension") != 2 ||
            ...
            return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::Unsupported, "requires embedded BC3 Texture2D with one image and known color space", source.path, {}, "asset.import"));
        ...
        if (!parsed || !parsed.value().isObject())
            return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::ParseError, "generated native texture definition is invalid", source.path, {}, "asset.import"));
