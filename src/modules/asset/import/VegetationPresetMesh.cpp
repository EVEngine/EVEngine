...
Result<T> failure(DiagnosticCode code, std::string message) {
    return Result<T>::failure(
        Diagnostic::error(code, std::move(message), {}, {}, "asset.import.vegetation-preset.mesh"));
}
...
Result<int> integer(std::string_view value) {
    ...
    if (ec != std::errc{} || end != value.data() + value.size())
        return failure<int>(DiagnosticCode::ParseError, "mesh rule option is not an integer");
...
Result<std::vector<float>> mask(const asset::CanonicalMeshData& mesh, const std::vector<std::string>& rule,
    ...
    if (rule.size() < 2) return failure<std::vector<float>>(DiagnosticCode::ParseError, "mesh mask rule lacks option");
    ...
    if (rule[0] == "GET_MASK_FROM_CHANNEL") {
        ...
        for (std::size_t i = 0; i < count; ++i) {
            ...
            switch (mode) {
                ...
                default:
                    return failure<std::vector<float>>(DiagnosticCode::Unsupported,
                                                       "unsupported procedural mesh mask mode");
            }
        }
    } else if (rule[0] == "GET_MASK_FROM_TEXTURE") {
        if (rule.size() < 3)
            return failure<std::vector<float>>(DiagnosticCode::ParseError, "texture mask lacks property");
        ...
        if (!image->second.width || !image->second.height || pixels > 16ull * 1024ull * 1024ull ||
            ...
            return failure<std::vector<float>>(DiagnosticCode::InvalidArgument, "invalid texture mask input");
        int coord = 0;
        ...
        for (std::size_t i = 0; i < count; ++i) {
            ...
            if (option.value() == 0 || option.value() == 1) {
                ...
                return failure<std::vector<float>>(DiagnosticCode::Unsupported,
                                                   "unsupported third-party vegetation mask mode");
            }
        }
    } else {
        return failure<std::vector<float>>(DiagnosticCode::Unsupported,
                                           "mesh mask source requires texture or vendor adapter");
    }
    if (rule.size() >= 3 && rule.back().starts_with("ACTION_")) {
        ...
        if (action == "ACTION_ONE_MINUS")
            ...
            return failure<std::vector<float>>(DiagnosticCode::Unsupported, "unsupported mesh mask action");
    }
...
Result<void> applyNormals(asset::CanonicalMeshData& mesh, const std::vector<std::string>& rule, float height) {
    if (rule.size() != 2 || rule[0] != "GET_NORMALS_PROCEDURAL")
        return failure<void>(DiagnosticCode::Unsupported, "unsupported normal rule");
    auto mode = integer(rule[1]);
    if (!mode || mode.value() < 0 || mode.value() > 6)
        return failure<void>(DiagnosticCode::ParseError, "invalid normal mode");
    ...
    if (mode.value() == 0) {
        ...
        for (std::size_t i = 0; i + 2 < mesh.indices.size(); i += 3) {
            ...
            if (a >= count || b >= count || c >= count)
                return failure<void>(DiagnosticCode::InvalidArgument, "mesh index out of range");
...
Result<void> recalculateTangents(asset::CanonicalMeshData& mesh) {
    ...
    if (uv->second.size() != count * 2)
        return failure<void>(DiagnosticCode::InvalidArgument, "canonical UV0 has an invalid tangent input size");
    ...
    for (std::size_t k = 0; k + 2 < mesh.indices.size(); k += 3) {
        ...
        if (a >= count || b >= count || c >= count)
            return failure<void>(DiagnosticCode::InvalidArgument, "mesh index out of range while rebuilding tangents");
...
Result<asset::CanonicalMeshData> executeVegetationMeshRules(
    ...
    if (!count || source.positions.size() != count * 3 || source.normals.size() != count * 3 ||
        ...
        return failure<asset::CanonicalMeshData>(DiagnosticCode::InvalidArgument, "invalid mesh geometry or seed");
    try {
        ...
        if (auto found = candidate.meshRules.find("SetPivots"); found != candidate.meshRules.end()) {
            if (found->second.size() == 1 && found->second[0] == "NONE") {
                ...
                if (found->second.size() != 2 || found->second[0] != "GET_PIVOTS_PROCEDURAL" || found->second[1] != "0")
                    return failure<asset::CanonicalMeshData>(DiagnosticCode::Unsupported, "unsupported pivot rule");
        ...
        return failure<asset::CanonicalMeshData>(DiagnosticCode::Failed, "mesh conversion allocation failed");
    }
}
