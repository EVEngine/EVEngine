...
Result<Fixture> parseFixture(const LegacyAnimationImportRequest& request) {
    if (request.text.size() > request.limits.maximumSourceBytes)
        return Result<Fixture>::failure(Diagnostic::error(
            DiagnosticCode::InvalidArgument, "animation fixture exceeds source budget", {}, {}, "asset.import"));
    Fixture result;
    ...
    while (std::getline(input, text)) {
        ...
        if (!magic) {
            ...
            if (tag != "EVA" || !(line >> version) || version != 1 || !complete(line))
                return Result<Fixture>::failure(Diagnostic::error(DiagnosticCode::UnknownVersion,
            ...
            if (!(line >> declaredBones) || declaredBones == 0 ||
                ...
                return Result<Fixture>::failure(
                    Diagnostic::error(DiagnosticCode::ParseError, "invalid skeleton count", path, {}, "asset.import"));
            ...
            if (!(line >> index >> bone.parent) || index != result.bones.size())
                return Result<Fixture>::failure(
                    Diagnostic::error(DiagnosticCode::ParseError, "invalid bone index", path, {}, "asset.import"));
            ...
            if (bone.name.empty() || bone.name.size() > request.limits.maximumStringBytes ||
                ...
                return Result<Fixture>::failure(
                    Diagnostic::error(DiagnosticCode::ParseError, "invalid bone hierarchy", path, {}, "asset.import"));
            result.bones.push_back(std::move(bone));
        } else if (tag == "bind") {
            std::uint32_t index = 0;
            if (!(line >> index) || index >= result.bones.size())
                return Result<Fixture>::failure(
                    Diagnostic::error(DiagnosticCode::ParseError, "invalid bind index", path, {}, "asset.import"));
            ...
            if (!(line >> bone.position[0] >> bone.position[1] >> bone.position[2] >>
                ...
                return Result<Fixture>::failure(
                    Diagnostic::error(DiagnosticCode::ParseError, "invalid bind transform", path, {}, "asset.import"));
            bone.hasBind = true;
        } else if (tag == "clip") {
            ...
            if (result.clipName.empty() || result.clipName.size() > request.limits.maximumStringBytes)
                return Result<Fixture>::failure(
                    Diagnostic::error(DiagnosticCode::ParseError, "invalid clip name", path, {}, "asset.import"));
        } else if (tag == "duration") {
            if (!(line >> result.duration) || !complete(line) || !finite(result.duration) || result.duration < 0)
                return Result<Fixture>::failure(
                    Diagnostic::error(DiagnosticCode::ParseError, "invalid clip duration", path, {}, "asset.import"));
        } else if (tag == "rate") {
            if (!(line >> result.rate) || !complete(line) || !finite(result.rate) || result.rate <= 0)
                return Result<Fixture>::failure(
                    Diagnostic::error(DiagnosticCode::ParseError, "invalid sample rate", path, {}, "asset.import"));
        } else if (tag == "loop") {
            int value = 0;
            if (!(line >> value) || (value != 0 && value != 1) || !complete(line))
                return Result<Fixture>::failure(
                    Diagnostic::error(DiagnosticCode::ParseError, "invalid loop flag", path, {}, "asset.import"));
            result.loop = value != 0;
        } else if (tag == "sync") {
            Marker marker;
            if (!(line >> marker.time) || !finite(marker.time))
                return Result<Fixture>::failure(
                    Diagnostic::error(DiagnosticCode::ParseError, "invalid sync marker", path, {}, "asset.import"));
            std::getline(line, marker.name); marker.name = trim(std::move(marker.name));
            if (marker.name.empty())
                return Result<Fixture>::failure(
                    Diagnostic::error(DiagnosticCode::ParseError, "empty sync marker", path, {}, "asset.import"));
            result.markers.push_back(std::move(marker));
            if (result.markers.size() > request.limits.maximumAssets)
                return Result<Fixture>::failure(Diagnostic::error(DiagnosticCode::InvalidArgument,
                                                                  "too many sync markers", path, {}, "asset.import"));
        } else if (tag == "track") {
            Track value;
            if (!(line >> value.bone >> value.expectedPosition >> value.expectedRotation >> value.expectedScale) ||
                ...
                return Result<Fixture>::failure(
                    Diagnostic::error(DiagnosticCode::ParseError, "invalid track header", path, {}, "asset.import"));
            result.tracks.push_back(std::move(value)); track = &result.tracks.back();
            if (result.tracks.size() > request.limits.maximumAssets)
                return Result<Fixture>::failure(Diagnostic::error(
                    DiagnosticCode::InvalidArgument, "too many animation tracks", path, {}, "asset.import"));
        } else if (tag == "p" || tag == "s") {
            VecKey key;
            if (!track || !(line >> key.time >> key.value[0] >> key.value[1] >> key.value[2]) ||
                ...
                return Result<Fixture>::failure(
                    Diagnostic::error(DiagnosticCode::ParseError, "invalid vector key", path, {}, "asset.import"));
            ...
            if (!track || !(line >> key.time >> key.value[0] >> key.value[1] >> key.value[2] >> key.value[3]) ||
                ...
                return Result<Fixture>::failure(
                    Diagnostic::error(DiagnosticCode::ParseError, "invalid rotation key", path, {}, "asset.import"));
            track->rotation.push_back(key);
        } else if (tag == "end") {
            if (!complete(line))
                return Result<Fixture>::failure(
                    Diagnostic::error(DiagnosticCode::ParseError, "invalid end", path, {}, "asset.import"));
            ended = true; break;
        } else {
            return Result<Fixture>::failure(Diagnostic::error(
                DiagnosticCode::ParseError, "unknown animation fixture record", path, {}, "asset.import"));
        }
    }
    if (!magic || !ended || result.bones.size() != declaredBones || result.clipName.empty() || result.rate <= 0)
        return Result<Fixture>::failure(
            Diagnostic::error(DiagnosticCode::ParseError, "animation fixture is incomplete", {}, {}, "asset.import"));
    for (const auto& bone : result.bones)
        if (!bone.hasBind)
            return Result<Fixture>::failure(
                Diagnostic::error(DiagnosticCode::ParseError, "bone bind is missing", bone.name, {}, "asset.import"));
    for (const auto& value : result.tracks)
        if (value.position.size() != value.expectedPosition || value.rotation.size() != value.expectedRotation ||
            ...
            return Result<Fixture>::failure(Diagnostic::error(
                DiagnosticCode::ParseError, "track key count does not match declaration", {}, {}, "asset.import"));
    ...
    for (const auto& value : result.tracks) {
        if (!trackBones.emplace(value.bone).second || !validTimes(value.position) ||
            ...
            return Result<Fixture>::failure(Diagnostic::error(
                DiagnosticCode::Conflict, "tracks must be unique and key times canonical", {}, {}, "asset.import"));
        ...
        if (keys > request.limits.maximumDecodedBytes / 24 ||
            ...
            return Result<Fixture>::failure(Diagnostic::error(
                DiagnosticCode::InvalidArgument, "animation key data exceeds decoded budget", {}, {}, "asset.import"));
