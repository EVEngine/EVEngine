...
Result<PrimitiveOutput> decodePrimitive(const Value::Object& root, const Value::Object& primitive,
    ...
    if (mode.value() != 4)
        return Result<PrimitiveOutput>::failure(Diagnostic::error(DiagnosticCode::Unsupported, "only glTF TRIANGLES primitives are supported", {}, {}, "asset.import"));
    ...
    if (!attributes)
        return Result<PrimitiveOutput>::failure(Diagnostic::error(DiagnosticCode::ParseError, "primitive attributes are required", {}, {}, "asset.import"));
    ...
    if (positions.value().count == 0 || positions.value().components != 3 || positions.value().componentType != 5126 ||
        ...
        return Result<PrimitiveOutput>::failure(Diagnostic::error(DiagnosticCode::Unsupported, "POSITION must be a bounded FLOAT VEC3 accessor", {}, {}, "asset.import"));
    ...
    if (const Value* normal = member(*attributes, "NORMAL")) {
        ...
        if (decoded.value().components != 3 || decoded.value().componentType != 5126 ||
            ...
            return Result<PrimitiveOutput>::failure(Diagnostic::error(DiagnosticCode::Unsupported, "NORMAL must match POSITION", {}, {}, "asset.import"));
    ...
    for (const auto& [name, texcoord] : *attributes) {
        ...
        if (parsed.ec != std::errc{} || parsed.ptr != suffix.data() + suffix.size() || std::to_string(set) != suffix)
            return Result<PrimitiveOutput>::failure(Diagnostic::error(DiagnosticCode::ParseError, "invalid UV set semantic", {}, {}, "asset.import"));
        ...
        if (uv.components != 2 || uv.count != positions.value().count ||
            ...
            return Result<PrimitiveOutput>::failure(Diagnostic::error(DiagnosticCode::Unsupported, "UV sets must match POSITION and use FLOAT or normalized unsigned VEC2", {}, {}, "asset.import"));
    ...
    for (const auto& [name, value] : *attributes) {
        ...
        if (name.starts_with("COLOR_")) {
            ...
            if (parsed.ec != std::errc{} || parsed.ptr != suffix.data() + suffix.size() ||
                ...
                return Result<PrimitiveOutput>::failure(Diagnostic::error(DiagnosticCode::ParseError, "invalid color set semantic", {}, {}, "asset.import"));
        }
        ...
        if (a.count != positions.value().count || a.components < 1 || a.components > 4 ||
            ...
            return Result<PrimitiveOutput>::failure(Diagnostic::error(DiagnosticCode::Unsupported, "unsupported vertex attribute", name, {}, "asset.import"));
    ...
    if (const Value* indicesValue = member(primitive, "indices")) {
        ...
        if (accessor.value().count == 0 || accessor.value().count > limits.maximumIndicesPerPrimitive ||
            ...
            return Result<PrimitiveOutput>::failure(Diagnostic::error(DiagnosticCode::InvalidArgument, "triangle index count is invalid", {}, {}, "asset.import"));
        ...
        for (std::uint32_t item = 0; item < accessor.value().count; ++item) {
            ...
            if (value.value() >= positions.value().count)
                return Result<PrimitiveOutput>::failure(Diagnostic::error(DiagnosticCode::ParseError, "mesh index exceeds vertex count", {}, {}, "asset.import"));
            indices.push_back(value.value());
        }
    } else {
        if (positions.value().count % 3 != 0)
            return Result<PrimitiveOutput>::failure(Diagnostic::error(DiagnosticCode::ParseError, "unindexed triangle vertex count must be divisible by three", {}, {}, "asset.import"));
    ...
    if (decodedSize > limits.maximumDecodedBytes || decodedSize > std::numeric_limits<std::size_t>::max())
        return Result<PrimitiveOutput>::failure(Diagnostic::error(DiagnosticCode::InvalidArgument, "canonical mesh exceeds decoded budget", {}, {}, "asset.import"));
...
Result<PreparedAssetImport> prepareGltfImport(const GltfImportRequest& request) {
    if (request.sourceName.empty() || request.sourceName.size() > request.limits.maximumStringBytes)
        return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::InvalidArgument, "glTF source name is invalid", {}, {}, "asset.import"));
    ...
    if (!root) return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::ParseError, "glTF root must be an object", {}, {}, "asset.import"));
    if (const auto* required = member(*root, "extensionsRequired")) {
        const auto* extensions = required->getIf<Value::Array>();
        if (!extensions)
            return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::ParseError, "extensionsRequired must be an array", {}, {}, "asset.import"));
        for (const auto& extension : *extensions)
            if (!extension.isString() || !gltf::supportsMaterialExtension(extension.asString()))
                return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::Unsupported, "required glTF extension is not supported", "extensionsRequired", {}, "asset.import"));
    }
    ...
    if (!version || !version->isString() || !version->asString().starts_with("2."))
        return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::UnknownVersion, "only glTF 2.x is supported", "$.asset.version", {}, "asset.import"));
    ...
    if (!meshes || meshes->empty())
        return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::ParseError, "glTF contains no meshes", "$.meshes", {}, "asset.import"));
    ...
    for (std::size_t meshIndex = 0; meshIndex < meshes->size(); ++meshIndex) {
        ...
        if (!primitives || primitives->empty())
            return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::ParseError, "glTF mesh has no primitives", {}, {}, "asset.import"));
        for (std::size_t primitiveIndex = 0; primitiveIndex < primitives->size(); ++primitiveIndex) {
            if (++assetCount > request.limits.maximumAssets)
                return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::InvalidArgument, "glTF primitive asset count exceeds limits", {}, {}, "asset.import"));
            const auto* primitive = (*primitives)[primitiveIndex].getIf<Value::Object>();
            if (!primitive)
                return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::ParseError, "glTF primitive is malformed", {}, {}, "asset.import"));
            if (member(*primitive, "targets"))
                return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::Unsupported, "morph targets cannot be represented by the canonical mesh", "meshes.primitives.targets", {}, "asset.import"));
            ...
            if (decoded.value().blob.size() > request.limits.maximumDecodedBytes ||
                ...
                return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::InvalidArgument, "total canonical mesh budget is exceeded", {}, {}, "asset.import"));
    ...
    if (const auto* used = member(*root, "extensionsUsed")) {
        ...
        if (!extensions)
            return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::ParseError, "extensionsUsed must be an array", {}, {}, "asset.import"));
        for (const auto& extension : *extensions) {
            if (!extension.isString())
                return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::ParseError, "extension name must be a string", {}, {}, "asset.import"));
            ...
            for (const auto& [key, unused] : attributes) {
                ...
                if (key.starts_with("JOINTS_") || key.starts_with("WEIGHTS_")) {
                    if (!member(*root, "skins"))
                        return Result<PreparedAssetImport>::failure(Diagnostic::error(DiagnosticCode::ParseError, "joint attributes require a skin binding", key, {}, "asset.import"));
                    continue;
                }
