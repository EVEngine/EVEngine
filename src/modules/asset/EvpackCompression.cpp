...
Result<std::vector<std::uint8_t>> compressEvpackChunk(
    ...
    if (codec != EvpackCodec::Zstd)
        return Result<std::vector<std::uint8_t>>::failure(Diagnostic::error(
            DiagnosticCode::Unsupported, "unknown runtime chunk codec", {}, {}, "asset.evpack.codec"));
    if (decoded.size() > ZSTD_MAX_INPUT_SIZE)
        return Result<std::vector<std::uint8_t>>::failure(Diagnostic::error(
            DiagnosticCode::InvalidArgument, "chunk exceeds the Zstd input limit", {}, {}, "asset.evpack.codec"));
...
Result<std::vector<std::uint8_t>> decompressEvpackChunk(
    ...
    if (codec == EvpackCodec::None) {
        if (stored.size() != decodedSize)
            return Result<std::vector<std::uint8_t>>::failure(
                Diagnostic::error(DiagnosticCode::ParseError, "uncompressed chunk size does not match the TOC", {}, {},
                                  "asset.evpack.codec"));
        return Result<std::vector<std::uint8_t>>::success(
    ...
    if (size != decoded.size())
        return Result<std::vector<std::uint8_t>>::failure(Diagnostic::error(
            DiagnosticCode::ParseError, "decoded chunk size does not match the TOC", {}, {}, "asset.evpack.codec"));
