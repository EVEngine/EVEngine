...
void StyleRecipe::compile(graphics::Graphics *gfx) {
    ...
    std::stable_sort(instances_.begin(), instances_.end(), [](StyleInstance *a, StyleInstance *b) {
    ...
    for (StyleInstance *instance : instances_) {
        if (instance->getStage() != stage_)
            throw eve::Exception("StyleRecipe.compile: cannot mix stages '%s' and '%s'",
                                 stage_.c_str(), instance->getStage().c_str());
...
void StyleRecipe::apply(graphics::Graphics *gfx, graphics::Texture *source,
...
void StyleRecipe::applyCanvas(graphics::Graphics *gfx, graphics::Canvas *source,
                              graphics::Canvas *dest) {
    if (!source || !source->getTexture())
        throw eve::Exception("StyleRecipe.applyCanvas: source has no texture");
