...
const StyleParameterDesc &requireParam(const std::string &style, const std::string &name) {
    ...
    if (!desc)
        throw eve::Exception("StyleInstance: style '%s' has no parameter '%s'", style.c_str(),
                             name.c_str());
    return *desc;
}
}  // namespace
...
StyleInstance::StyleInstance(std::string style) : style_(std::move(style)) {
    if (!findStyleDefinition(style_))
        throw eve::Exception("StyleInstance: unknown style '%s'", style_.c_str());
}
...
float StyleInstance::getParamDefault(const std::string &name) const {
    return requireParam(style_, name).defaultValue;
}
...
bool StyleInstance::hasParam(const std::string &name) const {
    return findStyleParameter(style_, name) != nullptr;
}

bool StyleInstance::isOverridden(const std::string &name) const {
