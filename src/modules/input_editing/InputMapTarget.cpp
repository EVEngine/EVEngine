...
EditorValue action(const InputActionValue& v) {
    return EditorValue::Object{{"id", v.id.value()}, {"name", v.name}, {"kind", v.kind}};
}
EditorValue binding(const InputBindingValue& v) {
    return EditorValue::Object{{"id", v.id.value()},   {"action", v.action.value()}, {"device", v.device},
                               {"control", v.control}, {"scale", v.scale},           {"deadzone", v.deadzone},
                               {"invert", v.invert}};
}
PropertyDescriptor prop(std::string p, PropertyType t, EditorValue d, const char* c, double lo = -1e9,
                        double hi = 1e9) {
    PropertyDescriptor x;
    x.path            = PropertyPath(p);
    x.displayNameKey  = "editor.input." + p;
    x.category        = c;
    x.type            = t;
    x.defaultValue    = std::move(d);
    x.flags           = PropertyFlag::Runtime;
    x.numeric.minimum = lo;
    x.numeric.maximum = hi;
    return x;
}
bool errors(const std::vector<EditorDiagnostic>& d) {
    return std::any_of(d.begin(), d.end(),
                       [](const auto& x) { return x.severity() == DiagnosticSeverity::Error; });
}
}  // namespace
InputMapTarget::InputMapTarget(std::string id) : id_(std::move(id)) {}
TargetDescriptor InputMapTarget::describe() const {
    return {TargetId(id_), "input-map", revision_, false, {CapabilityId("eve.editor.target.input-map-properties")}};
}
void* InputMapTarget::queryCapability(const CapabilityId& c) {
    return c == CapabilityId("eve.editor.target.input-map-properties") ? static_cast<IPropertyProvider*>(this)
                                                                       : nullptr;
}
bool InputMapTarget::matches(const SelectionSnapshot& s) const {
    if (s.items.empty()) return false;
    const auto type = s.items[0].type;
    for (const auto& i : s.items) {
        if (i.target != TargetId(id_) || i.type != type) return false;
        bool found = type == "input.action" ? std::any_of(actions_.begin(), actions_.end(),
                                                          [&](const auto& v) { return v.id.value() == i.item.value(); })
                                            : std::any_of(bindings_.begin(), bindings_.end(), [&](const auto& v) {
                                                  return v.id.value() == i.item.value();
                                              });
        if (!found) return false;
    }
    return type == "input.action" || type == "input.binding";
}
eve::Result<eve::Revision> InputMapTarget::currentRevision(const SelectionSnapshot& s) const {
    if (!matches(s))
        return eve::Result<eve::Revision>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::InvalidArgument, "Input selection mismatch", "editor.input.selection"));
    return eve::Result<eve::Revision>::success(eve::Revision(revision_));
}
PropertySchema InputMapTarget::schema(const SelectionSnapshot& s) const {
    PropertySchema x;
    if (!s.items.empty() && s.items[0].type == "input.binding") {
        x.typeId                  = "input.binding";
        x.properties              = {prop("binding.action", PropertyType::ObjectRef, "", "binding"),
                                     prop("binding.device", PropertyType::Enum, "keyboard", "binding"),
                                     prop("binding.control", PropertyType::String, "", "binding"),
                                     prop("binding.scale", PropertyType::Float, 1.0, "response", -100, 100),
                                     prop("binding.deadzone", PropertyType::Float, 0.0, "response", 0, .99),
                                     prop("binding.invert", PropertyType::Bool, false, "response")};
        x.properties[1].enumItems = {"keyboard", "mouse", "gamepad", "joystick", "touch"};
    } else {
        x.typeId                  = "input.action";
        x.properties              = {prop("action.name", PropertyType::String, "", "action"),
                                     prop("action.kind", PropertyType::Enum, "button", "action")};
        x.properties[1].enumItems = {"button", "axis1d"};
    }
    return x;
}
PropertyReadResult InputMapTarget::read(const SelectionSnapshot& s, const PropertyPath& p) const {
    if (!matches(s) || !schema(s).find(p)) return {};
    std::optional<EditorValue> common;
    for (const auto& i : s.items) {
        EditorValue v;
        if (i.type == "input.action") {
            const auto& a = *std::find_if(actions_.begin(), actions_.end(),
                                          [&](const auto& x) { return x.id.value() == i.item.value(); });
            v             = p == PropertyPath("action.name") ? EditorValue(a.name) : EditorValue(a.kind);
        } else {
            const auto& b = *std::find_if(bindings_.begin(), bindings_.end(),
                                          [&](const auto& x) { return x.id.value() == i.item.value(); });
            if (p == PropertyPath("binding.action"))
                v = b.action.value();
            else if (p == PropertyPath("binding.device"))
                v = b.device;
            else if (p == PropertyPath("binding.control"))
                v = b.control;
            else if (p == PropertyPath("binding.scale"))
                v = b.scale;
            else if (p == PropertyPath("binding.deadzone"))
                v = b.deadzone;
            else
                v = b.invert;
        }
...
EditorValue InputMapTarget::contentValue() const {
    EditorValue::Array a, b;
    for (const auto& v : actions_) a.push_back(action(v));
...
EditorResult<DomainOperation> InputMapTarget::replacement(EditorValue v, std::string p) const {
    ...
    op.payload     = std::move(v);
    op.inverse     = contentValue();
    op.hasInverse  = true;
    if (!p.empty()) op.affectedProperties.push_back(p);
...
EditorResult<DomainOperation> InputMapTarget::makeSet(const SelectionSnapshot& s, const PropertyPath& p,
    ...
    for (const auto& i : s.items)
        if (i.type == "input.action") {
            auto& a = *std::find_if(c.actions_.begin(), c.actions_.end(),
                                    [&](const auto& x) { return x.id.value() == i.item.value(); });
            if (p == PropertyPath("action.name"))
                a.name = *v.getIf<std::string>();
            else
                a.kind = *v.getIf<std::string>();
        } else {
            auto& b = *std::find_if(c.bindings_.begin(), c.bindings_.end(),
                                    [&](const auto& x) { return x.id.value() == i.item.value(); });
            if (p == PropertyPath("binding.action"))
                b.action = ObjectId(*v.getIf<std::string>());
            else if (p == PropertyPath("binding.device"))
                b.device = *v.getIf<std::string>();
            else if (p == PropertyPath("binding.control"))
                b.control = *v.getIf<std::string>();
            else if (p == PropertyPath("binding.scale"))
                b.scale = *v.getIf<double>();
            else if (p == PropertyPath("binding.deadzone"))
                b.deadzone = *v.getIf<double>();
            else
...
std::vector<EditorDiagnostic> InputMapTarget::validate() const {
    ...
    std::set<std::string>         ids, names, controls;
    for (const auto& a : actions_) {
        if (a.id.empty() || a.name.empty() || !ids.insert(a.id.value()).second || !names.insert(a.name).second ||
            (a.kind != "button" && a.kind != "axis1d"))
            d.push_back(eve::editing::ruleDiagnostic(
                eve::DiagnosticCode::PreconditionViolation, RuleId("editor.input.action"),
                DiagnosticSeverity::Error, "Action IDs/names must be unique and kinds supported"));
    }
    for (const auto& b : bindings_) {
        const bool target =
            std::any_of(actions_.begin(), actions_.end(), [&](const auto& a) { return a.id == b.action; });
        if (b.id.empty() || !ids.insert(b.id.value()).second || !target || b.control.empty() ||
            !std::isfinite(b.scale) || b.scale == 0 || std::abs(b.scale) > 100 || !std::isfinite(b.deadzone) ||
            b.deadzone < 0 || b.deadzone >= 1)
            d.push_back(eve::editing::ruleDiagnostic(
                eve::DiagnosticCode::PreconditionViolation, RuleId("editor.input.binding"),
                DiagnosticSeverity::Error, "Binding identity, reference or response is invalid"));
        const std::string key = b.device + ":" + b.control;
        if (!controls.insert(key).second)
            d.push_back(eve::editing::ruleDiagnostic(
                eve::DiagnosticCode::Conflict, RuleId("editor.input.conflict"),
                DiagnosticSeverity::Warning, "Control is bound more than once: " + key));
    }
    if (actions_.size() > 1024 || bindings_.size() > 8192)
        d.push_back(eve::editing::ruleDiagnostic(
            eve::DiagnosticCode::PreconditionViolation, RuleId("editor.input.budget"),
...
EditorResult<void> InputMapTarget::applyDomainOperation(const DomainOperation& op) {
    ...
    for (const auto& v : *aa) {
        ...
        auto* is = i ? i->getIf<std::string>() : nullptr;
    ...
    for (const auto& v : *ba) {
        auto *i = field(v, "id"), *a = field(v, "action"), *dev = field(v, "device"), *ctl = field(v, "control"),
             *scale = field(v, "scale"), *dead = field(v, "deadzone"), *inv = field(v, "invert");
        auto* is = i ? i->getIf<std::string>() : nullptr;
        auto* as = a ? a->getIf<std::string>() : nullptr;
        auto* ds = dev ? dev->getIf<std::string>() : nullptr;
        auto* cs = ctl ? ctl->getIf<std::string>() : nullptr;
        auto* sn = scale ? scale->getIf<double>() : nullptr;
...
EditorResult<std::map<std::string, double>> InputMapEvaluator::evaluate(
    ...
    for (const auto& b : t.bindings()) {
        auto s = std::find_if(samples.begin(), samples.end(),
                              [&](const auto& x) { return x.device == b.device && x.control == b.control; });
        if (s == samples.end()) continue;
        double value = std::clamp(s->value, -1.0, 1.0);
        if (std::abs(value) <= b.deadzone)
            value = 0;
        else
            value = std::copysign((std::abs(value) - b.deadzone) / (1 - b.deadzone), value);
        if (b.invert) value = -value;
        value *= b.scale;
        auto a = std::find_if(t.actions().begin(), t.actions().end(), [&](const auto& x) { return x.id == b.action; });
        if (a->kind == "button")
            out[a->name] = std::max(out[a->name], std::abs(value) >= .5 ? 1.0 : 0.0);
        else
            out[a->name] = std::clamp(out[a->name] + value, -1.0, 1.0);
    }
    return eve::editing::applied<std::map<std::string, double>>(std::move(out));
}
void InputBindingCapture::begin(std::string device) {
    active_ = true;
    device_ = std::move(device);
}
void InputBindingCapture::cancel() {
    active_ = false;
    device_.clear();
}
std::optional<InputControlSample> InputBindingCapture::feed(InputControlSample s) {
    if (!active_ || (!device_.empty() && s.device != device_) || s.control.empty() || !std::isfinite(s.value) ||
        std::abs(s.value) < .5)
        return {};
    cancel();
    return s;
