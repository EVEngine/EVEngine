...
Event::~Event() { SDL_QuitSubSystem(SDL_INIT_EVENTS); }
...
Message *Event::wait() {
    for (;;) {
        if (Message *queued = poll()) return queued;
...
Message *Event::convert(const SDL_Event &e) {
    ...
    const bool consumed   = cap::forEachUntil<IPlatformEventSink>([&](IPlatformEventSink *sink) {
        if (sink->shouldConsumePlatformEvent(&e)) return true;
    ...
    switch (e.type) {
        case SDL_QUIT: return new Message("quit");
        case SDL_WINDOWEVENT:
            if (e.window.event == SDL_WINDOWEVENT_CLOSE) return new Message("quit");
            break;
        default: break;
    }
    return nullptr;
