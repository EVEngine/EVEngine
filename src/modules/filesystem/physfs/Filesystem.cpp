...
std::string getDriveRoot(const std::string &input) { return input.substr(0, getDriveDelim(input) + 1); }

std::string skipDriveRoot(const std::string &input) { return input.substr(getDriveDelim(input) + 1); }
...
std::string normalize(const std::string &input) {
    ...
    for (size_t i = 0; i < input.size(); ++i) {
        isSep = (input[i] == EVENGINE_PATH_SEPARATOR[0]);
        if (!isSep || !seenSep) out << input[i];
        seenSep = isSep;
    }
...
bool Filesystem::isFused() const {
    if (!fusedSet) return false;
    return fused;
}

bool Filesystem::setIdentity(std::string ident, bool appendToPath) {
    ...
    save_path_relative = std::string(EVENGINE_APPDATA_PREFIX EVENGINE_APPDATA_FOLDER EVENGINE_PATH_SEPARATOR) + save_identity;

    // Generate the full path to the game save folder.
    save_path_full = std::string(getAppdataDirectory()) + std::string(EVENGINE_PATH_SEPARATOR);
    if (fused)
        save_path_full += std::string(EVENGINE_APPDATA_PREFIX) + save_identity;
    else
...
#ifdef EVENGINE_ANDROID
    if (save_identity == "") save_identity = "unnamed";
    ...
    if (!android::directoryExists(save_path_full.c_str()) && !android::mkdir(save_path_full.c_str()))
    ...
    // identity.
    if (!old_save_path.empty()) PHYSFS_unmount(old_save_path.c_str());
...
std::string Filesystem::getIdentity() const { return save_identity.c_str(); }

bool Filesystem::setSource(std::string source) {
    if (!PHYSFS_isInit()) return false;
...
#ifdef EVENGINE_ANDROID
    if (!android::createStorageDirectories()) SDL_Log("Error creating storage directories!");

    new_search_path = "";

    PHYSFS_Io *gameLoveIO;
    bool       hasFusedGame = android::checkFusedGame((void **)&gameLoveIO);
    ...
    if (hasFusedGame) {
        if (gameLoveIO) {
            ...
            if (!android::initializeVirtualArchive()) {
    ...
    oldschool:
        new_search_path = android::getSelectedGameFile();
        ...
        if (!PHYSFS_mount(new_search_path.c_str(), nullptr, 1)) {
            ...
            SDL_Log("Mounting %s did not work. Loading to memory.", new_search_path.c_str());
            char * game_archive_ptr  = NULL;
            size_t game_archive_size = 0;
            if (!android::loadGameArchiveToMemory(new_search_path.c_str(), &game_archive_ptr,
                                                        &game_archive_size)) {
                SDL_Log("Failure memory loading archive %s", new_search_path.c_str());
                return false;
            }
            if (!PHYSFS_mountMemory(game_archive_ptr, game_archive_size, android::freeGameArchiveMemory,
                                    "archive.zip", "/", 0)) {
...
#else
    // Add the directory.
    if (!PHYSFS_mount(new_search_path.c_str(), nullptr, 1)) return false;
#endif
...
bool Filesystem::setupWriteDirectory() {
    ...
    if (!PHYSFS_setWriteDir(temp_writedir.c_str())) return false;
    ...
    if (!PHYSFS_setWriteDir(save_path_full.c_str())) return false;
...
bool Filesystem::mount(std::string archive, std::string mountpoint, bool appendToPath) {
    if (!PHYSFS_isInit() || archive.empty()) return false;
    ...
    if (it != allowedMountPaths.end())
        ...
        if (archive.size() == 0 || archive.find("..") != string::npos || archive == "/") return false;
        ...
        if (realPath.find(game_source) == 0) return false;

        realPath += EVENGINE_PATH_SEPARATOR;
        realPath += archive;
    }

    if (realPath.length() == 0) return false;

    return PHYSFS_mount(realPath.c_str(), mountpoint.c_str(), appendToPath) != 0;
}

bool Filesystem::mount(Data *data, std::string archivename, std::string mountpoint, bool appendToPath) {
    if (!PHYSFS_isInit()) return false;

    if (PHYSFS_mountMemory(data->getData(), data->getSize(), nullptr, archivename.c_str(), mountpoint.c_str(), appendToPath) != 0) {
...
bool Filesystem::unmount(std::string archive) {
    ...
    if (datait != mountedData.end() && PHYSFS_unmount(archive.c_str()) != 0) {
...
filesystem::File *Filesystem::newFile(std::string filename) const { return new File(filename); }

std::string Filesystem::getWorkingDirectory() {
    if (cwd.empty()) {
#ifdef EVENGINE_WINDOWS

        WCHAR w_cwd[EVENGINE_MAX_PATH];
        _wgetcwd(w_cwd, EVENGINE_MAX_PATH);
        cwd = to_utf8(w_cwd);
        replace_char(cwd, '\\', '/');
#else
        char *cwd_char = new char[EVENGINE_MAX_PATH];

        if (getcwd(cwd_char, EVENGINE_MAX_PATH))
...
std::string Filesystem::getSaveDirectory() { return save_path_full.c_str(); }
...
std::string Filesystem::getSourceBaseDirectory() const {
    ...
    if (base_end_pos == std::string::npos) return "";

    // If the source is in the unix root (aka '/'), we want to keep the '/'.
    if (base_end_pos == 0) base_end_pos = 1;

    return game_source.substr(0, base_end_pos);
}

std::string Filesystem::getRealDirectory(std::string filename) const {
    if (!PHYSFS_isInit()) throw Exception("PhysFS is not initialized.");

    const char* dir = PHYSFS_getRealDir(filename.c_str());

    if (dir == nullptr) throw Exception("File does not exist on disk.");

    return std::string(dir);
}

bool Filesystem::getInfo(std::string filepath, Info &info) const {
    if (!PHYSFS_isInit()) return false;

    PHYSFS_Stat stat = {};
    if (!PHYSFS_stat(filepath.c_str(), &stat)) return false;
...
bool Filesystem::createDirectory(std::string dir) {
    ...
    if (PHYSFS_getWriteDir() == 0 && !setupWriteDirectory()) return false;

    if (!PHYSFS_mkdir(dir.c_str())) return false;
...
bool Filesystem::remove(std::string file) {
    ...
    if (!PHYSFS_delete(file.c_str())) return false;

    return true;
}

FileData *Filesystem::read(std::string filename, int64_t size) const {
    physfs::File file(filename);
...
void Filesystem::write(std::string filename, const void *data, int64_t size) const {
    physfs::File file(filename);
...
void Filesystem::append(std::string filename, const void *data, int64_t size) const {
    physfs::File file(filename);
