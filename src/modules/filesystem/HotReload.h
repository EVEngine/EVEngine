...
/**
 * @brief Lightweight path→reload registry for soft hot reload.
...
public:
    ...
    /** @brief Register a path for explicit reload (kind: "auto"|"particle"|"tilemap"|"texture"). */
    void bind(std::string path, std::string kind = "auto");
    void unbind(std::string path);

    /**
     * @brief Reload assets matching path (normalized).
