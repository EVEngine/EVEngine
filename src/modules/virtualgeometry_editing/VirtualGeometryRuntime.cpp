...
std::uint64_t checksum(const virtualgeometry::VirtualGeometryAsset& a) {
    std::uint64_t hash = 1469598103934665603ull;
    hashBytes(hash, a.positions.data(), a.positions.size() * sizeof(float));
    hashBytes(hash, a.triangles.data(), a.triangles.size() * sizeof(std::uint32_t));
    hashBytes(hash, a.clusters.data(), a.clusters.size() * sizeof(virtualgeometry::VgCluster));
    return hash;
}
}  // namespace
...
EditorResult<VirtualGeometryBuildArtifact> VirtualGeometryBuildRuntime::build(
    ...
    input.vertexCount = static_cast<int>(vertices);
    input.positions   = mesh.positions.data();
    input.normals     = mesh.normals.empty() ? nullptr : mesh.normals.data();
    ...
    artifact.sourceTriangles = mesh.indices.size() / 3;
    artifact.storedTriangles = candidate->triangles.size() / 3;
    artifact.clusters        = static_cast<int>(candidate->clusters.size());
    artifact.checksum        = checksum(*candidate);
    for (const auto& cluster : candidate->clusters) {
    ...
    const float projection = static_cast<float>(v.previewHeight) / (2.f * std::tan(v.previewFov * pi / 180.f * .5f));
    for (int sample = 0; sample < v.distanceSamples; ++sample) {
        const float                alpha    = static_cast<float>(sample) / (v.distanceSamples - 1);
        const float                distance = v.nearDistance * std::pow(v.farDistance / v.nearDistance, alpha);
        std::vector<std::uint32_t> selected;
        virtualgeometry::selectClusters(*candidate, distance, projection, v.errorPixels, selected);
        VirtualGeometryLodSample point;
        point.distance = distance;
        point.clusters = static_cast<int>(selected.size());
        for (auto id : selected) point.triangles += candidate->clusters[id].triCount;
        virtualgeometry::lodHistogram(*candidate, selected, point.levelHistogram);
        artifact.lodCurve.push_back(std::move(point));
    }
    asset_             = std::move(candidate);
    revision_          = document.revision();
    return eve::editing::applied<VirtualGeometryBuildArtifact>(std::move(artifact), diagnostics);
}
}  // namespace eve::virtualgeometry_editing
