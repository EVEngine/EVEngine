...
PropertyDescriptor prop(std::string path, PropertyType type, EditorValue value, const char* category, double minimum,
                        double maximum) {
    PropertyDescriptor d;
    d.path            = PropertyPath(path);
    d.displayNameKey  = "editor.virtualgeometry." + path;
    d.category        = category;
    d.type            = type;
...
VirtualGeometryDocumentTarget::VirtualGeometryDocumentTarget(std::string id) : id_(std::move(id)) {}
...
void* VirtualGeometryDocumentTarget::queryCapability(const CapabilityId& c) {
    return c == CapabilityId("eve.editor.target.virtualgeometry-properties") ? static_cast<IPropertyProvider*>(this)
                                                                             : nullptr;
}
bool VirtualGeometryDocumentTarget::matches(const SelectionSnapshot& s) const {
    return s.items.size() == 1 && s.items.front().target == TargetId(id_) && s.items.front().item.value() == id_;
}
eve::Result<eve::Revision> VirtualGeometryDocumentTarget::currentRevision(const SelectionSnapshot& s) const {
    if (!matches(s))
...
PropertySchema VirtualGeometryDocumentTarget::schema(const SelectionSnapshot&) const {
    PropertySchema s;
    s.typeId     = "virtualgeometry.import";
    s.properties = {prop("source.asset", PropertyType::AssetRef, "", "source", 0, 0),
                    prop("cluster.maxVertices", PropertyType::Int, int64_t{64}, "cluster", 3, 256),
                    prop("cluster.maxTriangles", PropertyType::Int, int64_t{124}, "cluster", 1, 512),
                    prop("lod.levels", PropertyType::Int, int64_t{4}, "lod", 1, 16),
                    prop("lod.mergeFactor", PropertyType::Int, int64_t{4}, "lod", 2, 4),
                    prop("lod.targetRatio", PropertyType::Float, .5, "lod", .01, .99),
                    prop("preview.fov", PropertyType::Float, 60.0, "preview", 1, 179),
                    prop("preview.height", PropertyType::Int, int64_t{1080}, "preview", 16, 16384),
                    prop("preview.errorPixels", PropertyType::Float, 1.0, "preview", .01, 100),
                    prop("preview.nearDistance", PropertyType::Float, 1.0, "preview", .001, 1e9),
                    prop("preview.farDistance", PropertyType::Float, 1000.0, "preview", .001, 1e9),
                    prop("preview.samples", PropertyType::Int, int64_t{32}, "preview", 2, 512)};
    s.properties[0].assetTypeFilters = {"model3d", "mesh"};
    return s;
}
EditorValue VirtualGeometryDocumentTarget::contentValue() const {
    const auto& v = value_;
    return EditorValue::Object{{"source", v.sourceAsset},
                               {"maxVertices", int64_t{v.builder.maxVerticesPerCluster}},
                               {"maxTriangles", int64_t{v.builder.maxTrianglesPerCluster}},
                               {"levels", int64_t{v.builder.minLodLevels}},
                               {"mergeFactor", int64_t{v.builder.mergeFactor}},
                               {"targetRatio", v.builder.lodTargetRatio},
                               {"fov", v.previewFov},
                               {"height", int64_t{v.previewHeight}},
                               {"errorPixels", v.errorPixels},
                               {"nearDistance", v.nearDistance},
                               {"farDistance", v.farDistance},
                               {"samples", int64_t{v.distanceSamples}}};
}
PropertyReadResult VirtualGeometryDocumentTarget::read(const SelectionSnapshot& s, const PropertyPath& p) const {
    if (!matches(s) || !schema(s).find(p)) return {};
    const auto& v = value_;
    EditorValue value;
    if (p == PropertyPath("source.asset"))
        value = v.sourceAsset;
    else if (p == PropertyPath("cluster.maxVertices"))
        value = int64_t{v.builder.maxVerticesPerCluster};
    else if (p == PropertyPath("cluster.maxTriangles"))
        value = int64_t{v.builder.maxTrianglesPerCluster};
    else if (p == PropertyPath("lod.levels"))
        value = int64_t{v.builder.minLodLevels};
    else if (p == PropertyPath("lod.mergeFactor"))
        value = int64_t{v.builder.mergeFactor};
    else if (p == PropertyPath("lod.targetRatio"))
        value = v.builder.lodTargetRatio;
    else if (p == PropertyPath("preview.fov"))
        value = v.previewFov;
    else if (p == PropertyPath("preview.height"))
        value = int64_t{v.previewHeight};
    else if (p == PropertyPath("preview.errorPixels"))
        value = v.errorPixels;
    else if (p == PropertyPath("preview.nearDistance"))
        value = v.nearDistance;
    else if (p == PropertyPath("preview.farDistance"))
        value = v.farDistance;
    else
...
EditorResult<DomainOperation> VirtualGeometryDocumentTarget::makeSet(const SelectionSnapshot& s, const PropertyPath& p,
    ...
    if (p == PropertyPath("source.asset"))
        ...
        v.builder.maxVerticesPerCluster = static_cast<int>(*value.getIf<int64_t>());
    else if (p == PropertyPath("cluster.maxTriangles"))
        v.builder.maxTrianglesPerCluster = static_cast<int>(*value.getIf<int64_t>());
    else if (p == PropertyPath("lod.levels"))
        v.builder.minLodLevels = static_cast<int>(*value.getIf<int64_t>());
    else if (p == PropertyPath("lod.mergeFactor"))
        v.builder.mergeFactor = static_cast<int>(*value.getIf<int64_t>());
    else if (p == PropertyPath("lod.targetRatio"))
        v.builder.lodTargetRatio = static_cast<float>(*value.getIf<double>());
    else if (p == PropertyPath("preview.fov"))
        v.previewFov = static_cast<float>(*value.getIf<double>());
    else if (p == PropertyPath("preview.height"))
        v.previewHeight = static_cast<int>(*value.getIf<int64_t>());
    else if (p == PropertyPath("preview.errorPixels"))
        v.errorPixels = static_cast<float>(*value.getIf<double>());
    else if (p == PropertyPath("preview.nearDistance"))
        v.nearDistance = static_cast<float>(*value.getIf<double>());
    else if (p == PropertyPath("preview.farDistance"))
        v.farDistance = static_cast<float>(*value.getIf<double>());
    else
...
std::vector<EditorDiagnostic> VirtualGeometryDocumentTarget::validate() const {
    ...
    if (v.sourceAsset.empty())
        d.push_back(eve::editing::ruleDiagnostic(eve::DiagnosticCode::PreconditionViolation,
                                                 ...
                                                 "VirtualGeometry source asset is not assigned"));
    if (v.builder.maxVerticesPerCluster < 3 || v.builder.maxVerticesPerCluster > 256 ||
        v.builder.maxTrianglesPerCluster < 1 || v.builder.maxTrianglesPerCluster > 512 || v.builder.minLodLevels < 1 ||
        v.builder.minLodLevels > 16 || v.builder.mergeFactor < 2 || v.builder.mergeFactor > 4 ||
        !std::isfinite(v.builder.lodTargetRatio) || v.builder.lodTargetRatio <= 0 || v.builder.lodTargetRatio >= 1)
        d.push_back(eve::editing::ruleDiagnostic(eve::DiagnosticCode::InvalidArgument,
                                                 RuleId("editor.virtualgeometry.builder"), DiagnosticSeverity::Error,
                                                 "VirtualGeometry builder settings exceed supported invariants"));
    if (!std::isfinite(v.previewFov) || v.previewFov < 1 || v.previewFov > 179 || v.previewHeight < 16 ||
        v.previewHeight > 16384 || !std::isfinite(v.errorPixels) || v.errorPixels <= 0 || v.errorPixels > 100 ||
        !std::isfinite(v.nearDistance) || !std::isfinite(v.farDistance) || v.nearDistance <= 0 ||
        v.farDistance <= v.nearDistance || v.farDistance > 1e9f || v.distanceSamples < 2 || v.distanceSamples > 512)
        d.push_back(eve::editing::ruleDiagnostic(eve::DiagnosticCode::InvalidArgument,
                                                 RuleId("editor.virtualgeometry.preview"), DiagnosticSeverity::Error,
...
EditorResult<void> VirtualGeometryDocumentTarget::applyDomainOperation(const DomainOperation& op) {
    ...
    int*        ints[]    = {&v.builder.maxVerticesPerCluster,
                             ...
                             &v.builder.minLodLevels,
                             &v.builder.mergeFactor,
                             &v.previewHeight,
                             &v.distanceSamples};
    const char* ifields[] = {"maxVertices", "maxTriangles", "levels", "mergeFactor", "height", "samples"};
    for (int i = 0; i < 6; ++i) {
    ...
    const char* ffields[] = {"targetRatio", "fov", "errorPixels", "nearDistance", "farDistance"};
    for (int i = 0; i < 5; ++i) {
