...
eve::Result<const eve::Value::Object*> readRoot(const eve::Value& value, std::string_view schemaId,
    ...
    if (!object)
        ...
        eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "climbing definition must be an object", {}, {}, "climbing.codec"));
    std::string  actualSchema;
    std::int64_t actualVersion = 0;
    if (!readString(*object, "schemaId", actualSchema) || actualSchema != schemaId)
        return eve::Result<const eve::Value::Object*>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "climbing definition schemaId is missing or mismatched", "schemaId", {}, "climbing.codec"));
    if (!readInt64(*object, "schemaVersion", actualVersion))
        return eve::Result<const eve::Value::Object*>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "climbing definition schemaVersion must be an integer", "schemaVersion", {}, "climbing.codec"));
    if (actualVersion != schemaVersion && actualVersion != schemaVersion - 1)
        return eve::Result<const eve::Value::Object*>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::UnknownVersion, "climbing definition schema version is unsupported", "schemaVersion", {}, "climbing.codec"));
...
eve::Result<void> validateClimbingActionDefinition(const ClimbingActionDefinition& action) {
    if (action.id.empty())
        ...
        eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument, "action id must not be empty", "id", {}, "climbing.codec"));
    if (!finite(action.minHeight) || !finite(action.maxHeight) || action.minHeight < 0.f ||
        action.maxHeight < action.minHeight)
        return eve::Result<void>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument, "action height range must be finite, non-negative, and ordered", "height", {}, "climbing.codec"));
    if (!finite(action.minSpeed) || action.minSpeed < 0.f || !finite(action.landingForward) ||
        ...
        return eve::Result<void>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument, "action speed, landing distance, and apex must be finite and non-negative", {}, {}, "climbing.codec"));
    if (action.duration.nanoseconds() <= 0)
        return eve::Result<void>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument, "action duration must be positive", "durationNs", {}, "climbing.codec"));
    if (!finite(action.hangBodyOffset) || action.hangBodyOffset < 0.f || !finite(action.hangFeetBelowLedge) ||
        ...
        return eve::Result<void>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument, "hang geometry and cancel window must be finite and ordered", {}, {}, "climbing.codec"));
    if (!finite(action.rootMotionScaleMin) || !finite(action.rootMotionScaleMax) || action.rootMotionScaleMin <= 0.f ||
        ...
        return eve::Result<void>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument, "root-motion scale and warp budgets must be finite, positive, and bounded", "warp", {}, "climbing.codec"));
    ...
    for (std::size_t index = 0; index < action.warpWindows.size(); ++index) {
        ...
        if (!finite(window.start) || !finite(window.end) || window.start < 0.f || window.end > 1.f ||
            ...
            return eve::Result<void>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument, "warp windows must be ordered, non-overlapping, and enable a channel", "warpWindows." + std::to_string(index), {}, "climbing.codec"));
    ...
    if (std::any_of(notifyNames.begin(), notifyNames.end(), [](const std::string& name) { return name.empty(); }) ||
        ...
        return eve::Result<void>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument, "required animation notifies must be non-empty and unique", "requiredNotifies", {}, "climbing.codec"));
    ...
    if (sourceBits == 0 || (sourceBits & ~static_cast<std::uint8_t>(ClimbingSourceMode::Any)) != 0)
        return eve::Result<void>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument, "sourceModes must contain a known mode", "sourceModes", {}, "climbing.codec"));
    if (!finite(action.minDepth) || !finite(action.maxDepth) || action.minDepth < 0.f ||
        ...
        return eve::Result<void>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument, "action geometry constraints are invalid", "geometry", {}, "climbing.codec"));
    if (!uniqueNonEmpty(action.tags) || !uniqueNonEmpty(action.requiredSupportTags) ||
        ...
        return eve::Result<void>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument, "action tags and event metadata must be unique", "metadata", {}, "climbing.codec"));
    if (action.animation.rootBone.empty())
        return eve::Result<void>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument, "animation rootBone must not be empty", "animation.rootBone", {}, "climbing.codec"));
    for (std::size_t index = 0; index < action.branchWindows.size(); ++index) {
        ...
        if (!finite(window.start) || !finite(window.end) || window.start < 0.f || window.end > 1.f ||
            ...
            return eve::Result<void>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument, "branch window is invalid", "branchWindows." + std::to_string(index), {}, "climbing.codec"));
        if (std::find(action.comboTags.begin(), action.comboTags.end(), window.comboTag) == action.comboTags.end())
            return eve::Result<void>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument, "branch window comboTag must be declared by the action", "branchWindows." + std::to_string(index) + ".comboTag", {}, "climbing.codec"));
    }
    for (std::size_t index = 0; index < action.contactConstraints.size(); ++index) {
        ...
        if (!finite(contact.start) || !finite(contact.end) || !finite(contact.maxWeight) || contact.start < 0.f ||
            ...
            return eve::Result<void>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument, "contact constraint is invalid", "contactConstraints." + std::to_string(index), {}, "climbing.codec"));
    }
    if (action.repetitionPenalty < 0 || !finite(action.staminaCost) || action.staminaCost < 0.f)
        return eve::Result<void>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument, "action cost metadata is invalid", "cost", {}, "climbing.codec"));
...
eve::Result<void> validateClimbingProfileDefinition(const ClimbingProfileDefinition& profile) {
    if (!finite(profile.capsuleRadius) || !finite(profile.capsuleHeight) || profile.capsuleRadius <= 0.f ||
        ...
        return eve::Result<void>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument, "capsule height must exceed twice its positive radius", "capsule", {}, "climbing.codec"));
    if (!finite(profile.compactCapsuleHeight) ||
        ...
        return eve::Result<void>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument, "compact capsule height must exceed the diameter and not exceed standing height", "compactCapsuleHeight", {}, "climbing.codec"));
    if (!finite(profile.skin) || profile.skin < 0.f || !finite(profile.maxProbeDistance) ||
        ...
        return eve::Result<void>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument, "profile probe, surface, skin, and warp limits are invalid", {}, {}, "climbing.codec"));
    if (!finite(profile.maxPlatformSpeed) || profile.maxPlatformSpeed <= 0.f || !finite(profile.dropInitialSpeed) ||
        ...
        return eve::Result<void>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument, "platform and drop speed limits must be finite", {}, {}, "climbing.codec"));
    if (profile.inputBufferTicks == 0 || profile.inputBufferTicks > 1000000 || profile.coyoteTicks > 1000000)
        return eve::Result<void>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument, "input buffer must be non-zero and tick windows must be bounded", "inputTicks", {}, "climbing.codec"));
    if (profile.pathValidationSegments < 2 || profile.pathValidationSegments > 64 ||
        ...
        return eve::Result<void>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument, "path validation samples and pelvis deviation are invalid", "motionValidation", {}, "climbing.codec"));
    if (!finite(profile.groundAcceleration) || profile.groundAcceleration < 0.f || !finite(profile.groundBraking) ||
        ...
        return eve::Result<void>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument, "profile locomotion and probe policies are invalid", "policies", {}, "climbing.codec"));
    if (!uniqueNonEmpty(profile.defaultActionIds) || !uniqueNonEmpty(profile.allowedActionTags) ||
        ...
        return eve::Result<void>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument, "profile action ids and tags must be unique", "actionPolicy", {}, "climbing.codec"));
    ...
    if (weights.direction < 0 || weights.approachSpeed < 0 || weights.height < 0 || weights.distance < 0 ||
        ...
        return eve::Result<void>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument, "score weights must be non-negative", "scoreWeights", {}, "climbing.codec"));
    if (profile.staminaPolicy == ClimbingStaminaPolicy::RequireProvider && profile.staminaAdapter.empty())
        return eve::Result<void>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument, "RequireProvider stamina policy needs a stable adapter id", "staminaAdapter", {}, "climbing.codec"));
    ...
    for (const ClimbingActionDefinition& action : profile.actions) {
        ...
        if (std::max(action.horizontalWarpBudget, action.verticalWarpBudget) >
            ...
            return eve::Result<void>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument, "action translation warp budgets exceed profile maximum", "actions." + action.id, {}, "climbing.codec"));
        if (!ids.emplace(action.id).second)
            return eve::Result<void>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::AlreadyExists, "profile contains duplicate action id", "actions." + action.id, {}, "climbing.codec"));
    }
    for (const std::string& id : profile.defaultActionIds)
        if (!ids.contains(id))
            return eve::Result<void>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::NotFound, "default action id is not registered in the profile", "defaultActionIds." + id, {}, "climbing.codec"));
    return eve::Result<void>::success();
}
...
eve::Result<ClimbingActionDefinition> decodeClimbingActionDefinition(const eve::Value& value) {
    ...
    if (!readString(object, "id", candidate.id) || !readString(object, "kind", kind) ||
        ...
        eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "climbing action has missing or invalid known fields", {}, {}, "climbing.codec"));
    ...
    for (std::size_t index = 0; index < notifies->size(); ++index) {
        ...
        if (!notify)
            return eve::Result<ClimbingActionDefinition>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "animation notify must be a string", "requiredNotifies." + std::to_string(index), {}, "climbing.codec"));
    ...
    for (std::size_t index = 0; warpWindows && index < warpWindows->size(); ++index) {
        ...
        if (!valueObject || !readFloat(*valueObject, "start", window.start) ||
            ...
            return eve::Result<ClimbingActionDefinition>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "warp window has missing or invalid fields", "warpWindows." + std::to_string(index), {}, "climbing.codec"));
    ...
    if (!readOptionalStringArray(object, "tags", candidate.tags) ||
        ...
        return eve::Result<ClimbingActionDefinition>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "climbing action has invalid v4 metadata", {}, {}, "climbing.codec"));
    ...
    if (const eve::Value* animationValue = field(object, "animation")) {
        ...
        if (!animation || !readString(*animation, "clipId", candidate.animation.clipId) ||
            ...
            return eve::Result<ClimbingActionDefinition>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "animation binding has missing or invalid fields", "animation", {}, "climbing.codec"));
    }
    if (const eve::Value* branchValue = field(object, "branchWindows")) {
        const auto* branches = branchValue->getIf<eve::Value::Array>();
        if (!branches)
            return eve::Result<ClimbingActionDefinition>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "branchWindows must be an array", "branchWindows", {}, "climbing.codec"));
        ...
        for (std::size_t index = 0; index < branches->size(); ++index) {
            ...
            if (!valueObject || !readFloat(*valueObject, "start", window.start) ||
                ...
                return eve::Result<ClimbingActionDefinition>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "branch window has missing or invalid fields", "branchWindows." + std::to_string(index), {}, "climbing.codec"));
    ...
    if (const eve::Value* contactsValue = field(object, "contactConstraints")) {
        ...
        if (!contacts)
            return eve::Result<ClimbingActionDefinition>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "contactConstraints must be an array", "contactConstraints", {}, "climbing.codec"));
        ...
        for (std::size_t index = 0; index < contacts->size(); ++index) {
            ...
            if (!valueObject ||
                ...
                return eve::Result<ClimbingActionDefinition>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "contact constraint has missing or invalid fields", "contactConstraints." + std::to_string(index), {}, "climbing.codec"));
...
eve::Result<ClimbingProfileDefinition> decodeClimbingProfileDefinition(const eve::Value& value) {
    ...
    if (!readFloat(object, "capsuleRadius", candidate.capsuleRadius) ||
        ...
        eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "climbing profile has missing or invalid known fields", {}, {}, "climbing.codec"));
    ...
    if (const eve::Value* weightsValue = field(object, "scoreWeights")) {
        ...
        if (!weights || !readInt64(*weights, "direction", direction) ||
            ...
            return eve::Result<ClimbingProfileDefinition>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "scoreWeights has missing or invalid fields", "scoreWeights", {}, "climbing.codec"));
        ...
        if (!inRange(direction) || !inRange(approachSpeed) || !inRange(height) || !inRange(distance) ||
            ...
            return eve::Result<ClimbingProfileDefinition>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::ParseError, "scoreWeights exceeds int32 range", "scoreWeights", {}, "climbing.codec"));
    ...
    for (std::size_t index = 0; index < actions->size(); ++index) {
        ...
        if (!decoded)
            return eve::Result<ClimbingProfileDefinition>::failure(
        eve::Diagnostic::error(decoded.error()->code(), decoded.error()->message(), "actions." + std::to_string(index) + "." + decoded.error()->path(), {}, "climbing.codec"));
