...
eve::Result<void> validateFullCapsulePath(physics::World3D& world, Vec3 start, const ClimbingCandidate& candidate,
    ...
    if (segmentCount == 0) {
        ...
        eve::Diagnostic::error(eve::DiagnosticCode::PreconditionViolation, "climbing.query_budget.exceeded", "candidate.path", {}, "climbing.selection"));
    }
    ...
    for (std::uint32_t segment = 1; segment <= segmentCount; ++segment) {
        ...
        if (length(subtract(desired, actual)) > profile.skin + 0.001f)
            return eve::Result<void>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::PreconditionViolation, "climbing.candidate.path_blocked", "candidate.path." + std::to_string(segment), {}, "climbing.selection"));
        current = target;
    }
...
eve::Result<ClimbingRuntime::PreparedBegin> ClimbingRuntime::prepareBegin(physics::World3D&   world,
    ...
    if (activePhase(phase_))
        ...
        eve::Diagnostic::error(eve::DiagnosticCode::Conflict, "a climbing execution is already active", "runtime.phase", {}, "climbing.selection"));
    if (nextExecutionId_ == 0 || nextExecutionId_ == std::numeric_limits<std::uint64_t>::max())
        return eve::Result<PreparedBegin>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::PreconditionViolation, "climbing execution id space is exhausted", "runtime.nextExecutionId", {}, "climbing.selection"));
    ...
    if (values.empty())
        return eve::Result<PreparedBegin>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::NotFound, "no valid climbing candidate was found", "candidate", {}, "climbing.selection"));
...
eve::Result<ClimbingRuntime::PreparedBegin> ClimbingRuntime::prepareBeginCandidate(
    ...
    if (!action)
        return eve::Result<PreparedBegin>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::InvariantViolation, "selected action definition disappeared", "candidate.actionId", {}, "climbing.selection"));
    if (!action->requiredNotifies.empty() && !validatedAnimationActions_.contains(action->id))
        return eve::Result<PreparedBegin>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::PreconditionViolation, "climbing.animation.notify_missing: action clip contract was not validated", "candidate.actionId", {}, "climbing.selection"));
...
eve::Result<ClimbingStart> ClimbingRuntime::commitBegin(PreparedBegin prepared) {
    if (activePhase(phase_) || execution_)
        return eve::Result<ClimbingStart>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::Conflict, "climbing state changed before commit", "runtime.phase", {}, "climbing.selection"));
    if (prepared.executionId.isZero() || prepared.executionId.value() != nextExecutionId_)
        return eve::Result<ClimbingStart>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::Conflict, "prepared climbing execution is stale", "executionId", {}, "climbing.selection"));
    if (!prepared.conditionsSatisfied)
        return eve::Result<ClimbingStart>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::Unsupported, "required climbing conditions were not evaluated by a service authority", "action.requiredConditionTags", {}, "climbing.selection"));
...
eve::Result<ClimbingStart> ClimbingSelectionSystem::tryStart(ClimbingRuntime& runtime, physics::World3D& world,
    ...
    if (!ClimbingInputSystem::peek(intent, command, tick))
        return eve::Result<ClimbingStart>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::NotFound, "climbing.input.no_match", "intent.commands", {}, "climbing.selection"));
    ...
    if (!commandMatches(prepared.value().action.requiredCommand, command))
        return eve::Result<ClimbingStart>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::NotFound, "selected climbing action does not accept the buffered command", "action.requiredCommand", {}, "climbing.selection"));
    ...
    if (!consumed || !consumed.value()) {
        ...
        return eve::Result<ClimbingStart>::failure(
        eve::Diagnostic::error(eve::DiagnosticCode::Conflict, "eligible climbing input changed before transaction commit", "intent.commands", {}, "climbing.selection"));
    }
