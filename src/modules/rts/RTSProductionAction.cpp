...
eve::Result<void> failure(eve::DiagnosticCode code, std::string message,
                          std::string path = {}) {
...
public:
    OrderParticipant(orders::CommandQueue& queue, std::string kind, int priority,
                     double timeout, std::string product, std::string owner)
        : queue_(queue), kind_(std::move(kind)), priority_(priority), timeout_(timeout),
          product_(std::move(product)), owner_(std::move(owner)) {}

    [[nodiscard]] std::string_view name() const noexcept override {
        return "rts-build-order";
    }
    ...
    [[nodiscard]] eve::Result<void> prepare(const transaction::TransactionContext&) override {
        if (prepared_ || committed_)
            return failure(eve::DiagnosticCode::Conflict,
                           "RTS order participant is already in flight", "orders");
        try {
            before_ = queue_;
            staged_ = queue_;
        } catch (const std::exception& exception) {
            return failure(eve::DiagnosticCode::Failed,
                           std::string("failed to stage RTS order: ") + exception.what(), "orders");
        }
        ...
        if (!order)
            return failure(eve::DiagnosticCode::InvariantViolation,
                           "staged RTS order disappeared", "orders");
    ...
    [[nodiscard]] eve::Result<void> commit(const transaction::TransactionContext&) override {
        if (!prepared_ || committed_)
            return failure(eve::DiagnosticCode::Conflict,
                           "RTS order participant has no prepared stage", "orders");
        try {
            queue_ = *staged_;
        } catch (const std::exception& exception) {
            return failure(eve::DiagnosticCode::Failed,
                           std::string("failed to publish RTS order: ") + exception.what(), "orders");
        }
        committed_ = true;
        return eve::Result<void>::success(eve::Status::success(eve::StatusCode::Applied));
    }
    ...
    [[nodiscard]] eve::Result<void> rollback(const transaction::TransactionContext&) override {
        if (committed_)
            return failure(eve::DiagnosticCode::Conflict,
                           "committed RTS order requires compensation", "orders");
        staged_.reset();
        prepared_ = false;
        return eve::Result<void>::success(eve::Status::success(eve::StatusCode::Applied));
    }
    ...
    [[nodiscard]] eve::Result<void> compensate(const transaction::TransactionContext&) override {
        if (!committed_)
            return failure(eve::DiagnosticCode::Conflict,
                           "RTS order has no committed state to compensate", "orders");
        try {
            queue_ = *before_;
        } catch (const std::exception& exception) {
            return failure(eve::DiagnosticCode::Failed,
...
public:
    ProductionParticipant(production::WorkQueue& queue, std::string owner,
                          std::string kind, std::string product, eve::Value context,
                          Duration duration, int priority)
        : queue_(queue), owner_(std::move(owner)), kind_(std::move(kind)),
          product_(std::move(product)), context_(std::move(context)), duration_(duration),
          priority_(priority) {}

    [[nodiscard]] std::string_view name() const noexcept override {
        return "rts-production-queue";
    }

    [[nodiscard]] eve::Result<void> prepare(const transaction::TransactionContext&) override {
        if (prepared_ || committed_)
            return failure(eve::DiagnosticCode::Conflict,
                           "RTS production participant is already in flight", "production");
    ...
    [[nodiscard]] eve::Result<void> commit(const transaction::TransactionContext&) override {
        if (!prepared_ || committed_)
            return failure(eve::DiagnosticCode::Conflict,
                           "RTS production participant has no prepared stage", "production");
    ...
    [[nodiscard]] eve::Result<void> rollback(const transaction::TransactionContext&) override {
        if (committed_)
            return failure(eve::DiagnosticCode::Conflict,
                           "committed production task requires compensation", "production");
        staged_.reset();
        prepared_ = false;
        return eve::Result<void>::success(eve::Status::success(eve::StatusCode::Applied));
    }
    ...
    [[nodiscard]] eve::Result<void> compensate(const transaction::TransactionContext&) override {
        if (!committed_)
            return failure(eve::DiagnosticCode::Conflict,
                           "production queue has no committed task to compensate", "production");
        auto current = queue_.snapshot();
        if (!current) return eve::Result<void>::failure(current.status());
        if (current.value() != afterJson_)
            return failure(eve::DiagnosticCode::StaleHandle,
                           "RTS production queue changed before compensation", "production");
...
public:
    ActionParticipant(action::ActionRuntime& runtime, action::ActionDefinition definition,
        ...
        : runtime_(runtime), definition_(std::move(definition)), request_(std::move(request)),
          tick_(tick), delta_(delta) {}

    [[nodiscard]] std::string_view name() const noexcept override {
        return "rts-build-action";
    }

    [[nodiscard]] eve::Result<void> prepare(const transaction::TransactionContext&) override {
        if (execution_) return failure(eve::DiagnosticCode::Conflict,
                                       "RTS Action participant is already in flight", "action");
    ...
    [[nodiscard]] eve::Result<void> commit(const transaction::TransactionContext&) override {
        if (!prepared_ || committed_ || !execution_)
            return failure(eve::DiagnosticCode::Conflict,
                           "RTS Action participant has no prepared execution", "action");
    ...
    [[nodiscard]] eve::Result<void> rollback(const transaction::TransactionContext&) override {
        if (committed_)
            return failure(eve::DiagnosticCode::Conflict,
                           "committed RTS Action requires compensation", "action");
        ...
        if (current != nullptr && current->phase() != action::ActionPhase::Failed &&
            current->phase() != action::ActionPhase::Cancelled &&
            current->phase() != action::ActionPhase::Completed) {
...
eve::Result<RTSBuildReceipt> RTSProductionActionAdapter::build(
    ...
    if (!subject.isValid())
        return failure<RTSBuildReceipt>(
...
eve::Result<RTSBuildReceipt> RTSProductionActionAdapter::build(RTSBuildRequest request) {
    if (request.production == nullptr || request.orders == nullptr || request.action == nullptr ||
        ...
        return failure<RTSBuildReceipt>(eve::DiagnosticCode::InvalidArgument,
                                        "RTS build requires production, orders, Action and account ports",
                                        "request");
    ...
    if (request.duration.nanoseconds() <= 0 || request.actionDelta.nanoseconds() < 0)
        return failure<RTSBuildReceipt>(eve::DiagnosticCode::InvalidArgument,
                                        "RTS build durations are invalid", "duration");

    if (!request.actionDefinition.id.isValid()) {
        auto id = eve::LogicalId::fromParts("rts", "build." + request.product);
        if (!id) return failure<RTSBuildReceipt>(eve::DiagnosticCode::InvalidArgument,
    ...
    if (!request.actionRequest.actionId.isValid())
        request.actionRequest.actionId = request.actionDefinition.id;
    ...
    OrderParticipant order(*request.orders, request.orderKind, request.orderPriority,
                           request.orderTimeoutSeconds, request.product, request.owner);
    ProductionParticipant production(*request.production, request.owner, request.productionKind,
                                      request.product, std::move(request.context), request.duration,
                                      request.priority);
    ActionParticipant action(*request.action, std::move(request.actionDefinition),
                             std::move(request.actionRequest), request.tick,
                             request.actionDelta);
    ...
    auto committed = transaction::AtomicResourcePayment::execute(
        context, *request.account, request.cost,
        std::span<transaction::ITransactionParticipant*>(participants));
    if (!committed)
        return eve::Result<RTSBuildReceipt>::failure(committed.status());
    ...
    return eve::Result<RTSBuildReceipt>::success(std::move(result),
