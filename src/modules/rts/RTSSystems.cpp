    ...
    for (std::size_t index = 0; index < selected.size(); ++index) {
        ...
        if (!order) {
            ...
            for (std::size_t rollback = 0; rollback < receipt.orderIds.size(); ++rollback) {
                auto cancelled =
                    selected[rollback]->orders()->values.cancel(receipt.orderIds[rollback], "command fan-out rollback");
...
Result<std::size_t> MotionSystem::step(const SimulationStep& step) {
    ...
    for (auto it = view.begin(); it != view.end(); ++it) {
        ...
        Unit* unit = identity == nullptr ? nullptr : dynamic_cast<Unit*>(ecs::try_get(identity->self));
        if (unit == nullptr)
            return failure<std::size_t>(DiagnosticCode::StaleHandle, "RTS motion identity link is stale",
                                        "unit.identity.self");
...
Result<std::size_t> OrderActionSystem::step(const SimulationStep& step, IRTSActionExecutor& executor) {
    ...
    for (auto it = view.begin(); it != view.end(); ++it) {
        ...
        Unit* unit = identity == nullptr ? nullptr : dynamic_cast<Unit*>(ecs::try_get(identity->self));
        if (unit == nullptr)
            return failure<std::size_t>(DiagnosticCode::StaleHandle, "RTS action identity link is stale",
                                        "unit.identity.self");
        auto current = readCurrent(orders->values);
        if (!current) return failureFrom<std::size_t>(current.status());
        auto record = std::move(current).takeValue();
...
Result<std::size_t> BuildingProductionSystem::step(const SimulationStep& step) {
    ...
    for (auto it = view.begin(); it != view.end(); ++it) {
        ...
        if (building == nullptr)
            return failure<std::size_t>(DiagnosticCode::StaleHandle, "RTS production identity link is stale",
...
Result<std::size_t> EffectSystem::step(const SimulationStep& step) {
    ...
    std::size_t processed = 0;
    {
        ...
        for (auto it = view.begin(); it != view.end(); ++it) {
            ...
            Unit* unit = identity == nullptr ? nullptr : dynamic_cast<Unit*>(ecs::try_get(identity->self));
            if (unit == nullptr)
                return failure<std::size_t>(DiagnosticCode::StaleHandle, "RTS unit effects identity link is stale",
                                            "unit.identity.self");
    ...
    }
    {
        ...
        for (auto it = view.begin(); it != view.end(); ++it) {
            ...
            Building* building = identity == nullptr ? nullptr : dynamic_cast<Building*>(ecs::try_get(identity->self));
            if (building == nullptr)
                return failure<std::size_t>(DiagnosticCode::StaleHandle, "RTS building effects identity link is stale",
                                            "building.identity.self");
            auto advanced = advanceEffects(effects->values, step);
            if (!advanced) return failureFrom<std::size_t>(advanced.status());
            processed += std::move(advanced).takeValue();
