...
eve::Result<void> RTSEffectExecutor::validate(const RTSEffectTarget& target) const {
    if (!std::isfinite(target.morale) || target.morale < 0.0 || target.morale > 100.0)
        return eve::Result<void>::failure(eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument,
                                                                 "RTS morale must be in [0,100]", "target.morale"));
...
eve::Result<RTSEffectUpdate> RTSEffectExecutor::settle(RTSEffectTarget&             target,
    ...
    auto valid = validate(target);
    if (!valid) return eve::Result<RTSEffectUpdate>::failure(valid.status());
    ...
    return eve::Result<RTSEffectUpdate>::success(std::move(result), eve::Status::success(eve::StatusCode::Applied));
}
...
std::size_t RTSEffectAdapter::count() const noexcept { return static_cast<std::size_t>(container_.effectCount()); }
...
eve::Result<void> RTSEffectComponent::bindSubject(eve::SubjectRef subject) {
    if (!subject.isValid())
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::InvalidArgument, "RTS effect component requires a valid subject", "subject"));
...
eve::Result<effects::EffectHandle> RTSEffectComponent::apply(const RTSEffectDefinition& definition) {
    if (!subject_.isValid())
        return eve::Result<effects::EffectHandle>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::InvalidArgument, "RTS effect component has no bound subject", "subject"));
    return adapter_.apply(definition, subject_);
}

eve::Result<void> RTSEffectComponent::remove(effects::EffectHandle handle) { return adapter_.remove(handle); }

eve::Result<RTSEffectUpdate> RTSEffectComponent::advance(const eve::SimulationStep& step) {
