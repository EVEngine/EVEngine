...
[[nodiscard]] const GarrisonMember* payloadMember(
    const eve::container::ContainerObject& object, GarrisonMember& fallback) {
    const auto* payload =
        dynamic_cast<const BuildingGarrisonContainerObject*>(object.payload.get());
    if (payload != nullptr) {
...
[[nodiscard]] bool sameLayout(const eve::container::ContainerSnapshot& lhs,
    ...
    if (lhs.id != rhs.id || lhs.revision != rhs.revision ||
        lhs.entries.size() != rhs.entries.size())
        return false;
    for (const auto& left : lhs.entries) {
        const auto right = std::find_if(
            rhs.entries.begin(), rhs.entries.end(), [&](const auto& entry) {
                return entry.membership.object == left.membership.object;
            });
        if (right == rhs.entries.end() || left.membership.slot != right->membership.slot ||
            left.membership.generation != right->membership.generation ||
            left.object.id != right->object.id || left.object.type != right->object.type ||
            left.object.quantity != right->object.quantity ||
...
[[nodiscard]] game_event::GameEvent makeEvent(
    const eve::container::ContainerId& container, const eve::container::MembershipId& object,
    ...
    event.tick = tick;
    event.payload = "{\"containerId\":" + quote(container.value()) +
                    ",\"objectId\":" + quote(object.value()) +
                    ",\"slot\":" + std::to_string(slot.isValid() ? slot.value() : -1) +
...
void notify(const eve::container::GameEventSink& sink,
            const eve::container::ContainerId& container,
            const eve::container::MembershipId& object, eve::container::ContainerEventKind kind,
            eve::container::SlotIndex slot, eve::Revision revision, std::uint64_t serial,
            eve::SimulationTick tick, std::string_view reason, bool& callbackFailed) noexcept {
    if (!sink) return;
    try {
...
[[nodiscard]] eve::Status appliedStatus(bool callbackFailed) {
    ...
    try {
        ...
        diagnostics.emplace_back(eve::Diagnostic::warning(
            ...
            "container membership committed but an event observer threw"));
...
class BuildingGarrisonContainerAdapter::PreparedState final
    : public eve::container::IContainer::PreparedState {
public:
    PreparedState(BuildingGarrisonContainerAdapter& owner, std::vector<GarrisonMember> staged,
                  eve::Revision expectedRevision, std::size_t expectedSize,
                  eve::Revision revision)
        : owner_(owner), staged_(std::move(staged)), expectedRevision_(expectedRevision),
          expectedSize_(expectedSize), revision_(revision) {}
...
BuildingGarrisonContainerAdapter::BuildingGarrisonContainerAdapter(
    eve::container::ContainerId id, PlacementWorld* world, int instanceId,
    eve::container::Capacity capacity, eve::container::AcceptedCondition accepted)
    : descriptor_{std::move(id), capacity, eve::container::Ordering::Insertion,
                   std::move(accepted)},
      world_(world), instanceId_(instanceId) {}
...
eve::container::ContainerObject BuildingGarrisonContainerAdapter::describe(
    const GarrisonMember& member) {
...
eve::Result<eve::container::ContainerSnapshot>
BuildingGarrisonContainerAdapter::snapshot() const {
    const auto* value = building();
    if (!descriptor_.id.isValid() || value == nullptr)
        return eve::Result<eve::container::ContainerSnapshot>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::StaleHandle,
                                   "building garrison adapter is stale or unbound"));
    if (!descriptor_.capacity.isUnlimited() &&
        value->garrison.size() > descriptor_.capacity.value())
        return eve::Result<eve::container::ContainerSnapshot>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::InvariantViolation,
                                   "building garrison exceeds configured capacity"));
    ...
    for (std::size_t index = 0; index < value->garrison.size(); ++index) {
        if (index > static_cast<std::size_t>(std::numeric_limits<std::int32_t>::max()))
            return eve::Result<eve::container::ContainerSnapshot>::failure(
                eve::Diagnostic::error(eve::DiagnosticCode::InvariantViolation,
                                       "building garrison has too many indexed memberships"));
        ...
        if (!eve::container::MembershipId::from(member.id) || member.type.empty() ||
            ...
            return eve::Result<eve::container::ContainerSnapshot>::failure(
                eve::Diagnostic::error(eve::DiagnosticCode::Conflict,
                                       "building garrison contains invalid or duplicate member"));
        const auto id = eve::container::MembershipId(member.id);
        result.entries.push_back({{id, eve::container::SlotIndex(static_cast<std::int32_t>(index)),
                                   eve::Generation(1)},
                                  describe(member)});
    }
...
eve::Result<void> BuildingGarrisonContainerAdapter::validateInsert(
    ...
    if (!descriptor_.id.isValid() || value == nullptr)
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::StaleHandle, "building garrison adapter is stale or unbound"));
    if (!object.id.isValid() || object.type.empty() || object.quantity != 1u)
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::InvalidArgument, "building garrison object facts are invalid"));
    GarrisonMember fallback;
    if (payloadMember(object, fallback) == nullptr)
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::StaleHandle, "building garrison payload is stale"));
    for (const auto& member : value->garrison) {
        if (member.id == object.id.value() &&
            (!ignoredObject || *ignoredObject != object.id))
            return eve::Result<void>::failure(eve::Diagnostic::error(
                eve::DiagnosticCode::Conflict, "building garrison already contains member"));
    }
    const std::size_t effectiveSize = value->garrison.size() -
        ((ignoredObject && std::any_of(value->garrison.begin(), value->garrison.end(),
                                       [&](const auto& member) {
                                           return member.id == ignoredObject->value();
                                       }))
             ? 1u
             : 0u);
    if (destination &&
        (!destination->isValid() || static_cast<std::size_t>(destination->value()) > effectiveSize))
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::InvalidArgument, "building garrison slot is out of range"));
    ...
    if (!descriptor_.capacity.isUnlimited() && effectiveSize >= descriptor_.capacity.value())
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::Conflict, "building garrison capacity is full"));
    return eve::Result<void>::success();
}

eve::Result<std::unique_ptr<eve::container::IContainer::PreparedState>>
BuildingGarrisonContainerAdapter::prepare(const eve::container::ContainerSnapshot& expected,
                                           const eve::container::ContainerSnapshot& candidate) {
    const auto* value = building();
    if (!descriptor_.id.isValid() || value == nullptr)
    ...
    if (expected.id != descriptor_.id || candidate.id != descriptor_.id)
        return eve::Result<std::unique_ptr<eve::container::IContainer::PreparedState>>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument,
                                   "building garrison prepare targets another container"));
    ...
    if (!sameLayout(current.value(), expected))
        return eve::Result<std::unique_ptr<eve::container::IContainer::PreparedState>>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::StaleHandle,
                                   "building garrison expected snapshot is stale"));
    const auto next = expected.revision.incremented();
    if (!next || candidate.revision != *next)
        return eve::Result<std::unique_ptr<eve::container::IContainer::PreparedState>>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::InvariantViolation,
                                   "building garrison candidate revision is not the next revision"));
    if (!descriptor_.capacity.isUnlimited() &&
        candidate.entries.size() > descriptor_.capacity.value())
        return eve::Result<std::unique_ptr<eve::container::IContainer::PreparedState>>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::Conflict,
                                   "building garrison candidate exceeds capacity"));
    ...
    for (std::size_t index = 0; index < candidate.entries.size(); ++index) {
        ...
        if (!entry.membership.object.isValid() || !entry.membership.slot.isValid() ||
            ...
            entry.object.quantity != 1u || !identities.insert(entry.membership.object).second)
            return eve::Result<std::unique_ptr<eve::container::IContainer::PreparedState>>::failure(
                eve::Diagnostic::error(eve::DiagnosticCode::InvariantViolation,
    ...
    std::unique_ptr<eve::container::IContainer::PreparedState> prepared =
        std::make_unique<PreparedState>(*this, std::move(staged), expected.revision,
                                        expected.entries.size(), *next);
    return eve::Result<std::unique_ptr<eve::container::IContainer::PreparedState>>::success(
        std::move(prepared));
}

eve::Result<eve::container::MembershipId> BuildingGarrisonContainerAdapter::enter(
    std::string memberId, std::string type, std::vector<std::string> tags,
    eve::container::GameEventSink sink, eve::SimulationTick tick) {
    ...
    if (!next) {
        const auto status = eve::Status::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::InvariantViolation, "building garrison revision exhausted"));
        bool ignored = false;
        notify(sink, descriptor_.id, objectId, eve::container::ContainerEventKind::Rejected,
               eve::container::SlotIndex::invalid(), candidate.revision, serial, tick,
               "revision_exhausted", ignored);
        return eve::Result<eve::container::MembershipId>::failure(status);
    }
    candidate.revision = *next;
    candidate.entries.push_back({{objectId,
                                  eve::container::SlotIndex(static_cast<std::int32_t>(
                                      current.value().entries.size())),
                                  eve::Generation(1)},
                                 std::move(object)});
    ...
    notify(sink, descriptor_.id, objectId, eve::container::ContainerEventKind::Accepted, slot,
           *next, serial, tick, {}, callbackFailed);
    notify(sink, descriptor_.id, objectId, eve::container::ContainerEventKind::Enter, slot,
           *next, serial, tick, {}, callbackFailed);
    return eve::Result<eve::container::MembershipId>::success(
        objectId, appliedStatus(callbackFailed));
}

eve::Result<eve::container::MembershipId> BuildingGarrisonContainerAdapter::exit(
    ...
    candidate.entries.erase(std::remove_if(candidate.entries.begin(), candidate.entries.end(),
                                            [&](const auto& entry) {
                                                return entry.membership.object == memberId;
                                            }),
                             candidate.entries.end());
    for (std::size_t index = 0; index < candidate.entries.size(); ++index)
        candidate.entries[index].membership.slot =
    ...
    if (!prepared) {
        ...
        const auto* diagnostic = status.primaryDiagnostic();
        notify(sink, descriptor_.id, memberId, eve::container::ContainerEventKind::Rejected, slot,
               current.value().revision, serial, tick,
               diagnostic == nullptr ? "prepare_failed" : diagnostic->message(), ignored);
        return eve::Result<eve::container::MembershipId>::failure(status);
    }
    auto staged = std::move(prepared).takeValue();
    staged->commit();
    bool callbackFailed = false;
    notify(sink, descriptor_.id, memberId, eve::container::ContainerEventKind::Accepted, slot,
           *next, serial, tick, {}, callbackFailed);
    notify(sink, descriptor_.id, memberId, eve::container::ContainerEventKind::Exit, slot, *next,
           serial, tick, {}, callbackFailed);
    return eve::Result<eve::container::MembershipId>::success(
        memberId, appliedStatus(callbackFailed));
}
