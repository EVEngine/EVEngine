...
eve::Result<double> VehicleAttributeAdapter::read(VehicleEntity& vehicle, std::string_view attribute) {
    ...
    if (!projected) return failure<double>(projected.code(), "vehicle compatibility projection failed", "health");
...
eve::Result<eve::attributes::ModifierId> VehicleAttributeAdapter::addModifier(
    ...
    auto ready = ensure(vehicle);
    if (!ready)
        return failure<eve::attributes::ModifierId>(ready.code(), "vehicle attributes are not available", "attributes");
    ...
    auto projected = project(vehicle);
    if (!projected)
        return failure<eve::attributes::ModifierId>(projected.code(), "vehicle compatibility projection failed",
                                                    "health");
    return added;
}
