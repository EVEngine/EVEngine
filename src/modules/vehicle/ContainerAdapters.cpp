...
[[nodiscard]] const VehicleEntity::Seats* seatsOf(const VehicleEntity* value) noexcept {
    return value == nullptr ? nullptr
                            : &*const_cast<VehicleEntity*>(value)->seats();
}
...
[[nodiscard]] int payloadOccupant(const eve::container::ContainerObject& object) noexcept {
    ...
    const auto expectedId = eve::container::MembershipId(
        std::string(kOccupantPrefix) + std::to_string(payload->occupantId));
    if (payload->occupantId <= 0 || object.id != expectedId)
        return 0;
...
class VehicleSeatContainerAdapter::PreparedState final
    : public eve::container::IContainer::PreparedState {
public:
    PreparedState(VehicleSeatContainerAdapter& owner, std::vector<std::optional<int>> staged,
    ...
    void commit() noexcept override {
        ...
        if (seats == nullptr || seats->list.size() != staged_.size())
            std::terminate();
...
VehicleSeatContainerAdapter::VehicleSeatContainerAdapter(
    eve::container::ContainerId id, VehicleEntity* vehicle,
    eve::container::AcceptedCondition accepted)
    : descriptor_{std::move(id),
                  vehicle != nullptr
                      ? eve::container::Capacity::fixed(seatsOf(vehicle)->list.size())
...
eve::container::MembershipId VehicleSeatContainerAdapter::occupantId(int occupantId) {
    return eve::container::MembershipId(
        std::string(kOccupantPrefix) + std::to_string(occupantId));
}

eve::container::ContainerObject VehicleSeatContainerAdapter::describe(int occupantId,
                                                                        std::string driver) {
    ...
    if (!descriptor_.id.isValid() || seats == nullptr)
        return eve::Result<eve::container::ContainerSnapshot>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::StaleHandle,
                                   "vehicle seat adapter is stale or unbound"));
...
eve::Result<void> VehicleSeatContainerAdapter::validateInsert(
    ...
    if (!descriptor_.id.isValid() || seats == nullptr)
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::StaleHandle, "vehicle seat adapter is stale or unbound"));
    if (!object.id.isValid() || object.type != "vehicle.occupant" || object.quantity != 1u)
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::InvalidArgument, "vehicle seat object facts are invalid"));
    const int occupant = payloadOccupant(object);
    if (occupant <= 0)
        return eve::Result<void>::failure(eve::Diagnostic::error(
            eve::DiagnosticCode::StaleHandle, "vehicle occupant identity is stale"));
    ...
    for (std::size_t index = 0; index < seats->list.size(); ++index) {
        if (occupiedBy(index) && (!ignoredObject || *ignoredObject != object.id))
            return eve::Result<void>::failure(eve::Diagnostic::error(
                eve::DiagnosticCode::Conflict, "vehicle occupant is already seated"));
    }
    if (destination) {
        if (!destination->isValid() ||
            static_cast<std::size_t>(destination->value()) >= seats->list.size())
            return eve::Result<void>::failure(eve::Diagnostic::error(
                eve::DiagnosticCode::InvalidArgument, "vehicle seat slot is out of range"));
        const auto& seat = seats->list[static_cast<std::size_t>(destination->value())];
        if (seat.occupied &&
            (!ignoredObject || occupantId(seat.occupant) != *ignoredObject))
            return eve::Result<void>::failure(eve::Diagnostic::error(
                eve::DiagnosticCode::Conflict, "vehicle seat slot is occupied"));
    }
...
VehicleSeatContainerAdapter::prepare(const eve::container::ContainerSnapshot& expected,
    ...
    if (expected.id != descriptor_.id || candidate.id != descriptor_.id)
        return eve::Result<std::unique_ptr<eve::container::IContainer::PreparedState>>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument,
                                   "vehicle seat prepare targets another container"));
    ...
    if (!sameLayout(current.value(), expected))
        return eve::Result<std::unique_ptr<eve::container::IContainer::PreparedState>>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::StaleHandle,
                                   "vehicle seat expected snapshot is stale"));
    const auto next = expected.revision.incremented();
    if (!next || candidate.revision != *next)
        return eve::Result<std::unique_ptr<eve::container::IContainer::PreparedState>>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::InvariantViolation,
                                   "vehicle seat candidate revision is not the next revision"));
    if (candidate.entries.size() > seats->list.size())
        return eve::Result<std::unique_ptr<eve::container::IContainer::PreparedState>>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::Conflict,
                                   "vehicle seat candidate exceeds capacity"));
    ...
    for (const auto& entry : candidate.entries) {
        if (!entry.membership.object.isValid() || !entry.membership.slot.isValid() ||
            ...
            entry.object.id != entry.membership.object || entry.object.type != "vehicle.occupant" ||
        ...
        if (occupant <= 0)
            return eve::Result<std::unique_ptr<eve::container::IContainer::PreparedState>>::failure(
                eve::Diagnostic::error(eve::DiagnosticCode::StaleHandle,
                                       "vehicle seat candidate payload is stale"));
...
eve::Result<eve::container::MembershipId> VehicleSeatContainerAdapter::enter(
    eve::container::SlotIndex seatIndex, int occupant, eve::container::GameEventSink sink,
    ...
    if (!current) {
        ...
        notify(sink, descriptor_.id, objectIdValue, eve::container::ContainerEventKind::Rejected,
               seatIndex, revision(), serial, tick,
    ...
    if (!accepted) {
        ...
        bool ignored = false;
        const auto* diagnostic = status.primaryDiagnostic();
        notify(sink, descriptor_.id, object.id, eve::container::ContainerEventKind::Rejected,
               seatIndex, current.value().revision, serial, tick,
    ...
    if (!next) {
        const auto status = eve::Status::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::InvariantViolation,
                                   "vehicle seat revision exhausted"));
        bool ignored = false;
        notify(sink, descriptor_.id, object.id, eve::container::ContainerEventKind::Rejected,
               seatIndex, candidate.revision, serial, tick, "revision_exhausted", ignored);
        return eve::Result<eve::container::MembershipId>::failure(status);
    }
    ...
    notify(sink, descriptor_.id, objectIdValue, eve::container::ContainerEventKind::Accepted,
           seatIndex, *next, serial, tick, {}, callbackFailed);
    notify(sink, descriptor_.id, objectIdValue, eve::container::ContainerEventKind::Enter,
           seatIndex, *next, serial, tick, {}, callbackFailed);
    return eve::Result<eve::container::MembershipId>::success(
        objectIdValue, appliedStatus(callbackFailed));
}

eve::Result<eve::container::MembershipId> VehicleSeatContainerAdapter::exit(
    ...
    candidate.entries.erase(std::remove_if(candidate.entries.begin(), candidate.entries.end(),
                                            [&](const auto& entry) {
                                                return entry.membership.slot == seatIndex;
                                            }),
    ...
    if (!next) {
        ...
        bool ignored = false;
        notify(sink, descriptor_.id, objectIdValue, eve::container::ContainerEventKind::Rejected,
               seatIndex, current.value().revision, serial, tick, "revision_exhausted", ignored);
        return eve::Result<eve::container::MembershipId>::failure(status);
    }
    candidate.revision = *next;
    auto prepared = prepare(current.value(), candidate);
    ...
    notify(sink, descriptor_.id, objectIdValue, eve::container::ContainerEventKind::Accepted,
           seatIndex, *next, serial, tick, {}, callbackFailed);
    notify(sink, descriptor_.id, objectIdValue, eve::container::ContainerEventKind::Exit,
           seatIndex, *next, serial, tick, {}, callbackFailed);
    return eve::Result<eve::container::MembershipId>::success(
        objectIdValue, appliedStatus(callbackFailed));
}
