...
eve::Result<double> checkedNumber(double value, std::string_view name) {
    if (!std::isfinite(value))
        return failure<double>(eve::DiagnosticCode::InvariantViolation,
                               std::string(name) + " must be finite", std::string(name));
...
RPGSettlementAdapter::RPGSettlementAdapter(RPGActor& target, SubjectRef targetRef,
                                           RPGActor* source, SubjectRef sourceRef)
    : RPGSettlementAdapter(target, std::move(targetRef), source, std::move(sourceRef), Config{}) {}

RPGSettlementAdapter::RPGSettlementAdapter(RPGActor& target, SubjectRef targetRef,
                                           RPGActor* source, SubjectRef sourceRef, Config config)
    : target_(target), targetRef_(targetRef), source_(source), sourceRef_(sourceRef),
...
eve::Result<double> RPGSettlementAdapter::readBase(std::string_view name) const {
    if (name.empty())
        return failure<double>(eve::DiagnosticCode::InvalidArgument,
                               "RPG settlement attribute name must not be empty", "attribute");
    const auto& attributes = target_.attributes()->values;
    if (!attributes.has(std::string(name)))
        return failure<double>(eve::DiagnosticCode::NotFound,
                               "RPG settlement attribute is not present", std::string(name));
...
eve::Result<double> RPGSettlementAdapter::readSourceFinal(std::string_view name,
...
eve::Result<void> RPGSettlementAdapter::validate(settlement::SettlementContext& context) {
    if (!(context.request().target == targetRef_))
        return failure<void>(eve::DiagnosticCode::Conflict,
                             "RPG settlement request target does not match adapter target",
                             "target");
    const bool damage = isDamage(context);
    const bool healing = isHealing(context);
    if (!damage && !healing)
        return failure<void>(eve::DiagnosticCode::Unsupported,
                             "RPG settlement adapter supports damage and healing kinds only",
                             "kind");
    ...
    if (health.value() > maximum.value())
        return failure<void>(eve::DiagnosticCode::InvariantViolation,
                             "RPG health exceeds max health", "health");
...
eve::Result<void> RPGSettlementAdapter::sourceModifiers(settlement::SettlementContext& context) {
    ...
    if (!std::isfinite(multiplier) || multiplier < 0.0)
        return failure<void>(eve::DiagnosticCode::InvalidArgument,
                             "RPG source multiplier must be finite and non-negative",
                             "source_multiplier");
    ...
    if (critical) {
        ...
        if (!std::isfinite(criticalMultiplier) || criticalMultiplier < 1.0)
            return failure<void>(eve::DiagnosticCode::InvalidArgument,
                                 "RPG critical multiplier must be finite and at least one",
                                 "critical_multiplier");
...
eve::Result<void> RPGSettlementAdapter::armorShield(settlement::SettlementContext& context) {
    ...
    const double penetration = std::max(0.0, numberValue(contextValue(context, "penetration"))
                                                   .value_or(0.0));
...
eve::Result<settlement::PreparedApply> RPGSettlementAdapter::prepareApply(
    ...
    const double nextHealth = isDamage(context) ? health.value() - context.magnitude()
                                                : health.value() + context.magnitude();
    ...
    auto pending = std::make_shared<Pending>(Pending{&target_, target_.attributes()->values,
        ...
        settlement::PreparedApply(
            ...
            [pending]() {
                if (pending->published)
                    pending->actor->attributes()->values = pending->before;
            }));
}
