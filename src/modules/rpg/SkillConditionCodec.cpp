...
eve::Result<eve::decision::ConditionKind> parseKind(const eve::Value& value) {
    ...
    static const std::pair<std::string_view, K> names[] = {
        {"all", K::All}, {"any", K::Any}, {"not", K::Not}, {"compare", K::Compare},
        {"has_tag", K::HasTag}, {"has_attribute", K::HasAttribute},
        {"has_resource", K::HasResource}, {"state_equals", K::StateEquals},
        {"authority_check", K::AuthorityCheck}, {"policy_call", K::PolicyCall},
    };
...
eve::Result<eve::decision::CompareOperator> parseOperator(const eve::Value& value) {
    const auto* text = value.getIf<std::string>();
    if (text == nullptr) return invalid<eve::decision::CompareOperator>("condition operator must be a string", "operator");
...
eve::Result<eve::decision::DeterminismLevel> parseDeterminism(const eve::Value& value) {
    const auto* text = value.getIf<std::string>();
    if (text == nullptr) return invalid<eve::decision::DeterminismLevel>("determinism must be a string", "scriptDeclaration.determinism");
    using D = eve::decision::DeterminismLevel;
    static const std::pair<std::string_view, D> names[] = {
        {"bit_exact", D::BitExact}, {"tick_deterministic", D::TickDeterministic},
...
eve::Result<std::vector<eve::decision::Condition>> decodeChildren(const eve::Value::Object& object) {
    ...
    if (array == nullptr) return invalid<std::vector<eve::decision::Condition>>("condition children must be an array", "children");
...
eve::Result<eve::decision::Condition> decodeNode(const eve::Value& value) {
    ...
    if (parsedKind == K::All || parsedKind == K::Any || parsedKind == K::Not) {
        if (!exactFields(*object, {"kind", "children"})) return invalid<eve::decision::Condition>("logical condition contains an unknown field");
        ...
        if (parsedKind == K::Not && values.size() != 1) return invalid<eve::decision::Condition>("not condition requires one child", "children");
        if (parsedKind == K::All) return eve::Result<eve::decision::Condition>::success(eve::decision::Condition::all(std::move(values)));
        if (parsedKind == K::Any) return eve::Result<eve::decision::Condition>::success(eve::decision::Condition::any(std::move(values)));
        return eve::Result<eve::decision::Condition>::success(eve::decision::Condition::not_(std::move(values.front())));
    }
    if (parsedKind == K::Compare) {
        if (!exactFields(*object, {"kind", "key", "operator", "expected"})) return invalid<eve::decision::Condition>("compare condition contains an unknown field");
        auto key = requiredString(*object, "key");
        const auto* op = field(*object, "operator");
        const auto* expected = field(*object, "expected");
        if (!key || op == nullptr || expected == nullptr) return invalid<eve::decision::Condition>("compare condition is incomplete");
        auto parsedOp = parseOperator(*op);
        if (!parsedOp) return eve::Result<eve::decision::Condition>::failure(parsedOp.status());
        return eve::Result<eve::decision::Condition>::success(eve::decision::Condition::compare(std::move(key).takeValue(), parsedOp.value(), *expected));
    }
    if (parsedKind == K::StateEquals) {
        if (!exactFields(*object, {"kind", "key", "expected"})) return invalid<eve::decision::Condition>("state condition contains an unknown field");
        auto key = requiredString(*object, "key");
        const auto* expected = field(*object, "expected");
        if (!key || expected == nullptr) return invalid<eve::decision::Condition>("state condition is incomplete");
        return eve::Result<eve::decision::Condition>::success(eve::decision::Condition::stateEquals(std::move(key).takeValue(), *expected));
    }
    if (parsedKind == K::PolicyCall) {
        if (!exactFields(*object, {"kind", "key", "arguments", "scriptDeclaration"})) return invalid<eve::decision::Condition>("policy condition contains an unknown field");
        auto key = requiredString(*object, "key");
        ...
        if (const auto* value = field(*object, "scriptDeclaration")) {
            ...
            if (declarationObject == nullptr || !exactFields(*declarationObject, {"name", "dependencies", "determinism"})) return invalid<eve::decision::Condition>("script declaration is invalid", "scriptDeclaration");
            auto name = requiredString(*declarationObject, "name");
            const auto* dependencyValue = field(*declarationObject, "dependencies");
            const auto* dependencies = dependencyValue == nullptr ? nullptr : dependencyValue->getIf<eve::Value::Array>();
            const auto* determinism = field(*declarationObject, "determinism");
            if (!name || dependencies == nullptr || determinism == nullptr) return invalid<eve::decision::Condition>("script declaration is incomplete", "scriptDeclaration");
            ...
            for (const auto& dependency : *dependencies) {
                ...
                if (text == nullptr || text->empty()) return invalid<eve::decision::Condition>("script dependency is invalid", "scriptDeclaration.dependencies");
                parsed.dependencies.push_back(*text);
            }
            declaration = std::move(parsed);
        }
        return eve::Result<eve::decision::Condition>::success(eve::decision::Condition::policyCall(std::move(key).takeValue(), std::move(arguments), std::move(declaration)));
    }
    if (!exactFields(*object, {"kind", "key"})) return invalid<eve::decision::Condition>("leaf condition contains an unknown field");
    auto key = requiredString(*object, "key");
    if (!key) return eve::Result<eve::decision::Condition>::failure(key.status());
    switch (parsedKind) {
    case K::HasTag: return eve::Result<eve::decision::Condition>::success(eve::decision::Condition::hasTag(std::move(key).takeValue()));
    case K::HasAttribute: return eve::Result<eve::decision::Condition>::success(eve::decision::Condition::hasAttribute(std::move(key).takeValue()));
    case K::HasResource: return eve::Result<eve::decision::Condition>::success(eve::decision::Condition::hasResource(std::move(key).takeValue()));
    case K::AuthorityCheck: return eve::Result<eve::decision::Condition>::success(eve::decision::Condition::authorityCheck(std::move(key).takeValue()));
    ...
    if (condition.kind() == K::All || condition.kind() == K::Any || condition.kind() == K::Not) {
        ...
        if (condition.kind() == K::PolicyCall) {
            ...
            if (condition.scriptDeclaration()) {
                ...
                case eve::decision::DeterminismLevel::ExplicitlyNondeterministic: determinism = "explicitly_nondeterministic"; break;
                }
                object.emplace("scriptDeclaration", eve::Value(eve::Value::Object{
                    {"name", eve::Value(declaration.name)}, {"dependencies", eve::Value(std::move(dependencies))},
...
eve::Result<eve::decision::Condition> decodeSkillCondition(const eve::Value& value) {
