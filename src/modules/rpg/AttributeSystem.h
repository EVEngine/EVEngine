...
/**
 * @brief 面向 RPGActor 的属性操作静态入口。数据本身存放在 RPGActor::Attributes 组件里，
 * 这里只是围绕组件的一组无状态操作函数（+ 一张全局自定义 op 表）。
 */
class AttributeSystem {
public:
    /** @brief 全局自定义 op 注册表（进程生命周期内有效）。 */
    static AttributeOpTable &customOps();

    /** @brief 设置基础值（不存在则创建该属性）。 */
    ...
    /**
     * @brief 添加一条修改器，返回自动生成的唯一 id（用于之后精确移除）。
