...
EditorValue faces(const std::uint8_t* v) {
    EditorValue::Array a;
    for (int i = 0; i < 6; ++i) a.push_back(int64_t{v[i]});
    return a;
}
EditorValue entryValue(const VoxelPaletteEntryValue& v) {
    return EditorValue::Object{{"id", v.id.value()},
                               {"name", v.type.name},
                               {"faces", faces(v.type.faceTex)},
                               {"directional", v.type.directional},
...
EditorResult<VoxelPaletteEntryValue> parse(const EditorValue& v) {
    ...
    const auto* dirs   = directional ? directional->getIf<bool>() : nullptr;
    const auto* groups = group ? group->getIf<std::string>() : nullptr;
    ...
    out.type.composeGroup = *groups;
    out.type.connects     = *conns;
    for (int i = 0; i < 6; ++i) {
...
PropertyDescriptor prop(std::string path, PropertyType type, EditorValue value) {
    ...
    d.displayNameKey = "editor.voxel-palette." + path;
    d.category       = "cube-type";
    d.type           = type;
    d.defaultValue   = std::move(value);
    d.flags          = PropertyFlag::Runtime;
    if (type == PropertyType::Array) d.presenterHint = "face-texture-six";
    return d;
}
...
bool VoxelPaletteTarget::matches(const SelectionSnapshot& s) const {
    if (s.items.empty()) return false;
    for (const auto& i : s.items)
        if (i.target != TargetId(id_) || std::none_of(entries_.begin(), entries_.end(),
                                                      [&](const auto& v) { return v.id.value() == i.item.value(); }))
            return false;
    return true;
}
eve::Result<eve::Revision> VoxelPaletteTarget::currentRevision(const SelectionSnapshot& s) const {
    if (!matches(s))
...
PropertySchema VoxelPaletteTarget::schema(const SelectionSnapshot&) const {
    PropertySchema s;
    s.typeId     = "voxel.cube-type";
    s.properties = {prop("cube.faces", PropertyType::Array,
                         EditorValue::Array{int64_t{0}, int64_t{0}, int64_t{0}, int64_t{0}, int64_t{0}, int64_t{0}}),
                    prop("cube.directional", PropertyType::Bool, false),
                    prop("cube.composeGroup", PropertyType::String, ""),
                    prop("cube.connects", PropertyType::Bool, false)};
    return s;
}
PropertyReadResult VoxelPaletteTarget::read(const SelectionSnapshot& s, const PropertyPath& p) const {
    if (!matches(s) || !schema(s).find(p)) return {};
    std::optional<EditorValue> common;
    for (const auto& i : s.items) {
        const auto& v = std::find_if(entries_.begin(), entries_.end(), [&](const auto& x) {
                            return x.id.value() == i.item.value();
                        })->type;
        EditorValue value;
        if (p == PropertyPath("cube.faces"))
            value = faces(v.faceTex);
        else if (p == PropertyPath("cube.directional"))
            value = v.directional;
        else if (p == PropertyPath("cube.composeGroup"))
            value = v.composeGroup;
        else
            value = v.connects;
        if (!common)
            common = value;
...
EditorValue VoxelPaletteTarget::contentValue() const {
    EditorValue::Array a;
...
EditorResult<DomainOperation> VoxelPaletteTarget::makeSet(const SelectionSnapshot& s, const PropertyPath& p,
    ...
    for (const auto& i : s.items) {
        ...
        if (p == PropertyPath("cube.faces")) {
            ...
            v.directional = *value.getIf<bool>();
        else if (p == PropertyPath("cube.composeGroup"))
            v.composeGroup = *value.getIf<std::string>();
        else
...
std::vector<EditorDiagnostic> VoxelPaletteTarget::validate() const {
    std::vector<EditorDiagnostic> d;
    std::set<std::string>         ids, names;
    int                           variants = 0;
    for (const auto& v : entries_) {
        variants += v.type.directional ? 4 : 1;
        if (v.id.empty() || v.type.name.empty() || !ids.insert(v.id.value()).second ||
            !names.insert(v.type.name).second)
            d.push_back(eve::editing::ruleDiagnostic(eve::DiagnosticCode::Conflict,
                RuleId("editor.voxel-palette.identity"), DiagnosticSeverity::Error,
                "Voxel palette IDs and names must be non-empty and unique"));
        if (v.type.connects && v.type.composeGroup.empty())
            d.push_back(eve::editing::ruleDiagnostic(eve::DiagnosticCode::PreconditionViolation,
                RuleId("editor.voxel-palette.connect-group"), DiagnosticSeverity::Warning,
                "Connecting cube type has no compose group"));
    }
    if (variants > 255)
        d.push_back(eve::editing::ruleDiagnostic(eve::DiagnosticCode::PreconditionViolation,
            RuleId("editor.voxel-palette.variant-capacity"), DiagnosticSeverity::Error,
            "Voxel palette exceeds 255 runtime variants"));
    if (entries_.empty())
        d.push_back(eve::editing::ruleDiagnostic(eve::DiagnosticCode::PreconditionViolation,
