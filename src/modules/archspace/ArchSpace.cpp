...
void ArchSpace::expose(ssq::Table& table) {
    ...
    document.addFunc("bootstrap", [vm](Document* self, const std::string& siteId, const std::string& buildingId,
        ...
        if (!self) return eve::script::projectResult(
        vm, eve::Result<void>::failure(eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument, "ArchSpaceDocument receiver must not be null", "receiver",
                                                           {}, "archspace.squirrel")));
