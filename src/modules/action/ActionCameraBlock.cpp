...
Result<double> optionalNumber(const Value::Object& payload, std::string_view field, double fallback) {
    ...
    if (const auto* integer = found->second.getIf<std::int64_t>())
        ...
        return Result<double>::failure(
            Diagnostic::error(DiagnosticCode::InvalidArgument, "camera cue field must be numeric", std::string(field)));
    if (!std::isfinite(value))
        return Result<double>::failure(
            Diagnostic::error(DiagnosticCode::InvalidArgument, "camera cue field must be finite", std::string(field)));
...
Result<ActionCameraCueBinding> ActionCameraCueBinding::fromPayload(const Value::Object& payload) {
    ...
    if (!cue)
        return Result<ActionCameraCueBinding>::failure(
            Diagnostic::error(DiagnosticCode::InvalidArgument, "camera cue must be text", "cue"));
    auto parsedCue = LogicalId::parse(*cue);
    if (!parsedCue)
        return Result<ActionCameraCueBinding>::failure(
            Diagnostic::error(DiagnosticCode::InvalidArgument, "camera cue must be a LogicalId", "cue"));
    ...
    if (position.value() < 0.0)
        return Result<ActionCameraCueBinding>::failure(Diagnostic::error(
            DiagnosticCode::InvalidArgument, "camera position amplitude must be non-negative", "positionAmplitude"));
    if (rotation.value() < 0.0)
        return Result<ActionCameraCueBinding>::failure(Diagnostic::error(
            DiagnosticCode::InvalidArgument, "camera rotation amplitude must be non-negative", "rotationAmplitude"));
    const double maximumFloat = static_cast<double>(std::numeric_limits<float>::max());
    if (position.value() > maximumFloat)
        return Result<ActionCameraCueBinding>::failure(Diagnostic::error(
            DiagnosticCode::InvalidArgument, "camera position amplitude exceeds runtime range", "positionAmplitude"));
    if (rotation.value() > maximumFloat)
        return Result<ActionCameraCueBinding>::failure(Diagnostic::error(
            DiagnosticCode::InvalidArgument, "camera rotation amplitude exceeds runtime range", "rotationAmplitude"));
    if (std::abs(fov.value()) > maximumFloat)
        return Result<ActionCameraCueBinding>::failure(Diagnostic::error(
            DiagnosticCode::InvalidArgument, "camera FOV amplitude exceeds runtime range", "fovAmplitude"));
    if (duration.value() <= 0.0)
        return Result<ActionCameraCueBinding>::failure(Diagnostic::error(
            DiagnosticCode::InvalidArgument, "camera cue duration must be positive", "durationSeconds"));
    if (duration.value() > maximumFloat)
        return Result<ActionCameraCueBinding>::failure(Diagnostic::error(
            DiagnosticCode::InvalidArgument, "camera cue duration exceeds runtime range", "durationSeconds"));
    ...
    if (const auto foundSeed = payload.find("seed"); foundSeed != payload.end()) {
        ...
        if (!seed || *seed < 0 || static_cast<std::uint64_t>(*seed) > std::numeric_limits<std::uint32_t>::max())
            return Result<ActionCameraCueBinding>::failure(Diagnostic::error(
                DiagnosticCode::InvalidArgument, "camera cue seed must be an unsigned 32-bit integer", "seed"));
