...
eve::Result<eve::Value> readJsonValue(const Store& store, std::string_view subject, std::string_view key) {
    if (!store.has(std::string(subject), std::string(key)))
        return eve::Result<eve::Value>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::NotFound, "state value is absent", "state"));
...
std::optional<eve::Value> StatePatchStateAdapter::value(std::string_view subject, std::string_view key) const {
    auto result = readJsonValue(store_, subject, key);
    if (!result) return std::nullopt;
    return std::move(result).takeValue();
}

std::optional<bool> StatePatchStateAdapter::hasTag(std::string_view subject, std::string_view tag) const {
...
std::optional<eve::Value> StatePatchStateAdapter::state(std::string_view subject, std::string_view key) const {
    return value(subject, key);
}

eve::Result<eve::MutationReceipt> StatePatchStateAdapter::apply(std::span<const eve::StateMutation> mutations,
                                                                const eve::MutationContext&         context) {
    if (context.transactionId.empty())
        return failure(eve::DiagnosticCode::InvalidArgument, "persistent dialogue mutations require a transaction id",
                       "transactionId");

    using Key = std::pair<std::string, std::string>;
    std::map<Key, std::optional<eve::Value>> pending;
    auto                                     batchReference = store_.newBatch();
    if (!batchReference) return eve::Result<eve::MutationReceipt>::failure(batchReference.status());
    const PatchBatchHandleRef batchHandle = std::move(batchReference).takeValue();
    auto                      batch       = store_.resolveBatch(batchHandle);
    if (!batch.isBound())
        return failure(eve::DiagnosticCode::StaleHandle, "StatePatch patch batch became stale during allocation",
                       "batch");
    struct BatchLease {
        Store&              store;
    ...
    for (std::size_t index = 0; index < mutations.size(); ++index) {
        ...
        if (!mutation.persistent)
            return failure(eve::DiagnosticCode::Unsupported, "StatePatch adapter accepts persistent mutations only",
                           path);
        if (mutation.subject.empty() || mutation.key.empty())
            return failure(eve::DiagnosticCode::InvalidArgument, "persistent mutation requires subject and key", path);

        const std::string sourceKey = mutation.key;
        const std::string key =
            mutation.kind == eve::MutationKind::AddTag || mutation.kind == eve::MutationKind::RemoveTag
                ? tagKey(sourceKey)
                : sourceKey;
        ...
        switch (mutation.kind) {
            case eve::MutationKind::Set: {
                ...
                if (!json)
                    return failure(json.error() ? json.error()->code() : eve::DiagnosticCode::SerializationError,
            ...
            case eve::MutationKind::AddNumber: {
                ...
                if (!numeric(mutation.value, delta))
                    return failure(eve::DiagnosticCode::InvalidArgument, "AddNumber requires a finite numeric value",
                                   path + ".value");
                ...
                if (!std::isfinite(total))
                    return failure(eve::DiagnosticCode::InvalidArgument, "AddNumber result must be finite", path);
                const eve::Value result(total);
                auto             json = result.toJson();
                if (!json)
                    return failure(eve::DiagnosticCode::SerializationError, "AddNumber result cannot be serialized",
                                   path);
                appended = batch->set(mutation.subject, key, json.value());
        ...
        if (!appended) return failure(eve::DiagnosticCode::Failed, "StatePatch rejected a mutation", path);
    }

    eve::transaction::TransactionContext transaction(context.transactionId, context.correlationId, context.causationId);
