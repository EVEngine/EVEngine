...
std::optional<eve::Value> cardValue(const CardData* card, const CardDefinition& definition, std::string_view key) {
...
decision::ConditionResult CardPlayConditionAdapter::evaluate(const CardData* card, const CardDefinition& definition,
...
public:
    CardTransferParticipant(CardData& card, eve::container::IContainer& source, eve::container::IContainer& destination,
    ...
    [[nodiscard]] std::string_view name() const noexcept override { return "card-container-transfer"; }
    ...
    [[nodiscard]] eve::Result<void> commit(const eve::transaction::TransactionContext&) override {
        if (!prepared_ || committed_) return cardLifecycleConflict("card transfer has no prepared stage");
    ...
    [[nodiscard]] eve::Result<void> rollback(const eve::transaction::TransactionContext&) override {
        if (committed_) return cardLifecycleConflict("committed card transfer requires compensation");
    ...
    [[nodiscard]] eve::Result<void> compensate(const eve::transaction::TransactionContext&) override {
        if (!committed_) return cardLifecycleConflict("card transfer has no committed state to compensate");
...
private:
    ...
    static void renumber(eve::container::ContainerSnapshot& snapshot) noexcept {
        for (std::size_t index = 0; index < snapshot.entries.size(); ++index)
            snapshot.entries[index].membership.slot = eve::container::SlotIndex(static_cast<std::int32_t>(index));
    }
...
eve::Result<void> CardPlayParticipant::prepare(const eve::transaction::TransactionContext& context) {
    (void)context;
    if (prepared_ || committed_) return cardLifecycleConflict("card play participant is already in flight or terminal");
...
eve::Result<void> CardPlayParticipant::commit(const eve::transaction::TransactionContext& context) {
    (void)context;
    if (!prepared_ || committed_) return cardLifecycleConflict("card play participant has no uncommitted stage");
    auto state = card_.state();
...
eve::Result<void> CardPlayParticipant::rollback(const eve::transaction::TransactionContext& context) {
    (void)context;
...
eve::Result<void> CardPlayParticipant::compensate(const eve::transaction::TransactionContext& context) {
    (void)context;
    if (!committed_) return cardLifecycleConflict("card play participant has no committed effect to compensate");
    auto state   = card_.state();
...
eve::Result<std::optional<eve::resource::CostSpec>> CardPlayPaymentAdapter::manaCost(const CardDefinition& definition) {
    ...
    return eve::Result<std::optional<eve::resource::CostSpec>>::success(std::move(cost).takeValue());
}
...
eve::Result<eve::transaction::TransactionReceipt> CardPlayPaymentAdapter::play(CardPlayRequest request) {
    ...
    if (request.composition.source != nullptr) {
        transfer = std::make_unique<CardTransferParticipant>(
            card, *request.composition.source, *request.composition.destination, request.composition.sourceSlot,
            request.composition.destinationSlot);
