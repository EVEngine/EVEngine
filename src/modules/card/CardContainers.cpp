...
[[nodiscard]] bool hasId(const std::vector<CardData*>& cards,
                         const eve::container::MembershipId& id) {
...
class CardContainerAdapter::PreparedState final
    : public eve::container::IContainer::PreparedState {
public:
    PreparedState(CardContainerAdapter& owner, std::vector<CardData*> staged,
...
CardContainerAdapter::CardContainerAdapter(eve::container::ContainerId id, CardContainerKind kind,
                                           Deck* deck, Hand* hand,
                                           std::vector<CardData*>* discard,
    ...
    : descriptor_{std::move(id), capacity,
      ...
      kind_(kind), deck_(deck), hand_(hand), discard_(discard) {
    if (!acceptedKinds.empty()) {
        descriptor_.filter = eve::container::Filter(
            [acceptedKinds = std::move(acceptedKinds)](
                const eve::container::ContainerObject& object) {
                if (std::find(acceptedKinds.begin(), acceptedKinds.end(), object.type) ==
                    acceptedKinds.end())
    ...
    if (kind_ == CardContainerKind::Deck && deck_ == nullptr)
        descriptor_.filter = eve::container::Filter([](const eve::container::ContainerObject&) {
            return eve::Result<void>::failure(eve::Diagnostic::error(
                eve::DiagnosticCode::InvalidArgument, "card deck adapter has no Deck"));
        });
    if (kind_ == CardContainerKind::Hand && hand_ == nullptr)
        descriptor_.filter = eve::container::Filter([](const eve::container::ContainerObject&) {
            return eve::Result<void>::failure(eve::Diagnostic::error(
                eve::DiagnosticCode::InvalidArgument, "card hand adapter has no Hand"));
        });
    if (kind_ == CardContainerKind::Discard && discard_ == nullptr)
        descriptor_.filter = eve::container::Filter([](const eve::container::ContainerObject&) {
            return eve::Result<void>::failure(eve::Diagnostic::error(
                eve::DiagnosticCode::InvalidArgument, "card discard adapter has no storage"));
        });
}
...
const std::vector<CardData*>* CardContainerAdapter::cards() const noexcept {
    if (kind_ == CardContainerKind::Deck)
        return deck_ ? &deck_->membership()->cards : nullptr;
    if (kind_ == CardContainerKind::Hand) return hand_ ? &hand_->membership()->cards : nullptr;
    return discard_;
}
...
eve::Result<eve::container::ContainerSnapshot> CardContainerAdapter::snapshot() const {
    ...
    if (!descriptor_.id.isValid() || values == nullptr)
        return eve::Result<eve::container::ContainerSnapshot>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument,
                                   "card container adapter is not bound"));
    if (!descriptor_.capacity.isUnlimited() && values->size() > descriptor_.capacity.value())
        return eve::Result<eve::container::ContainerSnapshot>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::InvariantViolation,
                                   "card container exceeds configured capacity"));
    ...
    for (std::size_t i = 0; i < values->size(); ++i) {
        if (i > static_cast<std::size_t>(std::numeric_limits<std::int32_t>::max()))
            return eve::Result<eve::container::ContainerSnapshot>::failure(
                eve::Diagnostic::error(eve::DiagnosticCode::InvariantViolation,
                                       "card container has too many indexed memberships"));
        CardData* card = (*values)[i];
        if (card == nullptr || card->identity()->id.empty())
            return eve::Result<eve::container::ContainerSnapshot>::failure(
                eve::Diagnostic::error(eve::DiagnosticCode::InvariantViolation,
                                       "card container contains null or unidentified card"));
        const auto id = objectId(*card);
        if (!ids.insert(id).second)
            return eve::Result<eve::container::ContainerSnapshot>::failure(
                eve::Diagnostic::error(eve::DiagnosticCode::Conflict,
                                       "card container contains duplicate card identity"));
        snapshot.entries.push_back({{id, eve::container::SlotIndex(static_cast<std::int32_t>(i)),
                                     eve::Generation(1)},
                                    describe(card)});
    }
...
eve::Result<void> CardContainerAdapter::validateInsert(
    ...
    if (object.quantity != 1u)
        return error(eve::DiagnosticCode::InvalidArgument,
                     "a card container membership must have quantity one");
    const auto* payload = dynamic_cast<const CardContainerObject*>(object.payload.get());
    auto* card = payload == nullptr ? nullptr
                                    : static_cast<CardData*>(ecs::try_get(payload->card));
    ...
    const std::size_t effectiveSize = values->size() -
        ((ignoredObject && hasId(*values, *ignoredObject)) ? 1u : 0u);
    if (destination) {
        if (!destination->isValid() ||
            static_cast<std::size_t>(destination->value()) > effectiveSize)
...
CardContainerAdapter::prepare(const eve::container::ContainerSnapshot& expected,
    ...
    if (expected.id != descriptor_.id || candidate.id != descriptor_.id)
        return eve::Result<std::unique_ptr<eve::container::IContainer::PreparedState>>::failure(
            eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument,
                                   "card prepare targets another container"));
    ...
    std::sort(ordered.begin(), ordered.end(), [](const auto* lhs, const auto* rhs) {
        return lhs->membership.slot < rhs->membership.slot;
    });
    ...
    for (std::size_t index = 0; index < ordered.size(); ++index) {
        ...
        auto* card = payload == nullptr ? nullptr
                                        : static_cast<CardData*>(ecs::try_get(payload->card));
        if (payload == nullptr || card == nullptr ||
            card->identity()->id != entry->membership.object.value() ||
            entry->object.type != card->identity()->kind)
            return eve::Result<std::unique_ptr<eve::container::IContainer::PreparedState>>::failure(
                eve::Diagnostic::error(eve::DiagnosticCode::StaleHandle,
                                       "card candidate payload is stale"));
