...
void exposePrimitiveScriptBindings(ssq::Table& table, ssq::Class& cls) {
    ...
    cls.addFunc("newPrimitiveGrid3D", [vm](Graphics* self, ssq::Array xyz, int cellsU, int cellsV,
        ...
        if (cellsU <= 0 || cellsV <= 0 || cellsU > 4096 || cellsV > 4096)
            return eve::script::projectStatusResult(
                vm,
                eve::Result<PrimitiveHandle>::failure(eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument,
                                                                             "grid cells must be in [1,4096]", "cells",
                                                                             {}, "graphics.primitive.binding"))
                    .status(),
                false, false);
    ...
    cls.addFunc(
        "setPrimitiveTransforms3D", [vm, primitiveTag](Graphics* self, ssq::Array proxies, ssq::Array matrices) {
            auto failure = [vm](const std::string& message) {
                return eve::script::projectStatusResult(
                    vm,
                    eve::Result<std::size_t>::failure(eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument,
                                                                             message, "transforms", {},
                                                                             "graphics.primitive.binding"))
                        .status(),
                    false, false);
            ...
            try {
                for (std::size_t i = 0; i < proxies.size(); ++i) {
                    ...
                    if (!descriptor) return eve::script::projectStatusResult(vm, descriptor.status(), false, false);
    ...
    primitive.addFunc("setLayer", [vm](ScriptPrimitive3D* value, std::int64_t layer) {
        if (layer < 0 || layer > 0xffffffffLL)
            return eve::script::projectStatusResult(
                vm,
                eve::Result<PrimitiveUpdateStatus>::failure(eve::Diagnostic::error(eve::DiagnosticCode::InvalidArgument,
                                                                                   "layer must fit uint32", "layer", {},
                                                                                   "graphics.primitive.binding"))
                    .status(),
                false, false);
