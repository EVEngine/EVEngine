...
void main() {
    ...
    if (dot(Ngeom, V) < 0.0)
        Ngeom = -Ngeom;
    vec2 uv = parallaxMappedUV(textures[heightSlot], vUV, Ngeom, vWorldPos, V,
                               m.parallax.x, m.parallax.y, m.parallax.z);
    vec4 base = textureCellBomb(textures[albedoSlot], uv, bombScale, bombStrength,
                                bombRot) * vTint;
    if (base.a < 0.5)
        discard;
