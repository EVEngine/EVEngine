...
void main() {
    ...
    vec2 uv;
    GpuMaterialRecord m;
    GpuInstance shadingInstance;
    shadingInstance.reflectionProbeSlots = uvec4(kInvalidSlot, kInvalidSlot, 0u, 0u);
    shadingInstance.reflectionProbeCenter[0] = vec4(0.0);
    ...
    vec3 color = (vis.x & 0x80000000u) != 0u
                     ? shadeGpuDrivenPixelFlat(m, shadingInstance, N, V, worldPos, viewPos)
                     : shadeGpuDrivenPixel(m, shadingInstance, N, V, worldPos, viewPos, uv);
