#include "graphics/editor/ReflectionProbeVisualizer.h"
...
ReflectionProbeVisualizer::ReflectionProbeVisualizer(graphics::ReflectionProbeCapture* probe) : probe_(probe) {}

void ReflectionProbeVisualizer::setExtents(float x, float y, float z) {
    if (!probe_) return;
    probe_->configureInfluence(x, y, z, probe_->getInfluenceIntensity(), probe_->getInfluenceBlendDistance(),
...
float ReflectionProbeVisualizer::coordinate(int corner, int component) const {
    ...
    return center[component] + ((corner & (1 << component)) ? extent[component] : -extent[component]);
}
