...
bool Graphics::gpuDrivenResolveWanted() const {
    return gpuDrivenEnabled_ && gpuDrivenCaps_.gpuDrivenCullAvailable() && renderControl_ &&
           renderControl_->isEnabled("visResolve") && sceneColorSamples == vk::SampleCountFlagBits::e1;
}
...
void Graphics::createGpuDrivenVisResources(int width, int height) {
    ...
    // gl_VertexIndex); draws non-indexed with the cull chain's NI commands.
    {
        std::vector<uint32_t> visVert(mesh3d_gbuffer_vis_vert_spv,
                                      mesh3d_gbuffer_vis_vert_spv + mesh3d_gbuffer_vis_vert_spv_count);
        std::vector<uint32_t> visFrag(mesh3d_gbuffer_vis_frag_spv,
                                      mesh3d_gbuffer_vis_frag_spv + mesh3d_gbuffer_vis_frag_spv_count);
        vk::ShaderModule      visVertModule = vkb::PipelineBuilder::createShaderModule(device.instance, visVert);
        vk::ShaderModule      visFragModule = vkb::PipelineBuilder::createShaderModule(device.instance, visFrag);
        gbufferVisPipeline = device.createPipeline()
    ...
    // VG variant: same vis attachments, cluster-stream fetch (no vertex input).
    {
        std::vector<uint32_t> vgVisVert(mesh3d_gbuffer_vgvis_vert_spv,
                                        mesh3d_gbuffer_vgvis_vert_spv + mesh3d_gbuffer_vgvis_vert_spv_count);
        std::vector<uint32_t> vgVisFrag(mesh3d_gbuffer_vgvis_frag_spv,
                                        mesh3d_gbuffer_vgvis_frag_spv + mesh3d_gbuffer_vgvis_frag_spv_count);
        vk::ShaderModule      vgVisVertModule = vkb::PipelineBuilder::createShaderModule(device.instance, vgVisVert);
        vk::ShaderModule      vgVisFragModule = vkb::PipelineBuilder::createShaderModule(device.instance, vgVisFrag);
        gbufferVgVisPipeline = device.createPipeline()
...
void Graphics::createBindlessSet() {
    ...
    const vk::ShaderStageFlags allStages =
    ...
    const vk::ShaderStageFlags vertFrag    = vk::ShaderStageFlagBits::eVertex | vk::ShaderStageFlagBits::eFragment;
    std::vector<vk::DescriptorSetLayoutBinding> bindings{
        ...
        storageBinding(29, vk::ShaderStageFlagBits::eVertex | vk::ShaderStageFlagBits::eFragment |
                               vk::ShaderStageFlagBits::eCompute),  // VG asset models
        storageBinding(30, computeOnly),                            // non-indexed indirect commands (vis pass)
        storageBinding(31, vertFrag),                               // optional vegetation vertex streams
    };
    ...
    std::array<vk::DescriptorPoolSize, 3> poolSizes{
        ...
        vk::DescriptorPoolSize{vk::DescriptorType::eStorageBuffer, 26 * setCount},
    ...
    vk::DescriptorImageInfo white2D{white->sampler, white->imageView(), vk::ImageLayout::eShaderReadOnlyOptimal};
    ...
    auto chunkFill = [&](vk::DescriptorSet set, uint32_t binding, uint32_t total, const vk::DescriptorImageInfo& info) {
    ...
    meshTableBuffer_   = vkb::GenericBuffer(device, vk::BufferUsageFlagBits::eStorageBuffer,
                                            kMaxMeshRecords * sizeof(GpuMeshRecord), kHostVisibleCoherent);
    ...
    materialTableBuffer_   = vkb::GenericBuffer(device, vk::BufferUsageFlagBits::eStorageBuffer,
                                                kMaxMaterialRecords * sizeof(GpuMaterialRecord), kHostVisibleCoherent);
    ...
    gpuDrivenCullParamsPlaceholder_ =
        vkb::GenericBuffer(device, vk::BufferUsageFlagBits::eUniformBuffer, 256, kHostVisibleCoherent);
    {
    ...
    {
        vk::DescriptorImageInfo depthInfo{white->sampler, white->imageView(), vk::ImageLayout::eShaderReadOnlyOptimal};
        for (vk::DescriptorSet set : bindlessSets_) {
            vk::WriteDescriptorSet w{};
            w.dstSet          = set;
            w.dstBinding      = 11;
    ...
    for (vk::DescriptorSet set : bindlessSets_) {
        vk::DescriptorImageInfo visInfo{white->sampler, white->imageView(), vk::ImageLayout::eShaderReadOnlyOptimal};
        vk::WriteDescriptorSet  w{};
        w.dstSet          = set;
        w.dstBinding      = 15;
        w.descriptorCount = 1;
...
void Graphics::ensureGpuVertexPool() {
    ...
    gpuVertexPool_.vegetation  = vkb::GenericBuffer(device, vk::BufferUsageFlagBits::eStorageBuffer,
                                                    kInitialVertices * sizeof(GpuVegetationVertexRecord), hostMem);
...
void Graphics::growGpuVertexPool(uint32_t needVertices, uint32_t needIndices) {
    ...
    copyInto(gpuVertexPool_.vegetation, gpuVertexPool_.vegetation,
             vk::DeviceSize(oldVerts) * sizeof(GpuVegetationVertexRecord),
             vk::DeviceSize(newVerts) * sizeof(GpuVegetationVertexRecord));
    bindGpuVertexPoolBindless();
}
...
void Graphics::bindGpuVertexPoolBindless() {
    ...
    for (vk::DescriptorSet set : bindlessSets_) {
        ...
        bufWrite(set, 31, gpuVertexPool_.vegetation.buffer, vk::DeviceSize(cap) * sizeof(GpuVegetationVertexRecord));
    }
}

void Graphics::appendGpuMeshToPool(const Mesh& mesh, GpuMesh& gpu) {
    ...
    auto* vegDst = static_cast<GpuVegetationVertexRecord*>(gpuVertexPool_.vegetation.map());
    if (!posDst || !nrmDst || !uvDst || !idxDst || !vegDst) {
        gpu.vertices.unmap();
        gpu.indices.unmap();
        return;
    }
    ...
    vegDst += gpuVertexPool_.vertexCount;
    const auto& tangents       = mesh.importedTangents();
    const auto& bitangents     = mesh.importedBitangents();
    const auto  factors        = mesh.vegetationFactors();
    const auto  highlights     = mesh.motionHighlights();
    const auto  deformation    = mesh.vegetationDeformationFactors();
    const bool  hasTangent     = tangents.size() == size_t(nVerts) * 3u && bitangents.size() == tangents.size();
    const bool  hasFactors     = factors.size() == size_t(nVerts) * 5u;
    const bool  hasHighlights  = highlights.size() == size_t(nVerts);
    const bool  hasDeformation = deformation.size() == size_t(nVerts) * 9u;
    for (uint32_t i = 0; i < nVerts; ++i) {
        posDst[i] = glm::vec4(verts[i].pos, 0.f);
        nrmDst[i] = glm::vec4(verts[i].normal, 0.f);
        uvDst[i]  = verts[i].uv;
        GpuVegetationVertexRecord record{};
        if (hasTangent) {
            record.tangent   = glm::vec4(tangents[i * 3u], tangents[i * 3u + 1u], tangents[i * 3u + 2u], 1.f);
            record.bitangent = glm::vec4(bitangents[i * 3u], bitangents[i * 3u + 1u], bitangents[i * 3u + 2u], 0.f);
        }
        const size_t factorBase = size_t(i) * 5u;
        record.factors0 =
            glm::vec4(hasFactors ? factors[factorBase] : 0.f, hasFactors ? factors[factorBase + 1u] : 1.f,
                      hasFactors ? factors[factorBase + 2u] : 0.f, hasFactors ? factors[factorBase + 3u] : 0.f);
        record.factors1 =
            glm::vec4(hasFactors ? factors[factorBase + 4u] : 0.f, hasHighlights ? highlights[i] : 0.f,
                      float((hasFactors ? 1u : 0u) | (hasHighlights ? 2u : 0u) | (hasDeformation ? 4u : 0u)), 0.f);
        if (hasDeformation) {
            const size_t base = size_t(i) * 9u;
            record.deformation0 =
                glm::vec4(deformation[base], deformation[base + 1u], deformation[base + 2u], deformation[base + 3u]);
            record.deformation1   = glm::vec4(deformation[base + 4u], deformation[base + 5u], deformation[base + 6u],
                                              deformation[base + 7u]);
            record.deformation2.x = deformation[base + 8u];
        }
        vegDst[i] = record;
    }
    ...
    gpuVertexPool_.vegetation.unmap();
    gpu.vertices.unmap();
    gpu.indices.unmap();
...
uint32_t Graphics::registerBindlessTexture2D(GpuTexture* tex) {
    ...
    vk::DescriptorImageInfo img{tex->sampler, tex->imageView(), vk::ImageLayout::eShaderReadOnlyOptimal};
    for (vk::DescriptorSet set : bindlessSets_) {
...
void Graphics::unregisterBindlessTexture(GpuTexture* tex) {
    ...
    auto restore = [&](uint32_t binding, uint32_t slot, GpuTexture* placeholder, bool cube) {
        vk::DescriptorImageInfo img{placeholder->sampler,
...
void Graphics::ensureVgBuffers() {
    ...
    vgGpu_.positions =
        vkb::GenericBuffer(device, vk::BufferUsageFlagBits::eStorageBuffer, kInitVertFloats * sizeof(float), hostMem);
    vgGpu_.triangles =
        vkb::GenericBuffer(device, vk::BufferUsageFlagBits::eStorageBuffer, kInitTriangles * sizeof(uint32_t), hostMem);
    ...
    vgGpu_.clusterAssets =
        vkb::GenericBuffer(device, vk::BufferUsageFlagBits::eStorageBuffer, kInitClusters * sizeof(uint32_t), hostMem);
    ...
    vgGpu_.visible =
        vkb::GenericBuffer(device, vk::BufferUsageFlagBits::eStorageBuffer | vk::BufferUsageFlagBits::eIndirectBuffer,
                           kVisBytes * kAsyncResourceCopies, hostMem);
    vgGpu_.indirect =
        vkb::GenericBuffer(device, vk::BufferUsageFlagBits::eStorageBuffer | vk::BufferUsageFlagBits::eIndirectBuffer,
                           kIndBytes * kAsyncResourceCopies, hostMem);
    vgGpu_.assetMaterials = vkb::GenericBuffer(device, vk::BufferUsageFlagBits::eStorageBuffer,
                                               kMaxVgAssets * sizeof(uint32_t) * kAsyncResourceCopies, hostMem);
    vgGpu_.assetModels    = vkb::GenericBuffer(device, vk::BufferUsageFlagBits::eStorageBuffer,
...
void Graphics::growVgBuffers(uint32_t needClusters, uint32_t needVertices, uint32_t needTriangles) {
    device->waitIdle();
    const auto hostMem    = kHostVisibleCoherent;
    auto       growBuffer = [&](vkb::GenericBuffer& dst, uint32_t elementSize, uint32_t oldElems, uint32_t newElems) {
    ...
    growBuffer(vgGpu_.clusters, uint32_t(sizeof(GpuVgCluster)), curClusters, std::max(needClusters, curClusters * 2u));
    growBuffer(vgGpu_.clusterAssets, uint32_t(sizeof(uint32_t)), curClusters, std::max(needClusters, curClusters * 2u));
    growBuffer(vgGpu_.positions, uint32_t(sizeof(float)), curVertFloats, std::max(needVertices, curVertFloats * 2u));
    growBuffer(vgGpu_.triangles, uint32_t(sizeof(uint32_t)), curTriangles, std::max(needTriangles, curTriangles * 2u));
    bindVgPoolBindless();
}

void Graphics::bindVgPoolBindless() {
    if (bindlessSets_.empty() || !vgGpu_.positions.buffer) return;
    auto bufWrite = [&](vk::DescriptorSet set, uint32_t binding, vk::Buffer buffer, vk::DeviceSize offset,
                        vk::DeviceSize size) {
...
uint32_t Graphics::gpuDrivenVgUpload(const GpuVgAssetUpload& asset) {
    ...
    if (!asset.positions || asset.vertexCount <= 0 || !asset.triangles || asset.triangleCount <= 0 || !asset.clusters ||
        asset.clusterCount <= 0)
    ...
    std::memcpy(static_cast<char*>(clMap) + vgClusterCount_ * sizeof(GpuVgCluster), asset.clusters,
                size_t(asset.clusterCount) * sizeof(GpuVgCluster));
    {
    ...
    // Default identity model per slot; vgSetInstance overwrites the current slot.
    {
        ...
        for (size_t s = 0; s < kAsyncResourceCopies; ++s) models[s * kMaxVgAssets + assetId] = glm::mat4(1.f);
        vgGpu_.assetModels.unmap();
    }
    return assetId;
}
...
bool Graphics::gpuDrivenVgAttachToMesh(Mesh* mesh, uint32_t vgAssetId) {
    ...
    if (gpu->gpuRecordIndex != kInvalidBindlessSlot && gpu->gpuRecordIndex < meshTableRecords_.size()) {
...
bool Graphics::gpuDrivenVgSetInstance(uint32_t vgAssetId, const glm::mat4& model, uint32_t materialId) {
    if (vgAssetId >= vgAssetCount_ || vgAssetId >= kMaxVgAssets) return false;
...
uint32_t Graphics::registerMeshRecord(Mesh* mesh, GpuMesh* gpu) {
    if (!mesh || !gpu) return kInvalidBindlessSlot;
...
GpuMaterialRecord Graphics::buildMaterialRecord(Material* material) {
    ...
    rec.tint    = glm::vec4(material->getTintR(), material->getTintG(), material->getTintB(), material->getTintA());
    rec.pbr     = glm::vec4(material->getMetallic(), material->getRoughness(), material->getReceiveShadow() ? 1.f : 0.f,
    ...
    rec.surface  = glm::vec4(material->getAlphaCutoff(),
                             material->surfaceMode() == SurfaceMode::Masked        ? 1.f
                             : material->surfaceMode() == SurfaceMode::Transparent ? 2.f
                                                                                   : 0.f,
                             0.f, 0.f);
    ...
    rec.shadingModel        = (model == "unlit") ? 1u : (model == "hair") ? 2u : (model == "custom") ? 3u : 0u;
    if (material->getCastShadow()) rec.flags |= 1u;
    if (material->getCastOcclusion()) rec.flags |= 2u;
    if (material->getCameraFacing()) rec.flags |= 4u;
    return rec;
}
...
void Graphics::syncMaterialTable() {
    ...
    materialTableBuffer_.updateLocal(vkb::FrameSlot::gpuIdle(), materialTableRecords_.data(),
                                     materialTableRecords_.size() * sizeof(GpuMaterialRecord));
}

uint32_t Graphics::gpuDrivenMeshRecord(Mesh* mesh) {
    if (!mesh || !mesh->gpuHandle) return kInvalidBindlessSlot;
    auto* gpu = static_cast<GpuMesh*>(mesh->gpuHandle);
    if (gpu->gpuRecordIndex == kInvalidBindlessSlot) registerMeshRecord(mesh, gpu);
...
bool Graphics::gpuDrivenSubmitOpaque(const GpuInstance* instances, uint32_t instanceCount) {
    ...
    std::vector<GpuIndirectCommand> cmds  = builder.commands();
    const auto&                     order = builder.sortedInstanceOrder();

    // Stage-1 binds each mesh's private vertex/index buffers below. Mesh table
    // offsets address the shared visibility-resolve pool and are invalid for
    // those private buffers.
    ...
    cb.bindDescriptorSets(vk::PipelineBindPoint::eGraphics, mesh3dGpuDrivenPipelineLayout, 0, 1, &s0.set, 2,
                          dynOffsets);
    ...
    for (uint32_t i = 0; i < drawCount; ++i) {
        ...
        cb.drawIndexedIndirect(arena.buffer(), cmdAlloc.offset + vk::DeviceSize(i) * stride, 1, stride);
    }
    return true;
}
...
GpuResidentSubmitStatus Graphics::gpuDrivenSubmitResident(const GpuResidentInstanceBatch& batch) {
    ...
    for (uint32_t i = 0; i < batch.bucketCount; ++i) {
        ...
        // Resident stage-1 also binds the mesh's private buffers. Shared-pool
        // offsets are used only by the visibility/cull path.
        commands[i] = {mesh.indexCount, bucket.instanceCount, 0, 0, bucket.firstInstance};
...
void Graphics::ensureGpuDrivenCullResources(int width, int height) {
    ...
    if (gpuDrivenCullReady_ && gpuDrivenCullWidth == width && gpuDrivenCullHeight == height) return;
    ...
    for (auto& slot : gpuDrivenCullSlots_) {
        slot.visibleFlags =
            vkb::GenericBuffer(device, vk::BufferUsageFlagBits::eStorageBuffer, flagBytes, kHostVisibleCoherent);
        slot.compacted =
            vkb::GenericBuffer(device, vk::BufferUsageFlagBits::eStorageBuffer, compactBytes, kHostVisibleCoherent);
        slot.indirect = vkb::GenericBuffer(
            device, vk::BufferUsageFlagBits::eStorageBuffer | vk::BufferUsageFlagBits::eIndirectBuffer, indirectBytes,
            kHostVisibleCoherent);
        slot.indirectNI = vkb::GenericBuffer(
            device, vk::BufferUsageFlagBits::eStorageBuffer | vk::BufferUsageFlagBits::eIndirectBuffer, indirectNIBytes,
            kHostVisibleCoherent);
        slot.bucketCounters =
            vkb::GenericBuffer(device, vk::BufferUsageFlagBits::eStorageBuffer, counterBytes, kHostVisibleCoherent);
        slot.hzb = vkb::GenericBuffer(device, vk::BufferUsageFlagBits::eStorageBuffer, hzbBytes, kHostVisibleCoherent);
        slot.cullParams = vkb::GenericBuffer(device, vk::BufferUsageFlagBits::eUniformBuffer, sizeof(GpuCullParams),
                                             kHostVisibleCoherent);
    ...
    std::array<vk::DescriptorSetLayout, 2> computeLayouts{gpuDrivenComputeEmptyLayout_, bindlessSetLayout_};
    ...
    const auto spvOf = [](const uint32_t* p, size_t n) { return std::vector<uint32_t>(p, p + n); };
    hzbBuildPass_.create(device, gpuDrivenComputeLayout, spvOf(hzb_build_comp_spv, hzb_build_comp_spv_count));
    cullPass_.create(device, gpuDrivenComputeLayout, spvOf(gpu_cull_comp_spv, gpu_cull_comp_spv_count));
    emitPass_.create(device, gpuDrivenComputeLayout, spvOf(gpu_emit_comp_spv, gpu_emit_comp_spv_count));
    vgCullPass_.create(device, gpuDrivenComputeLayout, spvOf(vg_main_cull_comp_spv, vg_main_cull_comp_spv_count));
    gpuDrivenCullReady_ = hzbBuildPass_.pipeline() && cullPass_.pipeline() && emitPass_.pipeline();
}
...
void Graphics::recordGpuDrivenHzbBuild() {
    ...
    cb.pipelineBarrier(vk::PipelineStageFlagBits::eEarlyFragmentTests | vk::PipelineStageFlagBits::eLateFragmentTests,
                       vk::PipelineStageFlagBits::eComputeShader, {}, 0, nullptr, 0, nullptr, 1, &imb);
    ...
    for (uint32_t m = 0; m <= gpuDrivenCullMaxMip; ++m) {
        ...
        cb.pushConstants(gpuDrivenComputeLayout, vk::ShaderStageFlagBits::eCompute, 0, sizeof(push), &push);
...
bool Graphics::gpuDrivenCullBegin(const GpuInstance* instances, uint32_t instanceCount) {
    ...
    if (!gpuDrivenInstAlloc_.mapped || !gpuDrivenBucketIdAlloc_.mapped || !gpuDrivenBucketOffAlloc_.mapped) {
        gpuDrivenCullInstanceCount_ = 0;
        return false;
    }
    std::memcpy(gpuDrivenInstAlloc_.mapped, sorted.data(), instanceCount * sizeof(GpuInstance));
    std::memcpy(gpuDrivenBucketIdAlloc_.mapped, gpuDrivenBucketIds_.data(), instanceCount * sizeof(uint32_t));
    std::memcpy(gpuDrivenBucketOffAlloc_.mapped, gpuDrivenBucketOffsets_.data(), bucketCount * sizeof(uint32_t));
    ...
    auto bufWrite = [&](uint32_t binding, vk::Buffer buffer, vk::DeviceSize offset, vk::DeviceSize size) {
        vk::DescriptorBufferInfo info{buffer, offset, size};
        vk::WriteDescriptorSet   w{};
        w.dstSet          = bindless;
        w.dstBinding      = binding;
...
void Graphics::gpuDrivenRecordComputeSection(const glm::mat4& viewProj, const glm::vec3& eye, float fovYDeg,
                           ...
                           gbufferHeight > 0 ? gbufferHeight : int(swapchain.extent.height));
    {
        ...
        if (gbSlot && gbSlot->visIDGpu.sampler && gbSlot->visBaryGpu.sampler) {
            vk::DescriptorImageInfo visIDInfo{gbSlot->visIDGpu.sampler, gbSlot->visIDGpu.imageView(),
                                              vk::ImageLayout::eShaderReadOnlyOptimal};
            vk::DescriptorImageInfo visBaryInfo{gbSlot->visBaryGpu.sampler, gbSlot->visBaryGpu.imageView(),
                                                vk::ImageLayout::eShaderReadOnlyOptimal};
    ...
    params.clipNearFar = glm::vec4(nearZ, farZ, float(h) * 0.5f / std::tan(fovRad * 0.5f), 1.f);
    params.hzbInfo     = glm::vec4(float(gpuDrivenCullMaxMip), 0.f, float(w), float(h));
    params.counts      = glm::uvec4(gpuDrivenCullInstanceCount_, gpuDrivenBucketCount_, kMaxGpuDrivenBuckets, 0u);
    {
    ...
    bufWrite(6, slot.cullParams.buffer, 0, sizeof(GpuCullParams), vk::DescriptorType::eUniformBuffer);
    bufWrite(7, slot.visibleFlags.buffer, 0, VK_WHOLE_SIZE, vk::DescriptorType::eStorageBuffer);
    ...
    bufWrite(14, slot.bucketCounters.buffer, 0, VK_WHOLE_SIZE, vk::DescriptorType::eStorageBuffer);
    ...
    cb.bindDescriptorSets(vk::PipelineBindPoint::eCompute, gpuDrivenComputeLayout, 1, 1, &bindless, 0, nullptr);
    ...
    // HZB build writes (storage buffer) -> cull reads.
    {
        ...
        bmb.srcAccessMask = vk::AccessFlagBits::eShaderWrite;
        bmb.dstAccessMask = vk::AccessFlagBits::eShaderRead;
        cb.pipelineBarrier(vk::PipelineStageFlagBits::eComputeShader, vk::PipelineStageFlagBits::eComputeShader, {}, 0,
                           nullptr, 1, &bmb, 0, nullptr);
    }
}

void Graphics::gpuDrivenVgComputeSection(const glm::mat4& viewProj, const glm::vec3& eye, float fovYDeg, float nearZ,
                                         float farZ) {
...
void Graphics::gpuDrivenCullEmit(const glm::mat4& viewProj, const glm::vec3& eye, float fovYDeg, float nearZ,
    ...
    // Arena host writes + previous-frame HZB shader writes visible to cull.
    {
        ...
        bmb.dstAccessMask = vk::AccessFlagBits::eShaderRead;
        cb.pipelineBarrier(vk::PipelineStageFlagBits::eHost | vk::PipelineStageFlagBits::eComputeShader,
                           vk::PipelineStageFlagBits::eComputeShader, {}, 0, nullptr, 1, &bmb, 0, nullptr);
    }
    ...
    // Emit writes (compacted + commands) -> vertex read + indirect read.
    {
        ...
        cb.pipelineBarrier(vk::PipelineStageFlagBits::eComputeShader,
                           vk::PipelineStageFlagBits::eVertexShader | vk::PipelineStageFlagBits::eDrawIndirect, {}, 0,
                           nullptr, 2, bmb, 0, nullptr);
    }
}
...
Graphics::GpuDrivenFrameSet0 Graphics::gpuDrivenFrameSet0() {
    ...
    auto*    gpuNormal =
        static_cast<GpuTexture*>(mesh3dNormalTexture ? mesh3dNormalTexture->gpuHandle : flatNormalTexture3D->gpuHandle);
    auto* gpuHeight =
        static_cast<GpuTexture*>(mesh3dHeightTexture ? mesh3dHeightTexture->gpuHandle : flatHeightTexture3D->gpuHandle);
    ...
    uint32_t       probeSlots[ReflectionProbeUpload::kMaxProbes] = {kInvalidBindlessSlot, kInvalidBindlessSlot};
    ...
    ubo.bindlessEnv = glm::vec4(float(envSlot), mesh3dEnvIntensity, float(probeSlots[0]), float(probeSlots[1]));
    ...
    vk::DescriptorSet set = mesh3dSetFor(gpuTex, gpuNormal, gpuEnv, gpuHeight, gpuDepth, gpuSceneColor, gpuDecalAlb,
                                         gpuDecalNrm, gpuDecalPrm, fslots);
...
void Graphics::gpuDrivenRecordVisPass() {
    ...
    if (s0.set && bindless) {
        ...
        cb.bindDescriptorSets(vk::PipelineBindPoint::eGraphics, mesh3dGpuDrivenPipelineLayout, 0, 1, &s0.set, 2,
                              dynOffsets);
        cb.bindDescriptorSets(vk::PipelineBindPoint::eGraphics, mesh3dGpuDrivenPipelineLayout, 1, 1, &bindless, 0,
                              nullptr);
...
void Graphics::gpuDrivenResolve() {
    ...
    cb.bindDescriptorSets(vk::PipelineBindPoint::eGraphics, mesh3dGpuDrivenPipelineLayout, 0, 1, &s0.set, 2,
                          dynOffsets);
    cb.bindDescriptorSets(vk::PipelineBindPoint::eGraphics, mesh3dGpuDrivenPipelineLayout, 1, 1, &bindless, 0, nullptr);
    const uint32_t w = uint32_t(sceneColorPassOpen ? sceneColorWidth : int(swapchain.extent.width));
    const uint32_t h = uint32_t(sceneColorPassOpen ? sceneColorHeight : int(swapchain.extent.height));
    setViewportAndScissor(cb, w, h);
    cb.draw(3, 1, 0, 0);
}

void Graphics::createResolveVisPipeline(const vkb::BuiltRenderPass& rp, vk::SampleCountFlagBits samples) {
    destroyPipeline(device, resolveVisPipeline);
    if (!mesh3dGpuDrivenPipelineLayout) return;
    std::vector<uint32_t> vert(resolve_vis_vert_spv, resolve_vis_vert_spv + resolve_vis_vert_spv_count);
    std::vector<uint32_t> frag(resolve_vis_frag_spv, resolve_vis_frag_spv + resolve_vis_frag_spv_count);
    ...
    resolveVisPipeline = device.createPipeline()
...
void Graphics::drawVgClusters(vk::CommandBuffer cb) {
    ...
    cb.bindDescriptorSets(vk::PipelineBindPoint::eGraphics, mesh3dGpuDrivenPipelineLayout, 1, 1, &bindless, 0, nullptr);
    cb.drawIndirectCount(vgGpu_.indirect.buffer, slotInd * slot, vgGpu_.visible.buffer, slotVis * slot, kMaxVgClusters,
                         sizeof(glm::uvec4));
}
...
void Graphics::createMesh3DGpuDrivenPipeline() {
    ...
    mesh3dGpuDrivenPipeline =
        createMesh3DStylePipeline(embeddedSpirv(mesh3d_gpudriven_vert_spv), embeddedSpirv(mesh3d_gpudriven_frag_spv),
                                  mesh3dGpuDrivenPipelineLayout, renderpass, vk::SampleCountFlagBits::e1);
}
}  // namespace eve::graphics::vulkan
