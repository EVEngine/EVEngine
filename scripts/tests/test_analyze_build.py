...
class AnalyzeBuildTests(unittest.TestCase):
    def test_reports_header_fanout_templates_and_objects(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(headers[0].path, "src/engine/common/Box.h")
            self.assertEqual(headers[0].fanout, 2)
            self.assertEqual(headers[0].templates, 1)
            self.assertEqual(objects[0][0], 1024)
