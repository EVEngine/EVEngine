#!/usr/bin/env python3
"""Compile sprite-stack GLSL vert/frag to SPIR-V C++ include arrays."""
...
SHADER_DIR = ROOT / "src" / "modules" / "spritestack" / "shaders"

SHADERS = [
    ("sprite_stack.vert", "vert"),
    ("sprite_stack.frag", "frag"),
]
...
def compile_shader(src: Path, stage: str) -> Path:
    out = src.with_suffix(f".{stage}.spv")
...
def main() -> int:
    ...
    for name, stage in SHADERS:
        ...
        stem = src.stem  # e.g. sprite_stack
