...
2. 内部有众多 C++ 编写的高性能模块，你还可以把自己的 C++ 库编译成动态链接库与这些模块直接交互；
3. 有完整的开发环境：自己写的逻辑可以直接用 GUI 工具编辑，且编辑器运行时就是游戏运行时——只有发布用的运行时才会裁剪开发工具（也可以选择保留，供玩家二次创作）；
4. 内置资产管理、热重载，无需额外管线；
5. 可扫描类并自动为其生成编辑用 GUI；
6. GUI 可以即时根据脚本动态创建和同步；
7. 可暂停游戏循环进行调试；
8. 拥有众多独立状态机模型，支持局部热更新；
9. 支持 3D 与 2D 混合渲染。
