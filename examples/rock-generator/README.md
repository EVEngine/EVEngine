...
- **Next base shape** cycles through mixed, boulder, slab, block, shard, and cliff silhouettes.
...
`block`, `shard`, or `cliff`; `mixed` chooses a profile deterministically from the seed.
`cliff` biases toward a taller, more faceted outcrop with a steep primary face. Shape composition
