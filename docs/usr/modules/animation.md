...
- `AnimRetargetProfile`：用 `addBoneMapping()` / `clearBoneMappings()` 管理 Avatar 式骨骼映射，`setRootBones()`、`setAutoRootScale()`、`setRootTranslationScale()` 和 `setUseSkeletonSpaceRotation()` 配置重定向，通过匹配/未匹配计数和骨名接口读取最近一次烘焙诊断；配合 `AnimClip.retargetWithProfile()` 使用。
