...
- `AnimClip`：`setName()`、`getName()`、`setDuration()`、`getDuration()`、`setLoop()`、`getLoop()`、`setSampleRate()`、`addPositionKey()`、`addRotationKey()`、`addScaleKey()`、`compress()`、`retarget()`、`sample()`、`wrapTime()`；事件标记使用 `addEvent()`、`getEventCount()`、`getEventTime()`、`getEventName()`、`getEventPayload()`。
