...
- 体型差导致手贴胸、双脚交叉等部位关系被破坏时，可开启 `setSkinnedInteractionPreserve(true)`：引擎在 FK 烘焙后用骨骼附着传感器重建源动作的身体部位相对关系，并通过双骨 IK 拉回末端（经典 SMR/MeshRet 思路，不依赖神经网络权重）。可用 `addInteractionIkChain(root, mid, tip)` 指定手臂/腿链，用 `getInteractionCorrectionCount()` 确认修正帧数。
