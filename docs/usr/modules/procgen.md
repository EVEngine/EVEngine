...
world seed、level、x、z 派生的独立 seed、revision 和 PointSet 输出缓存。
...
`execute(outputId)` 只求值该输出的祖先节点；没有配置变化时复用缓存。
