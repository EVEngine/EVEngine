...
- `MapFog`：`newMapFog()`、`update()`、`setCloudTexture()`、`setMaskTexture()`、`setCloudTiling()`、`setCloudSpeed()`、`setDistort()`、`setFogColor()`、`setFogAlpha()`、`setEdgeSoftness()`、`setShadowEnabled()`、`setShadow()`、`setSelectStrength()`、`makeCloudTexture()`、`draw()`（大地图迷雾：双层云 + mask R/G/B）
