...
由 `stylize.render.cylinderStyleGallery` 生成（暖色棋盘 albedo、3/4 视角定向光）。  
`cartoon` 使用物体空间 cel 网格着色；`ink` / `watercolor` / `pixel` 在同一基础前向帧上做图像空间后处理。
...
图片路径：`docs/images/stylize/cylinder_*.png`（测试同时写入 `build/.../test/out/`）。
