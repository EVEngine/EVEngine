...
5. **双后端**：`IGraphics3D` 由 Vulkan 与 WebGPU 共同实现；WebGPU 已有
   `supportsGBufferPost() == false` 先例，贴花同样可以 Vulkan-only、WebGPU 报告不支持。
...
| 纹理 | 格式 | 内容 |
|------|------|------|
| `decalAlbedo` | RGBA8 | RGB = premultiplied albedo，A = coverage |
...
6. `coverage = atlasAlpha × fade × edgeFalloff`，以 premultiplied alpha
   （blend：ONE, ONE_MINUS_SRC_ALPHA）写入 3 个 RT。
...
if (cov > 0.001) {
    // 混合模式由材质参数决定（over / multiply / screen / add）
    albedo    = decalBlend(albedo, dA.rgb / max(cov, 1e-3), cov, blendMode);
    vec3 dN   = texture(decalNormalSampler, sUV).xyz * 2.0 - 1.0;
    N         = normalize(mix(N, dN, cov * normalStrength));
    vec4 dp   = texture(decalParamsSampler, sUV);
    roughness = mix(roughness, dp.r, cov * roughStrength);
    metallic  = mix(metallic,  dp.g, cov * metalStrength);
    emissive += dp.b * cov * emissiveStrength;    // 最后加到 outColor 前
}
```
...
- **WebGPU 后端**：`supportsDecal() == false`，feature 自动失效，不阻塞构建。
...
| 项 | P0 默认 | 后续优化 |
|----|---------|----------|
| Draw 数 | 每贴花 1 draw，只画 frustum 内 + 距离阈值内（≤ ~128 draw 合理） | 实例化单位盒单 draw（实例数据 SSBO，参照 clustered SSBO 模式） |
...
| **P1 完整 PBR** | normal/roughness/metallic/emissive/height 混合、图集 + JSON 资源、混合模式、淡入淡出、frustum 剔除、实例化批、配额、测试图廊 | 覆盖 §1 全部效果（凹坑=法线贴花） |
| **P2 扩展** | 模型常驻贴花（路线 B overlay 图层）、POM 假凹坑、编辑器 gizmo（`IDecalQuery` 能力）、血泊 SDF、shader 变体统一 | 角色战损/皮肤、可编辑放置 |
| **P3 可选** | 真几何位移（英雄道具）、半分辨率 decal layer、WebGPU 支持 | 移动端与旗舰表现 |
