...
| **scene color** | swapchain 同格式 RGBA，A=1 清屏（天空），mesh 写 A=线性深度 | 前向 mesh | FXAA `MainTex`；手动 `getSceneColorTexture()` | `Graphics.getSceneColorTexture()` |
| **scene color depth** | 与 swapchain 相同的 HW depth | 前向 z-test | **不采样**（`storeOp=DontCare`） | — |
...
| 3 Scene color | `"forward"` / `"hair"` | `begin3DFrame` → `drawMeshShader` | `beginSceneColorRenderPass` 内直接 draw | 见下表 mesh |
| 4 FXAA resolve | `"aa"`（关则阈值拉满≈拷贝） | `flushToSwapchain` → `queueSceneColorResolve` | swapchain pass 第一张全屏四边形 | `aa_fxaa.frag`（`textured.vert`） |
