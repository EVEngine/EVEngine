...
> 后端：`src/modules/graphics/vulkan/Graphics.cpp`。底层构造器：`external/VKBuilder/include/vkbuilder.hpp`（`vkb::`）。投影：`perspectiveVulkanRH_ZO`（RH、ZO、Vulkan Y 向下，见 `ClipSpace.h`）。
...
> **规划中的混合光照**：不透明 Clustered Deferred（**完整核心 PBR GBuffer**）+ 半透明 Forward+（可退回全 Forward+）。
> 见 [`superpowers/specs/2026-09-26-hybrid-clustered-deferred-forward-plus-design.md`](./superpowers/specs/2026-09-26-hybrid-clustered-deferred-forward-plus-design.md)。
> 今日仍只有前向 / clustered / `pbr_surface` 前向；GBuffer 的 3-bit metal/rough **不**算 deferred PBR。
> `GraphicsDeferredGraph` 仅表示 Shadow/GBuffer 的 CPU 延后录制。
...
| VKBuilder | `external/VKBuilder/include/vkbuilder.hpp`：`Present` / `PresentBuilder` / `PipelineBuilder` / `executeImmediately` |
...
ClassicScenes 每个场景一张 PNG：`build/win32-debug/test/out/classic_scenes/{tag}.png`（FullLit）。相位/诊断图：`EVENGINE_DUMP_PHASES=1`。观感节奏：`EVENGINE_VIEW_SECONDS` / `EVENGINE_VIEW_HZ`。
