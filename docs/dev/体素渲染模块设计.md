...
`test/voxel.cpp`（CPU，37 cases）：打包/别名、脏标记与 remeshDirty、负坐标 chunk、空/满层/柱/L/棋盘贪婪、邻接消隐、六轴朝向与视距/视锥、`viewRange≤0`、null viewProj、Decode 六面与绕序、`meshVoxels` API、`drawVisible(nullptr)` 安全。

`test/voxel_render.cpp`（GPU，18 cases）：smoke、空实例、非法 faceDir、需 begin3DFrame、faceDir 别名、null atlas、大实例数、远 chunk、深度近遮远、多帧复用、六向环视、tilesPerRow 图集、编辑 remesh 像素变化、手动 batch。
