...
`test/voxel.cpp`（CPU，56 cases）：打包/别名/掩码、脏标记与 remeshDirty、负坐标与跨 chunk 边界、空/满层/柱/L/棋盘/描边环/双条带贪婪、邻接消隐、六轴朝向与视距/视锥、`viewRange≤0`、null viewProj、Decode 六面/最大矩形/UV、`meshVoxels` 抛错、`drawVisible(nullptr)`、多 chunk 可见子集。

`test/voxel_render.cpp`（GPU，26 cases）：smoke、空世界、空实例、非法 faceDir、需 begin3DFrame、±faceDir 别名、null atlas、大实例/扩容、远 chunk、多 chunk 合批、深度近遮远、多帧复用、六向环视、tilesPerRow 红/绿/蓝、单面手动绘制、侧视墙、编辑 remesh 像素变化。
