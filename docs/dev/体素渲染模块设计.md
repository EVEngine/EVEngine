...
Vulkan：共享 unit quad（4 顶点 / 6 索引）+ per-draw instance VBO（`VK_VERTEX_INPUT_RATE_INSTANCE`），push constant 传 `viewProj` / chunk origin / faceDir / atlas 分格。

## 测试

`test/voxel.cpp`：打包往返、实心立方体 6 面各 1 矩形、单格 6 面、平板合并、朝向/距离/视锥裁剪、跨 chunk `setVoxel`。
