...
`test/voxel.cpp`（CPU）：打包往返与位域隔离、实心/单格/平板/空心壳贪婪合并、邻接消隐、六轴朝向裁剪、视距/视锥、可见 batch 几何解码（`VoxelRectDecode` 镜像着色器）、绕序与外法线一致。

`test/voxel_render.cpp`（GPU / 像素回读）：`drawVisible` smoke、空实例、非法 faceDir、红色体素中心像素、faceCull 批次数、超距无亮色、同帧多 batch、手动 `drawVoxelFaceInstances` 对齐。
