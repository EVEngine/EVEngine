...
| Basic 3D/PBR | partial, core materials verified | WebGPU now matches Vulkan's derivative-built tangent-space normal mapping, POM UV displacement, texture cell bombing, hemispheric wrap fill and emissive/tonemap order; shared pixel tests verify normal, parallax and cell-bomb effects on both backends | Add isolated environment-cubemap and cloud-shadow scenes, then compare stored artifacts |
