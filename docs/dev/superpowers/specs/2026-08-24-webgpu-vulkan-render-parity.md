...
| Stylize post effects | supported, core verified | Cartoon, watercolor, ink and pixel passes have native WGSL implementations with the same packed public parameters; shared pass/chain rendering tests run on both backends | Complete cartoon/ink mesh WGSL and compare the full cylinder gallery |
