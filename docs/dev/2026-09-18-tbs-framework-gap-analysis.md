...
| Phase 1.6（hexmap 接线） | 🟡 **路由器已落地并被 physics 采用**：① `sensing/LineOfSightRouter` 按坐标空间分发（无后端 → `Unsupported` 且诊断带空间名、跨空间 → `InvalidArgument`、重复认领 → `Conflict`、只能由认领者释放、不拥有后端）；② 新增 `sensing::ensureLineOfSightRouter()`：首次调用把路由器注册为 `ILineOfSightQuery` capability（`Applied`），再次调用 `NoOp`，**若槽位已被别的 provider 占用则返回 `Conflict` 并原样保留该 provider**（项目自己注册的实现不会被静默顶掉）；③ `physics` 不再直接 `provide` 该 capability，改为向路由器**认领 `World3D`**（`PhysicsCapabilities.cpp`），因此 grid 后端可以另外认领自己的空间，两者不再按加载顺序互相顶掉。**剩余**：hexmap grid 后端——hexmap 的可见性只由**射程 + 高度**决定、**没有任何阻挡物数据模型**（`HexSearch.cpp:270-297`），所以要么新增"挡视线"事实（带 schema 与迁移），要么用"`to` ∈ FOV(from) 且 range = 两点距离"近似并写明阴影投射 FOV 天然不对称。分层无障碍（hexmap L4 → sensing L1 合法向下边）。 | 已做：路由器 5 个用例 + physics 采用后 `sensing\|targeting\|physics` 共 **79/79** 通过（含 `ensureLineOfSightRouter` 的 Applied/NoOp/Conflict 与"外来 provider 被保留"） |
