...
| Phase 1.4–1.6 | ✅ **1.4 交互状态机、1.5 表现意图与回退契约、1.6 tactics 侧视线/掩体策略已完成**（见 §11.1）。**表现与视线已可从脚本使用**（`presentationIntents` / `visibleCellsInRange` / `lineOfSightAlgorithm`）；**交互状态机仍只有 C++ 接口**——把它绑到脚本需要给 `InteractionSession` 建一个脚本侧值对象（会话状态属于客户端，不属于战局），列为剩余项；`hexmap` → `sensing::ILineOfSightQuery` 的 provider 接线见下一行 |
