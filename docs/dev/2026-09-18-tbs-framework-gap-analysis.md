...
| Phase 1.6（sensing 接线） | ✅ **网格 LOS 已真正接入瞄准管线**：① `sensing/LineOfSightRouter` 按坐标空间分发（无后端 → `Unsupported` 且诊断带空间名、跨空间 → `InvalidArgument`、重复认领 → `Conflict`、只能由认领者释放、不拥有后端）；② `sensing::ensureLineOfSightRouter()` 首次调用把路由器注册为 `ILineOfSightQuery`（`Applied`），再次 `NoOp`，**槽位已被别的 provider 占用时返回 `Conflict` 并保留该 provider**；③ `physics` 改为向路由器**认领 `World3D`**（不再独占 capability）；④ **`tactics` 导入即认领 `Grid2D`/`Grid3D`**（`TacticsLineOfSightAdapter`），阻挡读持久化 `sight_blocker` tag——与模块自己的射程查询同一份数据；适配器**同时只服务一个棋盘**（`attachLineOfSightBoard`/`detachLineOfSightBoard`，C++ 接口，与 physics 注册 World3D 对称），以 **ECS handle 而非指针**保存绑定（战局释放后查询返回 `Unsupported`，不读已释放棋盘），未绑定也返回 `Unsupported`（≠"看不见"）。`hexmap` 接手同一空间现在是**可选**且不再阻塞：它没有阻挡事实，接手前需要先有（新增持久化字段 + schema + 迁移）；`addProvider` 对已认领空间返回 `Conflict`，因此必须显式替换 | 路由器 5 用例 + 提供者 3 用例；`sensing\|targeting\|physics` **79/79**、`sensing\|tactics` **98/98** |
