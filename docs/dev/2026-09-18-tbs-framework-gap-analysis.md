...
| Phase 1.6（hexmap 接线） | 🟡 **单槽位冲突已用 `sensing/LineOfSightRouter` 解决**（`sensing/LineOfSightRouter.h/.cpp` + 4 个用例）：每个后端按**坐标空间**认领，路由器是唯一注册的 provider；无后端 → `Unsupported`（诊断带空间名）、跨空间查询 → `InvalidArgument`（接口禁止隐式换算）、重复认领 → `Conflict`、释放只能由认领者发起（否则 `NoOp`）、路由器**不拥有**后端。**剩余（下一轮）**：① `physics` 把 `provide<ILineOfSightQuery>(&targetingLineOfSightAdapter())` 改为向路由器认领 `World3D`，并由一处 composition 注册路由器——**注意这会让 `World3D` 的 provider 语义从"替换"变成"认领"，需要确认 `sensing::TargetingResolver` 对单 provider 的假设仍然成立**；② hexmap 侧仍缺**段可见谓词**：`HexVisibility` 是多观察者 FOV 计数器、`HexSearch::collectVisibleCells` 一次算整片 FOV，而 `query(from,to)` 问的是一段；且 hexmap 的可见性只由**射程 + 高度**决定，**没有任何阻挡物数据模型**（`HexSearch.cpp:270-297`），所以要么新增"挡视线"事实（带 schema 与迁移），要么用"`to` ∈ FOV(from) 且 range = 两点距离"近似并写明阴影投射 FOV 天然不对称。**③ 分层无障碍**：`hexmap` LAYER 4、`sensing` LAYER 1，加依赖是合法向下边。 | 已做：路由器 4 个用例（按空间分发且不误调、未认领返回 `Unsupported` 且诊断带空间名、跨空间拒绝、认领冲突/释放所有权）。待办：physics 改认领 + hexmap 段谓词语义决策 + `LineOfSightMode::Required` 集成测试 |
