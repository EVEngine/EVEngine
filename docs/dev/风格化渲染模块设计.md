...
| `supports(style, feature)` | `"gbuffer"` 可用（深度/法线由 `graphics.RenderControl` 提供） |
...
- GLSL：`src/modules/stylize/shaders/*.frag`（专用顶点着色器如 `ysa_mesh.vert` 同目录）
- 预编译：`python3 scripts/compile_stylize_shaders.py`（需 `glslc`）→ `*_frag_spv.inc` / `*_vert_spv.inc`
